...
namespace Metaplay.Unity.CompanyId
{
    static partial class CompanyIdLoginFlowHelper
    {
        static Task<GameLoginAccessToken> EditorLogin(string companyIdBaseUrl, string companyIdEnvironment)
        {
            ...
            {
                HttpListener http;
                Uri chosenCallbackUri;
                try
                {
                    (http, chosenCallbackUri) = UnityOAuth2Util.CreateListenerForCallbackIntoEditor(new List<Uri>() { callbackUri });
                }
                catch (Exception ex)
                {
                    throw new CompanyIdLoginFlowException(phase: "Init", message: "Could not open callback listener", ex);
                }
            ...
            using (http)
            {
                ...
                for (;;)
                {
                    ...
                    if (errorMaybe != null)
                    {
                        ...
                        request.Response.OutputStream.Close();
                        request.Response.Close();
                        throw new CompanyIdLoginFlowException(phase: "HandleCallback", message: $"Authorization server refused to provide code: {errorMaybe}");
                    }
                    ...
                    catch(Exception ex)
                    {
                        ...
                        throw new CompanyIdLoginFlowException(phase: "CodeExchange", message: "Authorization server refused to exchange code for access token", ex);
                    }
                }
            }
        }
