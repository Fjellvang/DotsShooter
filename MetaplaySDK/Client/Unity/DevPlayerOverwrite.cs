...
public class DevPlayerOverwrite : MonoBehaviour
{
    ...
    void Update()
    {
        // \note: Null check so if there is a domain reload in Editor, we don't spam NRE
        if (MetaplaySDK.Connection != null
            && MetaplaySDK.Connection.State.Status == ConnectionStatus.Connected
            && !string.IsNullOrEmpty(ImportPlayer))
        {
            SubmitOverwrite();
        }
    }
