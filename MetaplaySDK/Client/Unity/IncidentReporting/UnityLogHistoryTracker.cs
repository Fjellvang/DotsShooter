...
namespace Metaplay.Unity.IncidentReporting
{
    ...
    public class UnityLogHistoryTracker : IDisposable
    {
        ...
        public List<ClientLogEntry> GetLogHistory()
        {
            ...
            TruncateClientLogToLimits(entries);

            return entries;
        }

        /// <summary>
        /// Possibly truncate the log (in place) to not exceed <see cref="PlayerUploadIncidentReport.MaxLogEntriesTotalUtf8Size"/>.
        /// The purpose of this is to prefer truncating the log instead of exceeding <see cref="PlayerUploadIncidentReport.MaxUncompressedPayloadSize"/>
        /// and thus dropping the whole incident report.
        /// </summary>
        void TruncateClientLogToLimits(List<ClientLogEntry> entries)
        {
            if (entries == null) // Just defensive
                return;
        ...
        void HandleLog(string logString, string stackTrace, LogType type)
        {
            ...
            if (reportException)
            {
                TruncateClientLogToLimits(exceptionLogs);
