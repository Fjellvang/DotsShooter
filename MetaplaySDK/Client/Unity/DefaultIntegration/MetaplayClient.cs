...
namespace Metaplay.Unity.DefaultIntegration
{
    public class MetaplayClientOptions
    {
        ...
        /// <inheritdoc cref="MetaplaySDKConfig.ActiveEnvironmentIdOverride"/>
        public string ActiveEnvironmentIdOverride; // Optional
    }

    public delegate IPlayerClientContext MetaplayClientCreatePlayerContextFunc(IPlayerModelBase model, SessionProtocol.InitialPlayerState initialState, EntityId playerId, int logicVersion, MetaTime now);
    ...
    public class MetaplayClientState : ISessionContextProvider, ISessionStartHook, IDisposable
    {
        ...
        bool _hasHandledCurrentConnectionError;
        bool _hasHadSessionForCurrentConnection;
        ...
        public MetaplayClientState(MetaplayClientOptions options, Action deinitializeFunc)
        {
            ...
            {
                BuildVersion = appInfoOptions.BuildVersion ?? new BuildVersion(version: Application.version, buildNumber: null, commitId: null),
                ...
                SessionContext = this,
                ActiveEnvironmentIdOverride = options.ActiveEnvironmentIdOverride,
            };
            ...
            IMetaplaySubClientServices clientServices = new MetaplayUnitySubClientServices(ClientStore);
            ...
            // Connection.State starts with NotConnected. Ignore that.
        ...
        public void OnSessionStarted(SessionProtocol.SessionStartSuccess success, ClientSessionStartResources startResources)
        {
        ...
        void OnPlayerChecksumMismatch(PlayerChecksumMismatch mismatch)
        {
            ...
            MetaplaySDK.IncidentTracker.ReportPlayerChecksumMismatch(
                actionNdx:                  mismatch.ActionIndex,
                conflictTick:               details.TickNumber,
                playerModelDiff:            details.PlayerModelDiff,
                vagueDifferencePathsMaybe:  details.VagueDifferencePathsMaybe,
                playerAction:               action);
        ...
        public void Connect()
        {
            ...
            _hasHadSessionForCurrentConnection = false;
            Connection.Connect();
        }
        ...
        public void Update()
        {
            ...
            // If _hasHandledCurrentConnectionError has been left on after an Error status, turn it off now. (NotConnected is a result of Close() which we also want to handle).
            if (_hasHandledCurrentConnectionError && !(Connection.State.Status == ConnectionStatus.Error || Connection.State.Status == ConnectionStatus.NotConnected))
                _hasHandledCurrentConnectionError = false;
            ...
            {
                _hasHadSessionForCurrentConnection = true;
            }
            ...
            if (_initialState != null)
            {
                ...
                ISharedGameConfig   playerGameConfig    = Connection.SessionStartResources.GetConfigForPlayer(initialState.PlayerState);
            ...
            // Handle connection errors. NotConnected is the result of Close() (the initial state is ignored).
            if ((Connection.State.Status == ConnectionStatus.Error || Connection.State.Status == ConnectionStatus.NotConnected) && !_hasHandledCurrentConnectionError)
            {
                _hasHandledCurrentConnectionError = true;
        ...
        IPlayerClientContext DefaultCreatePlayerContext(IPlayerModelBase model, SessionProtocol.InitialPlayerState initialState, EntityId playerId, int logicVersion, MetaTime now)
        {
        ...
        void HandleConnectionError(ConnectionState connectionState, ConnectionStatistics connectionStatistics)
        {
            ...
            else if (connectionState is TransientError.SessionForceTerminated sessionForceTerminated)
            {
                ...
                MetaplayClientErrorInfo errorInfo
                    ...
                    : terminatedReason is SessionForceTerminateReason.GameConfigUpdated         ? MetaplayClientConnectionErrors.SessionTerminatedGameConfigUpdated
                    : terminatedReason is SessionForceTerminateReason.LogicVersionUpdated       ? MetaplayClientConnectionErrors.SessionTerminatedLogicVersionUpdated
            ...
            else if (connectionState is TransientError.GameConfigUpdateRequired)
            {
                OnConnectionLost(new ConnectionLostEvent(
                    MetaplayClientConnectionErrors.SessionTerminatedGameConfigUpdated,
                    // Auto-reconnect is not recommended here, as a game config update will cause all connected players
                    // to trigger this error more or less at the same time which could put the server under a fair amount
                    // of pressure.
                    autoReconnectRecommended: false,
                    technicalError: connectionState,
                    connStats));
            ...
            else if (connectionState is TerminalError.InvalidGameMagic invalidGameMagic)
            {
                OnConnectionLost(new ConnectionLostEvent(
                    ...
                    extraTechnicalInfo: Invariant($"server_{invalidGameMagic.Magic:x8}"),
                    autoReconnectRecommended: false,
                    technicalError: connectionState,
                    connStats));
            }
            else if (connectionState is TerminalError.ProjectNameMismatchError projectNameMismatch)
            {
                OnConnectionLost(new ConnectionLostEvent(
                    MetaplayClientConnectionErrors.ProjectNameMismatch,
                    extraTechnicalInfo: Invariant($"server_{projectNameMismatch.ServerProjectName}"),
                    autoReconnectRecommended: false,
                    technicalError: connectionState,
                    connStats));
            }
            ...
            else if (connectionState is TerminalError.ClientPatchVersionTooOld clientPatchVersionTooOld)
            {
                MetaplayClientErrorInfo errorInfo = MetaplayClientConnectionErrors.ClientPatchVersionTooOld;
                OnConnectionLost(new ConnectionLostEvent(
                    errorInfo,
                    extraTechnicalInfo: Invariant($"current_{clientPatchVersionTooOld.CurrentClientPatchVersion}_minimum_{clientPatchVersionTooOld.MinimumPatchVersionRequired}"),
                    autoReconnectRecommended: false,
                    technicalError: connectionState,
                    connStats));
            }
            ...
            else if (connectionState is NotConnected)
            {
                OnConnectionLost(new ConnectionLostEvent(
                    MetaplayClientConnectionErrors.ClientEndedConnection,
                    autoReconnectRecommended: false,
                    technicalError: connectionState,
                    connStats));
            }
        ...
        {
            if (_hasHadSessionForCurrentConnection)
    ...
    public abstract class MetaplayClientBase<TPlayerModel> where TPlayerModel : IPlayerModelBase
    {
        ...
        public static string ConfigVersion
        {
            get
            {
                ContentHash? version = Connection?.LatestGameConfigInfo?.BaselineVersion;
                if (version == null)
                    return "unavailable";
        ...
        public static void Initialize(MetaplayClientOptions options)
        {
            ...
            {
                // Make sure the editor playmode auto-deinit trigger is registered
        ...
        static void OnPlayModeStateChanged(PlayModeStateChange change)
        {
            ...
            {
                // Auto-deinitialize MetaplayClient on editor playmode exit if not done by integration code.
                // The deinitialization is done on entering edit mode so that all playmode behaviours have been
                // destroyed.
                EditorApplication.playModeStateChanged -= OnPlayModeStateChanged;
                if (State != null)
                    Deinitialize();
        ...
        public static void Deinitialize()
        {
            ...
            if (state != null)
                ...
                UnityEngine.Debug.LogError("MetaplayClient.Deinitialize called but client was already deinitialized!");
        }
...
#region Pseudo connection errors, invoked from within MetaplayClient itself
    ...
    public class PlayerChecksumMismatchConnectionError : TransientError
    {
        ...
        public PlayerChecksumMismatchConnectionError(long tickNumber, PlayerActionBase action, string modelDiff, List<string> vagueDifferencePathsMaybe)
        {
