...
namespace Metaplay.Unity
{
        ...
        , IPlayerModelServerListenerCore
    {
        ...
        MetaDictionary<LanguageId, ContentHash> _localizationVersions;
        ...
        public virtual async Task InitializeAsync(MetaplayOfflineOptions offlineOptions)
        {
            ...
            _localizationVersions = new MetaDictionary<LanguageId, ContentHash>();
            ...
            if (_offlineOptions.PersistState)
            {
                ...
                if (bytes != null)
                {
                    ...
                    if (persisted.PlayerSchemaVersion >= migrator.SupportedSchemaVersions.MinVersion && persisted.PlayerSchemaVersion <= migrator.SupportedSchemaVersions.MaxVersion)
                    {
                        ...
                        elapsedAsPersisted = now - persisted.PersistedAt;
        ...
        protected virtual void OnClientSessionHandshake(SessionStartState sessionStartState, SessionProtocol.SessionStartRequest sessionStart)
        {
        }
        ...
        void IOfflineServer.HandleMessage(MetaMessage msg)
        {
            ...
            switch (msg)
            {
                case Handshake.ClientHello login:
                {
                    SendToClient(new Handshake.ClientHelloAccepted(
                        serverOptions: new Handshake.ServerOptions(
                            ...
                            gameTimeOffset:                                         MetaDuration.Zero)
                        ));
                    break;
                }
                ...
                case SessionProtocol.SessionStartRequest sessionStart:
                {
                    ...
                    OnClientSessionHandshake(_sessionStartState.Value, sessionStart);
                    ...
                    // On Editor, default to consistency checks. On device builds, consistency
                    // on offline is useless since:
                    // 1) we don't get logs
                    // 2) we don't have anywhere to send incident reports.
#if UNITY_EDITOR
                    bool clientConsistencyChecks = true;
#else
                    bool clientConsistencyChecks = false;
#endif
                    EntityDebugConfig debugConfig = new EntityDebugConfig()
                    {
                        ServerConsistencyChecks = false,
                        ClientConsistencyChecks = clientConsistencyChecks,
                        ChecksumGranularity = ChecksumGranularity.None
                    };
                ...
                case MetaRequestMessage request:
                    ...
                    switch (request.Payload)
                    {
                        ...
                        {
                            MetaResponse response = new ImmutableXLoginChallengeResponse(message: "Login is not supported in OfflineMode. This is a dummy request to allow you test the login flow.", "Test login from offline mode", OfflinePlayerId, DateTime.UtcNow);
                            SendToClient(new SessionMetaResponseMessage() { RequestId = request.Id, Payload = response });
                            break;
                        }
                ...
                case PlayerScheduleDeletionRequest _:
                    ExecutePlayerServerAction(new PlayerSetIsScheduledForDeletionAt(MetaTime.Now + MetaDuration.FromDays(7), PlayerDeletionSource.User));
                    break;
        ...
        SessionProtocol.SessionResourceCorrection GetResourceCorrection(SessionProtocol.SessionStartRequest sessionStart, SessionStartState startState)
        {
            ...
            if (!sessionStart.ResourceProposal.ConfigVersions.Contains(GameConfigArchive.Version))
            {
                SessionProtocol.SessionResourceCorrection entityCorrection = new SessionProtocol.SessionResourceCorrection();
                    entityCorrection.ConfigUpdates.Add(new SessionProtocol.SessionResourceCorrection.ConfigArchiveUpdateInfo(
                        sharedGameConfigVersion: GameConfigArchive.Version,
                        urlSuffix: null));

                correction = SessionProtocol.SessionResourceCorrection.Combine(correction, entityCorrection);
            }
        ...
        MetaDictionary<int, ChannelEntity> _channelEntities = new MetaDictionary<int, ChannelEntity>();
        ...
        (EntityInitialState, ChannelEntity) CreateMultiplayerEntityInitialState(ClientSlot slot, IMultiplayerModel model, EntityId? memberId = null)
        {
            ...
            EntityInitialState initialState = new EntityInitialState(
                state:          state,
                channelId:      channelId,
                clientData:     CreateEntityClientData(slot, memberId));
        ...
        EntityClientData CreateEntityClientData(ClientSlot slot, EntityId? memberId)
        {
            return new EntityClientData.Default(slot);
        }
        ...
        EntityTimelineUpdateMessage TryGetEntityTickUpdate(ChannelEntity manager)
        {
            ...
            long                firstTick   = manager.Model.CurrentTick + 1;
