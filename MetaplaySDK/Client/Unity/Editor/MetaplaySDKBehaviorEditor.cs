...
namespace Metaplay.Unity
{
    ...
    public class MetaplaySDKBehaviorEditor : Editor
    {
        ...
        void LocationOverrideField(MetaplaySDKBehavior sdk)
        {
            string LocationToString(PlayerLocation loc)
            {
                return $"{loc.ContinentCodeMaybe}-{loc.Country.IsoCode}";
            }

            PlayerLocation[] premadeLocations = new PlayerLocation[]
            {
                new PlayerLocation(new CountryId("FI"), "EU"),
                new PlayerLocation(new CountryId("US"), "NA"),
                new PlayerLocation(new CountryId("JP"), "AS"),
            };

            int locationOverrideIndex;
            List<string> items = new List<string>();

            items.Add("None");
            foreach (PlayerLocation loc in premadeLocations)
                items.Add(LocationToString(loc));

            if (sdk._overrideLocationString == null)
            {
                locationOverrideIndex = 0;
            }
            else
            {
                locationOverrideIndex = -1;
                for (int ndx = 0; ndx < premadeLocations.Length; ++ndx)
                {
                    if (LocationToString(premadeLocations[ndx]) == sdk._overrideLocationString)
                    {
                        locationOverrideIndex = ndx + 1;
                        break;
                    }
                }
                if (locationOverrideIndex == -1)
                {
                    locationOverrideIndex = items.Count;
                    items.Add(sdk._overrideLocationString);
                }
            }

            int newIndex = EditorGUILayout.Popup(new GUIContent("Override client location", "Only for development environments and developer players"), locationOverrideIndex, items.ToArray());
            if (newIndex != locationOverrideIndex)
            {
                if (newIndex == 0)
                {
                    sdk._overrideLocationString = null;
                }
                else
                {
                    sdk._overrideLocationString = LocationToString(premadeLocations[newIndex - 1]);
                }
            }
        }
        ...
        void LanguageGUI()
        {
            ...
            GUILayout.Label(new GUIContent("Languages:", "Only shows localizations defined in the Languages field in the active game config."));
            ISharedGameConfig gameConfig = MetaplaySDK.SessionContext?.PlayerContext?.Journal?.StagedModel?.GameConfig;
            if (gameConfig != null && MetaplaySDK.LocalizationManager != null)
            {
                IGameConfigLibrary<LanguageId,LanguageInfo> gameConfigLanguages = gameConfig.Languages;

                foreach ((LanguageId language, ContentHash builtinLocalizationVersion) in BuiltinLanguageRepository.GetBuiltinLanguages())
                {
                    if (!gameConfigLanguages.ContainsKey(language))
                        continue;
                    ...
                    {
                        _ = MetaplaySDK.LocalizationManager.FetchLanguageAsync(language, serverLocalizationVersion, MetaplaySDK.CdnAddress, numFetchAttempts: 3, fetchTimeout: TimeSpan.FromSeconds(10), CancellationToken.None);
                    }
                    GUI.enabled = true;
        ...
        {
            if (!MetaplayCore.IsInitialized || !MetaplayCore.Options.FeatureFlags.EnableGuilds)
                return;
            ...
            if (EditorGUILayout.BeginFoldoutHeaderGroup(_showGuildGroup, "Guild State"))
            {
                ...
                switch (phase)
                {
                        ...
                        if (GUILayout.Button("Promote/Demote random member"))
                        {
                            ...
                            for (int i = 0; i < 30; ++i)
                            {
                                ...
                                if (!((IGuildModelBase)model).HasPermissionToChangeRoleTo(guildClient.PlayerId, target, role))
                                    continue;
                                MetaDictionary<EntityId, GuildMemberRole> changes = ((IGuildModelBase)model).ComputeRoleChangesForRoleEvent(GuildMemberRoleEvent.MemberEdit, target, role);
                                if (changes.Count == 0)
                                    continue;
                    ...
                    case GuildClientPhase.LoadingEntity:
                    default:
                    {
