...
namespace Metaplay.Client.Unity
{
    public class ConfigReferenceAnalyzerWindow : EditorWindow
    {
        ...
        class CommonResources
        {
            ...
            public MetaDictionary<ExperimentVariantPair, PatchStats> PatchStats;

            public CommonResources(DynamicTraversal.Resources traversalResources, FullGameConfigImportResources fullConfigImportResources, int numBaselineItems, MetaDictionary<ExperimentVariantPair, PatchStats> patchStats)
            {
        ...
        {
            public MetaDictionary<Type, TypeItemsInfo> ItemsByType = new MetaDictionary<Type, TypeItemsInfo>();
        ...
        class ItemSetState
        {
            public MetaDictionary<Type, ItemTypeState> ItemsByType = new MetaDictionary<Type, ItemTypeState>();
        ...
        {
            DynamicTraversal.Resources traversalResources = DynamicTraversal.Resources.Create(MetaplayServices.Get<MetaSerializerTypeRegistry>().TypeInfo);
            ...
            MetaDictionary<ExperimentVariantPair, PatchStats> patchStats = new MetaDictionary<ExperimentVariantPair, PatchStats>();
            ...
            if (_helpExpanded = EditorGUILayout.Foldout(_helpExpanded, "Help, what's this?", toggleOnLabelClick: true))
            {
                ...
                GUILayout.Label(
                    text:
                        ...
                        "it may be worth the hassle to eliminate those MetaRefs, by means of refactoring them into MetaConfigId " +
                        "and performing the config lookup explicitly in the game code." +
                        "\n\n" +
            ...
            else
            {
                ...
                if (EditorGUILayout.DropdownButton(new GUIContent(dropdownTitle), FocusType.Keyboard))
                {
                    ...
                    if (_patchDropdownSorting == PatchDropdownSorting.MostPropagatingFirst)
                    {
                        ...
                        MetaDictionary<PlayerExperimentId, int> experimentMaxNumIndirect =
                    ...
                    GUILayout.Label(
                        new GUIContent(
                            ...
                            tooltip:
                                ...
                                "\n\nYou can use the results to decide which MetaRefs are worth refactoring away (and using " +
                                "MetaConfigId instead)."),
                            EditorStyles.boldLabel);
        ...
        ItemsReachabilityState ResolveItemsReachability(CommonResources resources, TaskProgress progress, CancellationToken ct)
        {
            ...
            MetaDictionary<ConfigItemId, OrderedSet<(ConfigItemId, string)>> reverseReferences = MetaRefAnalysisUtil.CollectItemReverseReferences(resources.TraversalResources, config);
            ...
            result.ItemsByType = new MetaDictionary<Type, ItemsReachabilityState.TypeItemsInfo>(
                result.ItemsByType
