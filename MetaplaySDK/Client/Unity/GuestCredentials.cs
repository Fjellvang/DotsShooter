    ...
    public static class GuestCredentialsSerializer
    {
        ...
        public static GuestCredentials TryDeserialize(byte[] blob)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(data.DeviceId) || string.IsNullOrEmpty(data.AuthToken))
                    return null;

                // PlayerId contents are only a hint and is optional.
                EntityId playerIdHint = EntityId.None;
                if (data.PlayerId.IsValid)
                    playerIdHint = data.PlayerId;
