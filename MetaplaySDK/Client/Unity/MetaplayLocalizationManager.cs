...
namespace Metaplay.Unity
{
    ...
    public sealed class MetaplayLocalizationManager
    {
        ...
        MetaDictionary<LanguageId, ContentHash>  _builtinLanguages;
        CancellationTokenSource                     _stopCts;
        MetaDictionary<LanguageId, ContentHash>  _serverLocalizationVersions;
        ...
        internal void Start(IMetaplayLocalizationDelegate managerDelegate)
        {
            ...
            {
                LocalizationLanguage noneLanguage = new LocalizationLanguage(LanguageId.FromString("none"), ContentHash.None, new MetaDictionary<TranslationId, string>());
        ...
        /// </summary>
        public Task<LocalizationLanguage> FetchLanguageAsync(LanguageId language, ContentHash version, MetaplayCdnAddress cdnAddress, int numFetchAttempts, TimeSpan fetchTimeout, CancellationToken ct)
        {
            if (!MetaplayCore.Options.FeatureFlags.EnableLocalizations)
        ...
        public void SetCurrentLanguage(LanguageId language, Action onCompleted)
        {
            ...
            if (activeGameConfig != null)
            {
                LanguageInfo languageInfoMaybe = activeGameConfig.Languages.GetValueOrDefault(language);
                if (languageInfoMaybe == null)
                    throw new InvalidOperationException($"Unable to switch to language '{language}' as it is not defined in SharedGameConfig.Languages, please add it to the library and rebuild your game configs.");
            }
            ...
            if (_hasSession)
            {
                ...
                if (versionOnServerMaybe != ContentHash.None && versionOnServerMaybe != builtinVersion)
                {
                    ...
                    EnqueueSwitchToLanguageOnComplete(WithCancelAtSdkStop(fetchOrFallbackTask), wrappedOnCompleted);
                    return;
                }
            }
