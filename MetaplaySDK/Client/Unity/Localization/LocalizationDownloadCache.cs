...
namespace Metaplay.Unity.Localization
{
    ...
    public class LocalizationDownloadCache
    {
        ...
        string _downloadDir;
        object _lock;
        MetaDictionary<LanguageId, LanguageLocalizationFiles> _cached;
        uint _lastTimestamp;
        Task _initTask;
        ...
        async Task InitializeAsync()
        {
            ...
            MetaDictionary<LanguageId, ContentHash> builtins = BuiltinLanguageRepository.GetBuiltinLanguages();
            MetaDictionary<LanguageId, List<string>> languageFiles = new MetaDictionary<LanguageId, List<string>>();
            MetaDictionary<LanguageId, LanguageLocalizationFiles> cached = new MetaDictionary<LanguageId, LanguageLocalizationFiles>();
        ...
        static bool ShouldKeepCached(string subpath, MetaDictionary<LanguageId, ContentHash> builtins, out LanguageId language, out uint timestamp, out ContentHash version)
        {
            language = null;
        ...
        /// </summary>
        public Task<LocalizationLanguage> GetLocalizationAsync(LanguageId language, ContentHash version, MetaplayCdnAddress cdnAddress, int numFetchAttempts, TimeSpan fetchTimeout, CancellationToken ct)
        {
            return MetaTask.Run(async () =>
            {
                ...
                for (;;)
                {
                    ...
                    Task                        cancellableTimeout  = MetaTask.Delay(fetchTimeout, ct);
                    ...
                    switch (fetchTask.Status)
                    {
                        ...
                        default:
                        {
                            // fetchTask may fail in the future. Observe it.
                            _ = fetchTask.ContinueWith((task) => { _ = task.Exception; }, TaskContinuationOptions.OnlyOnFaulted);
        ...
        {
            MetaDictionary<LanguageId, ContentHash> latestLocalizations = sessionStart.LocalizationVersions;
            PruneOldLocalizationsInBackground(latestLocalizations);
        }

        void OnUpdateLocalizationVersions(UpdateLocalizationVersions update)
        {
            MetaDictionary<LanguageId, ContentHash> latestLocalizations = update.LocalizationVersions;
            PruneOldLocalizationsInBackground(latestLocalizations);
        }

        void PruneOldLocalizationsInBackground(MetaDictionary<LanguageId, ContentHash> latestLocalizations)
        {
