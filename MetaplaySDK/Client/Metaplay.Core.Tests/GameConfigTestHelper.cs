...
namespace Metaplay.Core.Tests
{
    ...
    public static class GameConfigTestHelper
    {
        ...
        public static (string VariantIdMaybe, string Name, object Value)[] ParseKeyValueMembers<TKeyValue>(GameConfigBuildLog buildLog, GameConfigParseKeyValuePipelineConfig config, SpreadsheetContent spreadsheet)
        {
            ...
            return members.Select(member => (member.VariantIdMaybe, member.Member.MemberName, member.Member.MemberValue)).ToArray();
        }
        ...
        // \todo This is a dirty dynamic->generic helper - instead, should make GameConfigParsePipeline etc natively accept Type instead of just type parameters.
        public static (string VariantIdMaybe, string Name, object Value)[] ParseKeyValueMembersWithVariants(Type keyValueType, GameConfigParseKeyValuePipelineConfig config, SpreadsheetContent spreadsheet)
        {
            MethodInfo method = typeof(GameConfigTestHelper).GetMethod(nameof(ParseKeyValueMembersWithVariants), new Type[] { typeof(GameConfigParseKeyValuePipelineConfig), typeof(SpreadsheetContent) }).MakeGenericMethod(keyValueType);
            return ((string VariantIdMaybe, string Name, object Value)[])method.Invoke(null, new object[] { config, spreadsheet });
        }
        ...
        // \todo This is a dirty dynamic->generic helper - instead, should make GameConfigParsePipeline etc natively accept Type instead of just type parameters.
        public static (string Name, object Value)[] ParseNonVariantKeyValueMembers(Type keyValueType, GameConfigParseKeyValuePipelineConfig config, SpreadsheetContent spreadsheet)
        {
            MethodInfo method = typeof(GameConfigTestHelper).GetMethod(nameof(ParseNonVariantKeyValueMembers), new Type[] { typeof(GameConfigParseKeyValuePipelineConfig), typeof(SpreadsheetContent) }).MakeGenericMethod(keyValueType);
            return ((string Name, object Value)[])method.InvokeWithoutWrappingError(null, new object[] { config, spreadsheet });
        }
    }
}
