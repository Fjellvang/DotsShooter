...
namespace Metaplay.Core.Tests
{
    class MetaGuidTests
    {
        ...
        public void NoneValueTests()
        {
            ...
            TestTryParseValid(MetaGuid.None, "000000000000000-0-0000000000000000");
        ...
        public void InvalidValueTests()
        {
            ...
            TestTryParseInvalid(null);

            string[] invalidInputs = new string[]
            {
                "00000000000000A-0-0000000000000000", // uppercase
                "00000000000000-0-0000000000000000",  // too short timestamp
                "0000000000000000-0-0000000000000000", // too long timestamp
                "000000000000000--0000000000000000", // no reserved value
                "000000000000000-0-000000000000000", // too short random
                "000000000000000-0-00000000000000000", // too long random
                "000000000000000-1-0000000000000000", // invalid reserved value
            };

            foreach (string input in invalidInputs)
            {
                Assert.Throws<FormatException>(() => MetaGuid.Parse(input));
                TestTryParseInvalid(input);
            }
        }
        ...
        public void BasicStringConversionTest()
        {
            ...
            TestTryParseValid(guid, guid.ToString());
        ...
        static void TestTryParseValid(MetaGuid expected, string input)
        {
            Assert.IsTrue(MetaGuid.TryParse(input, out MetaGuid guid, out string error));
            Assert.AreEqual(expected, guid);
            Assert.IsNull(error);
        }

        static void TestTryParseInvalid(string input)
        {
            Assert.IsFalse(MetaGuid.TryParse(input, out MetaGuid guid, out string error));
            Assert.AreEqual(default(MetaGuid), guid);
