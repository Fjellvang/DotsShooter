...
namespace Metaplay.Core.Tests
{
    ...
    public class OrderedContainerTests
    {
        ...
        void ExecuteDictOperations<K, V>(DictOp<K, V>[] ops, bool addOverwrites, bool removeExtractsValue) where K : IEquatable<K>
        {
            ...
            MetaDictionary<K, V> dict = new MetaDictionary<K,V>();
        ...
        public void TestOrderedSetRemoveWhere()
        {
            ...
            {
                OrderedSet<int> set = new OrderedSet<int>() { 1, 2, 3 };
                Assert.AreEqual(2, set.RemoveWhere((v, s) => v != (int)s, state: 1));
                Assert.AreEqual(new List<int>() { 1 }, new List<int>(set));
            }
            {
                OrderedSet<int> set = new OrderedSet<int>() { 1, 2, 3 };
                Assert.AreEqual(2, set.RemoveWhere((v, s) => v != ((string)s).Length, state: "foo"));
                Assert.AreEqual(new List<int>() { 3 }, new List<int>(set));
            }
            {
                OrderedSet<int> set = new OrderedSet<int>() { 1, 2, 3 };
                Assert.AreEqual(2, set.RemoveWhere((v, s) => v != 2, state: null));
                Assert.AreEqual(new List<int>() { 2 }, new List<int>(set));
            }
            // Iteration order
            {
        ...
        public void TestOrderedSetAllocations()
        {
            ...
            NoAllocationsIn(() => intSet.RemoveWhere((int k, object state) => k < ((string)state).Length, state: "foo"));
            ...
            NoAllocationsIn(() => boxSet.RemoveWhere((string k, object state) => ((k?.Length ?? 0) + ((string)state).Length) % 2 == 0, state: "foo"));
            ...
            NoAllocationsIn(() => enumSet.RemoveWhere((TestEnum k, object state) => ((int)k + ((string)state).Length) % 2 == 0, state: "foo"));
        ...
        [Test]
        public void TestOrderedDictIDictionaryWithNulls()
        {
            IDictionary iDict = new MetaDictionary<string, string>();

            // Indexer
            iDict[null] = "a";
            iDict["x"] = null;
            Assert.AreEqual(new string[]{ null, "x" }, iDict.Keys);
            Assert.AreEqual("a", iDict[null]);
            Assert.AreEqual(null, iDict["x"]);
            iDict.Remove(null);
            Assert.AreEqual(new string[]{ "x" }, iDict.Keys);
            iDict.Remove("x");
            Assert.AreEqual(new string[]{}, iDict.Keys);

            // Add()
            iDict.Add(null, "b");
            iDict.Add("y", null);
            Assert.AreEqual(new string[]{ null, "y" }, iDict.Keys);
            Assert.AreEqual("b", iDict[null]);
            Assert.AreEqual(null, iDict["y"]);
            iDict.Remove(null);
            Assert.AreEqual(new string[]{ "y" }, iDict.Keys);
            iDict.Remove("y");
            Assert.AreEqual(new string[]{}, iDict.Keys);
        }

        [Test]
        public void TestOrderedDictIDictionaryWithNullableValueTypeNulls()
        {
            IDictionary iDict = new MetaDictionary<int?, int?>();

            // Indexer
            iDict[null] = 1;
            iDict[10] = null;
            Assert.AreEqual(new int?[]{ null, 10 }, iDict.Keys);
            Assert.AreEqual(1, iDict[null]);
            Assert.AreEqual(null, iDict[10]);
            iDict.Remove(null);
            Assert.AreEqual(new int?[]{ 10 }, iDict.Keys);
            iDict.Remove(10);
            Assert.AreEqual(new int?[]{}, iDict.Keys);

            // Add()
            iDict.Add(null, 2);
            iDict.Add(20, null);
            Assert.AreEqual(new int?[]{ null, 20 }, iDict.Keys);
            Assert.AreEqual(2, iDict[null]);
            Assert.AreEqual(null, iDict[20]);
            iDict.Remove(null);
            Assert.AreEqual(new int?[]{ 20 }, iDict.Keys);
            iDict.Remove(20);
            Assert.AreEqual(new int?[]{}, iDict.Keys);
        }
        ...
        public void TestOrderedDictCapacity()
        {
            ...
            foreach (int capacity in new int[] { 0, 1, 2, 3, 4, 5, 123, 1023, 1024, 1025, 1239 })
            {
                MetaDictionary<int, int> orderedDict = new MetaDictionary<int, int>(capacity);
        ...
        {
            Assert.AreEqual(new List<KeyValuePair<int, int>>(), new List<KeyValuePair<int, int>>(new MetaDictionary<int, int>()));
            Assert.AreEqual(new List<KeyValuePair<int, int>>(), new List<KeyValuePair<int, int>>(new MetaDictionary<int, int>(1)));
            Assert.Throws<ArgumentException>(() =>
                new List<KeyValuePair<int, int>>(new MetaDictionary<int, int>(new KeyValuePair<int, int>[]
                {
            ...
            Assert.AreEqual(
                ...
                new List<KeyValuePair<int, int>>(new MetaDictionary<int, int>()
                {
                    [1] = 2,
                    [2] = 3,
                }));
        ...
        public void TestOrderedDictConcurrentModification()
        {
            ...
            {
                // attempt to in-place filter
                MetaDictionary<int, int> dict = new MetaDictionary<int, int>() { [1] = 2, [0] = 2 };
            ...
            {
                // attempt to use keys
                MetaDictionary<int, int> dict = new MetaDictionary<int, int>() { [1] = 2, [0] = 3 };
        ...
        public void TestOrderedDictRemoveWhere()
        {
            // result
            {
                MetaDictionary<int, int> dict = new MetaDictionary<int, int>() { { 1, 1 }, { 2, 2 }, { 3, 3 } };
            ...
            // Iteration order
            {
                MetaDictionary<int, int> dict = new MetaDictionary<int, int>();
                for (int i = 0; i < 1000; ++i)
                    dict.Add(i, i);
        ...
        {
            MetaDictionary<string, string> dict = new MetaDictionary<string, string>()
            {
                [null] = "1",
                ["A"] = "2",
                ["foo"] = null,
        ...
        {
            MetaDictionary<int, int> intDict = new MetaDictionary<int, int>()
            {
                [1] = 1,
                [2] = 2,
                [50] = 50,
            ...
            MetaDictionary<int, IntStruct> structDict = new MetaDictionary<int, IntStruct>()
            {
        ...
        public void TestOrderedDictPrettyPrintable()
        {
            ...
            Assert.AreEqual(
                ...
                PrettyPrint.Compact(new MetaDictionary<string, string>() { { "1", null }, { null, "2" } }).ToString());
        }
        [Test]
        [Retry(100)] // First runs provoke some internal allocs.
        public void TestOrderedDictAllocations()
        {
            ...
            Predicate<MetaDictionary<int, int>.KeyValue> IsOdd = kv => kv.Key % 2 == 0;
            Predicate<MetaDictionary<string, string>.KeyValue> IsEvenLength = kv => (kv.Value?.Length ?? 0) % 2 != 0;
            Predicate<MetaDictionary<TestEnum, TestEnum>.KeyValue> IsOne = kv => kv.Key == TestEnum.One;
            MetaDictionary<int, int> intDict = new MetaDictionary<int, int>() { { 0, 1 }, { 1, 2 }, { 5, 3 }, { -3, 4 }, { 8, 5 }, { 3, 6 } };
            MetaDictionary<string, string> boxDict = new MetaDictionary<string, string>() { { "1", "foo" }, { "2", "Bar" }, { null, "nully" }, { "fo", "fu" }, { "bar", null } };
            MetaDictionary<TestEnum, TestEnum> enumDict = new MetaDictionary<TestEnum, TestEnum>() { { TestEnum.One, TestEnum.Twenty }, { TestEnum.Two, TestEnum.Twenty }, { TestEnum.Zero, TestEnum.Twenty  } };
        ...
        {
            MetaDictionary<int, int> idict = new MetaDictionary<int, int>() { [1] = 2, [0] = 3 };
            ...
            MetaDictionary<string, string> sdict = new MetaDictionary<string, string>() { ["A"] = "1", ["BC"] = "2" };
        ...
        public void TestOrderedDictGetValueOrDefault()
        {
            ...
            {
                MetaDictionary<int, string> dict = new MetaDictionary<int, string>() { { 1, "1" }, { 2, "2" }, { 3, "3" } };
                Assert.AreEqual("3", dict.GetValueOrDefault(3, defaultValue: "5"));
                Assert.AreEqual("5", dict.GetValueOrDefault(4, defaultValue: "5"));
            }
        }
...
#if !UNITY_EDITOR // Collection expressions aren't supported

        [Test]
        public void TestOrderedDictCollectionExpression()
        {
            MetaDictionary<int, int> dict =
                [
                    new MetaDictionary<int, int>.KeyValue(1, 1),
                    new MetaDictionary<int, int>.KeyValue(2, 2),
                    new MetaDictionary<int, int>.KeyValue(3, 3),
                ];
            ...
            dict = CompilerSupport.MetaDictionaryBuilder.Build<int, int>((new MetaDictionary<int, int>.KeyValue[]
                {
                    new MetaDictionary<int, int>.KeyValue(1, 1),
                    new MetaDictionary<int, int>.KeyValue(2, 2),
                    new MetaDictionary<int, int>.KeyValue(3, 3),
