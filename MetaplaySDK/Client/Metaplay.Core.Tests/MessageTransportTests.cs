    ...
    class MessageTransportTests
    {
        ...
        class MockNetworkStream : Stream
        {
            ...
            int InternalRead(byte[] buffer, int offset, int count)
            {
                ...
                int numToRead = System.Math.Min(remaining, count);
            ...
            public override async Task<int> ReadAsync(byte[] buffer, int offset, int count, CancellationToken ct)
            {
                ...
                if (ReadTakesTimeAfterNumBytes.HasValue && ReadTakesTimeAfterNumBytes.Value > numReadTotal)
                    numToRead = System.Math.Min(count, ReadTakesTimeAfterNumBytes.Value - numReadTotal);
                else
                    numToRead = count;
        ...
        class TestStreamMessageTransport : StreamMessageTransport
        {
            ...
            static ConfigArgs GenConfig(Action<ConfigArgs> editConfig)
            {
                ...
                ca.GameMagic = "TEST";
                ca.ClientVersion = new ClientVersion(1, 0);
                editConfig(ca);
                return ca;
            }
            ...
            public async Task<ErrorT> ExpectError<ErrorT>()
            {
                ...
                this.OnError += (MessageTransport.Error e) =>
                {
                    if (e is ErrorT et)
                        ...
                        result.SetException(new AssertionException($"expected error: got {e.GetType().Name}, expected {typeof(ErrorT).Name}: {PrettyPrint.Compact(e)}"));
                };
                Open();
            ...
            public async Task<ErrorT> ExpectConnectThenError<ErrorT>()
            {
                ...
                {
                    if (!gotConnect)
                        result.SetException(new AssertionException($"expected connect: got {e.GetType().Name}: {PrettyPrint.Compact(e)}"));
        ...
        [Test]
#if !UNITY_EDITOR
        [NonParallelizable] // Not supported on Unity, Unity runs there serially anyway
#endif
        [Retry(3)] // \note: even though this is not parallelizable test we cannot trust it to give us reasonable environment.
                   //        The CI might do CPU intensive work in parallel, such as building the dashboard.
#if UNITY_EDITOR
