...
namespace Metaplay.Core.Model
{
    ...
    public enum MetaMemberFlags
    {
        ...
        _LegacyDontUse_ExcludeFromGdprExport = 1 << 3,
        ExcludeFromEventLog     = 1 << 4,               // On members of AnalyticsEventBase or AnalyticsContextBase subclasses: the member is not included in the event log shown in the LiveOps Dashboard (but can still be sent to external analytics storage).
    }
    ...
    public sealed class MaxCollectionSizeAttribute : Attribute
    {
        ...
        /// <param name="size">The maximum number of elements a collection can contain, default is see MetaSerializationContext.DefaultMaxCollectionSize</param>
    ...
    /// <summary>
    /// Property or field is not included in the entity event log that is stored
    /// in the game backend's database and shown in the LiveOps Dashboard.
    /// The property or field can still be included in the data sent to external
    /// analytics storage (e.g. BigQuery).
    /// </summary>
    /// <remarks>
    /// This only has an effect on data that would be included in the analytics events
    /// in the first place; that is classes derived from AnalyticsEventBase or AnalyticsContextBase.
    /// This has no effect on data within PlayerModel, for example.
    /// </remarks>
    [AttributeUsage(AttributeTargets.Property | AttributeTargets.Field)]
    public sealed class ExcludeFromEventLogAttribute : MetaMemberFlagAttribute
    {
        public override MetaMemberFlags Flags => MetaMemberFlags.ExcludeFromEventLog;
    }
    ...
    /// method in MetaSerialization,
    /// as well as nested objects.
    /// </summary>
    /// <remarks>
    /// The method can optionally take a single parameter of type MetaOnDeserializedParams.
    ...
    /// it must take a single parameter of type Serialization.MetaMemberDeserializationFailureParams,
