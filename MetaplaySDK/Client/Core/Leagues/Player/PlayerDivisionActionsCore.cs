        ...
        public override MetaActionResult InvokeExecute(IPlayerDivisionModel division, bool commit)
        {
            if (commit)
            {
                ...
                if (participantIdx == -1) // New participant
                {
                    ...
                    division.Log.Debug("Adding new participant {ParticipantIndex} with Id {ParticipantId} in division {DivisionIndex}", participantIdx, ParticipantId, division.DivisionIndex);
