...
namespace Metaplay.Core.Offers
{
    ...
    public abstract class MetaOfferInfoBase : IGameConfigData<MetaOfferId>, IGameConfigPostLoad, IRefersToMetaOffers
    {
        ...
        /// <summary>
        /// Override price to display in the LiveOps Dashboard, instead of <see cref="InAppProductInfoBase.Price"/> from <see cref="InAppProduct"/>.
        /// You can override this if you have implemented offers which use soft-currency (non-IAP) prices.
        /// </summary>
        public virtual string CustomReferencePriceForDashboard => null;
        ...
        void IGameConfigPostLoad.PostLoad()
        {
            ...
            if (InAppProduct != null && !InAppProduct.Ref.HasDynamicContent)
                throw new InvalidOperationException($"Configuration of MetaOffer {OfferId} references an In-App Product ({InAppProduct.KeyObject}) which has {nameof(InAppProductInfoBase.HasDynamicContent)} set to false, for Offers {nameof(InAppProductInfoBase.HasDynamicContent)} is required to be true.");
        }
        ...
        /// <summary>
        /// Return a reference price string (to display in the LiveOps Dashboard).
        /// Either <see cref="CustomReferencePriceForDashboard"/> if defined,
        /// or <see cref="InAppProductInfoBase.Price"/> from <see cref="InAppProduct"/>,
        /// or null if neither the custom price nor <see cref="InAppProduct"/> is defined.
        /// </summary>
        public string TryGetReferencePriceString()
        {
            string custom = CustomReferencePriceForDashboard;
            if (custom != null)
                return custom;

            if (InAppProduct != null)
            {
                // \note Converting via `decimal`, because we want 2 fractional decimals always,
                //       but F64.ToString() doesn't support that.
                F64 priceF64 = InAppProduct.Ref.Price;
                decimal priceDecimal = decimal.Parse(priceF64.ToString(), CultureInfo.InvariantCulture);
                string priceString = priceDecimal.ToString("F2", CultureInfo.InvariantCulture);
                // \note Dollar for USD
                return "$" + priceString;
            }
