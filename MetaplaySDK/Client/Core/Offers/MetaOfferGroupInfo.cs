...
namespace Metaplay.Core.Offers
{
    ...
    public abstract class MetaOfferGroupInfoBase : IMetaActivableConfigData<MetaOfferGroupId>, IGameConfigPostLoad
    {
        ...
        /// <summary>
        /// Whether sold-out offers should be included when the offer group becomes active.
        /// <para>
        /// By default, this is true, which means sold-out offers can still become active and count against the
        /// <see cref="MaxOffersActive"/> limit, even though they're not purchasable. This is likely appropriate
        /// if you intend to still display sold-out items in the UI.
        /// (However, if an offer group would contain only sold-out items, it will not be activated.)
        /// </para>
        /// <para>
        /// If this is false, sold-out offers will not be included when the offer group becomes active.
        /// Note, however, that offers that become sold-out during the current activation can still
        /// remain active until the end of the activation.
        /// </para>
        /// </summary>
        [MetaMember(108)] public bool IncludeSoldOutOffers { get; private set; } = true;
        ...
        protected MetaOfferGroupInfoBase(MetaOfferGroupId groupId, string displayName, string description, OfferPlacementId placement, int priority, List<MetaRef<MetaOfferInfoBase>> offers, MetaActivableParams activableParams, int? maxOffersActive, bool includeSoldOutOffers)
        {
            ...
            IncludeSoldOutOffers = includeSoldOutOffers;
        }
        ...
        protected MetaOfferGroupInfoBase(MetaOfferGroupSourceConfigItemBase source)
            : this(source.GroupId, source.DisplayName ?? "", source.Description ?? "", source.Placement, source.Priority, source.Offers, source.GetActivableParams(), source.MaxOffersActive, source.IncludeSoldOutOffers)
        {
        }
    ...
    public abstract class MetaOfferGroupSourceConfigItemBase
    {
        ...
        public bool                                 IncludeSoldOutOffers { get; private set; } = true;
