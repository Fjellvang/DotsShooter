...
namespace Metaplay.Core.Offers
{
    ...
    public abstract class MetaOfferGroupModelBase : MetaActivableState<MetaOfferGroupId, MetaOfferGroupInfoBase>
    {
        ...
        /// </summary>
        [MetaMember(201)] public MetaDictionary<MetaOfferId, MetaOfferPerGroupStateBase> OfferStates { get; protected set; } = new MetaDictionary<MetaOfferId, MetaOfferPerGroupStateBase>();
    ...
    {
        [MetaMember(200)] public MetaDictionary<MetaOfferId, MetaOfferPerPlayerStateBase> OfferStates { get; protected set; } = new MetaDictionary<MetaOfferId, MetaOfferPerPlayerStateBase>();
        ...
        {
            return HasAnyOffersAvailableForPurchaseInNewActivation(player, groupInfo);
        }
        ...
        protected override void OnJustBeforeStartActivation(TOfferGroupInfo groupInfo, IPlayerModelBase player)
        {
            ...
            IEnumerable<MetaOfferInfoBase> offersToInclude =
                ...
                .Where(offerInfo => OfferIsEligibleForInclusionInNewActivation(player, groupInfo, group, offerInfo));
        ...
        bool TryActivateOfferDuringOngoingActivation(MetaOfferGroupModelBase group, MetaOfferInfoBase offerInfo, IPlayerModelBase player, bool commit)
        {
            ...
            if (!group.ActivableInfo.IncludeSoldOutOffers)
            {
                if (offerInfo.PerPlayerPurchaseLimitReached(GetOfferNumPurchasedByPlayer(offerInfo.OfferId)))
                    return false;

                if (offerInfo.PerGroupPurchaseLimitReached(group.GetOfferNumPurchasedInGroup(offerInfo.OfferId)))
                    return false;
            }
        ...
        /// <summary>
        /// Implementation for <see cref="RefreshPerOfferActivations"/> and <see cref="HasAnyPerOfferActivationsToRefresh"/>.
        /// If <paramref name="commit"/> is false, this acts as <see cref="HasAnyPerOfferActivationsToRefresh"/>; i.e.
        ...
        /// <summary>
        /// Whether any of the given group's offers are available for purchase in a new activation,
        ...
        /// </summary>
        protected bool HasAnyOffersAvailableForPurchaseInNewActivation(IPlayerModelBase player, TOfferGroupInfo offerGroupInfo)
        {
            MetaOfferGroupModelBase offerGroupMaybe = TryGetState(offerGroupInfo);
            int numOffersIncluded = 0;
            foreach (MetaOfferInfoBase offerInfo in offerGroupInfo.Offers.MetaRefUnwrap())
            {
                if (offerGroupInfo.MaxOffersActiveLimitReached(numOffersIncluded))
                    break;

                if (OfferIsEligibleForInclusionInNewActivation(player, offerGroupInfo, offerGroupMaybe, offerInfo))
                {
                    numOffersIncluded++;

                    if (!offerInfo.PerPlayerPurchaseLimitReached(GetOfferNumPurchasedByPlayer(offerInfo.OfferId))
                     && !offerInfo.PerGroupPurchaseLimitReached(offerGroupMaybe?.GetOfferNumPurchasedInGroup(offerInfo.OfferId) ?? 0))
                    {
                        return true;
                    }
                }
        ...
        protected bool OfferIsEligibleForInclusionInNewActivation(IPlayerModelBase player, TOfferGroupInfo offerGroupInfo, MetaOfferGroupModelBase offerGroupMaybe, MetaOfferInfoBase offerInfo)
        {
            if (offerInfo.PerPlayerActivationLimitReached(GetOfferNumActivatedByPlayer(offerInfo.OfferId)))
                return false;

            if (!offerGroupInfo.IncludeSoldOutOffers)
            {
                if (offerInfo.PerPlayerPurchaseLimitReached(GetOfferNumPurchasedByPlayer(offerInfo.OfferId)))
                    return false;
