...
namespace Metaplay.Core
{
    ...
    public struct MetaGuid : IEquatable<MetaGuid>, IComparable<MetaGuid>, IComparable
    {
        ...
        public static MetaGuid Parse(string str)
        {
            ...
            if (!TryParse(str, out MetaGuid metaGuid, out string error))
                throw new FormatException(error);

            return metaGuid;
        }

        public static bool TryParse(string str, out MetaGuid metaGuid, out string error)
        {
            if (str == null)
            {
                metaGuid = default;
            ...
            {
                metaGuid = default;
                error = $"Empty string is not a valid {nameof(MetaGuid)} format";
                return false;
            }
            ...
            {
                metaGuid = default;
                error = $"String '{str}' is not a valid {nameof(MetaGuid)} format";
                return false;
            }
            ...
            {
                metaGuid = default;
                error = $"String '{str}' has invalid reserved value (must be zero)";
                return false;
            }

            ulong high = (timestamp << 4) + reserved;
            metaGuid = new MetaGuid(new MetaUInt128(high, random));
            error = null;
            return true;
        }
