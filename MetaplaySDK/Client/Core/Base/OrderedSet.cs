...
#pragma warning restore IDE1006 // Naming Styles
        ...
        {
            return RemoveWhere((key, state) =>
                {
                    return ((Predicate<TKey>)state)(key);
                },
                state: predicate);
        }

        /// <summary>
        /// Calls predicate for each item in the set and the <paramref name="state"/>, and removes the items for which the predicate returns true.
        /// </summary>
        /// <returns>the number of items removed</returns>
        public int RemoveWhere(Func<TKey, object, bool> predicate, object state)
        {
            ++_version; // Even attempting to modify a container invalidates Enumerators
            ...
            while (true)
            {
                ...
                if (!predicate(_entries[walkNdx].key, state))
                {
        ...
        private void Resize(int newCapacity)
        {
            ...
            else
            {
                ...
                while (walkNdx != -1)
                {
                    ...
                    newEntry.key = _entries[walkNdx].key;
