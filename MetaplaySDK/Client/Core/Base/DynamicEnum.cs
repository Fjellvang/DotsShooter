...
namespace Metaplay.Core
{
    ...
    /// <summary>
    /// An attribute on DynamicEnum instances that declares a static method used to discover instances of the DynamicEnum.
    /// </summary>
    /// When provided, the `factoryMethod` attribute should name a static method on the class that enumerates all the
    /// possible values of the DynamicEnum. For example:
    /// <code>
    /// [DynamicEnumFactory(nameof(InstanceFactory))]
    /// public class FooEnumType : DynamicEnum&lt;FooEnumType>&gt;
    /// {
    ///     ...
    ///     static IEnumerable&lt;FooEnumType&gt; FactoryMethod()
    ///     {
    ///         return new[]
    ///         {
    ///             new FooEnumType(...),
    ...
    /// }
    /// </code>
    /// Note that the use of this attribute overrides the default behaviour of declaring values as static member variables.
    [AttributeUsage(AttributeTargets.Class, Inherited = false)]
    public sealed class DynamicEnumFactoryAttribute : Attribute
    {
        public string FactoryMethod { get; private set; }

        public DynamicEnumFactoryAttribute(string factoryMethod)
        {
    ...
    /// }                                                                                                                                                   <br/>
    /// </code>
    /// An alternative for listing the enum values as static member variables is to provide a DynamicEnum factory method, <see cref="DynamicEnumFactoryAttribute"/>.
    /// </summary>
        ...
        where TEnum : DynamicEnum<TEnum>
    {
        ...
        {
            // \note Negative id is forbidden, because MetaSerialization uses -1 to encode null DynamicEnum variables.
            if (id < 0)
                throw new ArgumentOutOfRangeException(nameof(id), $"{GetType().Name}'s id is {id}. Id cannot be negative.");
        ...
        private static List<TEnum> FindAllValues()
        {
            ...
            {
                IEnumerable<TEnum> instances;

                DynamicEnumFactoryAttribute factoryAttr = enumType.GetCustomAttribute<DynamicEnumFactoryAttribute>();
                if (factoryAttr != null)
                {
                    MethodInfo factoryMethod = enumType.GetMethod(factoryAttr.FactoryMethod, BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly);
                    if (factoryMethod == null)
                        throw new InvalidOperationException($"DynamicEnum factory method {factoryAttr.FactoryMethod} not found in class {enumType}");

                    instances = (IEnumerable<TEnum>)factoryMethod.InvokeWithoutWrappingError(obj: null, parameters: null);
                    if (enumType.GetStaticFieldsOfType<TEnum>().Count > 0)
                        throw new InvalidOperationException($"DynamicEnum class {enumType} has both a factory method ({factoryAttr.FactoryMethod}) and static instance variables!");
                }
                else
                {
                    instances = enumType.GetStaticFieldsOfType<TEnum>();
                }

                foreach (TEnum value in instances)
                {
                    // Skip invalid values
                    if (!value.IsValid)
                        continue;
