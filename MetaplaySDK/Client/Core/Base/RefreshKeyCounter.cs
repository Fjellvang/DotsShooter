...
namespace Metaplay.Core
{
    ...
    public class RefreshKeyCounter<TKey>
    {
        ...
        {
            readonly DateTime       _cycleStartAt;
            readonly TimeSpan       _refreshCycle;

            public CycleClock(TimeSpan refreshInterval)
            {
                _cycleStartAt = DateTime.UtcNow;
                _refreshCycle = refreshInterval;
            }

            public int GetCurrentCycle()
            {
                return (int)((long)(DateTime.UtcNow - _cycleStartAt).TotalMilliseconds / (long)_refreshCycle.TotalMilliseconds);
            }
        }
        ...
        /// </summary>
        public RefreshKeyCounter(TimeSpan refreshInterval)
        {
            _lock = new object();
