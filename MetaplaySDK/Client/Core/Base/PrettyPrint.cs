...
namespace Metaplay.Core
{
    ...
    public static class PrettyPrinter
    {
        ...
        {
            // Allow re-setting the same formatters
            _ = _customFormatters.TryAdd(type, formatter);
        }

        public static void RegisterFormatter<T>(Func<T, bool, string> formatter)
        {
            // Allow re-setting the same formatters
            _ = _customFormatters.TryAdd(typeof(T), (obj, isCompact) => formatter((T)obj, isCompact));
        }
            ...
            try
            {
                ...
                else // generic object
                {
                    ...
                    foreach (PropertyInfo prop in type.GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic))
                    {
                        ...
                        if (prop.GetGetMethod(true) == null || prop.GetGetMethod(true).IsStatic || prop.Name == "Descriptor" || prop.HasCustomAttribute<IgnoreDataMemberAttribute>(inherit: true))
                            continue;
                        ...
                        bool isMemberSensitive = prop.HasCustomAttribute<SensitiveAttribute>();
                    ...
                    foreach (var field in type.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic).Where(field => !field.Name.Contains("__BackingField")))
                    {
                        ...
                        if (field.IsStatic || field.HasCustomAttribute<IgnoreDataMemberAttribute>(inherit: true))
                            continue;
                        ...
                        PrettyPrintFlag memberFlags = (attrib != null) ? attrib.Flags : PrettyPrintFlag.None;
                        bool isMemberSensitive = field.HasCustomAttribute<SensitiveAttribute>();
