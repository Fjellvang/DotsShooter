...
namespace Metaplay.Core
{
    ...
    {
        void ResolveInPlace(IGameConfigDataResolver resolver);
    ...
    ///
    /// Unresolved MetaRefs should not be used as keys or elements
    /// for hashsets, during resolve they are updated in place and
    /// as such the hashcode can change.
    ///
    /// <para>
    ...
    /// or <see cref="ResolveInPlace"/> can be used to create
    /// or update a corresponding resolved MetaRef.
    ...
    /// (using <see cref="MetaSerialization.ResolveMetaRefs(Type, object, IGameConfigDataResolver, MetaSerializationSettings?)"/>
    ...
    ///       only supported when the key type is nullable
    ///       (either a reference type, or a <see cref="Nullable"/>),
    ///       or the IGameConfigData type defines static property ConfigNullSentinelKey.
        ...
        where TItem : class, IGameConfigData // \todo #generalized-reference
    {
        ...
        Type IMetaRefBase.ItemType => typeof(TItem);
        ...
        /// <summary>
        /// Using the given resolver, updates this MetaRef by resolving the key of this
        /// MetaRef. Note that the resolved MetaRef might have a different KeyObject
        /// than this one, due to config aliases. The resolved MetaRef will have the
        /// "canonical" key, i.e. the actual key of the item.
        /// </summary>
        public void ResolveInPlace(IGameConfigDataResolver resolver) // \todo #generalized-reference
        {
            ...
            _key  = KeyProperty.GetValue(item);
            _item = item;
        }
        ...
        {
            MetaRef<TItem> clone = FromKey(_key);
            clone.ResolveInPlace(resolver);
            return clone;
        }
    ...
    public static class MetaRefUtil
    {
        ...
        /// that <see cref="MetaSerialization.ResolveMetaRefs(Type, object, IGameConfigDataResolver, MetaSerializationSettings?)"/>
