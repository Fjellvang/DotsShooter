...
namespace Metaplay.Core
{
    ...
    /// </para>
    /// </summary>
    [System.Diagnostics.DebuggerTypeProxy(typeof(MetaDictionary<,>.DebugView))]
    [System.Diagnostics.DebuggerDisplay("Count = {Count}")]
    [System.Serializable]
    #if NET8_0_OR_GREATER
    [System.Runtime.CompilerServices.CollectionBuilder(typeof(CompilerSupport.MetaDictionaryBuilder), "Build")]
    #endif
    public class MetaDictionary<TKey, TValue>
...
#pragma warning restore IDE1006 // Naming Styles
        ...
        /// </summary>
        public MetaDictionary(IEnumerable<KeyValuePair<TKey, TValue>> enumerable)
        {
        ...
        /// If there are multiple mappings with the same key, an exception is thrown.
        /// </summary>
        public MetaDictionary(ReadOnlySpan<KeyValue> values)
        {
        ...
        /// If there are multiple mappings with the same key, an exception is thrown.
        /// </summary>
        public MetaDictionary(KeyValue[] array) : this(array.AsSpan()) { }
        ...
        {
            MetaDictionary<TKey, TValue> _container;
            int _ndx;
            uint _version;

            public IterationWalker(MetaDictionary<TKey, TValue> container)
            {
                _ndx = default;
                _version = default;
        ...
        public struct Enumerator : IEnumerator<KeyValuePair<TKey, TValue>>
        {
            ...
            public Enumerator(MetaDictionary<TKey, TValue> container)
            {
        ...
        public struct DictionaryEnumerator : IDictionaryEnumerator
        {
            ...
            public DictionaryEnumerator(MetaDictionary<TKey, TValue> container)
            {
                _walker = new IterationWalker(container);
                Reset();
            }
        ...
        public struct KeyEnumerator : IEnumerator<TKey>
        {
            ...
            public KeyEnumerator(MetaDictionary<TKey, TValue> container)
            {
                _walker = new IterationWalker(container);
                Reset();
            }
        ...
        public struct ValueEnumerator : IEnumerator<TValue>
        {
            ...
            public ValueEnumerator(MetaDictionary<TKey, TValue> container)
            {
                _walker = new IterationWalker(container);
                Reset();
            }
        ...
        public struct KeyCollection : IEnumerable<TKey>, ICollection<TKey>, ICollection
        {
            MetaDictionary<TKey, TValue> _container;
            uint _version;
            public KeyCollection(MetaDictionary<TKey, TValue> container)
            {
                _container = container;
                _version = _container._version;
            }
            ...
            {
                CheckVersion();
                MetaDictionary<TKey, TValue>.CopyEnumerableTo<TKey, KeyCollection>(array, arrayIndex, this);
            }
        }
        public struct ValueCollection : IEnumerable<TValue>, ICollection<TValue>, ICollection
        {
            MetaDictionary<TKey, TValue> _container;
            uint _version;
            public ValueCollection(MetaDictionary<TKey, TValue> container)
            {
                _container = container;
                _version = _container._version;
            }
            ...
            {
                CheckVersion();
                MetaDictionary<TKey, TValue>.CopyEnumerableTo<TValue, ValueCollection>(array, arrayIndex, this);
            }
        }
        ...
        private void Resize(int newCapacity)
        {
            ...
            else
            {
                ...
                while (walkNdx != -1)
                {
                    // If size increased over capacity, throw an exception
                    if (nextFreeIndex >= newCapacity)
                        throw new InvalidOperationException("Container was modified during resize");

                    int newBucketNdx = GetBucketIndex(hashcode: _entries[walkNdx].hashcode, capacity: newCapacity);
                    ref ChainEntry newEntry = ref newEntries[nextFreeIndex];

                    newEntry.bucketNext = newBuckets[newBucketNdx].headNdx;
                    newEntry.bucketPrev = BucketChainIndex.FromBucketIndex(newBucketNdx);
                    newEntry.iterationNext = nextFreeIndex + 1; // will point oob on last iteration. Fixed later.
                    newEntry.iterationPrev = nextFreeIndex - 1;
                    newEntry.hashcode = _entries[walkNdx].hashcode;
                    newEntry.kv = _entries[walkNdx].kv;

                    newBuckets[newBucketNdx].headNdx = nextFreeIndex;
                    if (newEntry.bucketNext != -1)
                        newEntries[newEntry.bucketNext].bucketPrev = BucketChainIndex.FromEntryIndex(nextFreeIndex);
        ...
        protected MetaDictionary(SerializationInfo info, StreamingContext context)
        {
        ...
        void ICollection.CopyTo(Array array, int arrayIndex) => CopyEnumerableTo<KeyValuePair<TKey, TValue>, MetaDictionary<TKey, TValue>>(array, arrayIndex, this);
        #endregion
        ...
        object IDictionary.this[object key]
        {
            ...
            set
            {
                ...
                else if (value == null && typeof(TValue).CanBeNull())
        ...
        bool TryCastToTypedKey(object key, out TKey typedKey)
        {
            ...
            else if (key == null && typeof(TKey).CanBeNull())
            {
        ...
        {
            private MetaDictionary<TKey,TValue> _container;
            private DebugView(MetaDictionary<TKey, TValue> container)
            {
                _container = container;
            }
    ...
    namespace CompilerSupport
    {
        ...
        /// </summary>
        public static class MetaDictionaryBuilder
        {
            public static MetaDictionary<TKey, TValue> Build<TKey, TValue>(ReadOnlySpan<MetaDictionary<TKey, TValue>.KeyValue> values) => new MetaDictionary<TKey, TValue>(values);
        }
        #endif
    }
    ...
    public class OrderedDictionary<TKey, TValue> : MetaDictionary<TKey, TValue>
    {
        /// <summary>
        /// Creates a new empty dictionary.
        /// </summary>
        ...
        /// If there are multiple mappings with the same key, an exception is thrown.
        /// </summary>
        public OrderedDictionary(IEnumerable<KeyValuePair<TKey, TValue>> enumerable) : base(enumerable) { }

        /// <summary>
        /// Creates a new dictionary with the elements of <paramref name="values"/>.
        /// If there are multiple mappings with the same key, an exception is thrown.
        /// </summary>
        public OrderedDictionary(ReadOnlySpan<KeyValue> values) : base(values) { }

        /// <summary>
        /// Creates a new dictionary with the elements of <paramref name="array"/>.
        /// If there are multiple mappings with the same key, an exception is thrown.
        /// </summary>
        public OrderedDictionary(KeyValue[] array) : base(array) { }
    }
