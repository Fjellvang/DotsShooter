...
namespace Metaplay.Core
{
    public static class Util
    {
        ...
        {
            Span<char> c = bytes.Length <= 128 ? stackalloc char[bytes.Length * 2] : new char[bytes.Length * 2];
        ...
        /// <summary>
        /// Compute a combined hash of a sequence of elements.
        /// </summary>
        /// <typeparam name="T">Type of element</typeparam>
        /// <param name="sequence">Sequence of elements to compute hash code for</param>
        /// <returns></returns>
        public static int ComputeSequenceHash<T>(IEnumerable<T> sequence)
        {
            HashCode hash = new();
        ...
        /// <summary>
        /// Attemps to parse exception type, message and stacktrace from a string. Returns an <see cref="ExceptionDetails"/>
        /// object with type, message and stacktrace strings. Returns null if parsing was unsuccessful.
        /// Expects exception string to match what Exception.ToString() produces by default, otherwise may provide unexpected results.
        /// This should only be used for human visualization, and not in places where absolutely accurate results are needed.
        /// </summary>
        public static ExceptionDetails ParseExceptionDetailsFromString(string exceptionString)
        {
            string exceptionType        = "";
            string messageAndStackTrace = "";
            // Match first non-word, non-period character to find the end of the exception type
            Match  exceptionTypeEnd = Regex.Match(exceptionString, "[^a-zA-Z0-9_+`\\[\\].]");
            if (exceptionTypeEnd.Success)
            {
                exceptionType        = exceptionString.Substring(0, exceptionTypeEnd.Index);
                messageAndStackTrace = exceptionString.Substring(exceptionTypeEnd.Index);
                // Remove any leading ":"
                if (messageAndStackTrace.StartsWith(":", StringComparison.Ordinal))
                {
                    messageAndStackTrace = messageAndStackTrace.Substring(1);
                }
            }
            else
            {
            ...
            // Find start of stacktrace
            Match stackTraceMatch = Regex.Match(messageAndStackTrace, "\n\\s+at\\s");

            string message    = "";
            string stackTrace = "";
            // Separate message and stacktrace
            if (stackTraceMatch.Success)
            {
                message = messageAndStackTrace.Substring(0, stackTraceMatch.Index);
                message = message.Trim();

                stackTrace = messageAndStackTrace.Substring(stackTraceMatch.Index);
        ...
        public static string TruncateStacktrace(string stackTrace)
        {
            ...
            const int maxStacktraceLength = 8*1024;
            return (stackTrace.Length > maxStacktraceLength) ? stackTrace.Substring(0, maxStacktraceLength) : stackTrace;
        }
    }
    ...
    public static class EnumUtil
    {
        ...
        /// <returns></returns>
        public static int ToInt<TEnum>(TEnum e) where TEnum : struct, Enum
        {
#if NETCOREAPP
