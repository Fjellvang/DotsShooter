...
namespace Metaplay.Core
{
    ...
    {
        /// <summary>
        /// Used for skipping forward in time for testing and debugging purposes.
        /// This skip is included in <see cref="MetaTime.Now"/> and can be controlled
        /// via the LiveOps Dashboard.
        /// <para>
        /// The time skip cannot be decreased without resetting the database, so this
        /// should only ever be used in development environments!
        /// </para>
        /// <para>
        /// When using time skipping, features may work in an unexpected or incorrect
        /// manner if they either do not account for the possibility of <see cref="MetaTime.Now"/>
        /// skipping forward by a large amount, or improperly mix real time (<see cref="DateTime.UtcNow"/>)
        /// with <see cref="MetaTime.Now"/>.
        /// </para>
        /// </summary>
        public static MetaDuration DebugTimeOffset
        {
            get => MetaDuration.FromMilliseconds(Interlocked.Read(ref s_debugTimeOffsetMilliseconds));
            set { Interlocked.Exchange(ref s_debugTimeOffsetMilliseconds, value.Milliseconds); }
        }
        static long s_debugTimeOffsetMilliseconds = 0;
    ...
    public static class DateTimeExtensionsForMetaTime
    {
        /// <summary>
        /// Equivalent to calling <see cref="MetaTime.FromDateTime"/> with <paramref name="dateTime"/>.
        /// Provided as an extension method for convenience, especially with nullable <see cref="DateTime"/>s.
        /// </summary>
        public static MetaTime ToMetaTime(this DateTime dateTime) => MetaTime.FromDateTime(dateTime);
    }
}
