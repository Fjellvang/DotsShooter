...
namespace Metaplay.Core.Model
{
    ...
    public static class JournalUtil
    {
        ...
        public static uint ComputeChecksum(IOBuffer buffer, IModel model)
        {
            ...
            return ChecksumUtil.ComputeHash(buffer);
        }
    }
        ...
        where TModel : class, IModel<TModel>
    {
        public abstract class Leader
        {
            ...
            class JournalChecksumEvaluator : IChecksumContext
            {
                ...
                int                                             _operation;
                int                                             _stepIndex;
                MetaDictionary<JournalPosition, StagedStep>  _dstSteps;
                ...
                public JournalChecksumEvaluator(JournalChecksumHelper checksumHelper, FlatIOBuffer recycledChecksumBuffer, ref MetaDictionary<JournalPosition, StagedStep> dstSteps)
                {
            ...
            public interface IListener
            {
                ...
                /// </para>
                /// </summary>
                void AfterTick(long tick, MetaActionResult result, uint checksumAfter, IOBuffer checksumSerializationBuffer);
            ...
            public Exception                        LastActionOrTickError       { get; private set; }
            public long                             LastActionOrTickErrorOnTick { get; private set; }
            public string                           LastActionOrTickTypeName    { get; private set; }
            ...
            public void StageTick()
            {
                ...
                if (_executingTick != -1)
                    throw new InvalidOperationException($"Trying to execute Tick() from within another Tick() call (tick: {_executingTick}). Tick is not allowed to call another Tick.");
                else if (_executingAction != null)
                    throw new InvalidOperationException($"Trying to execute Tick() from within Action Execute(). Actions are not allowed to call Tick(). (action: {PrettyPrint.Compact(_executingAction)})");

                try
                {
                    // Mark as executing action
                    _executingTick = StagedPosition.Tick + 1;
                ...
                {
                    _log.Error("Tick threw an exception. Ticks should not throw since they may leave model timeline in a broken state: Error: {Error}.", ex);
                    LastActionOrTickError = ex;
                    LastActionOrTickErrorOnTick = StagedPosition.Tick;
                    LastActionOrTickTypeName = "Tick";
                    throw;
                }
                finally
                {
            ...
            public MetaActionResult StageAction(ModelAction action)
            {
                ...
                if (_executingTick != -1)
                {
                    throw new InvalidOperationException(
                        $"Trying to execute Action from within a Tick() call. Ticks are not allowed to run Actions directly. " +
                        $"This is most likely either trying to run an action from a background thread or trying to run an action from ServerListener or ClientListener. " +
                        $"In that case, you should enqueue the Action instead, for example via MetaplaySDK.RunOnUnityThreadAsync(() => PlayerContext.ExecuteAction(action)). " +
                        $"(action: {PrettyPrint.Compact(action)}). ");
                }
                else if (_executingAction != null)
                {
                    throw new InvalidOperationException(
                        $"Trying to execute Action from within another action. Actions are not allowed to run other Actions directly. " +
                        $"This is most likely either trying to run an action from a background thread or trying to run an action from ServerListener or ClientListener. " +
                        $"In that case, you should enqueue the Action instead, for example via MetaplaySDK.RunOnUnityThreadAsync(() => PlayerContext.ExecuteAction(action)). " +
                        $"(Action attempted to be run: {PrettyPrint.Compact(action)}, action that was running: {PrettyPrint.Compact(_executingAction)}).");
                }
                ...
                {
                    _log.Error("Action {ActionType} threw an exception. Actions should not throw since they may leave model timeline in a broken state: Error: {Error}.", action.GetType().ToGenericTypeString(), ex);
                    LastActionOrTickError = ex;
                    LastActionOrTickErrorOnTick = StagedPosition.Tick;
                    LastActionOrTickTypeName = action.GetType().ToGenericTypeString();
                    throw;
                }
                finally
                {
        ...
        public abstract class Follower
        {
            ...
            class ExpectedStepEvaluator : IChecksumContext
            {
                ...
                public ExpectedStepEvaluator(ref MetaDictionary<JournalPosition, StagedStep>  dstSteps)
                {
                    _dstSteps = dstSteps;
                }
            ...
            class ExpectedStepValidator : IChecksumContext
            {
                ...
                bool                                            _bufferIsLocked;
                MetaDictionary<JournalPosition, StagedStep>  _srcSteps;
                long                                            _tick;
                int                                             _operation;
                int                                             _stepIndex;
                bool                                            _hasConflict;
                ...
                public ExpectedStepValidator(MetaDictionary<JournalPosition, StagedStep> srcSteps)
                {
            ...
            public interface IListener
            {
                ...
                /// Called just after a Tick() operation is applied.
                /// </summary>
                void AfterTick(long tick, MetaActionResult result);
            ...
            public Follower(LogChannel log, bool computeChecksums = true)
            {
                _log = log;
                _timeline = new Timeline<TModel, StagedOp, StagedStep>(new JournalChecksumHelper(computeChecksums));
                _recycledChecksumBuffer = new FlatIOBuffer();
            ...
            CommitResult CommitOperationRange(TimelineCommitPlan commitPlan, JournalPosition commitPosition)
            {
                ...
                CommitResult result;
                uint actualChecksum = _timeline.ChecksumHelper.ComputeChecksum(_recycledChecksumBuffer, commitModel);
