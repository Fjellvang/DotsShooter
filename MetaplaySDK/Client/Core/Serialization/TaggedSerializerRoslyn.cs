...
namespace Metaplay.Core.Serialization
{
    ...
    public class TaggedSerializerRoslyn
    {
        ...
        delegate void TraverseMetaRefsInObjectDelegate(ref MetaSerializationContext context, Type type, object value);
        ...
        readonly GetTypeInfoMethod                _getTypeInfoDelegateMaybe;

        TaggedSerializerRoslyn(Type generatedSerializer)
        {
            ...
            MethodInfo getTypeInfoMethod = generatedSerializer.GetMethod("GetTypeInfo");
            if (getTypeInfoMethod != null)
                _getTypeInfoDelegateMaybe = (GetTypeInfoMethod)getTypeInfoMethod.CreateDelegate(typeof(GetTypeInfoMethod));
        }
        ...
        {
            T result = (T)_readObjectDelegate(ref context, reader, typeof(T));
            context.DebugAssertZeroDepth();
            return result;
        }
        ...
        {
            object result = _readObjectDelegate(ref context, reader, type);
            context.DebugAssertZeroDepth();
            return result;
        }
        ...
        {
            IReadOnlyList<T> result = (IReadOnlyList<T>)_readTableDelegate(ref context, reader, type, maxCollectionSizeOverride);
            context.DebugAssertZeroDepth();
            return result;
        }

        public void TraverseMetaRefs(ref MetaSerializationContext context, Type type, object obj)
        {
            _traverseMetaRefsInObjectDelegate(ref context, type, obj);
            context.DebugAssertZeroDepth();
        }

        public void TraverseMetaRefs<T>(ref MetaSerializationContext context, T value)
        {
            _traverseMetaRefsInObjectDelegate(ref context, typeof(T), value);
            context.DebugAssertZeroDepth();
        }
        ...
        public bool HasTypeInfoMethod => _getTypeInfoDelegateMaybe != null;

        public MetaSerializerTypeInfo GetTypeInfo()
        {
            MetaSerializerTypeInfo typeInfo = new MetaSerializerTypeInfo();
            typeInfo.Specs        = new Dictionary<Type, MetaSerializableType>();
            typeInfo.FullTypeHash = _getTypeInfoDelegateMaybe(typeInfo.Specs);
            return typeInfo;
        }

        public static TaggedSerializerRoslyn CreateFromAssembly(Assembly assembly, bool forceRunInitEagerly)
        {
            ...
            return new TaggedSerializerRoslyn(generatedSerializer);
        }

        public static TaggedSerializerRoslyn CreateFromAssemblyByName(string assemblyName, bool forceRunInitEagerly)
        {
            Assembly assembly = Assembly.Load(assemblyName);
            if (assembly == null)
                throw new InvalidOperationException($"Could not load {assemblyName}.dll");
            return CreateFromAssembly(assembly, forceRunInitEagerly);
        }
    }
}
