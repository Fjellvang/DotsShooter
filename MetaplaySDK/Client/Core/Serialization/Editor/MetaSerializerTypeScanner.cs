...
{
    /// <summary>
    /// Runtime reflection based serializer type info provider. Used in Server, BotClient and Unity Editor.
    /// </summary>
    public class MetaSerializerTypeScanner
    {
        Dictionary<Type, MetaSerializableType> _typeSpecs;
        // Current set of scanned types as a MetaSerializerTypeRegistry for internal use
        MetaSerializerTypeRegistry             _registry;
        ...
        internal static readonly Dictionary<Type, List<MemberInfo>> TypeMemberOverrides = new Dictionary<Type, List<MemberInfo>>()
        {
        ...
        void PropagatePublic(MetaSerializableType typeSpec)
        {
            ...
            // For MetaRef<> and MetaConfigId<>, recurse into key type
            if (type.IsGenericTypeOf(typeof(MetaRef<>)) || type.IsGenericTypeOf(typeof(MetaConfigId<>)))
            {
        ...
        void ValidateTypeSpec(MetaSerializableType typeSpec)
        {
            ...
            if (typeSpec.IsConcreteObject)
            {
                ...
                MetaDictionary<MetaSerializableType, TypeMemberTagReservation> typeMemberReservations = new MetaDictionary<MetaSerializableType, TypeMemberTagReservation>();
            ...
            if (typeSpec.IsConcreteObject && _attrCache.TryGetSealedAttributeOnTypeOrAncestor<MetaAllowNoSerializedMembers>(typeSpec.Type) == null
                && !typeSpec.IsSystemNullable && !typeSpec.IsGameConfigData && !typeSpec.IsGameConfigDataContent && !typeSpec.IsMetaRef && !typeSpec.IsMetaConfigId)
            {
        ...
        MetaSerializableAttribute InternalTryGetMetaSerializableAttribute(Type type)
        {
            ...
            foreach (Type interfaceType in type.GetInterfaces())
            {
                ...
                if (attributeInInterface != null)
                {
                    ...
                    if (!_attrCache.IsMetaFeatureEnabled(interfaceType))
                        continue;
            ...
            //        allows type-checks to perform more complex operation that are possible during early type scan.
            if (!_attrCache.IsMetaFeatureEnabled(type))
                return null;
            ...
            [DoesNotReturn]
            void ThrowMissingAttribute(Type type, Type baseType, Type attributeType)
            {
                throw new InvalidOperationException($"Type {type.ToGenericTypeString()} extends a {baseType.ToGenericTypeString()} base class but is missing a [{attributeType.ToGenericTypeString()}] attribute.");
            }
            ...
            {
                if (resultAttributeSourceType.Implements<MetaMessage>())
                    ThrowMissingAttribute(type, typeof(MetaMessage), typeof(MetaMessageAttribute));
                else if (resultAttributeSourceType.Implements<LiveOpsEventContent>())
                    ThrowMissingAttribute(type, typeof(LiveOpsEventContent), typeof(LiveOpsEventAttribute));
                else if (resultAttributeSourceType.Implements<AnalyticsEventBase>())
                    ThrowMissingAttribute(type, typeof(AnalyticsEventBase), typeof(AnalyticsEventAttribute));
                else if (resultAttributeSourceType.Implements<ModelAction>())
                    ThrowMissingAttribute(type, typeof(ModelAction), typeof(ModelActionAttribute));
                else if (resultAttributeSourceType.IsAbstract)
        ...
        MetaSerializableType InternalTryRegisterType(Type type)
        {
            ...
            {
                WireDataType wireType = TaggedWireSerializer.GetWireType(type, _registry);
                ...
                {
                    (Type keyType, Type valueType) = type.GetDictionaryKeyAndValueTypes();
                    if (keyType.ImplementsInterface<IMetaRef>())
                        throw new InvalidOperationException("MetaRef's are not supported as keys for dictionary types.");
                ...
                else // value collection
                {
                    ...
                    if (type.Implements(typeof(OrderedSet<>))
                        || type.Implements(typeof(HashSet<>))
                        || type.Implements(typeof(SortedSet<>))
                        || type.Implements(typeof(ImmutableSortedSet<>)))
                    {
                        if (elemType.Implements<IMetaRef>())
                            throw new InvalidOperationException("MetaRef's are not supported as items in hash based collections.");
                    }
            ...
            else if (type.IsGenericTypeOf(typeof(Nullable<>)))
            {
                ...
                WireDataType wireType = elemType.IsEnum
                                        ? TaggedWireSerializer.PrimitiveTypeWrapNullable(TaggedWireSerializer.GetWireType(elemType.GetEnumUnderlyingType(), _registry))
            ...
            else if (type.IsGenericTypeOf(typeof(MetaRef<>)) || type.IsGenericTypeOf(typeof(MetaConfigId<>)))
            {
                Type            infoType    = type.GetGenericArguments()[0];
                ...
                WireDataType wireType = TaggedWireSerializer.GetWireType(keyType, _registry);
            ...
            {
                WireDataType wireType = TaggedWireSerializer.GetWireType(type.GetEnumUnderlyingType(), _registry);
                MetaSerializableType typeSpec = new MetaSerializableType(type, wireType, serializableAttrib != null);
            ...
            else if ((serializableAttrib != null || isGameConfigData) && (type.IsClass || type.IsInterface || (type.IsValueType && !type.IsEnum && !type.IsPrimitive))) // user/config class/interface/struct
            {
                ...
                typeSpec.AddedInVersion = type.GetCustomAttribute<AddedInVersionAttribute>()?.LogicVersion;

                #if NETCOREAPP
                int maxLogicVersion = MetaplayCore.Options.SupportedLogicVersions.MaxVersion;
                if (typeSpec.AddedInVersion > maxLogicVersion)
                    throw new InvalidOperationException($"Type '{type.FullName}' was added in version {typeSpec.AddedInVersion} however the max supported logic version is {maxLogicVersion}. This is not supported, please lower or remove the AddedInVersion.");
                #else
                int maxLogicVersion = MetaplayCore.Options.ClientLogicVersion;
                if (typeSpec.AddedInVersion > maxLogicVersion)
                    throw new InvalidOperationException($"Type '{type.FullName}' was added in version {typeSpec.AddedInVersion} however the max supported logic version is {maxLogicVersion} in this client. This is not supported, please lower or remove the AddedInVersion.");
                #endif
        ...
        static bool IsPublicNamespace(string nsToCheck, string[] sharedNamespaces)
        {
            return sharedNamespaces.Any(sharedNs => nsToCheck.StartsWith(sharedNs, StringComparison.Ordinal))
                || (nsToCheck.StartsWith("Metaplay.Core", StringComparison.Ordinal) && !nsToCheck.StartsWith("Metaplay.Core.Tests", StringComparison.Ordinal));
        }
        ...
        protected virtual IEnumerable<Type> AllTypes(Assembly[] relevantAssemblies)
        {
            return relevantAssemblies.SelectMany(assembly => assembly.GetTypes());
        }

        void ScanTypes(string[] sharedNamespaces)
        {
            ...
            using (ProfilerScope.Create("MetaSerializerTypeScanner.RegisterTypes"))
            {
                ...
                foreach (Type type in AllTypes(assemblies))
                {
            ...
            using (ProfilerScope.Create("MetaSerializerTypeScanner.ScanTypeProviders"))
            {
                ...
                foreach (Type providerType in AllTypes(assemblies))
                {
                    ...
                    if (typeSpec.IsConcreteObject)
                    {
                        ...
                        foreach (MemberInfo metaMember in metaMembers)
                        {
                            ...
                            if (setValue == null && !typeSpec.UsesConstructorDeserialization)
                                throw new InvalidOperationException($"[MetaMember] {typeSpec.Name}.{metaMember.Name} does not have a setter, this is only supported using constructor based deserialization! Give it a setter or enable constructor based deserialization (use either [MetaDeserializationConstructor] or [MetaSerializable(MetaSerializableFlags.AutomaticConstructorDetection)]).");

                            if (metaMember is FieldInfo field && field.IsInitOnly && !typeSpec.UsesConstructorDeserialization)
                                throw new InvalidOperationException($"[MetaMember] Field {typeSpec.Name}.{field.Name} is readonly, this is only supported using constructor based deserialization! Make it writable or enable constructor based deserialization (use either [MetaDeserializationConstructor] or [MetaSerializable(MetaSerializableFlags.AutomaticConstructorDetection)]).");
            ...
            {
                foreach (MetaSerializableType typeSpec in _typeSpecs.Values.Where(x => x.UsesConstructorDeserialization))
                {
                    (ConstructorInfo bestMatching, List<MetaSerializableMember> constructorInitializedMembers) = typeSpec.FindBestMatchingConstructor();

                    typeSpec.DeserializationConstructor    = bestMatching;
                    typeSpec.ConstructorInitializedMembers = constructorInitializedMembers;
                    typeSpec.ConstructorParameterValues    = typeSpec.CollectConstructorParameterValues();
                }
            }
        ...
        static uint ComputeProtocolHash(IEnumerable<MetaSerializableType> typeSpecs)
        {
            ...
            #if DEBUG_PRINT_PROTOCOL_HASH
            System.IO.StreamWriter dumpStream = new System.IO.StreamWriter(new System.IO.FileStream("Metaplay_Protocol_Hash_"
                #if UNITY_EDITOR
                + "Unity",
                #else
                + "Server",
            ...
            for (int typeNdx = 0; typeNdx < sortedTypes.Count; typeNdx++)
            {
                ...
                #if DEBUG_PRINT_PROTOCOL_HASH
                DebugLog.Debug("Hash for #{0} {1}: 0x{2:X8} (cumulative=0x{3:X8})", typeNdx, typeSpec.Name, typeHash, fullProtocolHash);
                dumpStream.WriteLine(FormattableString.Invariant($"Hash for #{typeNdx} {typeSpec.Name}: 0x{typeHash:X8} (cumulative=0x{fullProtocolHash:X8})"));
                #endif
            }
        ...
        public MetaSerializerTypeInfo GetTypeInfo()
        {
            ...
            _registry = new MetaSerializerTypeRegistry(
                new MetaSerializerTypeInfo()
                {
                    FullTypeHash = 0,
                    Specs        = _typeSpecs,
                });
            ScanTypes(MetaplayCore.Options.SharedNamespaces);
            MetaSerializerTypeInfo ret = new MetaSerializerTypeInfo();
