...
namespace Metaplay.Core.Serialization
{
    public class TaggedSerializedInspector : TaggedWirePushParser
    {
        public class ObjectInfo
        {
            ...
            /// </summary>
            public MetaDictionary<ObjectInfo, ObjectInfo> KeyValueCollection;
        ...
        TaggedSerializedInspector(MetaSerializerTypeRegistry typeRegistry)
        {
        ...
        {
            TaggedSerializedInspector inspector = new TaggedSerializedInspector(MetaplayServices.Get<MetaSerializerTypeRegistry>());
        ...
        MetaSerializableType TopSerializeableSpecAsConcreteSpec
        {
            get
            {
                ...
                if (!_typeRegistry.TryGetSerializableType(elementType, out MetaSerializableType typeSpec))
                    return null;
                return typeSpec;
            }
        }
        ...
        void SetNextType(Type type)
        {
            if (type != null)
            {
                if (_typeRegistry.TryGetSerializableType(type, out MetaSerializableType typeSpec))
                {
        ...
        protected override void OnEndParsePrimitive(IOReader reader, WireDataType wireType, object obj)
        {
            ...
            {
                if (TaggedWireSerializer.IsBuiltinType(_nextRawType) && TaggedWireSerializer.GetWireType(_nextRawType, _typeRegistry) == wireType)
                {
        ...
        protected override void OnBeginParseAbstractStruct(IOReader reader, int typecode)
        {
            ...
            if (typecode != 0
                ...
                && _typeRegistry.TryGetSerializableType(derivedType, out MetaSerializableType derivedSpec))
            {
        ...
        protected override void OnBeginParseKeyValueCollection(IOReader reader, int numElements, WireDataType keyType, WireDataType valueType)
        {
            ...
            if (numElements >= 0)
                Top.KeyValueCollection = new MetaDictionary<ObjectInfo, ObjectInfo>(capacity: numElements);
        ...
        protected override void OnBeginParseStructMember(IOReader reader, WireDataType wireType, int tagId)
        {
            ...
            if (TopSerializeableSpecAsConcreteSpec != null && TopSerializeableSpecAsConcreteSpec.Members != null)
            {
                foreach (MetaSerializableMember memberInfo in TopSerializeableSpecAsConcreteSpec.Members)
                {
                    if (memberInfo.TagId == tagId)
                    {
