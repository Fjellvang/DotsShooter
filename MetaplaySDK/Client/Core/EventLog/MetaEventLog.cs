...
namespace Metaplay.Core.EventLog
{
    ...
    public abstract class MetaEventLogEntry
    {
        ...
        /// <summary> <see cref="MetaTime.Now"/> when the entry was added. </summary>
    ...
    public abstract class MetaEventLog<TEntry> where TEntry : MetaEventLogEntry
    {
        ...
        /// <summary>
        /// The <see cref="MetaEventLogEntry.UniqueId"/> of the last entry of the previous segment.
        /// By storing this in each segment, we can get the "unique id of the previous entry" for the
        /// first entry of the segment, without needing to fetch (possibly from the database) the
        /// previous segment.
        /// </summary>
        /// <remarks>
        /// - "Unique id of the previous entry" is used in event log scan desync detection.<br/>
        /// - Zero value here should be understood as "unknown" rather than actual zero:
        ///   this is zero rather than a proper value if the segment was persisted before this field was added.<br/>
        /// </remarks>
        [MetaMember(107)] public MetaUInt128                        PreviousSegmentLastEntryUniqueId = MetaUInt128.Zero;
        /// <summary>
