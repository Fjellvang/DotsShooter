...
namespace Metaplay.Core.MultiplayerEntity
{
    ...
    public abstract class MultiplayerEntityClientBase : IMultiplayerEntityClient
    {
        ...
        /// </summary>
        protected virtual string LogChannelName => ClientSlot.Name.ToLowerInvariant();
        ...
        public virtual void OnSessionStart(SessionProtocol.SessionStartSuccess successMessage, ClientSessionStartResources sessionStartResources)
        {
            ...
            _enableConsistencyChecks = successMessage.PlayerState.DebugConfig.ClientConsistencyChecks;
            _logicVersion            = -1;
            _entityMessageDispatcher.ResetPeer();
            ...
            if (initialEntityIndex != -1)
            {
                ...
                if (gameConfig != null)
                {
                    ActivateWithState(initialState, gameConfig);
                }
                else
                {
                    ...
                    Context = null;
                    InvokePhaseChanged();
                    BeginWaitForNewEntityConfigLoad(initialState);
                }
            }
            else
            {
        ...
        void ActivateWithState(EntityInitialState state, ISharedGameConfig gameConfig)
        {
            ...
            Context?.OnEntityDetached();
            Context = null;
            _entityMessageDispatcher.ResetPeer(keepBufferedData: true);
        ...
        void OnEntitySwitchedMessage(EntitySwitchedMessage switched)
        {
            ...
            if (switched.OldChannelId != -1 && switched.OldChannelId == _entityMessageDispatcher.ChannelId)
            {
                if (Context != null)
                {
                    ...
                    // As there is nothing on the channel, reset the dispatcher
                    _entityMessageDispatcher.ResetPeer();
                }
            ...
            if (switched.NewState != null && (wasDetached || ShouldAttachToNewEntityChannel(switched.NewState)))
            {
                ...
                Context = null;
                InvokePhaseChanged();
                BeginWaitForNewEntityConfigLoad(switched.NewState);
            }
        }

        void BeginWaitForNewEntityConfigLoad(EntityInitialState newState)
        {
            // Start downloading the config and buffer all updates while at it.
            _waitingConfigs = Services.GetConfigAsync(newState.State);
            _waitingState = newState;

            // Reset old channel and start buffering new channel messages.
            _entityMessageDispatcher.ResetPeer();
            _entityMessageDispatcher.StartPeerBuffering(newState.ChannelId);
        }
        ...
        /// Conventional implementation will look into <see cref="EntityClientData.ClientSlot"/> in <see cref="EntityInitialState.ClientData"/>, or <see cref="EntityInitialState.State"/>
        ...
        protected virtual bool ShouldAttachToNewEntityChannel(EntityInitialState initialState) =>
            initialState.ClientData.ClientSlot == ClientSlot;
        ...
        where TModel : class, IMultiplayerModel<TModel>
    {
        ...
        protected override MultiplayerEntityClientContext<TModel> CreateActiveModelContext(EntityInitialState state, ISharedGameConfig gameConfig)
        {
            TModel model = DefaultDeserializeModel(state.State, gameConfig);
            return new MultiplayerEntityClientContext<TModel>(DefaultInitArgs(model, state));
        }
    }
