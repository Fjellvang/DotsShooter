...
namespace Metaplay.Core.MultiplayerEntity
{
        ...
        where TModel : class, IMultiplayerModel<TModel>
    {
        ...
        public MultiplayerEntityClientContext(ClientMultiplayerEntityContextInitArgs initArgs)
        {
            ...
            long        currentTick             = Journal.StagedModel.CurrentTick;
        ...
        void OnTimelineUpdateMessage(EntityTimelineUpdateMessage msg)
        {
            ...
            long updateTick = GetLatestServerKnownTick();
        ...
        MetaTime GetPresentationTimeOfTick(long tick)
        {
        ...
        void ExtractOpsForPlayback(MetaTime currentTime, List<PlaybackOp> outOps, out bool outAdvancingWasThrottledByFrameLimit)
        {
            ...
            long maxNumTicksPerFrame = System.Math.Max(10, stagedModel.TicksPerSecond);
            long numTicksGathered = 0;
            for (;;)
            {
                ...
                long        nextTick                    = stagedModel.CurrentTick + numTicksGathered + 1;
        ...
        bool UpdatePlayback(MetaTime currentTime)
        {
            ...
            long            nextTick                    = Journal.StagedModel.CurrentTick + 1;
            ...
            if (PlaybackMode == ClientPlaybackMode.Smooth)
            {
                ...
                long        nextUpdateContainsTick          = GetLatestServerKnownTick() + 1;
            ...
            else
            {
                ...
                long            furthestTick                    = GetLatestServerKnownTick();
        ...
        /// </summary>
        public long GetLatestServerKnownTick()
        {
            long tick = Journal.StagedModel.CurrentTick;
