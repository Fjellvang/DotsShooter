...
namespace Metaplay.Core
{
    ...
    {
        public static ConfigParser Instance => MetaplayServices.Get<ConfigParser>();

        Dictionary<Type, ParseFunc> _parseFuncs = new Dictionary<Type, ParseFunc>();

        public ConfigParser(IIntegrationRegistry integrationRegistry, GameConfigRepository gameConfigRepository)
        {
            RegisterBasicTypeParsers();
            RegisterGameConfigs(gameConfigRepository);
            RegisterCustomParsers(integrationRegistry);
        }

        public bool TryParse(Type type, ConfigLexer lexer, out object result)
        {
            if (_parseFuncs.TryGetValue(type, out ParseFunc parseFunc))
            {
        ...
        public static T EnsureExact<T>(Func<ConfigLexer, T> parseFunc, string input)
        {
        ...
        public T ParseExact<T>(string input)
        {
            ConfigLexer lexer = new ConfigLexer(input);
        ...
        public T Parse<T>(ConfigLexer lexer)
        {
        ...
        void RegisterParseFunc(Type type, ParseFunc parseFunc)
        {
            ...
            _parseFuncs.Add(type, parseFunc);
        }

        void RegisterParseFunc<T>(ParseFunc parseFunc)
        {
            RegisterParseFunc(typeof(T), parseFunc);
        }

        public void RegisterCustomParseFunc<T>(ParseFunc parseFunc)
        {
            ...
            for (Type ancestor = typeof(T).BaseType;
                ancestor.HasCustomAttribute<UseCustomParserFromDerivedAttribute>(inherit: false);
        ...
        // Variants for parse functions that require access to parser instance

        void RegisterParseFunc<T>(Func<ConfigParser, ConfigLexer, object> parseFunc)
        {
            RegisterParseFunc(typeof(T), lexer => parseFunc(this, lexer));
        }

        public void RegisterCustomParseFunc<T>(Func<ConfigParser, ConfigLexer, object> parseFunc)
        {
            RegisterCustomParseFunc<T>(lexer => parseFunc(this, lexer));
        }

        public bool HasRegisteredParseFunc(Type type)
        {
            return _parseFuncs.ContainsKey(type);
        }

        void RegisterBasicTypeParsers()
        {
        ...
        bool TryParseBuiltin(Type type, ConfigLexer lexer, out object result)
        {
        ...
        void RegisterGameConfigs(GameConfigRepository gameConfigRepository)
        {
            foreach (GameConfigTypeInfo typeInfo in gameConfigRepository.AllGameConfigTypes)
            {
        ...
        void RegisterGameConfigLibrary(Type type)
        {
        ...
        void RegisterGameConfigLibrary(Type keyType, Type infoType)
        {
            ...
            {
                if (!_parseFuncs.ContainsKey(infoTypeAncestor))
                    RegisterGameConfigInfoReferenceTypeParsers(keyType, infoTypeAncestor);
            }
        }

        void RegisterCustomParsers(IIntegrationRegistry integrations)
        {
            foreach (ConfigParserProvider parserProvider in integrations.CreateAll<ConfigParserProvider>())
            {
                parserProvider.RegisterParsers(this);
            }
        }

        void RegisterGameConfigInfoReferenceTypeParsers(Type keyType, Type infoType)
        {
            ...
            // Register MetaRef and MetaConfigId parsers
            RegisterMetaRefParsers(keyType, infoType, typeof(MetaRef<>));
            RegisterMetaRefParsers(keyType, infoType, typeof(MetaConfigId<>));
        }

        void RegisterMetaRefParsers(Type keyType, Type infoType, Type refType)
        {
            if (!refType.IsGenericTypeOf(typeof(MetaRef<>)) && !refType.IsGenericTypeOf(typeof(MetaConfigId<>)))
                throw new ArgumentException($"Invalid refType {refType.ToGenericTypeString()}");

            Type genericRefType = refType.MakeGenericType(infoType);
            RegisterParseFunc(genericRefType, (lexer) =>
            {
                // \note Empty input always parses to null MetaRef/MetaConfigId, even when key type is not nullable.
                ...
                if (key != null)
                {
                    return refType == typeof(MetaRef<>) ? MetaRefUtil.CreateFromKey(metaRefType: genericRefType, key) : MetaConfigIdUtil.CreateFromKey(metaConfigIdType: genericRefType, key);
                }
                else
                    return null;
            });
        ...
        public static MetaDuration ParseMetaDuration(ConfigLexer lexer)
        {
        ...
        public static bool TryParseCorePlayerPropertyId(ConfigLexer lexer, out PlayerPropertyId resultPropertyId)
        {
            ...
            if (lexer.CurrentToken.Type == ConfigLexer.TokenType.Identifier)
            {
                ...
                switch (type)
                {
                    ...
                    {
                        lexer.Advance();
                        resultPropertyId = new PlayerPropertyLogicVersion();
                        return true;
                    }
                }
            }
    ...
    {
        public abstract void RegisterParsers(ConfigParser parser);
    }
}
