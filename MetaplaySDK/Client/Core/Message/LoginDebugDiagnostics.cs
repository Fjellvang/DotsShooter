...
namespace Metaplay.Core.Message
{
    ...
    public class LoginSessionDebugDiagnostics
    {
        ...
        /// <summary> Serialization type codes of the first few PlayerActions within the PlayerFlushActions among the MetaMessages the client has sent and the server hasn't yet acknowledged. </summary>
        [MetaMember(7)] public int[] FirstNRememberedSentActionTypeCodes    { get; set; }
        ...
        public string[] FirstNRememberedSentActionTypeNames =>
            FirstNRememberedSentActionTypeCodes?
            .Select(typeCode =>
            {
                if (ModelActionRepository.Instance == null)
                    return "<uninit>";
                else if (ModelActionRepository.Instance.SpecFromCode.TryGetValue(typeCode, out ModelActionSpec spec))
