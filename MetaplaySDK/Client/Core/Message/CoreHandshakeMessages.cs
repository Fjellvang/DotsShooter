...
namespace Metaplay.Core.Message
{
    ...
    public static class Handshake
    {
        ...
        {
            /// <summary> Version string of the server (mainly for informational purposes) </summary>
            [MetaMember(1)] public string       ServerVersion       { get; private set; }

            /// <summary> Build number of the server (mainly for informational purposes) </summary>
            [MetaMember(2)] public string       BuildNumber         { get; private set; }

            /// <summary> Hash of the network protocol (includes all members/types of public messages, actions, and models) </summary>
            [MetaMember(3)] public uint         FullProtocolHash    { get; private set; }

            /// <summary> Commit hash of the server (for compatibility checks) </summary>
            [MetaMember(4)] public string       CommitId            { get; private set; }

            /// <summary>
            /// The technical name of the project from <see cref="MetaplayCoreOptions.ProjectName"/>.
            /// Used to detect the server is the same game project as the client.
            /// </summary>
            [MetaMember(5)] public string       ProjectName         { get; private set; }
        ...
        {
            [MetaMember(1)]  public string           BuildVersion           { get; private set; }
            [MetaMember(2)]  public string           BuildNumber            { get; private set; }
            // Client logic version information in clients using Metaplay SDK before R31, before introduction
            // of client patch version. Newer clients use property `ClientVersionPayload` instead of this.
            [MetaMember(3)]  MetaVersionRange        SupportedLogicVersionsLegacy { get; set; }
            [MetaMember(13)] ClientVersion           ClientVersionPayload   { get; set; }
            ...
            [MetaMember(6)] MetaTime                    _timestamp;
            ...
            [MetaMember(12)] public string              TargetHostname              { get; private set; }

            public ClientVersion ClientVersion => ClientVersionPayload.LogicVersion == 0 ? new ClientVersion(SupportedLogicVersionsLegacy.MaxVersion, 0) : ClientVersionPayload;
            public DateTime      Timestamp     => _timestamp.ToDateTime();
            ...
            public ClientHello(string buildVersion, string buildNumber, ClientVersion clientVersion, uint fullProtocolHash, string commitId, DateTime timestamp, uint appLaunchId, uint clientSessionNonce, uint clientSessionConnectionNdx, ClientPlatform platform, int loginProtocolVersion, string targetHostname)
            {
                ...
                BuildNumber                = buildNumber;
                ClientVersionPayload       = clientVersion;
                FullProtocolHash           = fullProtocolHash;
                CommitId                   = commitId;
                _timestamp                 = MetaTime.FromDateTime(timestamp);
                ...
                // Set the legacy field for compatibility. This is read by servers before R31
                SupportedLogicVersionsLegacy = new MetaVersionRange(clientVersion.LogicVersion, clientVersion.LogicVersion);
            }
        }
        ...
        public class ClientAbandon : MetaMessage
        {
            ...
            public DateTime ConnectionStartedAt => _connectionStartedAt.ToDateTime();
            public DateTime ConnectionAbandonedAt => _connectionAbandonedAt.ToDateTime();
            public DateTime AbandonedCompletedAt => _abandonedCompletedAt.ToDateTime();
            // \note Should be DateTime[Offset] to reflect that it's real time, but can't for backwards compatibility.
            //       So we use helper conversions instead (in the getter property and in constructor).
            [MetaMember(1), PrettyPrint(PrettyPrintFlag.Hide)] MetaTime _connectionStartedAt;
            [MetaMember(2), PrettyPrint(PrettyPrintFlag.Hide)] MetaTime _connectionAbandonedAt;
            [MetaMember(3), PrettyPrint(PrettyPrintFlag.Hide)] MetaTime _abandonedCompletedAt;
            ...
            {
                _connectionStartedAt = MetaTime.FromDateTime(connectionStartedAt);
                _connectionAbandonedAt = MetaTime.FromDateTime(connectionAbandonedAt);
                _abandonedCompletedAt = MetaTime.FromDateTime(abandonedCompletedAt);
                Source = source;
            }
        }
        ...
        /// <summary>
        /// Client's patch version is too old for the server.
        /// </summary>
        [MetaMessage(MessageCodesCore.ClientPatchVersionTooOld, MessageDirection.ServerToClient, hasExplicitMembers: true)]
        public class ClientPatchVersionTooOld : MetaMessage
        {
            [MetaMember(1)] public int MinimumPatchVersionRequired { get; private set; }
        ...
        /// </summary>
        [MetaSerializable]
        [MetaBlockedMembers(7, 8)] // \note Reserved for company-id specific members. In company-id branch, you should remove this block.
        public struct ServerOptions
        {
            ...
            /// <summary>
            /// The offset for the time skip applied in this environment, to be assigned to <see cref="MetaTime.DebugTimeOffset"/>.
            /// This is zero if no time skip has been applied.
            /// </summary>
            [MetaMember(9)] public MetaDuration GameTimeOffset;

            public ServerOptions(int pushUploadPercentageSessionStartFailedIncidentReport, bool enableWireCompression, MetaDuration deletionRequestSafetyDelay, string gameServerGooglePlaySignInOAuthClientId, string immutableXLinkApiUrl, string gameEnvironment, MetaDuration gameTimeOffset)
            {
