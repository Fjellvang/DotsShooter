        ...
        if (methodInfo.isAsync) {
          browserModule[methodInfo.methodName] = function asyncMethodWrapper(args) {
            return new Promise((resolve, reject) => {
              ...
              try {
                getWasmTableEntry(invokeAsyncMethod)(reqId, methodInfo.methodId, argsBuffer)
              } finally {
                _free(argsBuffer);
              }
            });
          ...
          browserModule[methodInfo.methodName] = function syncMethodWrapper(args) {
            ...
            try {
              if (methodInfo.hasReturnValue) {
                const replyPtr = getWasmTableEntry(invokeSyncMethod)(methodInfo.methodId, argsBuffer)
                try {
                ...
                getWasmTableEntry(invokeSyncMethod)(methodInfo.methodId, argsBuffer)
                return undefined;
              }
            } finally {
              _free(argsBuffer);
