...
const WebSocketConnectorPlugin = {
  ...
  WebSocketConnectorJs_Initialize: function (closeCallback, errorCallback, openCallback, messageCallback) {
    ...
    _WebSocketState.sendError = function(connId, err) {
      ...
      try {
        {{{ makeDynCall('vii', '_WebSocketState.errorCallback') }}} (connId, errorBufferPtr);
      } finally {
    ...
    _WebSocketState.receiveMessage = function(connId, dataBuffer) {
      ...
      try {
        ...
        {{{ makeDynCall('viii', '_WebSocketState.messageCallback') }}} (connId, msgBufferPtr, dataBuffer.length);
      } finally {
        _free(msgBufferPtr)
      }
    }
  ...
  WebSocketConnectorJs_Open: function (connId, connectUrl) {
    ...
    // Close gets called after the connection has been successfully closed, or closed due to an error.
    _WebSocketState.sockets[connId].addEventListener('close', (event) => {
      ...
      try {
        {{{ makeDynCall('viiii', '_WebSocketState.closeCallback') }}} (connId, event.code, reasonBufferPtr, event.wasClean);
      } finally {
    ...
    _WebSocketState.sockets[connId].addEventListener('open', (event) => {
      {{{ makeDynCall('vi', '_WebSocketState.openCallback') }}} (connId);
    })
