...
const WebApiBridgePlugin = {
  ...
  WebApiBridgeJs_JsonCallSync: function(methodId, argsPtr) {
    ...
    // console.log(_webApiBridge.apiRegistry.syncBrowserMethods)
    // console.log(`WebApiBridgeJs_JsonCallSync(method=${method}:`, args);
    try {
      const result = method(args);
      if (result !== undefined)
      {
        return MetaplayUtil.stringToUTF8(JSON.stringify(result));
      }
    } catch(err) {
      console.log(`WebApiBridgeJs_JsonCallSync(): Call to browser method #${methodId}, ${method} failed with ${err}`);
    }
  },
