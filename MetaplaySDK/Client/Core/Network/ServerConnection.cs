...
namespace Metaplay.Core.Network
{
    ...
    {
        public static bool CanHandleClientVersionMismatch(this ServerHandshakePhase phase)
        {
            switch (phase)
            {
    ...
    public class ServerConnection
    {
        ...
        public class ClientPatchVersionTooOldError : MessageTransport.Error
        {
            public int CurrentClientPatchVersion { get; }
            public int MinimumPatchVersionRequired { get; }
            public ClientPatchVersionTooOldError(int currentClientPatchVersion, int minimumPatchVersionRequired)
            {
        ...
        public class ProjectNameMismatchError : MessageTransport.Error
        {
        ...
        {
            public bool GenerateIncidentReport { get; }

            public SessionResumeFailed(bool generateIncidentReport)
            {
        ...
        public class Config
        {
            ...
            public PlayerLocation?                                  OverrideLocation;
        };
        ...
        public LoginSessionDebugDiagnostics TryGetLoginSessionDebugDiagnostics()
        {
            lock (_currentSessionSendQueueLock)
            {
                ...
                else
                {
                    ...
                    int[] firstFewRememberedSentMessageTypeCodes =
                    ...
                    int[] firstFewRememberedSentActionTypeCodes =
                        pendingPlayerFlushActions
                        .Where(flush => flush.ActionTypesDebug != null)
                        .SelectMany(flush => flush.ActionTypesDebug)
                        .Take(5)
                        .Select(actionType => ModelActionRepository.Instance.SpecFromType.TryGetValue(actionType, out ModelActionSpec spec)
                                       ? spec.TypeCode
                                       : -1)
                        .ToArray();
                    ...
                    return new LoginSessionDebugDiagnostics
                    {
                        ...
                        FirstNRememberedSentMessageTypeCodes    = firstFewRememberedSentMessageTypeCodes,
                        FirstNRememberedSentActionTypeCodes     = firstFewRememberedSentActionTypeCodes,
        ...
        void SetupTransport()
        {
            lock (_transportLock)
            {
                ...
                SetWatchdog(_config.ConnectTimeout + TimeSpan.FromSeconds(5));
        ...
        /// <param name="terminateSynchronouslyIfNoTransport">If there is no transport, should the connection be closed synchronously. If false, connection is not terminated at all</param>
        public Task EnqueueCloseAsync(object payload, bool terminateSynchronouslyIfNoTransport = true)
        {
            Task completionTask;
            ...
            lock (_enqueueCloseLock)
            {
                ...
                else if (terminateSynchronouslyIfNoTransport)
                {
                ...
                {
                    hadTransportToClose = false; // dummy value to satisfy compiler
                }

                completionTask = _terminatingErrorTask.Task;
            }

            if (!hadTransportToClose && terminateSynchronouslyIfNoTransport)
            {
        ...
        public void RetrySessionStart(SessionProtocol.SessionResourceProposal resourceProposal, ClientAppPauseStatus clientAppPauseStatus)
        {
            ...
            _transport.EnqueueSendMessage(new SessionProtocol.SessionStartRequest(
                ...
                clientAppPauseStatus:           _clientAppPauseStatus,
                overrideLocation:               _config.OverrideLocation));
        }
        ...
        void DispatchToInternalHandlers(MetaMessage message)
        {
            ...
            if (message is Handshake.LogicVersionMismatch mismatch && _handshakePhase.CanHandleClientVersionMismatch())
            {
                HandleLogicVersionMismatch(mismatch);
                return;
            }
            if (message is Handshake.ClientPatchVersionTooOld patchVersionTooOld && _handshakePhase.CanHandleClientVersionMismatch())
            {
                HandleClientPatchVersionTooOld(patchVersionTooOld);
                return;
            }
        ...
        {
            // Verify ProjectName of the server
            // \note: Tolerate missing project name for protocol compatibility
            if (serverHello.ProjectName != null && serverHello.ProjectName != MetaplayCore.Options.ProjectName)
            {
                Log.Warning("Project name mismatch: client={ClientProject}, server={ServerProject}. Is this the correct game server?", MetaplayCore.Options.ProjectName, serverHello.ProjectName);
                TerminateWithError(new ProjectNameMismatchError(MetaplayCore.Options.ProjectName, serverHello.ProjectName));
                return;
            }
            ...
            if (serverHello.FullProtocolHash != MetaSerializerTypeRegistry.Instance.FullProtocolHash)
            {
                Log.Warning("Full protocol hash mismatch: client=0x{ClientProtocol:X8} vs server=0x{ServerProtocol:X8}", MetaSerializerTypeRegistry.Instance.FullProtocolHash, serverHello.FullProtocolHash);
                EnqueueInfo(new FullProtocolHashMismatchInfo(
                    clientProtocolHash: MetaSerializerTypeRegistry.Instance.FullProtocolHash,
        ...
        void HandleClientPatchVersionTooOld(Handshake.ClientPatchVersionTooOld patchVersionToOld)
        {
            Log.Warning("Client patch version is too old for the server: current={CurrentVersion}, required={MinimumRequiredVersion}", MetaplayCore.Options.ClientVersion.Patch, patchVersionToOld.MinimumPatchVersionRequired);
            TerminateWithError(new ClientPatchVersionTooOldError(MetaplayCore.Options.ClientVersion.Patch, patchVersionToOld.MinimumPatchVersionRequired));
        }
        ...
        void HandleSessionStartFailure(SessionProtocol.SessionStartFailure sessionFailure)
        {
            switch (sessionFailure.Reason)
            {
                ...
                case SessionProtocol.SessionStartFailure.ReasonCode.Deleted:
                {
                    // On login, the player account was found to be deleted. The server clears the associated authentication
                    // record and client should retry login.
                    TerminateWithError(new SessionStartFailed("Player deleted"));
                    return;
                }
        ...
        {
            // Only failures with GenerateIncidentReport=true are severe. No need to log
            // with Warning level for non-severe cases.
            Log.LogEvent(
                level: failed.GenerateIncidentReport ? LogLevel.Warning : LogLevel.Debug,
                ex: null,
                format: "Session resumption failure");

            TerminateWithError(new SessionResumeFailed(failed.GenerateIncidentReport));
        }
        ...
        public void ResumeSessionAfterConnectionDrop()
        {
            ...
            {
                // Invalid use, caller most likely closed the connection. Ignore.
                return;
            }
        ...
        bool TryDeferSessionTransportLossErrorToSupervisor(MessageTransport.Error error)
        {
            ...
            //
            // If there was no session, i.e. we were never successfully connected, always stop. There is nothing useful in reusing this
