...
namespace Metaplay.Core.Player
{
        ...
        #endif
    {
        ...
        const int CurrentBaseFixupVersion = 5;
        ...
        [MetaMember(2), Transient, ExcludeFromGdprExport] public long                               CurrentTick                 { get; private set; }   // Current tick, since start of session. Advanced at rate of TicksPerSecond.
        ...
        [MetaMember(17), NoChecksum] public MetaDictionary<string, InAppPurchaseEvent>           PendingInAppPurchases           { get; private set; } = new MetaDictionary<string, InAppPurchaseEvent>();                    // IAP transactions waiting to be validated or claimed
        [MetaMember(35)] public MetaDictionary<InAppProductId, PendingDynamicPurchaseContent>    PendingDynamicPurchaseContents  { get; private set; } = new MetaDictionary<InAppProductId, PendingDynamicPurchaseContent>(); // Intended contents for dynamic-content products
        [MetaMember(41)] public MetaDictionary<InAppProductId, PendingNonDynamicPurchaseContext> PendingNonDynamicPurchaseContexts { get; private set; } = new MetaDictionary<InAppProductId, PendingNonDynamicPurchaseContext>(); // Intended analytics contexts for non-dynamic products
        ...
        [MetaMember(23), ServerOnly] public MetaDictionary<string, PlayerDeviceEntry>            DeviceHistory               { get; private set; } = new MetaDictionary<string, PlayerDeviceEntry>();
        [MetaMember(24), ServerOnly] public List<PlayerLoginEvent>                                  LoginHistory                { get; private set; } = new List<PlayerLoginEvent>();
        [MetaMember(25), NoChecksum] public MetaDictionary<AuthenticationKey, LegacyPlayerAuthEntry> LegacyAttachedAuthMethods   { get; private set; } = null;
        [MetaMember(54), NoChecksum] public MetaDictionary<AuthenticationKey, PlayerAuthEntryBase>   AttachedAuthMethods         { get; private set; } = new MetaDictionary<AuthenticationKey, PlayerAuthEntryBase>();
        ...
        [MetaMember(53)] public MetaDictionary<ClientSlot, PlayerSubClientStateBase>             PlayerSubClientStates       { get; private set; } = new MetaDictionary<ClientSlot, PlayerSubClientStateBase>();

        [MetaMember(55)] public PlayerLiveOpsEventsModel                                            LiveOpsEvents               { get; private set; } = new PlayerLiveOpsEventsModel();

        [MetaMember(56), ServerOnly] public PlayerSessionDebugMode                                  SessionDebugModeOverride    { get; set; }
            ...
            switch (fromVersion)
            {
                ...
                case 2:
                    ...
                    if (DeviceHistory == null)
                        DeviceHistory = new MetaDictionary<string, PlayerDeviceEntry>();
                ...
                case 4:
                    // R27 -> R28: Breaking schema changes in LiveOps Events (which was only in an early preview state in R27).
                    LiveOpsEvents = new PlayerLiveOpsEventsModel();
                    break;
        ...
        public void DebugForceSetModelTime(MetaTime timeAtFirstTick, long currentTick)
        {
            TimeAtFirstTick = timeAtFirstTick;
        ...
        protected virtual PlayerAuthEntryBase FixupLegacyPlayerAuthEntry(AuthenticationKey key, LegacyPlayerAuthEntry auth)
        {
            switch (key.Platform)
            {
                ...
                case AuthenticationPlatform.Steam:
                    return new PlayerAuthEntryBase.Default(auth.AttachedAt);

                case AuthenticationPlatform._ReservedDontUse1:
                case AuthenticationPlatform._ReservedDontUse2:
                    // Invalid internal state.
                    throw new InvalidOperationException($"Authentication entry with {key.Platform} platform is not allowed");
            }
            return null;
        }
