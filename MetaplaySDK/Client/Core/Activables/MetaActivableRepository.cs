...
namespace Metaplay.Core.Activables
{
    ...
    public class MetaActivableRepository
    {
        ...
        public static MetaActivableRepository Instance => MetaplayServices.Get<MetaActivableRepository>();

        MetaDictionary<MetaActivableCategoryId, CategorySpec>    _categories = new MetaDictionary<MetaActivableCategoryId, CategorySpec>();
        MetaDictionary<MetaActivableKindId, KindSpec>            _kinds      = new MetaDictionary<MetaActivableKindId, KindSpec>();
        ...
        public MetaActivableRepository(GameConfigRepository gameConfigRepository)
        {
            Type sharedGameConfigType = gameConfigRepository.SharedGameConfigType;
            ...
            {
                // \note: check if any relevant attribute is present before materializing all attributes.
                if (type.HasCustomAttribute<MetaActivableCategoryMetadataAttribute>(inherit: false)
                    || type.HasCustomAttribute<MetaActivableKindMetadataAttribute>(inherit: false)
                    || type.HasCustomAttribute<MetaActivableConfigDataAttribute>(inherit: false)
                    || type.HasCustomAttribute<MetaActivableSetAttribute>(inherit: false))
                {
            ...
            foreach (MemberInfo configEntryMember in gameConfigRepository.GetGameConfigTypeInfo(sharedGameConfigType).Entries.Values.Select(entry => entry.MemberInfo))
            {
