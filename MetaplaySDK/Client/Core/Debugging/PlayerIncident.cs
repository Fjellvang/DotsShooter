...
namespace Metaplay.Core.Debugging
{
    ...
    public class UnitySystemInfo
    {
        ...
        DateTime _nextBatteryUpdateAt;

        public void UpdateDynamic(DateTime now)
        {
            ...
            {
                BatteryLevel = UnityEngine.SystemInfo.batteryLevel;
                _nextBatteryUpdateAt = now + TimeSpan.FromSeconds(120);
            }
    ...
    public class UnityPlatformInfo
    {
        ...
        DateTime _nextReachabilityUpdateAt;
        public void UpdateDynamic(DateTime now)
        {
            ...
            {
                InternetReachability = EnumToStringCache<UnityEngine.NetworkReachability>.ToString(UnityEngine.Application.internetReachability);
                _nextReachabilityUpdateAt = now + TimeSpan.FromSeconds(1);
            }
    ...
    public abstract class PlayerIncidentReport
    {
        ...
        [MetaMember(108)] public NetworkDiagnosticReport    NetworkReport       { get; set; }          // Optional network diagnostic report, populated late
            ...
            NetworkDiagnosticReport networkReport = null)
        {
            ...
            {
                IncidentId = PlayerIncidentUtil.EncodeIncidentId(occurredAt, RandomPCG.CreateNew().NextULong());
            }
            else
            {
        ...
        public PlayerIncidentReport(SharedIncidentInfo info, NetworkDiagnosticReport networkReport = null)
        {
            IncidentId         = PlayerIncidentUtil.EncodeIncidentId(info.OccurredAt, RandomPCG.CreateNew().NextULong());
            OccurredAt         = info.OccurredAt;
            ClientLogEntries   = info.ClientLogEntries;
            ClientSystemInfo   = info.ClientSystemInfo;
            ClientPlatformInfo = info.ClientPlatformInfo;
            GameConfigInfo     = info.GameConfigInfo;
        ...
        public class TerminalNetworkError : PlayerIncidentReport
        {
            ...
            [MetaMember(5)] public NetworkDiagnosticReport  NetworkReportObsolete { get; private set; } // Moved to base class, old reports handled specially on the dashboard side
            [MetaMember(6)] public string                   TlsPeerDescription    { get; private set; } // Information about the TLS peer used in Server connection

            // Provide the specific name dashboard is looking for to visualize the error in a nice way
            public string StackTrace => NetworkError;
            ...
            public TerminalNetworkError(
                SharedIncidentInfo sharedIncidentInfo,
                string errorType,
                string networkError,
                string reasonOverride,
                ServerEndpoint endpoint,
        ...
        public class SessionCommunicationHanged : PlayerIncidentReport
        {
            ...
            public SessionCommunicationHanged(
                ...
                int pingId,
                TimeSpan elapsedSinceCommunication)
                : base(sharedIncidentInfo)
            {
                IssueType = issueType;
                IssueInfo = issueInfo;
                DebugDiagnostics = debugDiagnostics;
                RoundtripEstimate = MetaDuration.FromTimeSpan(roundtripEstimate);
                ServerGateway = serverGateway;
                NetworkReachability = networkReachability;
                //NetworkReport = networkReport;
                TlsPeerDescription = tlsPeerDescription;
                SessionToken = sessionToken;
                PingId = pingId;
                ElapsedSinceCommunication = MetaDuration.FromTimeSpan(elapsedSinceCommunication);
            }
        ...
        /// <summary>
        /// CompanyID Login failed  on the client.
        /// </summary>
        [MetaSerializableDerived(5)]
        public class CompanyIdLoginError : PlayerIncidentReport
        {
            [MetaMember(1)] public string   Phase               { get; private set; }
            [MetaMember(2)] public string   Message             { get; private set; }
            [MetaMember(3)] public string   Exception           { get; private set; }

            public override string Type     => nameof(CompanyIdLoginError);
            public override string SubType  => Phase;
            public override string GetReason() => Phase;

            // For Dashboard
            public string StackTrace => $"{Message}\n{Exception}";

            CompanyIdLoginError() { }
            public CompanyIdLoginError(
                SharedIncidentInfo sharedIncidentInfo,
                string phase,
                string message,
                string exception)
        ...
        {
            [MetaMember(1)] public int              ActionNdx                 { get; private set; }
            [MetaMember(2)] public long             ConflictTick              { get; private set; }
            [MetaMember(3)] public string           PlayerModelDiff           { get; private set; }
            [MetaMember(4)] public List<string>     VagueDifferencePathsMaybe { get; private set; }
            [MetaMember(5)] public PlayerActionBase PlayerAction              { get; private set; }

            public override string Type    => nameof(PlayerChecksumMismatch);
            public override string SubType => ((PlayerAction != null) ? ("ActionType: " + PlayerAction.GetType().Name) : "Tick");
            public override string GetReason()
            {
                string reason = ((PlayerAction != null) ? ("ActionType: " + PlayerAction.GetType().Name) : "Tick") + ", ";

                if (VagueDifferencePathsMaybe != null)
                {
                    reason += "DiffPaths: " + string.Join(", ", VagueDifferencePathsMaybe);
                }
                else if (PlayerModelDiff != null)
                {
                    reason += "DiffReport: " + PlayerModelDiff;
                }
                else
                {
        ...
        {
            [MetaMember(1)] public string ExceptionType    { get; set; }
            [MetaMember(2)] public string ExceptionMessage { get; set; }
            [MetaMember(3)] public string StackTrace   { get; set; }

            public override string Type        => nameof(PlayerActorCrashed);
            public override string SubType     => ExceptionType;
            public override string GetReason() => ExceptionMessage;

            PlayerActorCrashed() { }
            public PlayerActorCrashed(string id,
                MetaTime occurredAt,
                List<ClientLogEntry> logEntries,
                UnitySystemInfo systemInfo,
                UnityPlatformInfo platformInfo,
    ...
    public static class PlayerIncidentUtil
    {
        ...
        public static bool IsValidIncidentId(string incidentId)
        {
            if (incidentId == null || incidentId.Length != 32)
                return false;
            foreach (char c in incidentId)
            {
                if (!(c >= '0' && c <= '9') && !(c >= 'a' && c <= 'f'))
                    return false;
            }
