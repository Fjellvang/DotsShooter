...
namespace Metaplay.Core.Guild
{
    ...
    public class GuildCreateInvitationResult
    {
        ...
        /// <summary>
        /// <c>Success</c> if query completed successfully. The error code of the failure otherwise.
        /// </summary>
    ...
    public enum GuildClientPhase
    {
        ...
        /// <inheritdoc cref="MultiplayerEntityClientPhase.LoadingEntity"/>
        LoadingEntity,
    }
    ...
    public class GuildClient : IEntitySubClient
    {
        ...
        {
            public readonly DateTime RunAt;
            public readonly Action Action;

            public DelayedAction(DateTime runAt, Action action)
            {
                RunAt = runAt;
                Action = action;
            }
        ...
        public void OnSessionStart(SessionProtocol.SessionStartSuccess successMessage, ClientSessionStartResources sessionStartResources)
        {
            ...
            _logicVersion            = sessionStartResources.LogicVersion;
            _playerConsistencyChecks = successMessage.PlayerState.DebugConfig.ClientConsistencyChecks;

            int initialEntityIndex = -1;
            for (int ndx = 0; ndx < successMessage.EntityStates.Count; ++ndx)
            {
                bool shouldAttachToNewEntityChannel = successMessage.EntityStates[ndx].ClientData.ClientSlot == ClientSlot;
            ...
            if (initialEntityIndex != -1)
            {
                // Initial login contains the entity for this client. If we have the game config for it, start immediately. Otherwise load config in background.
                EntityInitialState  initialState = successMessage.EntityStates[initialEntityIndex];
                ISharedGameConfig   gameConfig   = sessionStartResources.TryGetConfigForEntity(initialState.State);

                if (gameConfig != null)
                {
                    ActivateGuildWithState(initialState.State, initialState.ChannelId, guildUpdatesMaybe: null, gameConfig);
                }
                else
                {
                    // \note: This can only happen if GuildActor sets RequireClientHasGameConfigPresentOnSessionStart = false
                    Phase = GuildClientPhase.LoadingEntity;
                    GuildContext?.OnEntityDetached();
                    GuildContext = null;
                    PhaseChanged?.Invoke();
                    BeginWaitForNewEntityConfigLoad(initialState.State, initialState.ChannelId);
                }
            }
            else
            {
        ...
        void OnGuildCreateResponse(GuildCreateResponse response)
        {
            ...
            BeginWaitForNewEntityConfigLoad(response.GuildState.Value, response.GuildChannelId);
            _waitingGuildCreationCompletion = _pendingGuildCreationCompletion;
            _pendingGuildCreationCompletion = null;
        }

        void BeginWaitForNewEntityConfigLoad(EntitySerializedState guildState, int channelId)
        {
            _waitingState = guildState;
            _waitedOps = new List<GuildTimelineUpdateMessage>();
            _waitedChannelId = channelId;
            _waitingConfigs = _services.GetConfigAsync(guildState);
        }
        ...
        void OnGuildViewResponse(GuildViewResponse response)
        {
            ...
            state.ConfigDownload = _services.GetConfigAsync(response.GuildState);
        }
        ...
        void OnGuildSwitchedMessage(GuildSwitchedMessage changed)
        {
            ...
            else
            {
                ...
                BeginWaitForNewEntityConfigLoad(changed.GuildState.Value, changed.GuildChannelId);
                _waitingGuildJoinCompletion = _pendingGuildJoinCompletion;
                _pendingGuildJoinCompletion = null;
            }
        }
        ...
        void OnGuildCreateInvitationResponse(GuildCreateInvitationResponse response)
        {
            ...
            if (response.Status == GuildCreateInvitationResponse.StatusCode.RateLimited)
            {
                ...
                RunAfter(delay: TimeSpan.FromSeconds(1), () =>
                {
                    if (Phase != GuildClientPhase.GuildActive)
                        return;
        ...
        void RunAfter(TimeSpan delay, Action action)
        {
            _delayedActions.Add(new DelayedAction(runAt: DateTime.UtcNow + delay, action));
        }
