...
namespace Metaplay.Core.Web3
{
    ...
    public class NftMetadataSpec
    {
        ...
        {
            object value;
            if (memberValue is IMetaRefBase metaRefBase)
                value = metaRefBase.KeyObject;
            else
                value = memberValue;
        ...
        {
            Type type;
            if (memberType.IsGenericTypeOf(typeof(MetaRef<>)) || memberType.IsGenericTypeOf(typeof(MetaConfigId<>)))
            ...
            else if (memberType.IsGenericTypeOf(typeof(MetaConfigId<>)))
                memberValue = MetaConfigIdUtil.CreateFromKey(memberType, value);
            else
                memberValue = value;
        ...
        static NftPropertyType? TryGetPropertyType(Type memberType)
        {
            ...
            else if (memberType.IsGenericTypeOf(typeof(MetaRef<>)) || memberType.IsGenericTypeOf(typeof(MetaConfigId<>)))
            {
                // MetaRef and MetaConfigId map to whatever their key maps to.
