...
{
    [MetaSerializable]
    public struct MetaCalendarDateTime : IEquatable<MetaCalendarDateTime>
    {
        ...
        public static bool operator ==(MetaCalendarDateTime left, MetaCalendarDateTime right)
        {
        ...
        public bool Equals(MetaCalendarDateTime other)
        {
        ...
        {
            return obj is MetaCalendarDateTime other && Equals(other);
        }
    ...
    public struct MetaCalendarPeriod : IEquatable<MetaCalendarPeriod>
    {
        ...
        const int MaxDaysPerMonth = 31;
        const int MinDaysPerYear = 365;
        const int MaxDaysPerYear = 366;
        ...
        {
            // \note We want to produce a *lower* total estimate; therefore, it depends on the sign of the
            //       number of months whether we want to multiply it by the minimum or maximum days-per-month.
            //       Positive months means we want the smallest number of days-per-month (= total days for the months will be positive, as close to zero as possible).
            //       Negative months we want the largest number of days-per-month (= total days for the months will be negative, as far from zero as possible).
            //       Similarly for years.
            int daysForMonths = Months >= 0
                                ? Months * MinDaysPerMonth
                                : Months * MaxDaysPerMonth;
            int daysForYears = Years >= 0
                                ? Years * MinDaysPerYear
                                : Years * MaxDaysPerYear;
            ...
            return MetaDuration.FromSeconds(Seconds)
                ...
                + MetaDuration.FromDays(daysForMonths)
                + MetaDuration.FromDays(daysForYears);
        }
        ...
        {
            // \note We want to produce an *upper* total estimate; therefore, it depends on the sign of the
            //       number of months whether we want to multiply it by the maximum or minimum days-per-month.
            //       Positive months means we want the largest number of days-per-month (= total days for the months will be positive, as far from zero as possible).
            //       Negative months we want the smallest number of days-per-month (= total days for the months will be negative, as close to zero as possible).
            //       Similarly for years.
