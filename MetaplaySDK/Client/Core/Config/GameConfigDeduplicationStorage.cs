...
namespace Metaplay.Core.Config
{
    ...
    public class ConfigPatchIdSet
    {
        ...
        public ConfigPatchIdSet(OrderedSet<ExperimentVariantPair> presentPatchIds, MetaDictionary<ExperimentVariantPair, ConfigPatchIndex> allPatchIndexes)
        {
    ...
    {
        public readonly MetaDictionary<ExperimentVariantPair, ConfigPatchIndex> PatchIdToIndex;
        public readonly MetaDictionary<ConfigPatchIndex, ExperimentVariantPair> PatchIndexToId;

        public readonly MetaDictionary<TKey, GameConfigLibraryPatchedItemEntry<TInfo>> PatchedItemEntries = new MetaDictionary<TKey, GameConfigLibraryPatchedItemEntry<TInfo>>();
        public int NumBaselineItems;

        public readonly MetaDictionary<ConfigPatchIndex, PatchInfo> PatchInfos = new MetaDictionary<ConfigPatchIndex, PatchInfo>();

        public int NumSinglePatchDirectlyDuplicatedItems => PatchInfos.Values.Sum(patch => patch.DirectlyPatchedItems.Count);
        public int NumSinglePatchIndirectlyDuplicatedItems => PatchInfos.Values.Sum(patch => patch.IndirectlyPatchedItems.Count);
        ...
        {
            PatchIdToIndex = new MetaDictionary<ExperimentVariantPair, ConfigPatchIndex>();
            PatchIndexToId = new MetaDictionary<ConfigPatchIndex, ExperimentVariantPair>();
        ...
        /// </remarks>
        public void PopulateBaseline(MetaDictionary<TKey, TInfo> baselineItems)
        {
            if (PatchedItemEntries.Count != 0)
                throw new InvalidOperationException($"{nameof(PopulateBaseline)} called more than once");

            MetaDictionary<TKey, OrderedSet<ConfigItemId>> references = GameConfigUtil.CollectReferencesFromItems<TKey, TInfo>(baselineItems.Values.ToList());
        ...
        public void PopulatePatch(ConfigPatchIndex patchIndex, GameConfigLibraryPatch<TKey, TInfo> patch)
        {
            ...
            MetaDictionary<TKey, OrderedSet<ConfigItemId>> references = GameConfigUtil.CollectReferencesFromItems<TKey, TInfo>(allItemsInPatch.Select(kv => kv.Value).ToList());
    ...
    {
        public MetaDictionary<ConfigItemId, OrderedSet<ConfigItemId>> BaselineReferences = null;
        public MetaDictionary<ConfigItemId, OrderedSet<ConfigItemId>> BaselineReverseReferences = null;
    }
}
