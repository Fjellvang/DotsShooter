...
namespace Metaplay.Core.Config
{
    ...
    public struct GameConfigSpecializationKey : IEquatable<GameConfigSpecializationKey>
    {
        ...
        {
            if (ReferenceEquals(VariantIds, other.VariantIds))
                return true;
            if (VariantIds == null || other.VariantIds == null)
                return false;
        ...
        {
            // \note: Null is not equal to the empty key. Use different hash.
            if (ReferenceEquals(VariantIds, null))
                return 1;
            unchecked
            {
                uint hashCode = 0;
