...
namespace Metaplay.Core.Config
{
    ...
    public class FullGameConfigImportResources
    {
        ...
        public readonly int NumPatches;
        public readonly int TotalSinglePatchDirectDuplication;
        public readonly int TotalSinglePatchIndirectDuplication;
        ...
        FullGameConfigImportResources(ConfigArchive fullArchive, List<PlayerExperimentId> experimentIds, GameConfigMetaData metaData, GameConfigImportResources shared, GameConfigImportResources server)
        {
            ...
            // \note NumPatches can be taken from either `shared` or `server` (should not sum them together),
            //       because the same set of patches exists for both (only the contents of the patches differ).
            NumPatches = shared.Patches?.Count ?? 0;
            TotalSinglePatchDirectDuplication = shared.TotalSinglePatchDirectDuplication + server.TotalSinglePatchDirectDuplication;
            TotalSinglePatchIndirectDuplication= shared.TotalSinglePatchIndirectDuplication + server.TotalSinglePatchIndirectDuplication;
        }
            ...
            CancellationToken ct = default)
        {
            ...
            MetaDictionary<PlayerExperimentId, PlayerExperimentInfo> experiments;
            if (serverArchive.ContainsEntryWithName($"{ServerGameConfigBase.PlayerExperimentsEntryName}.mpc"))
                ...
                experiments = new MetaDictionary<PlayerExperimentId, PlayerExperimentInfo>();

            // Extract the patches (in envelope, i.e. serialized, form) from the archive.

            MetaDictionary<ExperimentVariantPair, GameConfigPatchEnvelope> sharedConfigPatches = new MetaDictionary<ExperimentVariantPair, GameConfigPatchEnvelope>();
            MetaDictionary<ExperimentVariantPair, GameConfigPatchEnvelope> serverConfigPatches = new MetaDictionary<ExperimentVariantPair, GameConfigPatchEnvelope>();
