...
namespace Metaplay.Core.Config
{
    public static partial class ConfigArchiveBuildUtility
    {
        ...
        /// </summary>
        internal static (ContentHash hash, byte[] bytes) ToBytes(MetaTime timestamp, IReadOnlyList<ConfigArchiveEntry> entries, CompressionAlgorithm compression = CompressionAlgorithm.Deflate, int minimumSizeForCompression = 32)
        {
            ContentHash version = ComputeVersionHashForEntries(entries.Select(x => (x.Name, x.Hash)));
            ...
            using (FlatIOBuffer buffer = new FlatIOBuffer())
            {
                using (IOWriter writer = new IOWriter(buffer))
                {
                    ...
                    foreach (ConfigArchiveEntry entry in entries)
                    {
