...
namespace Metaplay.Core.Config
{
    ...
    public class GameConfigSpecializationPatches
    {
        ...
        [MetaMember(2)] public MetaDictionary<PlayerExperimentId, MetaDictionary<ExperimentVariantId, byte[]>>    Patches { get; private set; }

        GameConfigSpecializationPatches() { }
        GameConfigSpecializationPatches(ContentHash version, MetaDictionary<PlayerExperimentId, MetaDictionary<ExperimentVariantId, byte[]>> patches)
        {
            Version = version;
            Patches = patches;
        }
        ...
        /// in patch order. As per convention, <c>null</c> <paramref name="assignment"/> is interpreted as empty.
        /// </summary>
        public GameConfigSpecializationKey CreateKeyFromAssignment(MetaDictionary<PlayerExperimentId, ExperimentVariantId> assignment)
        {
            ExperimentVariantId[] variants = new ExperimentVariantId[Patches.Count];
            int ndx = 0;
            foreach ((PlayerExperimentId patchExperimentId, MetaDictionary<ExperimentVariantId, byte[]> patchVariants) in Patches)
            {
                ExperimentVariantId variantId;
                if (assignment == null || !assignment.TryGetValue(patchExperimentId, out ExperimentVariantId variantInAssignment))
                {
                    // not assigned.
                    variantId = null;
                }
        ...
        /// </summary>
        public MetaDictionary<ExperimentVariantPair, GameConfigPatchEnvelope> GetPatchesForSpecialization(GameConfigSpecializationKey specializationKey)
        {
            MetaDictionary<ExperimentVariantPair, GameConfigPatchEnvelope> patchesToApply = new MetaDictionary<ExperimentVariantPair, GameConfigPatchEnvelope>();
            int ndx = 0;
            foreach ((PlayerExperimentId experimentId, MetaDictionary<ExperimentVariantId, byte[]> variantPatches) in Patches)
            {
        ...
        public static GameConfigSpecializationPatches FromContents(ContentHash version, MetaDictionary<PlayerExperimentId, MetaDictionary<ExperimentVariantId, byte[]>> patches)
        {
