...
namespace Metaplay.Core.Config
{
    ...
    public static class GameConfigSyntaxTree
    {
        ...
        {
            public MetaDictionary<NodeMemberId, NodeBase> Members { get; private set; }

            // \todo Support passing location explicitly to allow non-null location for empty collections (we still usually know the cells where the content would have been)
            public ObjectNode(MetaDictionary<NodeMemberId, NodeBase> members) : base(ComputeLocationUnion(members.Values))
            {
                Members = members;
            }
            ...
            public override NodeBase Clone() => new ObjectNode(Members.ToMetaDictionary(kv => kv.Key, kv => kv.Value.Clone()));
        }
