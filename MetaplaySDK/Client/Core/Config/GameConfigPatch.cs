...
namespace Metaplay.Core.Config
{
    ...
    public class GameConfigPatch
    {
        ...
        public GameConfigPatch(Type configType, Dictionary<string, GameConfigEntryPatch> entryPatches)
        {
            ...
            MetaDictionary<string, MemberInfo> entryMemberInfos = GetPatchableGameConfigEntryMembers(ConfigType);
        ...
        static MetaDictionary<string, MemberInfo> GetPatchableGameConfigEntryMembers(Type type)
        {
            return type
    ...
    public class GameConfigPatchEnvelope
    {
        ...
        /// in-place if an <see cref="GameConfigLibrary{TKey,TInfo}"/> otherwise a new object is created and returned.
        /// (For an explanation of the "content" of an entry, see comment on <see cref="GameConfigEntryPatch.CreatePatchedContentDangerously"/>).
        ...
        /// </summary>
        public object CreatePatchedEntryContent(object entryContent, string entryName, Type entryPatchType)
        {
            object content = entryContent;
            if (TryDeserializeEntryPatch(entryName, entryPatchType: entryPatchType, out GameConfigEntryPatch entryPatch))
                content = entryPatch.CreatePatchedContentDangerously(entryContent);
    ...
    public static class GameConfigPatchUtil
    {
        ...
        public class PatchBuildResolver : IGameConfigDataResolver
        {
            ...
            MetaDictionary<string, GameConfigEntryPatch> _entryPatches = new MetaDictionary<string, GameConfigEntryPatch>();
            ...
            public PatchBuildResolver(IGameConfigDataResolver baseResolver, IEnumerable<KeyValuePair<string, GameConfigEntryPatch>> initialEntryPatches = null)
            {
                ...
                _entryPatches = new MetaDictionary<string, GameConfigEntryPatch>(initialEntryPatches ?? Enumerable.Empty<KeyValuePair<string, GameConfigEntryPatch>>());
            }
    ...
    public abstract class GameConfigEntryPatch
    {
        ...
        /// The "content" of an entry depends on the kind of entry, and may
        /// or may not be the same as the entry itself:
        ///   - for a <see cref="GameConfigLibrary{TKey, TInfo}"/> entry, the
        ///     content is the <see cref="MetaDictionary{TKey, TInfo}"/>
        ...
        /// is that <see cref="GameConfigLibrary{TKey, TInfo}"/> and <see cref="GameConfigKeyValue{TKeyValue}"/>
        /// with constructor deserialization does not support in-place modification.
        ///
        /// Depending on the type of the entry, this has slightly different behaviour:
        ///   - for a <see cref="GameConfigLibrary{TKey, TInfo}"/> entry,
        ///     the dictionary is modified by replacing and appending new items.
        ///     The same instance of the dictionary is returned.
        ///   - for a <see cref="GameConfigKeyValue"/> entry, a new instance is
        ///     always created with the values changed according to the patch.
        ///
        ...
        /// Usage via <see cref="GameConfigPatchEnvelope.CreatePatchedEntryContent"/>
        /// is safe since a patch is deserialized, dangerously applied, and then
        /// discarded.
        /// </summary>
        internal abstract object CreatePatchedContentDangerously(object entryContent);
    ...
    {
        internal sealed override object CreatePatchedContentDangerously(object entryContent)
        {
            return CreatePatchedContentDangerously((TEntryContent)entryContent);
        }
        internal abstract TEntryContent CreatePatchedContentDangerously(TEntryContent entryContent);
    ...
    /// </summary>
    [MetaSerializable]
    public class GameConfigLibraryPatch<TKey, TInfo> : GameConfigEntryPatch<GameConfigLibrary<TKey, TInfo>, MetaDictionary<TKey, TInfo>>, IGameConfigLibraryPatch
        where TInfo : class, IGameConfigData<TKey>, new()
    {
        ...
        BuiltinGameConfigLibrary<TKey, TInfo> _replacedItems;
        [JsonProperty("appendedItems")]
        BuiltinGameConfigLibrary<TKey, TInfo> _appendedItems;

        [MetaMember(1), MaxCollectionSize(int.MaxValue), JsonIgnore]
        List<GameConfigDataContent<TInfo>> _replacedItemsForSerialization
        {
            ...
            set
            {
                _replacedItems = new BuiltinGameConfigLibrary<TKey, TInfo>(value.Select(wrapper => wrapper.ConfigData));
            }
        }

        [MetaMember(2), MaxCollectionSize(int.MaxValue), JsonIgnore]
        List<GameConfigDataContent<TInfo>> _appendedItemsForSerialization
        {
            ...
            set
            {
                _appendedItems = new BuiltinGameConfigLibrary<TKey, TInfo>(value.Select(wrapper => wrapper.ConfigData));
            }
        }
        ...
        public GameConfigLibraryPatch(IEnumerable<TInfo> replacedItems, IEnumerable<TInfo> appendedItems)
        {
            ...
            _replacedItems = new BuiltinGameConfigLibrary<TKey, TInfo>(replacedItems);
            _appendedItems = new BuiltinGameConfigLibrary<TKey, TInfo>(appendedItems);
        ...
        internal override MetaDictionary<TKey, TInfo> CreatePatchedContentDangerously(MetaDictionary<TKey, TInfo> libraryItems)
        {
        ...
        public override void ResolveMetaRefs(IGameConfigDataResolver resolver)
        {
            foreach (TInfo item in _replacedItems.Values.Concat(_appendedItems.Values))
            {
                ...
                MetaSerialization.ResolveMetaRefs(itemContent, resolver);
            }
        }
    ...
    public static class GameConfigPatchTypeInfoCache
    {
        class CacheEntry
        {
            public static CacheEntry Factory(MetaSerializableType type)
            {
                CacheEntry e = new CacheEntry();
                e.MemberByTagId = type.Members.ToDictionary(member => member.TagId);
                e.MemberByName = type.Members.ToDictionary(member => member.Name);
                return e;
            }
        ...
        static WebConcurrentDictionary<MetaSerializableType, CacheEntry> _cache = new WebConcurrentDictionary<MetaSerializableType, CacheEntry>();
#else
#pragma warning disable MP_WGL_00 // "Feature is poorly supported in WebGL". False positive, this is non-WebGL.
        static ConcurrentDictionary<MetaSerializableType, CacheEntry> _cache = new ConcurrentDictionary<MetaSerializableType, CacheEntry>();
#pragma warning restore MP_WGL_00
#endif

        public static Dictionary<int, MetaSerializableMember> GetTypeMemberByTagId(MetaSerializableType type)
        {
            return _cache.GetOrAdd(type, CacheEntry.Factory).MemberByTagId;
        }

        public static Dictionary<string, MetaSerializableMember> GetTypeMemberByName(MetaSerializableType type)
        {
            return _cache.GetOrAdd(type, CacheEntry.Factory).MemberByName;
        }
    }
    ...
    public class GameConfigStructurePatch<TStructure> : GameConfigEntryPatch<TStructure, TStructure>, IGameConfigStructurePatch
        where TStructure : GameConfigKeyValue, new()
    {
        [MetaMember(1)] TStructure      _replacementValues;
        ...
        public GameConfigStructurePatch(TStructure replacementValues, OrderedSet<int> replacedMemberTagIds)
        {
            ...
            {
                if (!GameConfigPatchTypeInfoCache.GetTypeMemberByTagId(structureTypeSpec).ContainsKey(memberTagId))
        ...
        public GameConfigStructurePatch(MetaDictionary<int, object> replacedMembersByTagId)
        ...
        public GameConfigStructurePatch(MetaDictionary<string, object> replacedMembersByName)
        ...
        internal override TStructure CreatePatchedContentDangerously(TStructure structure)
        {
            MetaSerializableType structureTypeSpec = MetaSerializerTypeRegistry.GetTypeSpec(typeof(TStructure));

            MetaDictionary<MetaSerializableMember, object> members = new MetaDictionary<MetaSerializableMember, object>();

            foreach (MetaSerializableMember member in structureTypeSpec.Members)
                members[member] = member.GetValue(structure);

            foreach (MetaSerializableMember member in EnumerateReplacedMemberSpecs())
                members[member] = member.GetValue(_replacementValues);

            TStructure newObj = MetaSerializationUtil.CreateAndPopulate<TStructure>(members);

            if (structureTypeSpec.OnDeserializedMethods != null)
            {
                foreach (MethodInfo onDeserializedMethod in structureTypeSpec.OnDeserializedMethods)
                {
                    if (onDeserializedMethod.GetParameters().Any())
                        MemberAccessGenerator.GenerateInvokeOnDeserializedMethodWithParams<TStructure>(onDeserializedMethod.Name)(ref newObj, new MetaOnDeserializedParams());
                    else
                        MemberAccessGenerator.GenerateInvokeOnDeserializedMethod<TStructure>(onDeserializedMethod.Name)(ref newObj);
                }
            }
        ...
        IEnumerable<MetaSerializableMember> EnumerateReplacedMemberSpecs()
        {
            ...
            return _replacedMemberTagIds
                .Select(memberTagId =>
                {
                    ...
                    if (!GameConfigPatchTypeInfoCache.GetTypeMemberByTagId(structureTypeSpec).TryGetValue(memberTagId, out MetaSerializableMember memberSpec))
                        return null;
        ...
        static TStructure CreateReplacementValuesStructureByTagIds(MetaDictionary<int, object> replacedMembersByTagId)
        {
            MetaSerializableType structureTypeSpec = MetaSerializerTypeRegistry.GetTypeSpec(typeof(TStructure));

            return CreateReplacementValuesStructure(structureTypeSpec, replacedMembersByTagId.Select(kv =>
                {
        ...
        static TStructure CreateReplacementValuesStructureByNames(MetaDictionary<string, object> replacedMembersByName)
        {
            MetaSerializableType structureTypeSpec = MetaSerializerTypeRegistry.GetTypeSpec(typeof(TStructure));

            return CreateReplacementValuesStructure(structureTypeSpec, replacedMembersByName.Select(kv =>
                {
                    (string memberName, object memberValue) = kv;
                    if (!GameConfigPatchTypeInfoCache.GetTypeMemberByName(structureTypeSpec).TryGetValue(memberName, out MetaSerializableMember memberSpec))
                        throw new InvalidOperationException($"No MetaMember with name {memberName} found in {typeof(TStructure).ToGenericTypeString()}");
                    return (memberSpec, memberValue);
                }));
        }

        static TStructure CreateReplacementValuesStructure(MetaSerializableType structureTypeSpec, IEnumerable<(MetaSerializableMember, object)> replacedMembers)
        {
            if (!structureTypeSpec.UsesConstructorDeserialization)
            {
            ...
            {
                MetaDictionary<MetaSerializableMember,object> memberToValue = replacedMembers.ToMetaDictionary(kv => kv.Item1, kv => kv.Item2);
                foreach (MetaSerializableMember metaSerializableMember in structureTypeSpec.Members)
                {
                    if (!memberToValue.ContainsKey(metaSerializableMember))
                        memberToValue[metaSerializableMember] = metaSerializableMember.Type.GetDefaultValue();
                }

                return MetaSerializationUtil.CreateAndPopulate<TStructure>(memberToValue);
            }
        }
        ...
        static IEnumerable<int> TagIdsToNames(IEnumerable<string> names)
        {
            ...
            return names.Select(name => GameConfigPatchTypeInfoCache.GetTypeMemberByName(structureTypeSpec)[name].TagId);
        }
        ...
        public override void ResolveMetaRefs(IGameConfigDataResolver resolver)
        {
            MetaSerialization.ResolveMetaRefs(_replacementValues, resolver);
        }
    }
}
