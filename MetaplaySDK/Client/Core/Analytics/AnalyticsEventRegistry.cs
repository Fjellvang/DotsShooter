...
namespace Metaplay.Core.Analytics
{
    ...
    public class AnalyticsEventSpec
    {
        ...
        public readonly string[]             Parameters;        // \note: only used for dashboard visualization
        [IgnoreDataMember]
        ...
        public AnalyticsEventSpec(Type type, AnalyticsEventAttribute attrib, AnalyticsEventCategoryAttribute categoryAttrib)
        {
            ...
            Parameters          = new string[typeSpec.Members.Count];

            for (int ndx = 0; ndx < typeSpec.Members.Count; ++ndx)
                Parameters[ndx] = typeSpec.Members[ndx].Name;
        ...
        static GetAnalyticsEventKeywordsFunc ResolveKeywordsFunc(Type eventType)
        {
            ...
            OrderedSet<string> keywordsForType = new OrderedSet<string>();
            foreach (AnalyticsEventKeywordsAttribute attr in eventType.GetCustomAttributes<AnalyticsEventKeywordsAttribute>(inherit: true))
            {
                foreach (string keyword in attr.Keywords)
                    keywordsForType.Add(keyword);
            }
        ...
        static string GetEventDefaultDisplayName(string className, string categoryName)
        {
            ...
            StringBuilder sb = new StringBuilder(capacity: classNameWithoutPrefix.Length + 5);
            foreach (char c in classNameWithoutPrefix)
            {
    ...
    public class AnalyticsEventRegistry
    {
        public static AnalyticsEventRegistry               Instance             => MetaplayServices.Get<AnalyticsEventRegistry>();
        public MetaDictionary<Type, AnalyticsEventSpec> EventSpecs           { get; private set; }
        public MetaDictionary<int, AnalyticsEventSpec>  EventSpecsByTypeCode { get; private set; }
        public IEnumerable<AnalyticsEventSpec> AllEventSpecs => EventSpecs.Values;

        public static AnalyticsEventSpec GetEventSpec(Type type)
        {
            if (Instance.EventSpecs.TryGetValue(type, out AnalyticsEventSpec spec))
                return spec;
            else
                throw new ArgumentException($"Type {type.ToGenericTypeString()} is not a known AnalyticsEvent", nameof(type));
        }

        public AnalyticsEventRegistry()
        {
            EventSpecs = ResolveEventSpecs();
            EventSpecsByTypeCode = EventSpecs.ToMetaDictionary(x => x.Value.TypeCode, x => x.Value);
        }

        static MetaDictionary<Type, AnalyticsEventSpec> ResolveEventSpecs()
        {
            MetaDictionary<Type, AnalyticsEventSpec> eventSpecs = new MetaDictionary<Type, AnalyticsEventSpec>();
            HashSet<Type> validatedTypes = new HashSet<Type>();
            List<string> validationErrors = new List<string>();

            foreach (Type eventType in MetaSerializerTypeRegistry.GetConcreteDerivedTypes<AnalyticsEventBase>())
            {
            ...
            foreach (Type contextTypes in MetaSerializerTypeRegistry.GetConcreteDerivedTypes<AnalyticsContextBase>())
            {
        ...
        [Conditional("NETCOREAPP")] // Validate on server ..
        [Conditional("UNITY_EDITOR")] // .. and editor -- skip devices
        ...
        static void ValidateEventTypeTree(Type eventType, bool allowConfigReferences, List<string> validationErrors, Type scanType, HashSet<Type> checkedTypes, Stack<string> path)
        {
            ...
            if (!MetaSerializerTypeRegistry.TryGetTypeSpec(scanType, out MetaSerializableType typeSpec))
                return;
            ...
            else if (typeSpec.IsMetaConfigId)
            {
                // MetaConfigIds are OK because they support deserialization without having access to the resolver.
            }
        ...
        static void ValidateSingleTypeIsRoundtripSerializable(Type eventType, List<string> validationErrors, MetaSerializableType typeSpec, Stack<string> path)
        {
            ...
            foreach (MemberInfo mi in typeSpec.Type.EnumerateInstanceDataMembersInUnspecifiedOrder())
            {
                ...
                if (mi.HasCustomAttribute<IgnoreDataMemberAttribute>())
                    continue;
