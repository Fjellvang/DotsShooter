...
namespace Metaplay.Core.Analytics
{
    ...
    {
        public static FirebaseAnalyticsFormatter Instance => MetaplayServices.Get<FirebaseAnalyticsFormatter>();
        MetaDictionary<Type, EventFormatter> _eventFormatters;

        public FirebaseAnalyticsFormatter(AnalyticsEventRegistry analyticsEventRegistry)
        {
        ...
        static EventFormatter TryCreateFormatterForEvent(AnalyticsEventSpec spec, Dictionary<Type, ConverterDelegate> converterMemo)
        {
            ...
            if (!spec.SendToAnalytics)
                return null;
            if (spec.Type.HasCustomAttribute<FirebaseAnalyticsIgnoreAttribute>())
                return null;
            ...
            foreach (MemberInfo memberInfo in spec.Type.EnumerateInstanceDataMembersInUnspecifiedOrder())
            {
                ...
                if (memberInfo.HasCustomAttribute<IgnoreDataMemberAttribute>())
                    continue;
                // If [FirebaseAnalyticsIgnore] on member, skip
                if (memberInfo.HasCustomAttribute<FirebaseAnalyticsIgnoreAttribute>())
                    continue;
                ...
                if (valueType.HasCustomAttribute<FirebaseAnalyticsIgnoreAttribute>())
                    continue;
