...
meta-list-card(
  ...
  template(#item-card="{ item: season }")
    MListItem {{ season.name }}
      template(
        v-if="season.currentSeason && singleLeagueData.enabled"
        #badge
        )
        MBadge(
      ...
      template(#top-right)
        MBadge(
          v-if="season.startedEarly && !season.endedEarly"
          variant="warning"
          ) Started Early
        MBadge(
          v-else-if="!season.startedEarly && season.endedEarly"
          variant="warning"
          ) Ended Early
        MBadge(
          v-else-if="season.startedEarly && season.endedEarly"
...
const allSeasonsDisplayData = computed((): SeasonDisplayData[] => {
  ...
  if (singleLeagueData.value.state.historicSeasons) {
    ...
    const mappedHistoricSeasonsData = singleLeagueData.value.state.historicSeasons.map(
      (pastSeason: any): SeasonDisplayData => ({
