...
<template lang="pug">
div(v-if="playerData")
  span(class="font-weight-bold") #[fa-icon(icon="coins")] Resources
  b-table-simple(
    small
    responsive
    class="tw-mt-1"
    ...
    b-tbody
      b-tr(
        v-for="resource in coreStore.gameSpecific.playerResources"
        :key="resource.displayName"
        )
        b-td {{ resource.displayName }}
        b-td(class="tw-text-right")
          span(
            v-if="!!resource.getAmount(playerData.model) || typeof resource.getAmount(playerData.model) === 'number'"
            ) {{ resource.getAmount(playerData.model) }}
          MBadge(
            v-else
            variant="danger"
            ) not found
