    ...
    MCollapse(extraMListItemMargin)
      ...
      template(#header)
        MListItem(noLeftPadding)
          ...
          template(#badge)
            fa-icon(
              v-if="Object.keys(slot.item.attachments).length > 0"
              icon="paperclip"
              size="sm"
          ...
          template(#top-right)
            MBadge(
              :tooltip="slot.item.statusTooltip"
              :variant="slot.item.statusBadgeVariant"
              ) {{ slot.item.statusText }}
          template(#bottom-right)
            MTextButton(:to="`/broadcasts/${slot.item.id}`") View broadcast
      //- Collapse content
      meta-generated-content(
        :value="slot.item.contents"
 ...
 */
const { data: broadcastsData, hasPermission: broadcastsHasPermission } = useSubscription(
  getBroadcastsSubscriptionOptions()
)
 ...
 */
function isPlayerTargeted(player: any, broadcastParams: any): boolean {
  ...
  if (broadcastParams.isTargeted) {
    if (broadcastParams.targetPlayers?.includes(player.id)) {
      ...
      if (
        broadcastParams.targetCondition.requireAllSegments.every((segment: any) =>
          player.matchingSegments.includes(segment)
        )
      ) {
        isTargeted = true
      }
      ...
      if (
        broadcastParams.targetCondition.requireAnySegment.some((segment: any) =>
          player.matchingSegments?.includes(segment)
        )
      ) {
        isTargeted = true
