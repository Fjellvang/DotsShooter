...
span(v-if="displayStrings")
  MBadge(
    v-for="displayString in displayStrings"
    :key="displayString"
    class="tw-ml-1"
    ) {{ displayString }}
span(v-else)
  slot
    //- pre {{ displayStrings }}
    span(class="text-danger") Error: undefined offer type '{{ offer.$type }}'
</template>
...
const displayStrings = computed(() => {
  ...
  return (
    coreStore.gameSpecific.iapContents.find((iap) => iap.$type === props.offer.$type)?.getDisplayContent(props.offer) ??
    null
  )
})
</script>
