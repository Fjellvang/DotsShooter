    ...
    MCollapse(extraMListItemMargin)
      ...
      template(#header)
        MListItem(noLeftPadding) {{ slot.item.config.displayName }}
          template(#badge)
            span(
              v-if="slot.item.referencePrice !== null"
              class="tw-mt-0.5 tw-text-sm tw-text-neutral-500"
              ) {{ slot.item.referencePrice }}
            MBadge(
              v-if="!isUsed(slot.item)"
              variant="warning"
              ) Unused
          template(#top-right)
          ...
          template(#bottom-right): MTextButton(
            :to="`/offerGroups/offer/${slot.item.config.offerId}`"
        ...
        MListItem(condensed) Rewards
          template(#top-right)
            div
              ...
              meta-reward-badge(
                v-for="reward in slot.item.config.rewards"
                :key="reward.type"
                :reward="reward"
                )
</template>
...
const { data: offersData } = useSubscription(getAllOffersSubscriptionOptions())

/**
 * Get the statistics for the given offer in the context of the offer group.
 */
function getOfferGroupStatisticsForOffer(offer: any): any {
 ...
 */
function isUsed(offer: any): boolean {
  // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
  return Object.values(offersData.value.offerGroups).some((offerGroup: any) => {
 ...
 */
function conversion(purchased: any, activated: any): number {
  if (activated === 0) {
    return 0
  } else {
 ...
 */
function conversionTooltip(purchased: any, activated: any): string {
  if (activated === 0) {
...
const searchFields = ['config.displayName', 'config.description']
</script>
