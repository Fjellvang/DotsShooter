...
import { addGeneratedUiFormComponent, addGeneratedUiViewComponent, addStringIdDecorator } from './generatedUiApis'
import {
...
import { addUiComponent, removeUiComponent, removeAllUiComponents } from './uiPlacementApis'
 ...
 */
export function addPlayerRewards(rewards: GameSpecificReward[]): void {
 ...
 */
export function addPlayerResources(resources: GameSpecificPlayerResource[]): void {
 ...
 */
export function addInAppPurchaseContents(purchases: GameSpecificInAppPurchaseContent[]): void {
  const coreStore = useCoreStore()
...
export function addActorInfoToOverviewPage(
  key: string,
  overviewListDisplayName?: string,
  chartDisplayName?: string,
  databaseListDisplayName?: string
): void {
  const coreStore = useCoreStore()
  if (overviewListDisplayName) {
    coreStore.actorOverviews.overviewListEntries.push({
      key,
      displayName: overviewListDisplayName,
    })
  }
  if (chartDisplayName) {
  ...
  if (databaseListDisplayName) {
    coreStore.actorOverviews.databaseListEntries.push({
      key,
      displayName: databaseListDisplayName,
    })
  }
}
 ...
 */
export function addNavigationEntry(entry: RouteRecordRaw, options: NavigationEntryOptions, replace = false): void {
  ...
  if (currentRoutes.find((x) => x.name === entry.name)) {
    if (!replace) {
      throw new Error(
        `Attempting to replace existing route entry for '${entry.name?.toString() ?? 'unknown'}' (${entry.path}) without specifying "replace"`
      )
    }
  }
 ...
 */
export async function getGameData(): Promise<{
  $type: string
  gameConfig: any
  serverGameConfig: any
}> {
 ...
 */
function setGameIconUrl(url: string): void {
  const coreStore = useCoreStore()
 ...
 */
export function setGameSpecificInitialization(
  initializationFunction: (initializationApi: typeof InitializationApi) => Promise<void>
): void {
  gameSpecificInitializationStep = async (): Promise<void> => {
    await initializationFunction(InitializationApi)
  }
}
 ...
 */
export let gameSpecificInitializationStep: () => Promise<void>
