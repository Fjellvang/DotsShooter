...
const playerPhaseInfos: Record<string, PhaseInfo> = {
  Preview: {
    sortOrder: 1,
...
const nonPlayerPhaseInfos: Record<string, PhaseInfo> = {
  Preview: {
    sortOrder: 1,
 ...
 */
export function allPhaseDisplayStrings(hasPlayer: boolean, hideErrorStrings = true): string[] {
 ...
 */
export function phaseSortOrder(
  activable: ActivableData,
  overridePhase: string | null = null,
  player: any = null
): number {
 ...
 * @see getPhaseInfo about the parameters.
 */
export function phaseDisplayString(
  activable: ActivableData,
  overridePhase: string | null = null,
  player: any = null
): string {
  return (
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
    getPhaseInfo(activable, overridePhase, player).displayString || `Error: ${overridePhase}`
  )
}
...
export function phaseTooltip(
  ...
  return (
    // prettier-ignore
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
    (getPhaseInfo(activable, overridePhase, player).tooltip ||
      `Error: ${overridePhase}`)
      // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
...
export function phaseBadgeVariant(
  ...
  return (
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
    getPhaseInfo(activable, overridePhase, player).badgeVariant || 'warning'
  )
}
