...
import type { NodeSet, RawNodeData } from '../clusterServerTypes'
import type { ErrorCountResponse, TelemetryMessagesResponse } from './generalTypes'
...
/**
 * Subscribe to game server status from the `/status` endpoint.
 * @returns Subscription object.
 */
 ...
 * @returns Subscription object.
 */
export function getServerErrorsSubscriptionOptions(): SubscriptionOptions<ErrorCountResponse> {
  return {
...
/**
 * Subscribe to backend telemetry messages and details from the `/telemetryMessages` endpoint.
 * @returns Subscription object.
 */
export function getTelemetryMessagesSubscriptionOptions(): SubscriptionOptions<TelemetryMessagesResponse> {
  return {
    permission: 'api.system.view_telemetry_messages',
    pollingPolicy: getPollingPolicyTimer(30000),
    fetcherPolicy: getFetcherPolicyGet('/telemetryMessages'),
    cacheRetentionPolicy: getCacheRetentionPolicyKeepForever(),
  }
}
...
interface StaticConfig {
  ...
  activablesMetadata: {
    categories: Record<string, ActivableCategoryMetadata>
    kinds: Record<string, ActivableKindMetadata>
  }
...
interface ActivableKindMetadata {
  displayName: string
  category: string
  description: string
...
interface ActivableCategoryMetadata {
  displayName: string
...
interface ServerConfigBuildInfo {
  buildSupported: boolean
  buildParametersType: string
  buildParametersNamespaceQualifiedName: string
  slotToAvailableSourcesMapping: Record<string, Array<{ displayName: string }>>
}
 ...
 * @returns Subscription object.
 */
export function getStaticConfigSubscriptionOptions(): SubscriptionOptions<StaticConfig> {
  return {
    permission: 'api.general.view',
    pollingPolicy: getPollingPolicyOnceOnly(),
...
/**
 * Subscribe to all generated form types from the `/getAllFormTypes` endpoint.
 * @returns Subscription object.
 */
export function getAllGeneratedFormTypes(): SubscriptionOptions {
  return {
    permission: 'api.general.view',
    pollingPolicy: getPollingPolicyOnceOnly(),
    fetcherPolicy: getFetcherPolicyGet('testing/getAllFormTypes'),
    cacheRetentionPolicy: getCacheRetentionPolicyKeepForever(),
  }
}
...
interface ClusterOptions {
  nodeSets: NodeSet[]
  autoscaling: NodeSet
  workload: RawNodeData
}

/**
 * Subscribe to cluster status from the `/cluster` endpoint.
 * @returns Subscription object.
 */
export function getClusterSubscriptionOptions(): SubscriptionOptions<ClusterOptions> {
  return {
    permission: 'api.system.view_cluster_nodes',
    pollingPolicy: getPollingPolicyTimer(2000),
    fetcherPolicy: getFetcherPolicyGet('/cluster/status'),
    cacheRetentionPolicy: getCacheRetentionPolicyKeepForever(),
  }
}
