...
export function getAllActivablesSubscriptionOptions(customEvaluationTime?: string): SubscriptionOptions {
  return {
    ...
    fetcherPolicy: getFetcherPolicyGet(
      customEvaluationTime ? `/activables?time=${encodeURIComponent(customEvaluationTime)}` : '/activables'
    ),
    cacheRetentionPolicy: customEvaluationTime
      ? getCacheRetentionPolicyDeleteImmediately()
      : getCacheRetentionPolicyTimed(10000),
  }
}
 ...
 * @param customEvaluationTime Optional: Custom evaluation time. If not provided, the current game server time will be used.
 */
export function getAllActivablesForPlayerSubscriptionOptions(
  playerId: string,
  customEvaluationTime?: string
): SubscriptionOptions {
  return {
    permission: 'api.activables.view',
    pollingPolicy: getPollingPolicyTimer(5000),
    fetcherPolicy: getFetcherPolicyGet(
      customEvaluationTime
        ? `/activables/${playerId}?time=${encodeURIComponent(customEvaluationTime)}`
        : `/activables/${playerId}`
    ),
    cacheRetentionPolicy: customEvaluationTime
      ? getCacheRetentionPolicyDeleteImmediately()
      : getCacheRetentionPolicyTimed(5000),
  }
}
