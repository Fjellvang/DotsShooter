...
export function getAllOffersSubscriptionOptions(customEvaluationTime?: string): SubscriptionOptions {
  return {
    ...
    fetcherPolicy: getFetcherPolicyGet(
      customEvaluationTime ? `/offers?time=${customEvaluationTime.replace('+', '%2B')}` : '/offers'
    ),
 ...
 * @param customEvaluationTime The time to evaluate the offers at. If not provided, the current time will be used.
 */
export function getAllOffersForPlayerSubscriptionOptions(
  playerId: string,
  ...
  return {
    ...
    fetcherPolicy: getFetcherPolicyGet(
      customEvaluationTime
        ? `/offers/${playerId}?time=${encodeURIComponent(customEvaluationTime)}`
        : `/offers/${playerId}`
    ),
    cacheRetentionPolicy: customEvaluationTime
      ? getCacheRetentionPolicyDeleteImmediately()
