...
MViewContainer(
  ...
  :error="databaseEntityError"
  )
  template(#overview)
    MPageOverviewCard(
      :id="id"
      title="Database Entity"
      data-testid="database-entity-overview-card"
      )
        ...
        b-tbody
          b-tr
            ...
            b-td(
              ...
              meta-time(
                :date="databaseEntityData.persistedAt"
                showAs="datetime"
                )
          b-tr
            b-td
              div Entry Schema Version
              div(class="small text-muted") The version of the payload in the database. This can be lower than the latest version if entity is not yet migrated.
            b-td(class="tw-text-right") {{ databaseEntityData.schemaVersion }}
          b-tr
            b-td
              div Actor Schema Version
              div(class="small text-muted") The current schema version in use. Entities on older versions will be migrated on the next actor wake-up.
            b-td(class="tw-text-right") {{ databaseEntityData.currentSchemaVersion }}
          b-tr
            b-td
              div Is Final
              div(class="small text-muted") An entity is 'final' when it is not currently in use and has been written to the database cleanly.
            b-td(class="tw-text-right")
              MBadge(:variant="databaseEntityData.isFinal ? 'success' : 'danger'") {{ databaseEntityData.isFinal }}
          b-tr
            b-td
              div Size on database
              div(class="small text-muted") The number of bytes this entity takes when stored in the database. The entity may be compressed.
            b-td(class="tw-text-right")
              meta-abbreviate-number(
                :value="databaseEntityData.compressedSize"
                unit="byte"
                )
          b-tr
            b-td
              div Uncompressed Size
              div(class="small text-muted") The size of the entity state when read from the database and uncompressed.
            b-td(class="tw-text-right")
              meta-abbreviate-number(
                :value="databaseEntityData.uncompressedSize"
                unit="byte"
                )

  b-card(
    class="mb-2 table-container tw-mt-4"
    data-testid="database-entity-data-card"
    )
    b-card-title Stored Data

    b-table-simple(
      v-if="databaseEntityData?.structure"
      small
      responsive
      class="text-break"
      )
      ...
      b-tbody(class="text-monospace small")
        tr(
          v-for="row in visibleRows"
          :key="row.id"
          :class="{ highlight: row.hasChildren }"
          class="small"
          @click="toggleExpandedKey(row.id)"
          )
          td
            //- Row header
            div(
              :style="'padding-left: ' + row.depth + 'rem'"
              :class="{ open: row.isExpanded, 'table-row-link': row.hasChildren }"
              class="table-row"
              )
              fa-icon(
            ...
            div(
              ...
              class="bg-light rounded border p-2 small"
              )
          ...
          //- Size info
          td(
            :class="rowSizeToCssClass(row.size)"
            style="width: 6rem"
            class="tw-text-right"
            ) {{ row.size }}#[small(class="text-muted") bytes]
          td(
            :class="rowSizeToCssClass(row.size)"
            style="width: 5rem"
            class="tw-text-right"
            ) ({{ row.sizePercent.toFixed(2) }}%)
    ...
    div(v-else)
      meta-alert(title="Entity State Not Available.")
        ...
        div(class="mt-2") If the problems persists then it may point to an issue.
</template>
...
import { MBadge, MPageOverviewCard, MTextButton, MViewContainer } from '@metaplay/meta-ui-next'
...
function getAllRows(expandedKeys?: any): any[] {
  if (!databaseEntityData.value?.structure) {
    return []
  }

  const totalSize =
    databaseEntityData.value.structure.EnvelopeEndOffset - databaseEntityData.value.structure.EnvelopeStartOffset

  // eslint-disable-next-line @typescript-eslint/max-params
  function addVisibleRows(rows: any, structure: any, path: any, depth: any, sortOrder: any, forcedName?: any): void {
      ...
      fn: () =>
        rows.push({
          ...
          name:
            forcedName ||
            structure.FieldName ||
            (structure.TagId && `[member ${structure.TagId}]`) ||
            structure.TypeName ||
            '[unknown]',
          size: envelopeSize,
    ...
    if (isExpanded) {
      ...
      if (hasMembers) {
        for (let ndx = 0; ndx < structure.Members.length; ++ndx) {
          insertFns.push({
            ...
            fn: () => {
              addVisibleRows(rows, structure.Members[ndx], `${path}.m${ndx}`, depth + 1, sortOrder)
            },
          })
        }
      }
      if (hasElements) {
        for (let ndx = 0; ndx < structure.Elements.length; ++ndx) {
          insertFns.push({
            ...
            fn: () => {
              addVisibleRows(rows, structure.Elements[ndx], `${path}.e${ndx}`, depth + 1, sortOrder)
            },
          })
        }
      }
      if (hasDictionary) {
        for (let ndx = 0; ndx < structure.Dictionary.length; ++ndx) {
          const keySize =
            structure.Dictionary[ndx].Key.EnvelopeEndOffset - structure.Dictionary[ndx].Key.EnvelopeStartOffset
          const valueSize =
            structure.Dictionary[ndx].Value.EnvelopeEndOffset - structure.Dictionary[ndx].Value.EnvelopeStartOffset
          ...
          insertFns.push({
            ...
            fn: () => {
              ...
              if (entryIsExpanded) {
                addVisibleRows(
                  rows,
                  structure.Dictionary[ndx].Key,
                  `${entryPath}.k`,
                  depth + 2,
                  sortOrder,
                  structure.Dictionary[ndx].Key.TypeName ? `[key: ${structure.Dictionary[ndx].Key.TypeName}]` : '[key]'
                )
                addVisibleRows(
                  rows,
                  structure.Dictionary[ndx].Value,
                  `${entryPath}.v`,
                  depth + 2,
                  sortOrder,
                  structure.Dictionary[ndx].Value.TypeName
                    ? `[value: ${structure.Dictionary[ndx].Value.TypeName}]`
                    : '[value]'
                )
              }
            },
...
function rowSizeToCssClass(size: number): string {
...
function toggleExpandedKey(id: string): void {
...
function toggleRowDetails(id: string): void {
