...
MViewContainer(
  ...
  template(#overview)
    MPageOverviewCard(
      :title="singleLeagueSeasonData.displayName"
      data-testid="league-season-detail-overview-card"
      )
      template(#subtitle)
        div(v-if="singleLeagueData") #[MTextButton(:to="`/leagues/${leagueId}`") {{ singleLeagueData.details.leagueDisplayName }}] / {{ singleLeagueSeasonData.displayName }}
        p(class="mt-2") {{ singleLeagueSeasonData.description }}
        ...
        b-tbody
          b-tr(v-if="!singleLeagueSeasonData.isCurrent")
            ...
            b-td(class="tw-text-right")
              meta-time(
                :date="singleLeagueSeasonData.startTime"
                showAs="datetime"
                )
          ...
          b-tr(v-if="!singleLeagueSeasonData.isCurrent")
            ...
            b-td(class="tw-text-right")
              meta-time(
                :date="singleLeagueSeasonData.endTime"
                showAs="datetime"
                )
          ...
          b-tr(v-if="!singleLeagueSeasonData.isCurrent")
            ...
            b-td(class="tw-text-right")
              meta-duration(
                :duration="DateTime.fromISO(singleLeagueSeasonData.endTime).diff(DateTime.fromISO(singleLeagueSeasonData.startTime))"
                showAs="top-two"
                )
          b-tr
            b-td Total participants
            b-td(class="text-right") {{ singleLeagueSeasonData.totalParticipantCount }}
          b-tr
            b-td First-time participants
            b-td(class="text-right") {{ singleLeagueSeasonData.newParticipantCount }}
          b-tr(v-if="!singleLeagueSeasonData.isCurrent")
            b-td Dropped at season's end
            b-td(class="text-right") {{ singleLeagueSeasonData.numDropped }}
          b-tr(v-if="singleLeagueSeasonData.isCurrent")
            b-td(colspan="2")
              league-progress-card(
                v-if="singleLeagueData && singleLeagueData.enabled"
                :leagueId="leagueId"
                )
  MSingleColumnLayout
    core-ui-placement(
      placementId="Leagues/Season/Details"
      :leagueId="leagueId"
      :seasonId="seasonId"
      )
        ...
        template(#item-card="{ item: singleDivisionData }")
          MListItem {{ singleDivisionData.displayName }}
            ...
            template(#bottom-right)
              MTextButton(
                link
                :to="`/leagues/${leagueId}/${seasonId}/${singleDivisionData.entityId}`"
                permission="api.leagues.view"
                ) View division

  meta-raw-data(
    :kvPair="singleLeagueSeasonData"
    name="singleLeagueSeasonData"
    )
  meta-raw-data(
    :kvPair="activeDivisionsData"
    name="activeDivisionsData"
    )
</template>
...
import {
  ...
  getActiveDivisionsForLeagueSeasonSubscriptionOptions,
...
const seasonIdError: DisplayError | undefined = isFinite(seasonId)
  ? undefined
  : new DisplayError('Invalid season ID', `Season ID must be a number, not '${route.params.seasonId.toString()}'.`)
 ...
 */
const { data: singleLeagueSeasonData, error: singleLeagueSeasonError } = useSubscription(() => {
  return seasonIdError ? undefined : getSingleLeagueSeasonSubscriptionOptions(leagueId, seasonId)
})
 ...
 */
const { data: activeDivisionsData } = useSubscription<any[] | undefined>(() => {
  return seasonIdError ? undefined : getActiveDivisionsForLeagueSeasonSubscriptionOptions(leagueId, seasonId)
})
...
const alerts = computed(() => {
  ...
  if (isNullOrUndefined(singleLeagueData.value) && isNullOrUndefined(singleLeagueSeasonData.value)) {
    ...
    if (
      singleLeagueSeasonData.value.isCurrent === true &&
      singleLeagueData.value.state.currentSeason.endedEarly === true
    ) {
      allAlerts.push({
    ...
    if (
      singleLeagueSeasonData.value.isCurrent === true &&
      singleLeagueData.value.state.currentSeason.startedEarly === true
    ) {
      allAlerts.push({
...
const pastSeasonAlertMessage = computed(() => {
  if (singleLeagueSeasonData.value?.startedEarly === true && singleLeagueSeasonData.value?.endedEarly === false) {
    ...
    singleLeagueSeasonData.value?.startedEarly === false &&
    singleLeagueSeasonData.value?.endedEarly === true
  ) {
