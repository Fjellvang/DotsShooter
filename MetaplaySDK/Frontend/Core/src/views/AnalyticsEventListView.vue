...
MViewContainer(
  ...
  template(#overview)
    MPageOverviewCard(
      title="View Analytics Event Types"
      data-testid="analytics-event-list-overview-card"
      )
      p These are all the currently implemented server-side analytics events.
      div(class="tw-text-xs+ tw-text-neutral-500") You can use this page to explore and inspect the individual analytics events. This might be especially useful for data analysts who need to know the exact contents and formatting of events.
  ...
  MTwoColumnLayout
    meta-list-card(
      ...
      :pageSize="20"
      data-testid="analytics-event-list-core-events-card"
      )
    ...
    meta-list-card(
      ...
      :pageSize="20"
      data-testid="analytics-event-list-custom-events-card"
      )
      template(v-slot:item-card="slot")
        analytics-type-list-group-item(:event="slot.item")

  meta-raw-data(
    :kvPair="allAnalyticsEventsData"
...
import { MPageOverviewCard, MViewContainer, MTwoColumnLayout } from '@metaplay/meta-ui-next'
import { useSubscription } from '@metaplay/subscriptions'

import AnalyticsTypeListGroupItem from '../components/analytics/AnalyticsTypeListGroupItem.vue'
import type { EventInfo } from '../components/mails/mailUtils'
import { getAllAnalyticsEventsSubscriptionOptions } from '../subscription_options/analyticsEvents'

/**
 * Subscribe to events data.
 */
const { data: allAnalyticsEventsData, error: allAnalyticsEventsError } = useSubscription(
 ...
 */
const searchFields = ['categoryName', 'displayName', 'docString', 'typeCode', 'eventType']
...
const filterSets = computed(() => {
  return [
    MetaListFilterSet.asDynamicFilterSet(
      allAnalyticsEventsData.value,
      'category',
      (eventSpec: any) => (eventSpec as EventInfo).categoryName
    ),
  ]
})
 ...
 * aka "custom".
 */
function isCoreEvent(event: EventInfo): boolean {
...
const coreEvents = computed(() => {
  return Object.values(allAnalyticsEventsData.value ?? {}).filter((e) => isCoreEvent(e))
})
