 ...
 */
export function getPollingPolicyTimer(timeoutInMs: number): PollingPolicyTimer {
 ...
 */
export function getPollingPolicyOnceOnly(): PollingPolicyOnceOnly {
...
export abstract class PollingPolicy {
   ...
   */
  public register(requestFetchCallback: RequestFetchCallback): void {
  ...
  /**
   * If the fetch fails, the poller gets to decide what to do.
   */
  public abstract onFetchFailed(): void
}
 ...
 * created.
 * Note that in the case where the initial fetch fails (eg: if we are offline) then we will try again until a fetch
 * succeeds. This still means that we only ever make one fetch of the data.
 */
class PollingPolicyOnceOnly extends PollingPolicy {
  ...
  /**
   * We never have any polling to cancel. See `onFetchFailed` for the exception.
   */
  cancel(): void {
    if (this.timerId !== null) {
      clearTimeout(this.timerId)
  ...
  /**
   * If the data fetch fails, we will try again in 1 second.
   */
  onFetchFailed(): void {
    this.timerId = setTimeout(() => {
      this.timerId = null
