...
MTooltip(
  ...
  MButton(
    v-if="fullSize"
    :disabled-tooltip="tooltips.disableButton"
    @click="copy(contents)"
...
/**
 * Tooltip content for the wrapping `MTooltip` component and the underlying button component.
 */
const tooltips = computed((): { title?: string; disableButton?: string } => {
  if (props.disabled) {
    // Copy is disabled.
    return {
      disableButton: 'Copy to clipboard is disabled.',
    }
  } else if (!props.contents) {
    // No content was supplied.
    return {
      disableButton: 'No content to copy!',
    }
  } else {
    // There is content to copy and the button is enabled.
    return {
      title: !copied.value ? 'Copy to clipboard.' : 'Copied!',
    }
  }
})
</script>
