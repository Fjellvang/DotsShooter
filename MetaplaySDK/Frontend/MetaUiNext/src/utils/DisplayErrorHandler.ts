...
export class DisplayError {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  addDetail(title: string, content: string | string[] | Record<string, any>): void {
 ...
 */
export function registerHandler(handler: ErrorHandler): void {
 ...
 */
function handleError(error: Error): DisplayError | undefined {
  if (error instanceof Error) {
    return new DisplayError('JavaScript Error', error.message, error.name, undefined, [
      {
 ...
 * @param error The error object we are interested in.
 */
function handleAxiosError(error: Error): DisplayError | undefined {
  if (axios.isAxiosError(error)) {
    ...
    displayError.addDetail('API Response', axiosErrorObject.response?.data || 'None')
 ...
 */
export function createDisplayError(error: Error): DisplayError {
