 ...
 */
export function validateEvents(events: EventStreamItemBase[]): boolean {
...
export function wrapSessions(events: EventStreamItemBase[]): EventStreamItemBase[] {
  ...
  const flush = (events: EventStreamItemBase[], startTerminated: boolean): EventStreamItemBase[] => {
    ...
    flushedEvents.push(new EventStreamItemSession(newPath, events[0], events[events.length - 1], events.length))
  ...
  events.forEach((srcEvent) => {
    if (srcEvent instanceof EventStreamItemEvent) {
      ...
      if (eventType === 'Metaplay.Core.Player.PlayerEventClientConnected') {
        ...
        if (sessionsStartedCount > 0) {
          ...
          sessionEvents.push(srcEvent)
          newEvents.push(...flush(sessionEvents, true))
          sessionEvents = []
        } else {
          ...
          newEvents = flush(newEvents, false)
        }
...
export function wrapDays(events: EventStreamItemBase[]): EventStreamItemBase[] {
  ...
  const flush = (events: EventStreamItemBase[]): EventStreamItemBase[] => {
    const flushedEvents: EventStreamItemBase[] = []
    ...
    // Finally, push a wrapper event.
    flushedEvents.push(new EventStreamItemDay(newPath, events[0], events[events.length - 1], events.length))
    return flushedEvents
  }
...
export function wrapRepeatingEvents(events: EventStreamItemBase[], minimumSize = 5): EventStreamItemBase[] {
  ...
  const flush = (events: EventStreamItemBase[]): EventStreamItemBase[] => {
    if (events.length >= minimumSize) {
      ...
      flushedEvents.push(
        new EventStreamItemRepeatedEvents(events[0].path, events[0], events[events.length - 1], events.length)
      )
  ...
  events.forEach((srcEvent) => {
    if (typeof srcEvent === typeof EventStreamItemEvent) {
...
export function generateStats(events: EventStreamItemBase[]): EventStreamStats {
  ...
  const numEvents = events.reduce((p, c) => {
    return p + (c instanceof EventStreamItemEvent ? 1 : 0)
  }, 0)
 ...
 */
export function getFiltersForEventStream(events: EventStreamItemBase[]): {
  eventTypes: string[]
