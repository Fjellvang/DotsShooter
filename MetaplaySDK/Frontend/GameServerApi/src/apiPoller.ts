 ...
 */
type ParamsCallback<ParamsDataType> = () => ParamsDataType | undefined
 ...
 */
type HandlerCallback<ReturnedDataType> = (data: ReturnedDataType) => void
...
export class ApiPoller<ParamsDataType = object, ReturnedDataType = any> {
  private readonly timeout: number | TimeoutCallback
  private readonly method: ApiPollerHttpMethod
  private readonly endpoint: string
  private readonly fnParams?: ParamsCallback<ParamsDataType>
  private readonly fnHandler: HandlerCallback<ReturnedDataType>
  private readonly fnError?: ErrorCallback
  ...
  public constructor(
    ...
    method: ApiPollerHttpMethod,
    endpoint: string,
    fnParams: ParamsCallback<ParamsDataType> | undefined,
    fnHandler: HandlerCallback<ReturnedDataType>,
  ...
  private async poll(): Promise<void> {
    try {
      ...
      let params: ParamsDataType | undefined
      if (this.fnParams) {
        params = this.fnParams()
      }
      const result = (
        await this.gameServerApi.request({
          url: this.endpoint,
          method: this.method,
      ...
      if (axios.isCancel(e)) {
        ...
        if (this.fnError) {
          ...
          console.error(`Error during API polling of ${this.endpoint}: ${String(e)}`)
        }
      }
    } finally {
      this.requestLockCount--
