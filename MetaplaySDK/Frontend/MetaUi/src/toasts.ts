...
/**
 * @deprecated Use the `useNotifications` composable from `@metaplay/meta-ui-next` instead.
 */
export function useToastsVuePlugin(app: App): void {
  app.use(Toast, {
  ...
  const warningMessagesToIgnore = ['(deprecation COMPONENT_FUNCTIONAL)']
  ...
  const traceItemsToIgnore = [
    ...
    'GeneratedUiViewDynamicComponent', // ATTR_FALSE_VALUE warning -> non-issue.
    'GeneratedUiViewContainerField', // ATTR_FALSE_VALUE warning -> non-issue.
    'GeneratedUiViewSectionContent', // ATTR_FALSE_VALUE warning -> non-issue.
    ...
    'MInputSingleSelectSwitch', // INSTANCE_ATTRS_CLASS_STYLE warning -> non-issue.
    'MInputDate', // ATTR_FALSE_VALUE warning that seemed to be intended behaviour.
    'MInputSwitch', // ATTR_FALSE_VALUE checked has value 'false' of course.
    'MInputTime', // ATTR_ENUMERATED_COERCION warning -> non-issue.
    'CoreUiPlacement', // CORE_ASYNC_COMPONENT warning -> non-issue.
    'MIconButton', // Attr inheritance warning -> non-issue.
    'MClipboardCopy', // WATCH_ARRAY warning -> non-issue.
    'MSidebarLink', // INSTANCE_ATTRS_CLASS_STYLE -> non-issue.
  ]

  // Vue warnings only happen in development builds, so this code will never fire in production.
  app.config.warnHandler = function (msg: string, instance: ComponentPublicInstance | null, trace: string): void {
    ...
    console.warn(`${msg}\n${trace}`)
    // eslint-disable-next-line @typescript-eslint/no-deprecated -- This should be re-implemented when getting rid of compat mode.
 ...
 * @example
   ...
   @deprecated Use the `useNotifications` composable from `@metaplay/meta-ui-next` instead.
 */
export function showSuccessToast(message: string, title = 'Done'): void {
 ...
 */
function show(message: string, title: string, toastOptions: ToastOptions): void {
