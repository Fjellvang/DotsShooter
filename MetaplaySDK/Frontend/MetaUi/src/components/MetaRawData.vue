  ...
  b-col(lg="10")
    div
      ...
      span(
        ...
        span for
        MBadge(class="tw-ml-1") '{{ name }}'
      span
        span(class="small text-muted d-inline-block tw-ml-1") (enabled in developer mode)

    div(v-if="kvPair != null")
      ul(
        v-if="Object.keys(kvPair).length > 0"
        class="tw-divide-y tw-divide-neutral-200 tw-text-xs"
        )
        li(
          v-for="(rootValue, rootKey) in kvPair"
          :key="rootKey"
          class="pl-0 pr-0 pb-0 pb-2 text-monospace tw-break-all tw-py-3"
          )
          b-row(
            no-gutters
            align-v="center"
            :class="{ 'not-collapsed': isPropOpen(rootKey) }"
            class="pointer text-break"
            @click="toggleProp(String(rootKey))"
            )
            fa-icon(
              icon="angle-right"
              size="1x"
          ...
          div(v-if="isPropOpen(rootKey)")
            ...
            //- Object.
            div(
              v-else-if="typeof rootValue === 'object'"
              class="pl-2"
              )
              div(
                v-for="childKey in Object.keys(rootValue)"
                :key="childKey"
                style="margin-bottom: 0.1rem"
                )
                div(
                  :class="{ 'not-collapsed': isPropOpen(rootKey, childKey) }"
                  class="pointer"
                  @click="toggleProp(rootKey, childKey)"
                  )
                  fa-icon(
                    icon="angle-right"
                    size="1x"
                  ...
                  //- Muted preview when closed.
                  span(
                    v-if="!isPropOpen(rootKey, childKey)"
                    class="ml-2 text-muted small"
                    )
                    //- null.
                ...
                div(
                  ...
                  MBadge(
                    v-if="rootValue[childKey] == null"
                    class="tw-mb-1"
                    ) null
                  MBadge(
                    v-else-if="rootValue[childKey] === ''"
                    class="tw-mb-1"
                    ) empty string
...
const props = defineProps<{
  kvPair?: Record<string | number, any>
  name?: string
}>()
...
const unwatch = watch(
  () => props.kvPair,
  (newVal: Record<string | number, any>, oldVal: Record<string | number, any>) => {
    if (!oldVal && newVal) {
...
function isPropOpen(...keys: any[]): boolean {
  const key = keys.toString()
  return openProps.value.includes(key)
}

function toggleProp(...keys: any[]): void {
  const key = keys.toString()
  if (openProps.value.includes(key)) {
    openProps.value = openProps.value.filter((e) => e !== key)
 ...
 */
function renderArray(rootValue: any[]): string {
  if (rootValue.length > 5) {
    const firstFive = rootValue
      .slice(0, 5)
      .map((value) => `${typeof value === 'object' ? 'object' : value},`)
      .join(' ')
 ...
 */
function renderObject(rootValue: object): string {
