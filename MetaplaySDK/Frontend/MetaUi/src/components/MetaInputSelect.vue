...
Multiselect(
  ...
  template(#singlelabel="item: { value: MultiselectOption<T> }")
    div(style="z-index: 1;").tw-w-full.multiselect-single-label
 ...
 */
function checkForUniqueIds(options: Array<MultiselectOption<T>>): void {
  const ids = options.map((option) => option.label)
  const uniqueIds = new Set(ids)
  if (ids.length !== uniqueIds.size) {
    console.error('Duplicate IDs found in options array of MetaInputSelect:', options)
  }
}
 ...
 */
async function lazyOptionsForMultiSelectComponent(searchString: string): Promise<Array<MultiselectOption<T>>> {
 ...
 */
function getOptionValueById(id: string): unknown {
  let option: unknown
  if (isOptionsAFunction) {
 ...
 */
function onSearchChange(query: any): void {
 ...
 */
function searchFilter(option: MetaInputSelectOption<T>): boolean {
  if (typeof option.value === 'object') {
    ...
    return (
      (option.value as string).toString().toLowerCase().includes(searchQuery.value) ||
      (option as any).label.toString().toLowerCase().includes(searchQuery.value)
    )
  } else {
    return true
  }
 ...
 */
function onUpdateInput(newValue?: MultiselectOption<T> | Array<MultiselectOption<T>>): void {
  ...
  if (!props.multiselect) {
    ...
    emit(
      'input',
      (newValue as any[]).map((option) => option.value)
    ) // Issue with TS generics here. Emit type defined above so this still works as expected.
  }
}
...
watch(multiselectRef, () => {
  ...
  if (clientWidth && children) {
    ...
    const dropdownElement = Object.values(children).find((el) =>
      (el as HTMLElement).className.includes('multiselect-dropdown')
    )
    if (dropdownElement) {
      const htmlElement = dropdownElement as HTMLElement
      htmlElement.style.minWidth = `${clientWidth}px`
    }
  }
})
</script>
