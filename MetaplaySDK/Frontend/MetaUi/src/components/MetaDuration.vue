...
// TODO: refactor to Luxon
import { computed } from 'vue'

import { MTooltip } from '@metaplay/meta-ui-next'
import { stringToSentenceCase } from '@metaplay/meta-utilities'

import { constructExactTimeCompactString, constructExactTimeVerboseString } from '../utils/metaDurationUtils'
...
const props = withDefaults(
  defineProps<{
     ...
     */
    // eslint-disable-next-line @typescript-eslint/no-redundant-type-constituents -- `number | string` and `any` are overlapping types.
...
const tooltipContent = computed(() => {
  ...
  if (
    ...
    moment.duration(props.duration).asMilliseconds() === 0 ||
...
const bodyContent = computed(() => {
  switch (props.showAs) {
    ...
    case 'humanizedSentenceCase':
      return stringToSentenceCase(humanizedString.value)
    case 'top-two':
      return topString(2)
    case 'top-three':
      return topString(3)
...
/**
 * Precise exact duration string. (e.g., '1y 1mo 1d 1h 1min 1s' when there are multiple units of time and '45 minutes' when there is only one unit of time).
 */
...
const exactTimeTooltip = computed(() => {
  return (
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
    constructExactTimeVerboseString(props.duration, props.hideMilliseconds) + ' exactly'
  )
})
 ...
 */
function topString(count: number): string {
  const values: string[] = []
  // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
  const duration = moment.duration(props.duration)
 ...
 */
function constructExactTimeUnit(current: string, value: number, unit: string): string {
  if (value === 0) {
    return current
  } else {
