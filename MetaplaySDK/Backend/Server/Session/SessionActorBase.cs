...
namespace Metaplay.Server
{
    ...
    public abstract class SessionActorBase<TGameSessionStartParams, TGameSessionState> : EphemeralEntityActor
    {
        ...
        DateTime?                                               _clientPauseStatusSetAt                     = null;
        ...
        DateTime                                                _nextInvitationInspectionEarliestAt         = DateTime.UnixEpoch;
        Dictionary<int, EntitySubscription>                     _guildViews                                 = new Dictionary<int, EntitySubscription>();
        bool                                                    _hasOngoingGuildSearch;
        #endif

        // Fields below are set once in successful session start
        ...
        int                                                     _numResourceCorrectionsSent;
        ...
        protected override async Task OnSubscriptionLostAsync(EntitySubscription subscription)
        {
        ...
        protected override async Task OnSubscriptionKickedAsync(EntitySubscription subscription, MetaMessage message)
        {
            if (subscription == _playerActor)
            {
        ...
        subscribeAgain:
        {
            ...
            SessionProtocol.SessionResourceCorrection               optionalCorrection  = new SessionProtocol.SessionResourceCorrection();
            ...
            MetaDictionary<ClientSlot, EntitySubscription>       slotToSubscription  = new MetaDictionary<ClientSlot, EntitySubscription>();
            ...
            catch (InternalPlayerSessionSubscribeRefused playerRefused)
            {
                ...
                else if (playerRefused.Result == InternalPlayerSessionSubscribeRefused.ResultCode.Deleted)
                {
                    // player is deleted, refuse session immediately
                    throw InternalSessionStartNewRefusal.ForPlayerIsDeleted();
                }
            ...
            for (int associatedEntityNdx = 0; associatedEntityNdx < pendingAssociations.Count; ++associatedEntityNdx)
            {
                ...
                try
                {
                    ...
                    PendingAssociatedEntitySubscription subscription = await BeginSubscribeToAssociatedEntityAsync(
                        ...
                        isDryRun:                       resourceCorrection.HasAnyCorrection(),
                ...
                catch (InternalEntitySubscribeRefusedBase.Builtins.DryRunSuccess refused)
                {
                    ...
                    // Combine optional resources
                    optionalCorrection = SessionProtocol.SessionResourceCorrection.Combine(optionalCorrection, refused.ResourceCorrection);
                }
                ...
                catch (InternalEntitySubscribeRefusedBase refusal)
                {
                    ...
                    await EntityAskAsync(association.SourceEntity, new InternalEntityAssociatedEntityRefusedRequest(association, refusal));
            ...
            {
                _numResourceCorrectionsSent += 1;
                if (_numResourceCorrectionsSent > 1)
                    _log.Warning("Client resources did not resolve after first attempt. Attempt {AttemptNum}. Correction: {Correction}", _numResourceCorrectionsSent, resourceCorrection);

                // If client will apply the mandatory corrections, it might as well apply optionals too. Include optionalCorrection.
                throw InternalSessionStartNewRefusal.ForResourceCorrection(SessionProtocol.SessionResourceCorrection.Combine(resourceCorrection, optionalCorrection));
            }
            ...
            _logicVersion                       = startNewRequest.ClientVersion.LogicVersion;
        ...
        protected DefaultPlayerSessionParams CreateDefaultPlayerSessionParams(SessionStartParams sessionStart, SessionToken sessionToken)
        {
            return new DefaultPlayerSessionParams(
                ...
                logicVersion:                   sessionStart.Meta.ClientVersion.LogicVersion,
                timeZoneInfo:                   sessionStart.Meta.PlayerTimeZoneInfo,
                location:                       sessionStart.Meta.PlayerLocation,
                clientVersion:                  sessionStart.Meta.ClientBuildVersion,
        ...
        InternalSessionResumeResponse ResumeSession(EntitySubscriber client, InternalSessionResumeRequest resumeRequest)
        {
            ...
            if (_phase != ActorPhase.InSession)
            {
                ...
                // The SessionActor is either in terminating state or waiting for new connection, which most likely means
                // that it has been terminated by timeout. This is expected behaviour, so communicate error as non-fatal.
                throw InternalSessionResumeRefusal.ForSessionTimeout();
            }
            if (_logicVersion != resumeRequest.LogicVersion)
            {
                _log.Error("Refused session resume: LogicVersion mismatch (session: {SessionLogicVersion}, request: {RequestLogicVersion})", _logicVersion, resumeRequest.LogicVersion);
                throw InternalSessionResumeRefusal.ForUnexpectedResumeFailure();
            }
                ...
                case SessionUtil.ResumeResult.Failure resumeFailure:
                    ...
                    switch (resumeFailure)
                    {
                        ...
                        default:
                            ...
                            if (resumeFailure is SessionUtil.ResumeResult.Failure.WeHaveForgottenTooMany)
                                throw InternalSessionResumeRefusal.ForSessionTimeout();
                            throw InternalSessionResumeRefusal.ForUnexpectedResumeFailure();
                    }
        ...
        SessionForceTerminateReason TryConvertPlayerForceKickOwnerReason(PlayerForceKickOwnerReason reason)
        {
            switch (reason)
            {
                ...
                case PlayerForceKickOwnerReason.GameConfigUpdated:              return new SessionForceTerminateReason.GameConfigUpdated();
                case PlayerForceKickOwnerReason.LogicVersionUpdated:            return new SessionForceTerminateReason.LogicVersionUpdated();
        ...
        void UpdateDebugChecks()
        {
            ...
            if (// There must have been a session resumption.
                _statistics.NumConnections > 1
                // Connection must be ongoing.
             && _statistics.RecentConnectionEvents.Last().Kind == SessionStatistics.ConnectionEvent.EventKind.Start
                // Client must not be paused. If the client goes to pause after sending the
                // session resumption request, but before receiving the response (which is when
                // it sends the ping) then it won't send the ping until coming back from pause.
             && _clientPauseStatus == ClientAppPauseStatus.Running)
            {
                 ...
                 && !diag.CurrentPauseDuration.HasValue) // Client wasn't on pause when it sent the login.
                {
                    ...
                    // The ping deadline is based on connection start time, except that if a client pause
                    // happened after the connection start, then the deadline is based on the unpause time (_clientPauseStatusSetAt).
                    DateTime                pingWaitStartedAt               = _clientPauseStatusSetAt.HasValue
                                                                              ? Util.Max(_clientPauseStatusSetAt.Value, connectionStartedAt)
                                                                              : connectionStartedAt;

                    if (!pingReceivedInThisConnection
                     && currentTime > pingWaitStartedAt + options.SessionPingDurationIncidentThreshold // Has been waiting for ping long enough. \note Doesn't include estimated roundtrip
                    ...
                    {
                        ReportSessionPingDurationThresholdExceeded(connectionIndex, durationWaitedForPing: currentTime - pingWaitStartedAt);
        ...
        void ReportSessionPingDurationThresholdExceeded(int connectionIndex, TimeSpan durationWaitedForPing)
        {
            ...
            PlayerIncidentReport incident = new PlayerIncidentReport.SessionCommunicationHanged(
                ...
                roundtripEstimate:          TimeSpan.Zero,
                serverGateway:              null,
                networkReachability:        null,
                tlsPeerDescription:         null,
                sessionToken:               sessionToken,
                pingId:                     connectionIndex,
                elapsedSinceCommunication:  durationWaitedForPing);
            ...
            SendMessage(_playerActor, new InternalPlayerAddIncidentReport(compressed));
        }
        ...
        Task HandleSocialAuthenticateRequest(SocialAuthenticateRequest authReq, int requestId)
        {
            ...
            ContinueTaskOnActorContext(
                Authenticator.AuthenticateSocialAccountAsync(_log, asker: this, claim),
                async (SocialAuthenticationResult authResult) =>
                {
                    ...
                    if (existingPlayerId.IsValid && existingPlayerId != PlayerId)
                    {
                        ...
                        try
                        {
                            InternalEntityStateResponse response                = await EntityAskAsync(existingPlayerId, InternalEntityStateRequest.Instance);
                    ...
                    else
                    {
                        ...
                        await EntityAskAsync(PlayerId, new InternalPlayerAddNewSocialAccountRequest(authResult.AuthKeys));
        ...
        async Task HandleSocialAuthenticateResolveConflict(SocialAuthenticateResolveConflict resolveConflict)
        {
            ...
            if (_pendingSocialAuths.Remove(resolveConflict.ConflictResolutionId, out PendingSocialAuthRequest pendingAuth))
            {
                ...
                if (resolveConflict.UseOther)
                {
                    ...
                    InternalPlayerResolveDeviceIdAuthConflictResponse removalResponse = await EntityAskAsync(PlayerId, InternalPlayerResolveDeviceIdAuthConflictRequest.ForDeviceMigrationSource(deviceAuthKey, pendingAuth.AuthKeys, pendingAuth.ExistingPlayerId));
                    _ = await EntityAskAsync(pendingAuth.ExistingPlayerId, InternalPlayerResolveDeviceIdAuthConflictRequest.ForDeviceMigrationDestination(deviceAuthKey, removalResponse.RemovedAuthEntryOrNull, pendingAuth.AuthKeys, PlayerId));
                ...
                else
                {
                    ...
                    InternalPlayerResolveSocialAuthConflictResponse removed = await EntityAskAsync(pendingAuth.ExistingPlayerId, InternalPlayerResolveSocialAuthConflictRequest.ForSocialAuthMigrationSource(pendingAuth.AuthKeys, PlayerId));
                    _ = await EntityAskAsync(PlayerId, InternalPlayerResolveSocialAuthConflictRequest.ForSocialAuthMigrationDestination(removed.RemovedSocialKeys, pendingAuth.ExistingPlayerId));
                }
            }
            else
            {
        ...
        async Task HandleGuildJoinRequest(GuildJoinRequest request)
        {
            ...
            InternalPlayerJoinGuildResponse response = await EntityAskAsync(PlayerId, new InternalPlayerJoinGuildRequest(request, _guildIncarnation));
            ...
            EntityChannelManager guildManager = new EntityChannelManager(_log, guildActor, guildResponse.State.ChannelId, guildResponse.State.ClientData.ClientSlot, isActiveOnClient: false, parentManager: GetPlayerChannel());
        ...
        async Task HandleGuildLeaveRequest(GuildLeaveRequest request)
        {
            ...
            InternalPlayerGuildLeaveResponse response = await EntityAskAsync(PlayerId, new InternalPlayerGuildLeaveRequest(guildChannel.EntityId, _guildIncarnation));
        ...
        async Task HandleGuildDiscoveryRequest(GuildDiscoveryRequest request)
        {
            ...
            try
            {
                response = await EntityAskAsync(GuildRecommenderActorBase.EntityId, new InternalGuildRecommendationRequest(context));
                metrics.OnSuccess();
            }
            catch
            {
        ...
        async Task HandleGuildSearchRequest(GuildSearchRequest request)
        {
            ...
            ContinueTaskOnActorContext(
                EntityAskAsync(GuildSearchActorBase.EntityIdOnCurrentNode, new InternalGuildSearchRequest(request.SearchParams, context)),
        ...
        async Task HandleGuildInspectInvitationRequest(GuildInspectInvitationRequest request)
        {
            ...
            if (_nextInvitationInspectionEarliestAt > now)
            {
            ...
            _nextInvitationInspectionEarliestAt = now + TimeSpan.FromSeconds(1);
            ...
            InternalGuildInspectInviteCodeResponse guildResponse = await EntityAskAsync(guildId, new InternalGuildInspectInviteCodeRequest(invitePointer.InviteId, request.InviteCode));
            ...
            InternalPlayerGetGuildInviterAvatarResponse playerResponse = await EntityAskAsync(guildResponse.PlayerId, InternalPlayerGetGuildInviterAvatarRequest.Instance);
        ...
        const string PauseHintDeviceTimeFormatString = "yyyy'-'MM'-'dd HH':'mm':'ss'.'fff zzz";
        ...
        void HandleClientLifecycleHintPausing(ClientLifecycleHintPausing clientPausing)
        {
            ...
            _log.Info("Client app paused (at {DeviceTime} device time). Announced max duration {MaxDuration} with reason {Reason}.", clientPausing.ClientTimestamp.ToString(PauseHintDeviceTimeFormatString, CultureInfo.InvariantCulture), clientPausing.MaxPauseDuration, sanitizedReason);

            _clientPauseStatus = ClientAppPauseStatus.Paused;
            _clientPauseStatusSetAt = DateTime.UtcNow;
            ...
            if (clientPausing.MaxPauseDuration.HasValue)
                clientMaxPauseDuration = Util.Min(clientPausing.MaxPauseDuration.Value, options.MaxClientPauseDuration);
            else
        ...
        {
            _log.Debug("Client app started unpausing (at {DeviceTime} device time).", clientUnpausing.ClientTimestamp.ToString(PauseHintDeviceTimeFormatString, CultureInfo.InvariantCulture));
        ...
        {
            _log.Info("Client app unpaused (at {DeviceTime} device time).", clientUnpaused.ClientTimestamp.ToString(PauseHintDeviceTimeFormatString, CultureInfo.InvariantCulture));
        ...
        public class PlayerOverwriteRequestPayload
        {
            ...
            public MetaDictionary<string, MetaDictionary<string, ExportedEntity>> Entities;
        };
        ...
        async Task HandleDevOverwritePlayerStateRequest(DevOverwritePlayerStateRequest overwrite, int requestId)
        {
            ...
            try
            {
                ...
                ImportResponse response = await EntityArchiveUtils.Import(
                    ...
                    userRemaps: new MetaDictionary<string, MetaDictionary<string, string>>()
                    {{
                        "player",
                        new MetaDictionary<string, string>() {{ sourcePlayerId, PlayerId.ToString() } }
                    }},
        ...
        async Task DoSubscribeToEntityAsync(List<PendingAssociatedEntitySubscription> pendingSubscriptions, EntitySubscription parentSubscriptionMaybe, AssociatedEntityRefBase association)
        {
            ...
            foreach (AssociatedEntityRefBase subAssociation in subscription.AssociatedEntities)
            {
                ...
                catch (InternalEntitySubscribeRefusedBase refusal)
                {
                    ...
                    await EntityAskAsync(association.AssociatedEntity, new InternalEntityAssociatedEntityRefusedRequest(subAssociation, refusal));
        ...
        protected async Task ExecutePlayerServerActionAsync(EntityId playerId, PlayerActionBase action, int logicVersion)
        {
            ...
            await EntityAskAsync(playerId, new InternalPlayerExecuteServerActionRequest(serialized));
        }
        ...
        {
            MetaSerialized<PlayerActionBase> serialized = MetaSerialization.ToMetaSerialized(action, MetaSerializationFlags.IncludeAll, logicVersion);
            await EntityAskAsync(playerId, new InternalPlayerEnqueueServerActionRequest(serialized));
        }
        ...
        /// <summary>
        /// Subscribes into the associated entity and creates a <see cref="PendingAssociatedEntitySubscription"/> representing the effects of an successful results. Implementation does not need to handle any error in <see cref="InternalEntitySubscribeRefusedBase.Builtins"/>
        /// and may forward them to the caller.
        /// </summary>
        /// <param name="parentSubscription">Subscription to the entity that declared the target entity in its associated entities. If the target entity is a root entity, e.g. player, the parent is <c>null</c>.</param>
        ...
        protected virtual EntityChannelManager CompleteSubscribeToAssociatedEntity(PendingAssociatedEntitySubscription subscription, bool isActiveOnClient)
        {
            ...
            // Default manager
            return new EntityChannelManager(_log, subscription.Actor, subscription.State.ChannelId, subscription.State.ClientData.ClientSlot, isActiveOnClient: isActiveOnClient, parentManager);
        }
