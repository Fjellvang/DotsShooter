...
namespace Metaplay.Server.MaintenanceJob
{
    ...
    public class EntityMaintenanceJobRegistry
    {
        public static EntityMaintenanceJobRegistry Instance => MetaplayServices.Get<EntityMaintenanceJobRegistry>();

        public MetaDictionary<EntityKind, Func<EntityRefresherJobSpec>>               RefreshJobSpecs;
        public MetaDictionary<EntityKind, Func<EntitySchemaMigratorJobSpec>>          SchemaMigrationJobSpecs;
        public MetaDictionary<(EntityKind, string), EntityMaintenanceJobAttribute>    GenericJobs;        // Key is (EntityKind, jobId)

        public EntityMaintenanceJobRegistry(EntityConfigRegistry entityConfigRegistry)
        {
            MetaDictionary<EntityKind, Func<EntityRefresherJobSpec>> refreshJobSpecs = new();
            MetaDictionary<EntityKind, Func<EntitySchemaMigratorJobSpec>> schemaMigrationJobSpecs = new();
            MetaDictionary<(EntityKind, string), EntityMaintenanceJobAttribute> genericJobs = new();

            foreach ((Type entityConfigType, EntityConfigBase entityConfig) in entityConfigRegistry.TypeToEntityConfig)
            {
                ...
                // Refresh Jobs
                if (entityConfigType.HasCustomAttribute<EntityMaintenanceRefreshJobAttribute>())
                {
                ...
                if (entityConfigType.HasCustomAttribute<EntityMaintenanceSchemaMigratorJobAttribute>())
                {
