...
namespace Metaplay.Server.MaintenanceJob.EntitySchemaMigrator
{
    ...
    public class EntitySchemaMigratorJobSpec : MaintenanceJob.MaintenanceJobSpec
    {
        ...
        public override MetaDictionary<string, object> CreateSummary(DatabaseScanProcessingStatistics statsParam)
        {
            ...
            MetaDictionary<string, object> summary = new MetaDictionary<string, object>();
    ...
    public class EntitySchemaMigratorProcessor : DatabaseScanProcessor
    {
        ...
        public override Task StartProcessItemBatchAsync(IContext context, IEnumerable<IPersistedEntity> items)
        {
            ...
            {
                int targetSchemaVersion = GetTargetSchemaVersion(EntityId.ParseFromString(item.EntityId));
            ...
            _migrations.Update(
                ...
                onRequestFailure: (MigrationSpec migration, RequestErrorInfo error) =>
                    RecordFinishedMigrationStatistics(
                        ...
                        entityId:       migration.EntityId,
                        fromVersion:    migration.FromSchemaVersion,
                        toVersion:      GetTargetSchemaVersion(migration.EntityId),
                        error:          error)
            );
        ...
        int GetTargetSchemaVersion(EntityId entity)
        {
            return EntityConfigRegistry.Instance.GetPersistedConfig(_entityKind).GetSupportedSchemaVersions(entity).MaxVersion;
        }
    }
}
