...
namespace Metaplay.Server
{
    ...
    [MetaMessage(MessageCodesCore.LocalizationDataChangedRequest, MessageDirection.ServerInternal)]
    [MetaImplicitMembersRange(1, 100)]
    public class LocalizationDataChangedRequest : LocalizationsDataManager.GameDataChangedRequest { }
    ...
    [LocalizationsEnabledCondition]
    public class PublishLocalizationRequest : EntityAskRequest<PublishLocalizationResponse>
    {
    ...
    [LocalizationsEnabledCondition]
    public class PublishLocalizationResponse : EntityAskResponse
    {
        [MetaSerializable]
    ...
    public class LocalizationsDataManager : GameDataManager<PersistedLocalizations>
    {
        ...
        {
            PersistedLocalizations persisted = await MetaDatabase.Get().TryGetAsync<PersistedLocalizations>(id.ToString());
        ...
        [EntityAskHandler]
        public async Task<EntityAskOk> HandleRemoveLocalizationRequest(RemoveLocalizationsRequest request)
        {
            if (_state.ActiveLocalizationsId == request.Id)
                throw new InvalidEntityAsk($"Can't remove current active localization {request.Id}");

            return await Remove(request);
        }

        [EntityAskHandler]
        public Task<EntityAskOk> HandleLocalizationDataChanged(LocalizationDataChangedRequest changedRequest)
        {
            OnAnyGameDataChanged(changedRequest.ChangedGuid);
            return Task.FromResult(EntityAskOk.Instance);
        }
