...
namespace Metaplay.Server
{
    ...
    public class CreateOrUpdateGameDataResponse : EntityAskResponse
    {
    ...
    public abstract class GameDataManager<TDataType> : EntityComponent where TDataType : PersistedGameData, new()
    {
        ...
        public abstract class CreateOrUpdateRequest : EntityAskRequest<CreateOrUpdateGameDataResponse>
        {
        ...
        [MetaImplicitMembersRange(100, 200)]
        public abstract class RemoveRequest : EntityAskRequest<EntityAskOk>
        {
            public MetaGuid Id { get; set; }
        }

        [MetaImplicitMembersRange(100, 200)]
        public abstract class GameDataChangedRequest : MetaMessage
        {
            public MetaGuid ChangedGuid { get; set; }
        }
        #endregion
        ...
        protected abstract void OnAnyGameDataChanged(MetaGuid guid);
        ...
        async Task<(MetaGuid id, bool wasStored)> FindOrStoreBuiltinGameDataAsync()
        {
            ...
            {
                _log.Warning("Failed to load {GameDataTypeDesc} from built-in storage at {BuiltinArchivePath}: {Error}", GameDataTypeDesc, archivePath, ex);
                return (MetaGuid.None, false);
            }
    ...
    public abstract class DeleteAllArchivedBackgroundTask<TDataType> : GameDataBuildBackgroundTask where TDataType : PersistedGameData, new()
    {
        public override async Task<IBackgroundTaskOutput> Run(BackgroundTaskContext context)
        {
            MetaDatabase db = MetaDatabase.Get();
            await db.DeleteAllArchivedGameData<TDataType>();

            await context.EntityAskAsync<EntityAskOk>(GlobalStateManager.EntityId, GetGameDataChangedRequest());
            return new Progress("done");
        }

        protected abstract GameDataManager<TDataType>.GameDataChangedRequest GetGameDataChangedRequest();
    }

    [MetaSerializableDerived(4)]
    public class DeleteAllArchivedGameConfigsBackgroundTask : DeleteAllArchivedBackgroundTask<PersistedStaticGameConfig>
    {
        protected override GameDataManager<PersistedStaticGameConfig>.GameDataChangedRequest GetGameDataChangedRequest()
        {
            return new GameConfigDataChangedRequest() {ChangedGuid = MetaGuid.None};
        }
    }

    [MetaSerializableDerived(5)]
    public class DeleteAllArchivedLocalizationsBackgroundTask : DeleteAllArchivedBackgroundTask<PersistedLocalizations>
    {
        protected override GameDataManager<PersistedLocalizations>.GameDataChangedRequest GetGameDataChangedRequest()
        {
            return new LocalizationDataChangedRequest() {ChangedGuid = MetaGuid.None};
        }
    }
}
