...
namespace Metaplay.Server.League
{
    ...
    public abstract class DivisionEntityConfigBase : PersistedEntityConfig
    {
        ...
        public override Type              EntityActorType      => typeof(DivisionActorBase<,,,>);
        ...
        {
            int leagueIndex = DivisionIndex.FromEntityId(entityId).League;

            if (LeagueManagerRegistry.Instance.DivisionActorTypeCache.TryGetValue(leagueIndex, out Type actorType))
                return actorType;

            throw new ArgumentOutOfRangeException(nameof(entityId), $"No division actor type found for entity id {entityId}");
        }
        ...
        public override Type GetPersistedPayloadType(EntityId entityId)
        {
            ...
            if (LeagueManagerRegistry.Instance.DivisionActorPayloadTypeCache.TryGetValue(leagueIndex, out Type payloadType))
                return payloadType;

            throw new ArgumentOutOfRangeException(nameof(entityId), $"No division persisted payload type found for entity id {entityId}");
        }
    }
    ...
    /// </summary>
    public abstract class DivisionActorBase<TModel, TServerModel, TPersisted, TOptions>
        ...
        where TOptions : LeagueManagerOptionsBase
    {
        ...
        protected TOptions LeagueManagerOptions { get; private set; }
        ...
        {
            LeagueManagerOptions = RuntimeOptionsRegistry.Instance.GetCurrent<TOptions>();
            ...
            _analyticsEventHandler = new AnalyticsEventHandler<IDivisionModel, DivisionEventBase>((model, payload) =>
            {
                ...
                if (eventSpec.SendToAnalytics)
                {
                    ...
                    MetaDictionary<AnalyticsLabel, string> labels = GetAnalyticsLabels((TModel)model);
        ...
        protected override Task SetUpModelAsync(TModel model, IMultiplayerEntitySetupParams setupParams)
        {
            ...
            model.DesiredParticipantCount = divisionSetup.DesiredParticipantCount;
        ...
        Task ScheduleOrDoConclude(MetaTime concludeTime)
        {
            ...
            {
                int maxDelayMilliseconds = LeagueManagerOptions.ConcludeSeasonMaxDelayMilliseconds;
        ...
        void HandleScoreEventMessage(InternalDivisionScoreEventMessage message)
        {
            if (Model.IsConcluded)
            {
                if(_log.IsDebugEnabled)
                    _log.Debug("Received a score update event from {Participant} after season is already concluded.", message.ParticipantId);
            ...
            {
                _log.Debug("Received a score update event from {Participant} when season is not ongoing.", message.ParticipantId);
                return;
            }
            ...
            {
                if(_log.IsDebugEnabled)
                    _log.Debug("Score event from {Participant} was refused", message.ParticipantId);
                return;
            }
        ...
        /// The returned history entry will be sent back to the asker.
        /// </summary>
        ...
        protected override EntityClientData CreateClientSessionStartEntityClientData(EntityId sessionId, EntityId playerId, InternalEntitySubscribeRequestBase requestBase, List<AssociatedEntityRefBase> associatedEntities)
        {
            return new DivisionEntityClientData(requestBase.AssociationRef.GetClientSlot(), participantId: requestBase.AssociationRef.SourceEntity);
        }
        ...
        /// <summary>
        /// Called when Session (i.e. client, i.e. user) subscribes into this Division. See <see cref="MultiplayerEntityActorBase{TModel, TAction}.OnClientSessionStart"/>
        /// </summary>
