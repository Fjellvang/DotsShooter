...
namespace Metaplay.Server.GameConfig
{
    public class GameConfigSpecializationPatchesBuilder
    {
        ...
        {
            int numVariants = 0;
            MetaDictionary<PlayerExperimentId, MetaDictionary<ExperimentVariantId, byte[]>> patches = new MetaDictionary<PlayerExperimentId, MetaDictionary<ExperimentVariantId, byte[]>>();
            ...
            foreach (PlayerExperimentId experimentId in activeSubset)
            {
                ...
                MetaDictionary<ExperimentVariantId, byte[]> variantPatches = new MetaDictionary<ExperimentVariantId, byte[]>();
