...
namespace Metaplay.Server
{
    ...
    public class SystemOptions : RuntimeOptionsBase
    {
        ...
        public TimeSpan     PlayerDeletionSweepTimeOfDay{ get; private set; } = TimeSpan.FromSeconds(-1); // negative value will default to a random value
        ...
        public override async Task OnLoadedAsync(RuntimeOptionsRegistry registry)
        {
            ...
            // Default deletion sweep time-of-day to a random value based on the project & environment name (the offset does not change over deployments)
            if (PlayerDeletionSweepTimeOfDay.Seconds < 0)
            {
                // Randomize time based on project name and environment name
                MetaplayCoreOptions coreOpts = IntegrationRegistry.Get<IMetaplayCoreOptionsProvider>().Options;
                EnvironmentOptions envOpts = await GetDependencyAsync<EnvironmentOptions>(registry);
                uint seed = MiniMD5.ComputeMiniMD5(coreOpts.ProjectName) + MiniMD5.ComputeMiniMD5(envOpts.Environment);
                const uint secondsPerDay = 24 * 60 * 60;
                int offsetSeconds = (int)(seed % secondsPerDay);

                // Quantize time to 30min resolution (operations starting at completely random times can be surprising)
                const int resolutionSeconds = 30 * 60;
                offsetSeconds = (offsetSeconds / resolutionSeconds) * resolutionSeconds;

                PlayerDeletionSweepTimeOfDay = TimeSpan.FromSeconds(offsetSeconds);
            }
