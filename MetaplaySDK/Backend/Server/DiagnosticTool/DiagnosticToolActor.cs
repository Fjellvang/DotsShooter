...
namespace Metaplay.Server
{
    ...
    public class TestEntityAskFailureRequest : EntityAskRequest<EntityAskOk>
    {
    ...
    public class TestAsyncHandlerEntityAskFailureRequest : EntityAskRequest<EntityAskOk>
    {
        public bool Controlled { get; private set; }
    ...
    public class DiagnosticToolActor : EphemeralEntityActor
    {
        ...
        class PeerState
        {
            ...
            public PeerState(ClusterNodeAddress address, EntityId entityId, bool isConnected)
            {
                Address = address;
                EntityId = entityId;
        ...
        public class PingState
        {
            ...
            public readonly DateTime    PingSentAt;

            public PingState(int pingId, EntityId peerId, DateTime pingSentAt)
            {
                PingId = pingId;
                PeerId = peerId;
        ...
        void HandleActorTick(ActorTick _)
        {
            ...
            DateTime pingExpireAt = now - PingExpireDuration;
            ...
            {
                // If peer was removed, don't warn about missing reply
                ClusterNodeAddress peerAddress = GetPeerAddress(state.PeerId);
                if (peerAddress != null)
                {
                    _log.Warning("Expired ping #{PingId} to {PeerId} on {PeerAddress}, sent {SentAgo} ago", state.PingId, state.PeerId, peerAddress, now - state.PingSentAt);
        ...
        void HandlePong(EntityId fromEntityId, DiagnosticToolPong pong)
        {
            ...
            if (state.PeerId != fromEntityId)
            {
                _log.Warning("Received pong from {FromEntity} but target peer was {PeerId} to unknown ping #{PingId}", fromEntityId, state.PeerId, pong.PingId);
                return;
            }

            TimeSpan duration = DateTime.UtcNow - state.PingSentAt;
            c_pingDurations.WithLabels(GetPeerName(fromEntityId)).Observe(duration.TotalSeconds);
        ...
        [CommandHandler]
        void HandleClusterChangedEvent(ClusterConnectionManager.ClusterChangedEvent changed)
        {
            ClusteringOptions opts = RuntimeOptionsRegistry.Instance.GetCurrent<ClusteringOptions>();

            _peers.Clear();

            // If this node is not a part of the cluster, then this node cannot communicate with other nodes
            // and there are no peers.
            if (!changed.Members.Any(member => member.Address == opts.SelfAddress))
                return;

            // Collect peers
            foreach (ClusterConnectionManager.ClusterChangedEvent.ClusterMember member in changed.Members)
            {
                if (member.Address == opts.SelfAddress)
                    continue;
                if (!opts.ClusterConfig.ResolveNodeShardId(EntityKindCloudCore.DiagnosticTool, member.Address, out EntityShardId shardId))
                    continue;

                EntityId entityId = DynamicServiceShardingStrategy.CreatePlacedEntityId(shardId);
                _peers.Add(new PeerState(member.Address, entityId, member.IsConnected));
            }
        }
    }
}
