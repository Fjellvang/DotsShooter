    ...
    public class WebSocketListener : EntityShard
    {
        ...
        void ReceiveWebSocketConnected(WebSocketConnected connected)
        {
            ...
            if (IsAllowingIncomingConnections(out ProtocolStatus rejectionReason))
            {
                ...
                _ = SpawnEntityWithCustomActor(connectionId, () =>
                {
                    return Context.ActorOf(Props.Create<WebSocketClientConnection>(connectionId, srcAddr, localPort, connected.Socket, connected.SocketFinished), connectionId.ToString());
                });
            }
            else
            {
        ...
        protected override async Task InitializeAsync()
        {
            ...
            if (options.Enabled)
            {
                ...
                // Start metrics timer
                Context.System.Scheduler.ScheduleTellRepeatedly(TimeSpan.Zero, UpdateMetricsInterval, _self, UpdateMetrics.Instance, ActorRefs.NoSender, _cancelTimers);
            }
