...
namespace Metaplay.Server.GuildDiscovery
{
    ...
    public abstract class GuildRecommenderActorBase : EphemeralEntityActor
    {
        ...
        [CommandHandler]
        async Task HandleTick(Tick tick)
        {
        ...
        {
            MetaDictionary<string, InternalGuildRecommenderTestGuildResponse.PoolStatus> pools = new MetaDictionary<string, InternalGuildRecommenderTestGuildResponse.PoolStatus>(capacity: _allPools.Count);
            foreach (PersistedGuildDiscoveryPool pool in _allPools)
            {
