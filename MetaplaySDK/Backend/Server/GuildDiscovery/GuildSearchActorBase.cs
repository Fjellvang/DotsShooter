...
namespace Metaplay.Server.GuildDiscovery
{
    ...
    public abstract class GuildSearchActorBase : EphemeralEntityActor
    {
        ...
        class WorkRequest
        {
            ...
            public EntityAsk<InternalGuildSearchResponse> Ask;
            public Stopwatch                        Stopwatch;

            public WorkRequest(GuildSearchParamsBase searchParams, GuildDiscoveryPlayerContextBase searchContext, EntityAsk<InternalGuildSearchResponse> ask, Stopwatch stopwatch)
            {
        ...
        [EntityAskHandler]
        void HandleInternalGuildSearchRequest(EntityAsk<InternalGuildSearchResponse> ask, InternalGuildSearchRequest request)
        {
        ...
        async Task<List<GuildDiscoveryInfoBase>> TrySearchAsync(WorkRequest workItem)
        {
            ...
            foreach (EntityId guildId in guildIds)
            {
                ...
                Task<InternalGuildDiscoveryGuildDataResponse> asyncResponse = ExecuteOnActorContextAsync(() => EntityAskAsync(guildId, InternalGuildDiscoveryGuildDataRequest.Instance));
