...
namespace Metaplay.Server.AdminApi.Controllers
{
    ...
    public class LeaguesController : GameAdminApiController
    {
        ...
        public async Task<ActionResult<LeagueStateResponse>> GetLeague(string leagueId)
        {
            ...
            if (!LeagueManagerRegistry.Instance.LeagueManagerIds.Contains(entityId))
                throw new MetaplayHttpException(404, "League not found!", $"League with id: {leagueId} not found.");

            InternalLeagueStateResponse response = await EntityAskAsync(entityId, InternalLeagueStateRequest.Instance);
            ...
            if (response.LeagueManagerState.HistoricSeasons != null && response.LeagueManagerState.HistoricSeasons.Count > 0)
            {
                ...
                participants -= response.LeagueManagerState.HistoricSeasons[^1].DroppedParticipants;
            }
        ...
        {
            IReadOnlyList<EntityId> allLeagues = LeagueManagerRegistry.Instance.LeagueManagerIds;

            InternalLeagueStateResponse[] responses = await Task.WhenAll(allLeagues.Select(entityId => EntityAskAsync(entityId, InternalLeagueStateRequest.Instance)));

            return new ActionResult<IEnumerable<LeagueEntityState>>(
                responses.Zip(allLeagues, (response, leagueId) => (response, leagueId))
                    .Select(zipped =>
                {
                    (InternalLeagueStateResponse response, EntityId leagueId) = zipped;
                    PlayerLocalTime time         = new PlayerLocalTime(MetaTime.Now, MetaDuration.Zero);
        ...
        {
            IReadOnlyList<LeagueManagerRegistry.LeagueInfo> allLeagues = LeagueManagerRegistry.Instance.LeagueInfos;
            InternalLeagueStateResponse[] responses = await Task.WhenAll(allLeagues.Select(league => EntityAskAsync(league.LeagueManagerId, InternalLeagueStateRequest.Instance)));

            EntityId participantId = EntityId.ParseFromString(entityIdString);

            IEnumerable<(InternalLeagueStateResponse, LeagueManagerRegistry.LeagueInfo)> zipped = responses.Zip(allLeagues, (response, leagueId) => (response, leagueId));

            List<LeagueParticipantSpecificDetailsItem> results = new List<LeagueParticipantSpecificDetailsItem>();

            foreach ((InternalLeagueStateResponse response, LeagueManagerRegistry.LeagueInfo leagueInfo) in zipped)
            {
                PlayerLocalTime time = new PlayerLocalTime(MetaTime.Now, MetaDuration.Zero);
                ...
                PersistedParticipantDivisionAssociation association = await MetaDatabase.Get().TryGetParticipantDivisionAssociation(leagueInfo.LeagueManagerId, participantId);
                ...
                if (participantId.IsOfKind(EntityKindCore.Player))
                {
                    ...
                    if (playerDetails.Model?.PlayerSubClientStates?.TryGetValue(leagueInfo.ClientSlot, out PlayerSubClientStateBase playerDivisionStateBase) ?? false)
                    {
                        IDivisionClientState playerDivisionState = playerDivisionStateBase as IDivisionClientState;
                        if (playerDivisionState?.HistoricalDivisions?.Any(historical => (ulong)historical.DivisionIndex.League == leagueInfo.LeagueManagerId.Value) ?? false)
                ...
                result.LeagueId          = leagueInfo.LeagueManagerId;
            ...
            return new ActionResult<IEnumerable<LeagueParticipantSpecificDetailsItem>>(results);
        }
        ...
        public async Task<ActionResult<IEnumerable<LeagueParticipantSpecificDetailsItem>>> RemoveParticipantFromLeague(string leagueIdString, string participantIdString)
        {
            ...
            if (!LeagueManagerRegistry.Instance.LeagueManagerIds.Contains(leagueId))
                throw new MetaplayHttpException(404, "League not found!", $"League with id: {leagueId} not found.");
        ...
        public async Task<ActionResult<IEnumerable<LeagueParticipantSpecificDetailsItem>>> AddParticipantToLeague(string leagueIdString, string participantIdString, string divisionIdString)
        {
            ...
            try
            {
                InternalLeagueDebugAddResponse response = await EntityAskAsync(
                    leagueId,
        ...
        public async Task<ActionResult<IEnumerable<LeagueParticipantSpecificDetailsItem>>> AddParticipantToLeagueRank(string leagueIdString, string participantIdString, int rankIndex)
        {
            ...
            try
            {
                InternalLeagueJoinResponse response = await EntityAskAsync(
                    leagueId,
