...
namespace Metaplay.Server.AdminApi.Controllers
{
    ...
    public class EntityDatabaseInfoController : GameAdminApiController
    {
        ...
        public async Task<ActionResult<EntityDetails>> GetDbInfo(string entityIdStr)
        {
            ...
            MetaVersionRange supportedSchemaVersions = entityConfig.GetSupportedSchemaVersions(entityId);

            if (persistedEntity.Payload != null)
            {
                // Inspect object tree and convert it into JSONified tree
                TaggedSerializedInspector.ObjectInfo objectTree = ParsePersistedEntityPayload(persistedEntity.Payload, entityConfig.GetPersistedPayloadType(EntityId.ParseFromString(persistedEntity.EntityId)), out int uncompressedSize);
                ...
                return new EntityDetails
                {
                    ...
                    CurrentSchemaVersion = supportedSchemaVersions.MaxVersion,
