...
namespace Metaplay.Server.AdminApi.Controllers
{
    ...
    public class PlayerExperimentsController : GameAdminApiController
    {
        ...
        public class ExperimentEdited : ExperimentEventPayloadBase
        {
            ...
            [MetaMember(6)]  public MetaDictionary<string, int>                   VariantWeights          { get; private set; }
            ...
            [MetaMember(9)]  public MetaDictionary<string, bool>                  VariantIsDisabled       { get; private set; }
                ...
                bool? isRolloutDisabled)
            {
                ...
                if (variantWeights != null) VariantWeights = new MetaDictionary<string, int>(variantWeights.Select((kv) => new KeyValuePair<string, int>(kv.Key?.ToString() ?? "$null", kv.Value)));
                TargetCondition = targetCondition;
                EnrollTrigger = enrollTrigger;
                if (variantIsDisabled != null) VariantIsDisabled = new MetaDictionary<string, bool>(variantIsDisabled.Select((kv) => new KeyValuePair<string, bool>(kv.Key?.ToString() ?? "$null", kv.Value)));
        ...
        public async Task<IActionResult> GetExperiments()
        {
            ...
            GlobalStateGetAllExperimentsResponse      response             = await EntityAskAsync(GlobalStateManager.EntityId, request);

            GlobalStateExperimentCombinationsRequest  combinationsRequest  = new GlobalStateExperimentCombinationsRequest();
            GlobalStateExperimentCombinationsResponse combinationsResponse = await EntityAskAsync(GlobalStateManager.EntityId, combinationsRequest);
            return Ok(new
            {
        ...
        public async Task<IActionResult> GetExperimentState(string experimentIdStr)
        {
            ...
            GlobalStateExperimentStateResponse response = await EntityAskAsync(GlobalStateManager.EntityId, request);
        ...
        public struct EditExperimentConfigInput
        {
            ...
            [JsonProperty(Required = Required.AllowNull)] public MetaDictionary<ExperimentVariantId, int>    VariantWeights      { get; private set; }
            [JsonProperty(Required = Required.AllowNull)] public MetaDictionary<ExperimentVariantId, bool>   VariantIsDisabled   { get; private set; }
        ...
        public async Task<IActionResult> EditExperimentConfig(string experimentIdStr)
        {
            ...
            GlobalStateModifyExperimentResponse response = await EntityAskAsync(GlobalStateManager.EntityId, request);
        ...
        public async Task<IActionResult> EditExperimentPhase(string experimentIdStr)
        {
            ...
            GlobalStateSetExperimentPhaseRequest request = new GlobalStateSetExperimentPhaseRequest(
                ...
                phase:                  input.Phase,
                force:                  input.Force);
            GlobalStateSetExperimentPhaseResponse response = await EntityAskAsync(GlobalStateManager.EntityId, request);
            if (response.ErrorStringOrNull == null)
            {
        ...
        public async Task<IActionResult> DeleteExperiment(string experimentIdStr)
        {
            ...
            GlobalStateDeleteExperimentResponse response = await EntityAskAsync(GlobalStateManager.EntityId, request);
            if (response.ErrorStringOrNull == null)
            {
