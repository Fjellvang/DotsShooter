...
namespace Metaplay.Server.AdminApi.Controllers
{
    public class ActivablesController : ActivablesControllerBase
    {
        ...
        public async Task<ActionResult> GetActivablesAsync([FromQuery] string time)
        {
            ...
            MetaDictionary<MetaActivableKindId, GeneralActivableKindData> activableDatas = MetaActivableRepository.Instance.AllKinds.Values.ToMetaDictionary(
                kind => kind.Id,
                kind =>
                {
                    MetaDictionary<object, GeneralActivableData> activables;
                    if (kind.GameConfigLibrary == null)
                        activables = new MetaDictionary<object, GeneralActivableData>();
                    else
                    {
                        IGameConfigLibraryEntry library = kind.GameConfigLibrary.GetMemberValue(context.SharedGameConfig);
                        activables = library.EnumerateAll().ToMetaDictionary(
        ...
        async public Task<ActionResult> GetActivablesForPlayer(string playerIdStr)
        {
            ...
            MetaDictionary<MetaActivableKindId, PlayerActivableKindData> activableDatas = MetaActivableRepository.Instance.AllKinds.Values.ToMetaDictionary(
                kind => kind.Id,
                kind =>
                {
                    MetaDictionary<object, PlayerActivableData> activables;
                    if (kind.GameConfigLibrary == null || !kind.PlayerSubModel.TryGetMemberValue(context.PlayerModel, out IMetaActivableSet playerActivables))
                        activables = new MetaDictionary<object, PlayerActivableData>();
                    else
                    {
                        IGameConfigLibraryEntry library = kind.GameConfigLibrary.GetMemberValue(context.SharedGameConfig);
                        activables = library.EnumerateAll().ToMetaDictionary(
