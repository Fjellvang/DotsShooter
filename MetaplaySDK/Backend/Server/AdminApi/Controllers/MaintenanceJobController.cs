    ...
    public class MaintenanceJobController : GameAdminApiController
    {
        ...
        public class GetJobsResponse
        {
            public class Entry
            {
                ...
                public required MaintenanceJobSpec  Spec    { get; init; }
            }

            public required Entry[] SupportedJobKinds { get; init; }
        }
        ...
        public ActionResult<GetJobsResponse> GetJobs()
        {
            ...
            {
                SupportedJobKinds = GetSupportedJobKinds().Select(kv => new GetJobsResponse.Entry { Id = kv.Key, Spec = kv.Value }).ToArray(),
            };
        }
        ...
        [RequirePermission(MetaplayPermissions.ApiSystemMaintenanceJobs)]
        public async Task DeleteEnqueuedJob(string maintenanceJobIdStr)
        {
            ...
            RemoveMaintenanceJobResponse response = await EntityAskAsync(DatabaseScanCoordinatorActor.EntityId, request);
        ...
        {
            public required int                 MaintenanceJobId    { get; init; }
            public required MaintenanceJobSpec  Spec                { get; init; }
        }
        ...
        [RequirePermission(MetaplayPermissions.ApiSystemMaintenanceJobs)]
        public async Task<ActionResult<EnqueueJobResponse>> EnqueueJob()
        {
            ...
            AddMaintenanceJobResponse response = await EntityAskAsync(DatabaseScanCoordinatorActor.EntityId, request);
            // \todo [nuutti] Figure out proper statusCode
            if (!response.IsSuccess)
        ...
        MetaDictionary<string, MaintenanceJobSpec> GetSupportedJobKinds()
        {
            MetaDictionary<string, MaintenanceJobSpec> specs = new MetaDictionary<string, MaintenanceJobSpec>();

            foreach ((EntityKind kind, Func<EntitySchemaMigratorJobSpec> factory) in EntityMaintenanceJobRegistry.Instance.SchemaMigrationJobSpecs)
                specs.Add($"Migrate_{kind}", factory());

            foreach ((EntityKind kind, Func<EntityRefresherJobSpec> factory) in EntityMaintenanceJobRegistry.Instance.RefreshJobSpecs)
                specs.Add($"Refresh_{kind}", factory());

            foreach (((EntityKind kind, string jobId), EntityMaintenanceJobAttribute attrib) in EntityMaintenanceJobRegistry.Instance.GenericJobs)
