...
namespace Metaplay.Server.AdminApi.Controllers
{
    ...
    public class DatabaseScanJobsController : GameAdminApiController
    {
        ...
        public class JobInfo
        {
            ...
            public MetaDictionary<string, object>    Summary;
            public bool                                 CanCancel;
            public string                               CannotCancelReason;

            JobInfo(JobApiId id, JobPhaseCategory phaseCategory, DatabaseScanJobSpec spec, MetaTime? startTime, JobPhase phase, MetaTime? endTime, DatabaseScanStatistics scanStatistics, DatabaseScanProcessingStatistics processingStatistics, int? numWorkers, MetaDictionary<string, object> summary, bool canCancel, string cannotCancelReason)
            {
                Id = id.ToString();
        ...
        public async Task<ActionResult<DatabaseScanJobsResponse>> GetDatabaseScanJobs([FromQuery] int jobHistoryLimit = 10)
        {
            ListDatabaseScanJobsRequest request = new ListDatabaseScanJobsRequest(jobHistoryLimit);
            ListDatabaseScanJobsResponse response = await EntityAskAsync(DatabaseScanCoordinatorActor.EntityId, request);

            // Return results
            return new DatabaseScanJobsResponse(
        ...
        public async Task<ActionResult<CancelDatabaseScanJobResponse>> CancelDatabaseScanJob(string jobIdStr)
        {
            JobApiId jobApiId;
            try
            {
            ...
            if (jobApiId is JobApiId.ProperDatabaseScanJobId properDatabaseScanJobId)
            {
                ...
                BeginCancelDatabaseScanJobResponse response = await EntityAskAsync(DatabaseScanCoordinatorActor.EntityId, request);
                ...
                return new CancelDatabaseScanJobResponse(Invariant($"Started cancellation of database scan job '{response.JobState.Spec.JobTitle}' (job #{response.JobState.Id.SequentialId})."));
            }
            else if (jobApiId is JobApiId.UpcomingJobId upcomingJobId)
            {
                ...
                if (upcomingJobId.ManagerKind == DatabaseScanJobManagerKind.MaintenanceJobManager)
                {
                    ...
                    RemoveMaintenanceJobResponse removeResponse = await EntityAskAsync(DatabaseScanCoordinatorActor.EntityId, removeRequest);
                    ...
                    if (removeResponse.IsSuccess)
                    {
                        ...
                        return new CancelDatabaseScanJobResponse(Invariant($"Deleted upcoming job '{removeResponse.JobSpec.JobTitle}'."));
                    }
                    else if (removeResponse.ActiveJobId.HasValue)
                    {
                        ...
                        BeginCancelDatabaseScanJobResponse cancelResponse = await EntityAskAsync(DatabaseScanCoordinatorActor.EntityId, cancelRequest);
                        // \todo [nuutti] Figure out proper statusCode
                        ...
                        return new CancelDatabaseScanJobResponse(Invariant($"Started cancellation of database scan job '{cancelResponse.JobState.Spec.JobTitle}' (job #{cancelResponse.JobState.Id.SequentialId})."));
                    }
                    else
                    {
                        // \todo [nuutti] Figure out proper statusCode
        ...
        [RequirePermission(MetaplayPermissions.ApiScanJobsCancel)]
        public async Task SetGlobalPause([FromBody] SetGlobalPauseBody body)
        {
            ...
            _ = await EntityAskAsync(DatabaseScanCoordinatorActor.EntityId, request);
