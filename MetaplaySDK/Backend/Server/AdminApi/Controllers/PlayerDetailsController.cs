...
namespace Metaplay.Server.AdminApi.Controllers
{
    ...
    public class PlayerDetailsController : GameAdminApiController
    {
        ...
        public class PlayerDetailsItem
        {
            ...
            public MetaDictionary<PlayerExperimentId, PlayerExperimentDetails> Experiments            { get; set; }
            ...
            public bool                                                           LogicVersionMismatched { get; set; }
        }
        ...
        public class PlayerDetailsIAPSubscriptionsExtraInfo
        {
            ...
            /// </remarks>
            public MetaDictionary<string, InstanceInfo> Instances { get; set; }

            public PlayerDetailsIAPSubscriptionsExtraInfo(MetaDictionary<string, InstanceInfo> instances) { Instances = instances; }
        ...
        public async Task<ActionResult<PlayerDetailsItem>> GetPlayerDetails(string playerIdStr)
        {
            // Parse parameters
            (bool isFound, bool isInitialized, PlayerDetails details) = await TryGetPlayerDetailsAsync(playerIdStr);

            if (!isFound)
                throw new Exceptions.MetaplayHttpException(404, "Persisted entity not found.", $"Cannot find {details.PlayerId}.");
            ...
            List<PlayerSegmentInfoBase>                                   matchingSegments  = null;
            MetaDictionary<PlayerExperimentId,PlayerExperimentDetails> experimentDetails = null;

            List<PlayerDetailsNftInfo>             nftInfos = null;
            PlayerDetailsIAPSubscriptionsExtraInfo iapSubscriptionsExtraInfo = null;
            ...
            if (isInitialized)
            {
                ...
                {
                    var guildResponse = await EntityAskAsync(details.Model.GuildState.GuildId, new InternalGuildPlayerDashboardInfoRequest(EntityId.ParseFromString(playerIdStr))).ConfigureAwait(false);
                ...
                experimentDetails = (await EntityAskAsync(details.PlayerId, InternalPlayerGetPlayerExperimentDetailsRequest.Instance)).Details;
                ...
                MetaDictionary<string, PlayerDetailsIAPSubscriptionsExtraInfo.InstanceInfo> iapSubscriptionInstanceExtraInfos = new MetaDictionary<string, PlayerDetailsIAPSubscriptionsExtraInfo.InstanceInfo>();
                ...
                iapSubscriptionsExtraInfo = new PlayerDetailsIAPSubscriptionsExtraInfo(iapSubscriptionInstanceExtraInfos);
                if (new Web3EnabledCondition().IsEnabled)
                {
                    List<MetaNft> nfts = (await EntityAskAsync(NftManager.EntityId, new QueryNftsRequest(owner: details.PlayerId)))
                        .Items
                    ...
                    MetaSerialization.ResolveMetaRefs(nfts, resolver);
            ...
            return new PlayerDetailsItem
            {
                ...
                Model                  = details.Model,
                PersistedSize          = details.Persisted.Payload?.Length ?? 0,
                ...
                Nfts                   = nftInfos,
                LogicVersion           = details.LogicVersion,
                LogicVersionMismatched = details.LogicVersionMismatched,
            };
        }
        ...
        public async Task<ActionResult<object>> GetRaw(string playerIdStr)
        {
            ...
            if (persistedPlayer.Data != null)
            {
                try
                {
                    playerStateResponse.SetData(await EntityAskAsync(playerMetaData.Data.Id, InternalEntityStateRequest.Instance));
                }
                catch (Exception ex)
                {
        ...
        private bool ShouldWriteAuditLogEvent(string userId, EntityId playerId)
        {
            lock (_playerViewCacheLock)
            {
                ...
                TimeSpan timeoutDuration = TimeSpan.FromMinutes(5);
                ...
                if (PlayerViewCache.ContainsKey(userId))
                    ...
                    // entry for it
                    PlayerViewCache[userId] = new Dictionary<EntityId, DateTime>();
        ...
        /// </summary>
        private static Dictionary<string, Dictionary<EntityId, DateTime>> PlayerViewCache = new Dictionary<string, Dictionary<EntityId, DateTime>>();
