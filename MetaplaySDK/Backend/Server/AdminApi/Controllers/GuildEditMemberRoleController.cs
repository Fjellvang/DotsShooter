...
namespace Metaplay.Server.AdminApi.Controllers
{
    ...
    public class GuildEditMemberRoleController : GameAdminApiController
    {
        ...
        /// <summary>
        /// HTTP response for member role edit preview
        /// </summary>
        public class MemberValidateRoleEditResult
        {
        ...
        public async Task<ActionResult<MemberValidateRoleEditResult>> ValidateEditRole(string guildIdStr)
        {
            // Parse parameters
            ...
            MemberValidateRoleEditResult                    result          = new MemberValidateRoleEditResult();
            MetaDictionary<EntityId, GuildMemberRole>    changesNow      = guildModel.ComputeRoleChangesForRoleEvent(GuildMemberRoleEvent.MemberEdit, subjectMemberId, subjectRole);
            result.Changes = new Dictionary<string, string>(changesNow.Select(kv => new KeyValuePair<string, string>(kv.Key.ToString(), kv.Value.ToString())));
            return result;
        }
        ...
        public async Task<IActionResult> EditRole(string guildIdStr)
        {
            ...
            InternalGuildAdminEditRolesResponse response = await EntityAskAsync(guildId, new InternalGuildAdminEditRolesRequest(subjectMemberId, subjectRole, changesNow));
            if (!response.Success)
