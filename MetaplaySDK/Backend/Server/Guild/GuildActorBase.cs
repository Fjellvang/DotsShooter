...
namespace Metaplay.Server.Guild
{
    ...
    public class PersistedGuildSearch : IPersistedItem
    {
        ...
        public const int ReservedLowestPriorityLevel = 64;  // Reserved lowest priority level used for deleted guilds
        ...
        where TPersisted : PersistedGuildBase, new()
    {
        ...
        MetaDictionary<int, MetaTime>        _nextInvitationCreationEarliestAt;
        ...
        /// <inheritdoc cref="MultiplayerEntity.MultiplayerEntityActorBase{TModel, TAction}.RequireClientHasGameConfigPresentOnSessionStart"/>
        protected virtual bool RequireClientHasGameConfigPresentOnSessionStart => true;
        ...
        protected GuildActorBase(EntityId guildId) : base(guildId)
        {
            ...
            _analyticsEventHandler = new AnalyticsEventHandler<IGuildModelBase, GuildEventBase>((model, payload) =>
            {
                ...
                GuildAnalyticsContext context = needContext ? GetAnalyticsContext() : null;
                ...
                if (eventSpec.IncludeInEventLog)
                {
                    ...
                    GuildEventBase payloadCopy = MetaSerialization.CloneTagged<GuildEventBase>(payload, MetaSerializationFlags.EntityEventLog, model.LogicVersion, resolver);

                    // \note Context is only included if so configured in eventLogOptions.
                    GuildAnalyticsContext contextCopy =
                    ...
                    GuildEventLogUtil.AddEntry(model.EventLog, eventLogOptions, collectedAt, uniqueId,
                        baseParams => new GuildEventLogEntry(baseParams, modelTime, schemaVersion, payloadCopy, contextCopy, labelsCopy));
        ...
        protected override void PreStart()
        {
            ...
            _nextInvitationCreationEarliestAt = new MetaDictionary<int, MetaTime>();
        }
        ...
        protected override sealed async Task PostLoad(TModel model, DateTime persistedAt, TimeSpan elapsedTime)
        {
            ...
            _persistedSearchName = (Model.SearchVersion == GuildModelConstants.GuildSearchTableVersion) ? Model.DisplayName : null;
            _log.Debug("Initializing name search: '{GuildName}' (SearchVersion={SearchVersion})", Model.DisplayName, Model.SearchVersion);
        }
        ...
        protected override sealed async Task PersistStateImpl(bool isInitial, bool isFinal)
        {
            ...
            string updateGuildName = (ShouldUpdateGuildSearch()) ? Model.DisplayName : null;
            if (updateGuildName != null)
                _log.Info("Name has changed from '{OldGuildName}' to '{NewGuildName}', search table update required (SearchVersion={SearchVersion})", _persistedSearchName, Model.DisplayName, Model.SearchVersion);

            // Mark the search as updated to current version
            Model.SearchVersion = GuildModelConstants.GuildSearchTableVersion;
            ...
            await MetaDatabase.Get().UpdateGuildAsync(_entityId, (PersistedGuildBase)persisted, updateNameSearch: updateGuildName, Model.LifecyclePhase);
            ...
            // Update member epochs and inform interested members of a successful commit
        ...
        /// <summary>
        /// When persisting guild, should the associated guild search entries be updated as well?
        /// </summary>
        protected virtual bool ShouldUpdateGuildSearch()
        {
            // \note Null and "" names are considered equal for search.
            return (Model.SearchVersion != GuildModelConstants.GuildSearchTableVersion) || ((Model.DisplayName ?? "") != (_persistedSearchName ?? ""));
        }
        ...
        InternalEntityStateResponse HandleInternalGuildStateRequest(InternalEntityStateRequest _)
        {
            ...
            return new InternalEntityStateResponse(serialized, _logicVersion, _activeGameConfig.BaselineStaticGameConfigId, _activeGameConfig.BaselineDynamicGameConfigId, specializationKey: null, logicVersionMismatched: false);
        }
        ...
        async Task<MetaMessage> HandleMemberSubscribeRequest(InternalEntitySubscribeRequestBase subscribeRequest)
        {
            ...
            else
            {
                ...
                MetaDictionary<int, GuildMemberPlayerOpLogEntry> pendingOps = TryGetMemberPendingPlayerOpsAfterEpoch(member, associationRef.LastPlayerOpEpoch);
                if (pendingOps != null)
            ...
            if (subscribeRequest.ResourceProposal.HasValue && (RequireClientHasGameConfigPresentOnSessionStart || subscribeRequest.IsDryRun))
            {
                ...
                if (resourceCorrection.HasAnyCorrection())
                {
                    ...
                    if (subscribeRequest.IsDryRun)
                        throw new InternalEntitySubscribeRefusedBase.Builtins.DryRunSuccess(resourceCorrection, associatedEntities);
                }
            }
            if (subscribeRequest.IsDryRun)
                throw new InternalEntitySubscribeRefusedBase.Builtins.DryRunSuccess(new SessionProtocol.SessionResourceCorrection(), associatedEntities);
            ...
            return new InternalEntitySubscribeResponseBase.Default(
                state: new EntityInitialState(
                    ...
                    clientData:     new EntityClientData.Default(ClientSlotCore.Guild)),
        ...
        async Task HandleInternalGuildTransactionGuildSyncBegin(EntitySynchronize sync, InternalGuildTransactionGuildSync.Begin begin)
        {
            ...
            MetaDictionary<int, GuildMemberPlayerOpLogEntry> preceedingPlayerOps = TryGetMemberPendingPlayerOpsAfterEpoch(member, begin.LastPlayerOpEpoch);
        ...
        bool ContainsAnyUncommittedGuildMemberGuildOp(EntityId playerId, MetaDictionary<int, GuildMemberGuildOpLogEntry> ops)
        {
        ...
        void ExecutePendingGuildMemberGuildOps(EntityId playerId, GuildMemberBase member, MetaDictionary<int, GuildMemberGuildOpLogEntry> ops)
        {
        ...
        MetaDictionary<int, GuildMemberPlayerOpLogEntry> TryGetMemberPendingPlayerOpsAfterEpoch(GuildMemberBase member, int afterEpoch) => TryGetMemberPendingPlayerOpsAfterEpoch(member.PendingPlayerOps, afterEpoch);
        MetaDictionary<int, GuildMemberPlayerOpLogEntry> TryGetMemberPendingPlayerOpsAfterEpoch(GuildPendingMemberKickState kickState, int afterEpoch) => TryGetMemberPendingPlayerOpsAfterEpoch(kickState.PendingPlayerOps, afterEpoch);
        MetaDictionary<int, GuildMemberPlayerOpLogEntry> TryGetMemberPendingPlayerOpsAfterEpoch(MetaDictionary<int, GuildMemberPlayerOpLogEntry> pendingOps, int afterEpoch)
        {
            MetaDictionary<int, GuildMemberPlayerOpLogEntry> preceedingPlayerOps = null;
            if (pendingOps != null)
            {
                foreach ((int epoch, GuildMemberPlayerOpLogEntry op) in pendingOps)
                {
                    ...
                    if (preceedingPlayerOps == null)
                        preceedingPlayerOps = new MetaDictionary<int, GuildMemberPlayerOpLogEntry>();
        ...
        void AfterMemberPendingPlayerOpsCommitted(ref MetaDictionary<int, GuildMemberPlayerOpLogEntry> pendingOps, int committedOpEpoch)
        {
            if (pendingOps == null)
                return;
        ...
        void EnqueueMemberPendingPlayerOp(GuildMemberBase member, GuildMemberPlayerOpLogEntry entry)
        {
            ...
            if (member.PendingPlayerOps == null)
                member.PendingPlayerOps = new MetaDictionary<int, GuildMemberPlayerOpLogEntry>();
            ...
            for (;;)
            {
                ...
                try
                {
                    PersistedGuildInviteCode persisted = new PersistedGuildInviteCode(
                        ...
                        inviteId:   inviteId,
                        createdAt:  MetaTime.Now.ToDateTime());
        ...
        async Task HandleTriggerEventLogFlushing(TriggerEventLogFlushing _)
        {
            ...
            if (GuildEventLogUtil.CanFlush(Model.EventLog, eventLogConfig))
                await GuildEventLogUtil.TryFlushAndCullSegmentsAsync(Model.EventLog, eventLogConfig, _entityId, _logicVersion, _log, PersistStateIntermediate);
        }
        ...
        InternalGuildAdminEditRolesResponse HandleInternalGuildAdminEditRolesRequest(InternalGuildAdminEditRolesRequest request)
        {
            ...
            MetaDictionary<EntityId, GuildMemberRole> actualChanges = Model.ComputeRoleChangesForRoleEvent(GuildMemberRoleEvent.MemberEdit, request.TargetPlayerId, request.TargetNewRole);
