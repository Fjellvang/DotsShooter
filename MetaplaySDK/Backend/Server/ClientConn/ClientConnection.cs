...
namespace Metaplay.Server
{
    ...
    public abstract class ClientConnection : EphemeralEntityActor
    {
        ...
        protected static readonly Prometheus.Counter      c_protocolHashMismatch      = Prometheus.Metrics.CreateCounter("game_client_protocol_hash_mismatch", "Client and server protocol hashes are mismatched (only checked when LogicVersion is exact match)");
        ...
        protected string                  _helloBuildVersion;
        ...
        protected ClientVersion           _helloClientVersion;
        ...
        protected override void PreStart()
        {
            ...
            SendToClient(new Handshake.ServerHello(
                ...
                MetaSerializerTypeRegistry.Instance.FullProtocolHash,
                CloudCoreVersion.CommitId,
                MetaplayCore.Options.ProjectName));
        ...
        protected async Task OnHealthCheckRequest(HealthCheckTypeBits typeBits)
        {
            ...
            if (typeBits.HasFlag(HealthCheckTypeBits.GlobalState))
            {
                try
                {
                    await EntityAskAsync(GlobalStateManager.EntityId, GlobalStatusRequest.Instance);
            ...
            if (typeBits.HasFlag(HealthCheckTypeBits.Database))
            {
                try
                {
                    await MetaDatabase.Get().HealthCheck();
        ...
        {
            string buildVersion = hello.BuildVersion == null ? null : Util.ShortenString(hello.BuildVersion, 256); // Shorten version string to some conservative size, shouldn't matter with legit clients

            // Log source IP only after a successful Hello. This cuts out the spam caused by test connections.
            _log.Info("Client handshake completed with {SourceAddress}: buildVersion={BuildVersion}, localPort={LocalPort}, targetHostname={TargetHostname}", IPAddressRedactionUtil.ToPrivacyProtectingString(_srcAddr), buildVersion, _localPort, hello.TargetHostname ?? "<null>");
            ...
            TimeSpan helloDelay = DateTime.UtcNow - hello.Timestamp;
            if (helloDelay.TotalMilliseconds >= 5_000 && helloDelay.TotalMilliseconds <= 2 * 60 * 1000) // cap to 2min to avoid noise from very bad client clocks
                _log.Info("Client connection was delayed by {HelloDelay:0.00}s", helloDelay.TotalSeconds);
            ...
            if (compatSettings.RedirectEnabled && hello.ClientVersion.LogicVersion > compatSettings.ActiveLogicVersion)
            {
                ServerEndpoint redirectServer = compatSettings.RedirectServerEndpoint;
                if (redirectServer != null)
                {
                    _log.Info("Redirecting future client: clientSupportedLogicVersions={ClientSupportedLogicVersion}, activeLogicVersion={ActiveLogicVersion}", hello.ClientVersion.LogicVersion, compatSettings.ActiveLogicVersionRange);
                    c_clientConnectionRefused.WithLabels("Redirected").Inc();
                    SendToClient(new Handshake.RedirectToServer(redirectServer));
                }
                else
                {
                    _log.Warning("Missing server redirect endpoint for client from future: clientSupportedLogicVersions={ClientSupportedLogicVersions}, activeLogicVersion={ActiveLogicVersion}!", hello.ClientVersion.LogicVersion, compatSettings.ActiveLogicVersionRange);
            ...
            _helloBuildVersion               = buildVersion;
            ...
            _helloClientVersion              = hello.ClientVersion;
            ...
            _logicVersion = _helloClientVersion.LogicVersion;
            ...
            SendToClient(new Handshake.ClientHelloAccepted(
                serverOptions: new Handshake.ServerOptions(
                    ...
                    gameTimeOffset:                                         GlobalStateProxyActor.ActiveGameTimeOffset.Get().Offset
                    )));
        }
        ...
        async Task ReceiveCreateGuestAccountRequest(Handshake.CreateGuestAccountRequest newGuestAccountRequest)
        {
            ...
            if (!CheckClientVersionSupported())
            {
                await GracefulStopAsync();
                return;
            }
        ...
        async Task ReceiveLoginRequest(Handshake.LoginRequest loginReq)
        {
            ...
            //        to be over.
            if (!isInMaintenance && !CheckClientVersionSupported())
            {
                await GracefulStopAsync();
                return;
            }
        ...
        /// </summary>
        bool CheckClientVersionSupported()
        {
            // Only accept the current ActiveLogicVersions
            ClientCompatibilitySettings compatSettings = GlobalStateProxyActor.ActiveClientCompatibilitySettings.Get().ClientCompatibilitySettings;

            if (_helloClientVersion.LogicVersion < compatSettings.ActiveLogicVersionRange.MinVersion)
            {
                _log.Info("Client logic version is too old: {ClientSupportedLogicVersion}, activeLogicVersionRange={ActiveLogicVersionRange}", _helloClientVersion.LogicVersion, compatSettings.ActiveLogicVersionRange);
                c_clientConnectionRefused.WithLabels("ClientLogicVersionTooOld").Inc();
                SendToClient(new Handshake.LogicVersionMismatch(compatSettings.ActiveLogicVersionRange));
                return false;
            }
            if (_helloClientVersion.LogicVersion > compatSettings.ActiveLogicVersionRange.MaxVersion)
            {
                _log.Warning("Refusing client from future: clientSupportedLogicVersion={ClientSupportedLogicVersion}, activeLogicVersionRange={ActiveLogicVersion}!", _helloClientVersion.LogicVersion, compatSettings.ActiveLogicVersionRange);
                c_clientConnectionRefused.WithLabels("ClientLogicVersionTooNew").Inc();
                SendToClient(new Handshake.LogicVersionMismatch(compatSettings.ActiveLogicVersionRange));
                return false;
            }
            ...
            foreach (ClientPatchVersionRequirement req in compatSettings.ClientPatchVersionRequirements)
            {
                if (!req.IsApplicableForClient(_helloClientVersion.LogicVersion, _helloPlatform))
                    continue;

                if (req.MinPatchVersion > _helloClientVersion.Patch)
                {
                    _log.Info("Client patch version is too old, at least {MinimumPatchVersion} required: clientLogicVersion={ClientLogicVersion}, clientPlatform={ClientPlatform}, clientPatchVersion={ClientPatchVersion}",
                        req.MinPatchVersion, _helloClientVersion.LogicVersion, _helloPlatform, _helloClientVersion.Patch);
                    c_clientConnectionRefused.WithLabels("ClientPatchVersionTooOld").Inc();
                    SendToClient(new Handshake.ClientPatchVersionTooOld(req.MinPatchVersion));
                    return false;
                }
            }

            // If SupportedLogicVersions MaxVersion match, warn about mismatching protocol hash.
            if (_helloClientVersion.LogicVersion == MetaplayCore.Options.SupportedLogicVersions.MaxVersion && _helloProtocolHash != MetaSerializerTypeRegistry.Instance.FullProtocolHash)
            {
                c_protocolHashMismatch.Inc();
                _log.Info(
        ...
        {
            // Location overriding
            PlayerLocation? location = Geolocation.Instance.TryGetPlayerLocation(_srcAddr);
            if (sessionRequest.OverrideLocation != null)
            {
                if (GlobalStateProxyActor.ActiveDevelopers.Get().IsPlayerDeveloper(_loginPlayerId) ||
                    RuntimeOptionsRegistry.Instance.GetCurrent<EnvironmentOptions>().EnableDevelopmentFeatures)
                {
                    location = sessionRequest.OverrideLocation;
                }
                else
                {
                    _log.Warning(
                        "Client {PlayerId} attempted to override location without permission. " +
                        "Player is not marked as a Developer Player nor is the server in development mode (Environment:EnableDevelopmentFeatures). " +
                        "Ignored.",
                        _loginPlayerId);
                }
            }
            ...
            InternalSessionStartNewRequest startRequest = new InternalSessionStartNewRequest(
                ...
                clientBuildVersion:             _helloBuildVersion,
        ...
        retry:
            ...
            try
            {
                ...
                // \note: Wait 20s rather than the normal 10s. Session Actor will be subscribing to entities with the default timeout. By waiting more here we can
                //        get the root timeout reason delivered to client. Essentially we will get "Session Crashed: Timeout when subscribing to Entity:XX" rather
                //        than less helpful "Timeout when starting session."
                (sessionSubscription, sessionResponse) = await WaitLongRunningOperationAsync(SubscribeToAsync<InternalSessionStartNewResponse>(sessionId, EntityTopic.Owner, startRequest, timeout: TimeSpan.FromSeconds(20)));
            }
            catch (InternalSessionStartNewRefusal refusal)
            {
                ...
                if (refusal.Result == InternalSessionStartNewRefusal.ResultCode.PlayerIsBanned)
                {
                ...
                else if (refusal.Result == InternalSessionStartNewRefusal.ResultCode.PlayerIsDeleted)
                {
                    _log.Debug("Session {SessionId} start rejected, player is deleted [{Milliseconds}ms]", sessionId, sw.ElapsedMilliseconds);
                    errorCode             = SessionProtocol.SessionStartFailure.ReasonCode.Deleted;
                    debugOnlyErrorMessage = null;
                }
                else if (refusal.Result == InternalSessionStartNewRefusal.ResultCode.LogicVersionDowngradeNotAllowed)
                {
                    _log.Debug("Session {SessionId} start rejected, player trying to downgrade their logicversion to {ClientLogicVersion}. The player has already connected with a higher logic version. [{Milliseconds}ms]", sessionId, _helloClientVersion.LogicVersion, sw.ElapsedMilliseconds);
            ...
            SendToClient(new SessionProtocol.SessionStartSuccess(
                ...
                logicVersion:                   _helloClientVersion.LogicVersion,
        ...
        async Task ReceiveLoginAndSessionResumeRequest(Handshake.LoginAndResumeSessionRequest loginAndResumeSessionRequest)
        {
            ...
            if (!SessionResumptionTokenUtil.ValidateResumptionToken(loginAndResumeSessionRequest.ClaimedPlayerId, loginAndResumeSessionRequest.SessionToResume.Token, loginAndResumeSessionRequest.ResumptionToken))
            {
                ...
                SendToClient(new SessionProtocol.SessionResumeFailure(generateIncidentReport: true));
                await GracefulStopAsync();
                return;
            }
            ...
            if (isInMaintenance)
            {
                ...
                // Check client version if it wasn't checked earlier
                if (!CheckClientVersionSupported())
                {
                    c_sessionResumeFails.WithLabels("in_maintenance").Inc();
                    await GracefulStopAsync();
            ...
            InternalSessionResumeRequest resumeRequest = new InternalSessionResumeRequest(
                ...
                clientSessionNonce:             _helloClientSessionNonce,
                clientSessionConnectionNdx:     _helloClientSessionConnectionNdx,
                buildVersion:                   _helloBuildVersion,
                debugDiagnostics:               _loginDebugDiagnostics,
                loginGamePayload:               _loginGamePayload,
            ...
            catch (InternalSessionResumeRefusal refusal)
            {
                ...
                SendToClient(new SessionProtocol.SessionResumeFailure(generateIncidentReport: refusal.Result == InternalSessionResumeRefusal.ResultCode.UnexpectedResumeFailure));
                return;
            }
            catch
            {
        ...
        async Task ReceiveSessionStartAbortReasonTrailer(SessionProtocol.SessionStartAbortReasonTrailer sessionAbortTrailer)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    await PlayerIncidentStorage.SerializeAndPersistIncidentAsync(_loginPlayerId, incident, source: PlayerIncidentStorage.IncidentSource.SessionStartAbortTrailer, informPlayerActor: true, sourceEntity: this);
                }
            }
            ...
            try
            {
                ...
                switch (req)
                {
                    ...
                    case Handshake.SocialAuthenticationLoginRequest socialReq:
                    {
                        ...
                        SocialAuthenticationResult authResult = await Authenticator.AuthenticateAccountViaSocialPlatform(_log, asker: this, socialReq.Claim, playerIdHint: socialReq.PlayerIdHint, socialReq.IsBot);
                        accountInfo = new LoginAuthParams(
                    ...
                    case Handshake.DualSocialAuthenticationLoginRequest socialReq:
                    {
                        ...
                        try
                        {
                            authResult = await Authenticator.AuthenticateAccountViaSocialPlatform(_log, asker: this, socialReq.Claim, playerIdHint: socialReq.PlayerIdHint, socialReq.IsBot);
                        }
                        catch (NoBoundPlayerAccountForValidSocialAccount noPlayerError)
                        {
                            ...
                            InternalPlayerLoginWithNewSocialAccountResponse response = await EntityAskAsync(guestAccountPlayerId, new InternalPlayerLoginWithNewSocialAccountRequest(noPlayerError.AuthKeys));
        ...
        static WirePacket? TryParsePacket(ByteString incomingBuffer)
        {
            ...
            {
                throw new InvalidOperationException($"Bad packet: Invalid packet type: {packetType}");
            }

            // Validate payloadLength
            if (payloadLength <= 0 || payloadLength > WireProtocol.MaxPacketWirePayloadSize)
                throw new InvalidOperationException($"Bad packet: Invalid payloadLength ({payloadLength}) header received");
            ...
            switch (compressionMode)
            {
                ...
                default:
                    throw new InvalidOperationException($"Bad packet: Invalid compression mode for packet: {compressionMode}");
            }
        ...
        PacketParsingErrorReaction HandlePacketParsingError(Exception exception)
        {
            ...
            if (trafficInfo == null)
            {
                // Not well-known traffic. Log first couple of bytes so they can be debugged, but otherwise handle it silently.
                _log.Warning("Got malformed first packet from {SourceAddress} (totalBytesReceived={TotalBytesReceived}): [ {IncomingBufferPrefix} ... ]", IPAddressRedactionUtil.ToPrivacyProtectingString(_srcAddr), _totalBytesReceived, Util.BytesToString(_incomingBuffer.Take(5).ToArray()));
            }
            else
            {
        ...
        async Task<T> WaitLongRunningOperationAsync<T>(Task<T> longRunningTask)
        {
            ...
            // Start background task for keep-alives
            ...
            // Run the long-running task while the background task updates the client.
            try
            {
    ...
    public class TcpClientConnection : ClientConnection
    {
        ...
        public TcpClientConnection(EntityId entityId, IActorRef socket, IPAddress srcAddress, int localPort) : base(entityId, srcAddress, localPort)
        {
            ...
            // Finish TCP binding
            Tell(_socket, new Tcp.Register(_self));
        }

        protected override void CloseSocket()
        {
            if (_socket is IActorRef notNullsocket)
                Tell(notNullsocket, Tcp.Close.Instance);
        }

        protected override void WriteBytesToSocket(ByteString data)
        {
            Tell(_socket, Tcp.Write.Create(data));
        }
