...
namespace Metaplay.Server.Matchmaking
{
    ...
    public class AsyncMatchmakingRequest : EntityAskRequest<AsyncMatchmakingResponse>
    {
    ...
    public class AsyncMatchmakingResponse : EntityAskResponse
    {
            ...
            where TMmPlayerModel : struct, IAsyncMatchmakerPlayerModel
        {
            return new AsyncMatchmakingResponse(
                ...
                mmModel.HasValue ? MetaSerialization.SerializeTagged(mmModel.Value, MetaSerializationFlags.IncludeAll, null) : null);
        }
    ...
    public class AsyncMatchmakerPlayerEnrollRequest : EntityAskRequest<AsyncMatchmakerPlayerEnrollResponse>
    {
    ...
    public class AsyncMatchmakerPlayerEnrollResponse : EntityAskResponse
    {
    ...
    public class AsyncMatchmakerInfoRequest : EntityAskRequest<AsyncMatchmakerInfoResponse>
    {
    ...
    public class AsyncMatchmakerInfoResponse : EntityAskResponse
    {
        [MetaSerializable]
        ...
        public sealed class LabelInfo
        {
            ...
            [MetaMember(2)] public string TypeName;
            [MetaMember(3)] public List<string> Options;

            public LabelInfo(string displayName, string typeName, List<string> options)
            {
        ...
        public AsyncMatchmakerInfoResponse(string name, string description, int playersInBuckets, float bucketsOverallFillPercentage, BucketInfo[] bucketInfos, LabelInfo[] labelInfos, int stateSizeInBytes,
    ...
    public class AsyncMatchmakerPlayerInfoRequest : EntityAskRequest<AsyncMatchmakerPlayerInfoResponse>
    {
    ...
    public class AsyncMatchmakerPlayerInfoResponse : EntityAskResponse
    {
        public EntityId PlayerId      { get; private set; }
    ...
    public class AsyncMatchmakerRebalanceBucketsRequest : EntityAskRequest<EntityAskOk>
    {
    ...
    public class AsyncMatchmakerClearStateRequest : EntityAskRequest<EntityAskOk>
    {
    ...
    public class AsyncMatchmakerInspectBucketRequest : EntityAskRequest<AsyncMatchmakerInspectBucketResponse>
    {
    ...
    public class AsyncMatchmakerInspectBucketResponse : EntityAskResponse
    {
            ...
            where TMmPlayerModel : struct, IAsyncMatchmakerPlayerModel
        {
            ...
            using (IOWriter ioW = new IOWriter(ioB))
            {
                foreach (TMmPlayerModel player in players.OrderByDescending(x => x.DefenseMmr).Skip(pageIndex * pageSize).Take(pageSize))
                    MetaSerialization.SerializeTagged(ioW, player, MetaSerializationFlags.IncludeAll, null);
            }
        ...
        public List<IAsyncMatchmakerPlayerModel> GetDeserializedPlayers(Type matchmakerModelType)
        {
            ...
            {
                object model = MetaSerialization.DeserializeTagged(ioR, matchmakerModelType, MetaSerializationFlags.IncludeAll, null, null);
    ...
    public class AsyncMatchmakerEntityKind
    {
        ...
        public AsyncMatchmakerEntityKind(EntityKind entityKind, Type queryType, Type modelType)
        {
            ...
            if (!EntityConfigRegistry.Instance.TryGetConfig(EntityKind, out EntityConfigBase entityConfig))
                throw new InvalidOperationException(Invariant($"No entity config found for entity kind {EntityKind}"));

#pragma warning disable CS0618 // Type or member is obsolete
            if (entityConfig.ShardingStrategy is StaticServiceShardingStrategy staticStrategy)
                NumMatchmakers = staticStrategy.IsSingleton ? 1 : clusterOptions.ClusterConfig.GetNodeCountForEntityKind(entityKind);
            else if (entityConfig.ShardingStrategy is StaticMultiServiceShardingStrategy multiServiceStrategy)
                NumMatchmakers = multiServiceStrategy.NumServices;
            else
                throw new InvalidOperationException(Invariant($"Unsupported sharding strategy for entity kind {EntityKind}"));
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        where TMatchmakerOptions : AsyncMatchmakerOptionsBase
    {
        ...
        {
            [MetaOnMemberDeserializationFailure("ClearSizingModelOnDeserializationFailure")]
            [MetaMember(1)] public DynamicBucketSizingModel                SizingModel;
            [MetaMember(2)] public int                                     LastPlayerCacheTotalSize;
            [MetaOnMemberDeserializationFailure("ClearBucketingStrategyStatesOnDeserializationFailure")]
            ...
            public static DynamicBucketSizingModel ClearSizingModelOnDeserializationFailure(MetaMemberDeserializationFailureParams failureParams) => null;
            public static List<IAsyncMatchmakerBucketingStrategyState> ClearBucketingStrategyStatesOnDeserializationFailure(MetaMemberDeserializationFailureParams failureParams) => null;
        }
        ...
        protected override sealed Task<TMatchmakerState> RestoreFromPersisted(PersistedMatchmakerState persisted)
        {
            ...
            if (state.SizingModel == null)
            {
                _log.Warning($"Failed to deserialize SizingModel, resetting to default");
                state.SizingModel = DynamicBucketSizingModel.CreateDefault(MmOptions);
            }
        ...
        void SetPlayersToDbCache()
        {
            ...
            if (State.CachedPlayers.Count > MetaSerializationContext.DefaultMaxCollectionSize) // Limit the amount of players in the cache
                State.CachedPlayers = State.CachedPlayers.Take(MetaSerializationContext.DefaultMaxCollectionSize).ToList();
        }
        ...
        /// </summary>
        [EntityAskHandler]
        public void HandleMatchmakingRequest(EntityShard.EntityAsk<AsyncMatchmakingResponse> entityAsk, AsyncMatchmakingRequest request)
        {
        ...
        public AsyncMatchmakerInfoResponse HandleMatchmakerInfoRequest(AsyncMatchmakerInfoRequest request)
        {
            ...
            float bucketsOverallFillPercentage = totalPlayers / (float)State.BucketPool.AllBuckets.Sum(b => b.Capacity);
            if (float.IsNaN(bucketsOverallFillPercentage))
                bucketsOverallFillPercentage = 0;

            AsyncMatchmakerInfoResponse.LabelInfo[] labelInfos = new AsyncMatchmakerInfoResponse.LabelInfo[BucketingStrategies.Count];

            for (int i = 0; i < BucketingStrategies.Count; i++)
            {
                IAsyncMatchmakerBucketingStrategy<TMmPlayerModel, TMmQuery> strategy = BucketingStrategies[i];

                IEnumerable<IBucketLabel> labels = State.BucketPool.AllBuckets.SelectMany(b => b.Labels)
                    .Where(l => l.GetType() == strategy.LabelType);

                if (strategy.LabelType.IsAssignableTo(typeof(IOrderedBucketLabel)))
                    labels = labels.Order();

                List<string> options = labels.Select(l => l.DashboardLabel)
                    .Distinct().ToList();

                labelInfos[i] = new AsyncMatchmakerInfoResponse.LabelInfo(
                    displayName: strategy.LabelDashboardName,
                    typeName: strategy.LabelType.FullName,
                    options: options);
            }
        ...
        public async Task<AsyncMatchmakerPlayerEnrollResponse> HandleMatchmakerPlayerEnrollRequest(AsyncMatchmakerPlayerEnrollRequest request)
        {
            ...
            InternalEntityStateResponse response          = await EntityAskAsync(request.PlayerId, InternalEntityStateRequest.Instance);
        ...
        async Task<(IEnumerable<TPlayerModel>, int scannedPlayersCount)> FetchBatchFromDb(EntityId startId, int batchSize, ISharedGameConfig config, int activeLogicVersion, ErrorCounter errorCounter)
        {
            ...
            {
                EntityId entityId = EntityId.ParseFromString(player.EntityId);
                MetaDebug.Assert(
                    SysOptions.AllowBotAccounts || entityId.Value >=
                ...
                try
                {
                    model = entityConfig.DeserializeDatabasePayload<TPlayerModel>(entityId, player.Payload, resolver, activeLogicVersion);
                    ...
                    SchemaMigrator migrator = SchemaMigrationRegistry.Instance.GetSchemaMigrator(entityConfig.GetPersistedPayloadType(entityId));
