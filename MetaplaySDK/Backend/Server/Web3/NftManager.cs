...
namespace Metaplay.Server.Web3
{
    ...
    {
        [MetaMember(1)] public MetaDictionary<NftCollectionId, Collection> Collections = new();
        ...
        public class Collection
        {
            ...
            [MetaMember(3)] public MetaDictionary<NftId, UninitializedNftInfo>   RecentUninitializedNfts = new();
    ...
    public class NftManager : PersistedEntityActor<PersistedNftManager, NftManagerState>
    {
        ...
        {
            public MetaDictionary<NftCollectionId, Collection> Collections = new();
        ...
        [EntityAskHandler]
        void HandleRefreshCollectionLedgerInfoRequest(EntityAsk<RefreshCollectionLedgerInfoResponse> ask, RefreshCollectionLedgerInfoRequest request)
        {
        ...
        async Task HandleFlushPendingMetadataWritesAsync(FlushPendingMetadataWrites _)
        {
            ...
            MetaDictionary<NftKey, byte[]> nftMetadatasToWrite = new();
        ...
        [EntityAskHandler]
        void HandleRefreshNftRequest(EntityAsk<RefreshNftResponse> ask, RefreshNftRequest request)
        {
            FullGameConfig gameConfig = GetActiveBaselineGameConfig();
        ...
        [EntityAskHandler]
        void HandleRefreshNftsOwnedByEntityRequest(EntityAsk<RefreshNftsOwnedByEntityResponse> ask, RefreshNftsOwnedByEntityRequest request)
        {
            FullGameConfig gameConfig = GetActiveBaselineGameConfig();
            ...
            ContinueTaskOnActorContext(
                ...
                handleSuccessAsync: async (MetaDictionary<NftKey, NftOwnerAddress> ownedNfts) =>
                {
                    try
                    {
        ...
        /// </summary>
        async Task UpdateOwnedTokensAsync(FullGameConfig gameConfig, EntityId ownerId, MetaDictionary<NftKey, NftOwnerAddress> newlyKnownOwnedNfts)
        {
            // Get NFTs that the server thinks (before applying the new info) the owner owns.
            MetaDictionary<NftKey, NftQueryResultItem> lastKnownOwnedNfts = (await StorageQueryNftsAsync(collection: null, owner: ownerId, gameConfig))
                                                                               .ToMetaDictionary(queryItem => queryItem.NftKey);
