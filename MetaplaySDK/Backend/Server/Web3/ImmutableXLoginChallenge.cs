...
namespace Metaplay.Server.Web3
{
    public readonly struct ImmutableXLoginChallenge
    {
        ...
        ImmutableXLoginChallenge(string message, string description, EthereumAddress ethAddress, EntityId playerId, DateTime timestamp)
        {
            Message = message;
        ...
        public static ImmutableXLoginChallenge Create(Web3Options web3Options, EnvironmentOptions environmentOpts, DateTime issuedAt, EntityId playerId, EthereumAddress ethAddress, StarkPublicKey imxAddress)
        {
            ...
            string signature = GetHmacString(
                value: $"sub:{playerId};eth:{ethAddress.GetAddressString()};imx:{imxAddress.GetPublicKeyString()};t:{issuedAt.ToString(format: "o", CultureInfo.InvariantCulture)}",
            ...
            MetaDictionary<string, string> fields = new MetaDictionary<string, string>()
            {
                ...
                { "{Timestamp}", issuedAt.ToString(format: "o", CultureInfo.InvariantCulture) },
                { "{Signature}", signature },
            };
        ...
        static string Format(string template, MetaDictionary<string, string> fields)
        {
