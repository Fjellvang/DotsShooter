...
namespace Metaplay.Server.Web3
{
    ...
    public class NftLedgerDataFetcher
    {
        ...
        public async Task<MetaDictionary<NftKey, NftOwnerAddress>> GetNftsOwnedByAddressesAsync(Web3Options web3Options, List<NftOwnerAddress> ownerAddresses)
        {
            MetaDictionary<NftKey, NftOwnerAddress> ownedNfts = new();

            foreach (Web3Options.NftCollectionSpec collection in web3Options.NftCollections.Values)
            {
                MetaDictionary<NftKey, NftOwnerAddress> ownedInCollection = await GetNftsInCollectionOwnedByAddressesAsync(web3Options, collection, ownerAddresses);
        ...
        async Task<MetaDictionary<NftKey, NftOwnerAddress>> GetNftsInCollectionOwnedByAddressesAsync(Web3Options web3Options, Web3Options.NftCollectionSpec collection, List<NftOwnerAddress> ownerAddresses)
        {
            switch (collection.Ledger)
            {
                case Web3Options.NftCollectionLedger.LocalTesting:
                    return new MetaDictionary<NftKey, NftOwnerAddress>();
        ...
        async Task<MetaDictionary<NftKey, NftOwnerAddress>> GetImmutableXNftsInCollectionOwnedByAddressesAsync(Web3Options web3Options, Web3Options.NftCollectionSpec collection, List<NftOwnerAddress> ownerAddresses)
        {
            MetaDictionary<NftKey, NftOwnerAddress> ownedNfts = new();
