...
namespace Metaplay.Server.EntityArchive
{
    ...
    public class DefaultImportGuildHandler : SimpleImportEntityHandler
    {
        ...
        protected override async Task DoWrite(EntityId entityId, bool createFirst, IEntityMessagingApi asker)
        {
            if (createFirst)
            {
            ...
            InternalGuildImportModelDataResponse response = await asker.EntityAskAsync(entityId, request);
            if (!response.Success)
            {
