...
namespace Metaplay.Server.EntityArchive
{
    ...
    public abstract class ImportEntityHandler
    {
        ...
        public async Task<EntityId> GenerateNewId(IEntityMessagingApi asker)
        {
            EntityId newId;
            do
            {
        ...
        public async Task Write(IEntityMessagingApi asker)
        {
        ...
        virtual public async Task<string> Diff(EntityId otherId, IEntityMessagingApi asker)
        {
        ...
        protected virtual Task<EntityId> DoGenerateNewId(IEntityMessagingApi asker)
        {
        ...
        protected internal abstract Task<bool> DoDoesExist(EntityId id);
        ...
        /// <returns></returns>
        protected abstract Task DoWrite(EntityId id, bool createFirst, IEntityMessagingApi asker);
        ...
        protected abstract Task<string> DoDiff(EntityId otherId, IEntityMessagingApi asker);
    }
...
#pragma warning restore CS0618 // Type or member is obsolete
    {
        ...
        public override void Initialize()
        {
            ...
            if (EntityConfig.IsPolymorphic)
                throw new ImportException("Polymorphic entities are not supported.", "Polymorphic entities are not supported in import handlers.");
        }
        ...
        protected override sealed void DoConstruct(string data, int? schemaVersion)
        {
            ...
            {
                SchemaMigrator migrator = SchemaMigrationRegistry.Instance.GetSchemaMigrator(EntityConfig.GetPersistedPayloadType(EntityId.None));
        ...
        protected internal override sealed async Task<bool> DoDoesExist(EntityId id)
        {
        ...
        protected override sealed async Task DoRemap(EntityImportEntityIdRemapper remapper)
        {
            ...
            Payload = EntityConfig.SerializeDatabasePayload(EntityId.None, model, LogicVersion);
        }
        ...
        protected IModel MaterializeModel(bool runMigrations)
        {
            ...
            try
            {
                model = EntityConfig.DeserializeDatabasePayload<IModel>(EntityId.None, Payload, resolver, LogicVersion);
            }
            catch (Exception ex)
            {
        ...
        protected override sealed async Task<string> DoDiff(EntityId otherId, IEntityMessagingApi asker)
        {
            ...
            if (SchemaVersion.HasValue)
            {
                ...
                if (SchemaVersion < toVersion)
                {
                    ...
                    proposedPayload = EntityConfig.SerializeDatabasePayload(EntityId.None, model, LogicVersion);
                }
            }

            // Ensure that the other entity's data is persisted to database
            _ = await asker.EntityAskAsync(otherId, PersistStateRequestRequest.Instance);
            ...
            comparer.Type = EntityConfig.GetPersistedPayloadType(EntityId.None);
