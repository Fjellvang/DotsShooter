...
namespace Metaplay.Server.EntityArchive
{
    ...
    public class EntityImportEntityIdRemapper : IModelEntityIdRemapper
    {
        ...
        MetaDictionary<EntityId, EntityId>                     _memo                   = new MetaDictionary<EntityId, EntityId>();
        MetaDictionary<EntityId, EntityId>                     _fixedMap               = new MetaDictionary<EntityId, EntityId>();
        MetaDictionary<EntityKind, Func<Task<EntityId>>>       _asyncIdGenerators      = new MetaDictionary<EntityKind, Func<Task<EntityId>>>();
        MetaDictionary<EntityKind, Func<EntityId, Task<bool>>> _entityExistsPredicates = new MetaDictionary<EntityKind, Func<EntityId, Task<bool>>>();
        ...
        public void SetEntityExistsPredicate(EntityKind kind, Func<EntityId, Task<bool>> entityExistsPredicate)
        {
            _entityExistsPredicates.Add(kind, entityExistsPredicate);
        }
        ...
        public async Task<EntityId> Get(EntityId sourceId)
        {
            ...
            {
                // Not in forced remap. In create mode, we create new ids as we go. Otherwise, we check if the Id exists, if so, the id stays, otherwise we use None
                ...
                // If the entity is currently being imported, or if it exists in the database, we want to keep the original EntityId
                else if (await DoesExist(sourceId))
                    destinationId = sourceId;
                // Otherwise, we know the entity won't exist and might cause errors, therefore we use None instead
                else
                    destinationId = EntityId.None;
        ...
        /// <summary>
        /// Checks to see if the given <paramref name="sourceId"/> exists in either the current imported entity set or in the database.
        /// </summary>
        /// <param name="sourceId">The id you wish to check</param>
        /// <returns><c>True</c> if the entity exists in the current import or in the database.</returns>
        Task<bool> DoesExist(EntityId sourceId)
        {
            if (!_entityExistsPredicates.TryGetValue(sourceId.Kind, out Func<EntityId, Task<bool>> getExistingAsyncFunc))
                return Task.FromResult(false);

            return getExistingAsyncFunc(sourceId);
        }

        Task<EntityId> IModelEntityIdRemapper.RemapEntityIdAsync(EntityId originalId) => Get(originalId);
    }

    public partial class EntityArchiveUtils
    {
        /// <summary>
        ...
        public static Task<ImportResponse> Import(
            int formatVersion,
            MetaDictionary<string, MetaDictionary<string, ExportedEntity>> importEntities,
            MetaDictionary<string, MetaDictionary<string, string>> userRemaps,
            OverwritePolicy overwritePolicy,
            bool verifyOnly,
            IEntityMessagingApi asker)
        {
            ...
            MetaDictionary<string, MetaDictionary<EntityId, ExportedEntity>>  convertedImportEntities = new MetaDictionary<string, MetaDictionary<EntityId, ExportedEntity>>();
            MetaDictionary<string, MetaDictionary<EntityId, EntityId>> convertedUserRemaps = new MetaDictionary<string, MetaDictionary<EntityId, EntityId>>();

            foreach (string entityType in importEntities.Keys)
            {
                if (!convertedImportEntities.ContainsKey(entityType))
                    convertedImportEntities.Add(entityType, new MetaDictionary<EntityId, ExportedEntity>());
            ...
            foreach (string entityType in userRemaps.Keys)
            {
                if (!convertedUserRemaps.ContainsKey(entityType))
                    convertedUserRemaps.Add(entityType, new MetaDictionary<EntityId, EntityId>());
...
#pragma warning disable CS0618 // Type or member is obsolete #LegacyEntityHandlerCompat
        static async Task<ImportResponse> Import(
            MetaDictionary<string, MetaDictionary<EntityId, ExportedEntity>> importEntities,
            MetaDictionary<string, MetaDictionary<EntityId, EntityId>> userRemaps,
            OverwritePolicy overwritePolicy,
            bool verifyOnly,
            IEntityMessagingApi asker)
        {
            ...
            foreach ((string entityType, ArchivingSpec archivingSpec) in s_archiveHandlers)
            {
                ...
                remapper.SetEntityExistsPredicate(handler.EntityKind, async entityId => (importEntities.ContainsKey(entityType) && importEntities[entityType].ContainsKey(entityId)) || await handler.DoDoesExist(entityId));

                if (userRemaps.TryGetValue(entityType, out MetaDictionary<EntityId, EntityId> typeRemaps))
                {
            ...
            foreach ((string entityType, ArchivingSpec archivingSpec) in s_archiveHandlers)
            {
                ...
                MetaDictionary<EntityId, ExportedEntity> entities = importEntities.GetValueOrDefault(entityType);
        ...
        static async Task<EntityResult> PrepareHandlerAsync(EntityImportEntityIdRemapper remapper, OverwritePolicy overwritePolicy, ImportEntityHandler handler, EntityId sourceId, ExportedEntity data, IEntityMessagingApi asker)
        {
