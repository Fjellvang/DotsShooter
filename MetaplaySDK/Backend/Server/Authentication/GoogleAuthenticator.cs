...
namespace Metaplay.Server.Authentication.Authenticators
{
    public abstract class GoogleAuthenticatorBase : SocialPlatformAuthenticatorBase
    {
        ...
        protected static async Task<GoogleOAuth2UserIds> GetGamesOAuth2UserIdsAsync(string oauth2ServerAuthCode)
        {
            ...
            {
                using FormUrlEncodedContent content = new FormUrlEncodedContent(new MetaDictionary<string, string>() {
            ...
            AppVerifyResponse verifyResponse = await HttpUtil.RequestJsonGetAsync<AppVerifyResponse>(
                s_httpClient,
                $"https://www.googleapis.com/games/v1/applications/{storeOpts.GooglePlayApplicationId}/verify",
            ...
            PlayerResponse playerResponse = await HttpUtil.RequestJsonGetAsync<PlayerResponse>(
                s_httpClient,
    ...
    public class GooglePlayV2Authenticator : GoogleAuthenticatorBase
    {
        ...
        {
            if (string.IsNullOrEmpty(googlePlayV2.ServerAuthCode))
                throw new AuthenticationError("Invalid google play games login claim. No ServerAuthCode.");
