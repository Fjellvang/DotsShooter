...
namespace Metaplay.Server.Authentication
{
    ...
    public class AuthenticationConflictAutoResolverBase : IMetaIntegrationSingleton<AuthenticationConflictAutoResolverBase>
    {
        ...
        /// </summary>
        public static async Task<IPlayerModelBase> GetPlayerModelAsync(IEntityMessagingApi asker, EntityId playerId)
        {
            InternalEntityStateResponse response            = await asker.EntityAskAsync(playerId, InternalEntityStateRequest.Instance);
        ...
        public virtual Task<EntityId> ResolveInternallyConflictingSocialPlatformAsync(IEntityMessagingApi asker, AuthenticatedSocialClaimKeys keys, EntityId[] possiblePlayers)
        {
