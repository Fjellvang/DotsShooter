...
namespace Metaplay.Server.Authentication.Authenticators
{
    ...
    {
        public static async Task<AuthenticatedSocialClaimKeys> AuthenticateAsync(SocialAuthenticationClaimImmutableX claim)
        {
            ...
            DateTime issuedAt = claim.ChallengeTimestamp;
            DateTime now = DateTime.UtcNow;
            if (issuedAt > now)
                throw new AuthenticationError("ChallengeTimestamp is in the future");
            if (issuedAt < now - web3Options.ImmutableXPlayerAuthenticationTimeLimit)
