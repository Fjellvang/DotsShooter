...
namespace Metaplay.Server.Authentication
{
    ...
    public static class Authenticator
    {
        ...
        static async Task<AuthenticatedSocialClaimKeys> AuthenticateSocialAuthenticationClaimAsync(SocialAuthenticationClaimBase claim)
        {
            switch (claim)
            {
                ...
                case SocialAuthenticationClaimImmutableX immutableX:
                    return await ImmutableXAuthenticator.AuthenticateAsync(immutableX);

                case SocialAuthenticationClaimSteam steam:
                    return await SteamAuthenticator.AuthenticateAsync(steam);
        ...
        /// <exception cref="AuthenticationError">if claim is not valid</exception>
        /// <exception cref="AuthenticationTemporarilyUnavailable">if validation services are temporarily unavailable</exception>
        static async Task<SocialAuthenticationAttachedPlayer?> TryGetAndSyncSocialAuthenticationAttachedPlayerAsync(IMetaLogger log, IEntityMessagingApi asker, AuthenticatedSocialClaimKeys authenticatedClaim)
        {
            ...
            foreach (MaintenanceMigration migration in migrations)
            {
                ...
                {
                    // migration
                    InternalPlayerResolveSocialAuthConflictResponse removed = await asker.EntityAskAsync(migration.FromPlayer, InternalPlayerResolveSocialAuthConflictRequest.ForSocialAuthMigrationSource(AuthenticatedSocialClaimKeys.FromSingleKey(migration.SocialKey), migration.ToPlayer));
                    _ = await asker.EntityAskAsync(migration.ToPlayer, InternalPlayerResolveSocialAuthConflictRequest.ForSocialAuthMigrationDestination(removed.RemovedSocialKeys, migration.FromPlayer));
                }
                else
                {
                    // new key
                    await asker.EntityAskAsync(migration.ToPlayer, InternalPlayerResolveSocialAuthConflictRequest.ForSocialAuthAddNewMigrationKey(new MetaDictionary<AuthenticationKey, PlayerAuthEntryBase>() { { migration.SocialKey, null } }));
                }
            }
        ...
        /// <exception cref="AuthenticationError">if claim is not valid</exception>
        /// <exception cref="AuthenticationTemporarilyUnavailable">if validation services are temporarily unavailable</exception>
        public static async Task<SocialAuthenticationResult> AuthenticateSocialAccountAsync(IMetaLogger log, IEntityMessagingApi asker, SocialAuthenticationClaimBase claim)
        {
            // Authenticate (or throw) the social claim
            AuthenticatedSocialClaimKeys authenticatedSocialClaim = await AuthenticateSocialAuthenticationClaimAsync(claim).ConfigureAwait(false);
            ...
            else // AuthEntry not found in database
            {
                if (isBot)
                {
                    ...
                    catch (Exception exception)
                    {
                        ...
                        //
                        // Note that we cannot detect botness from the existing player entity. Hence if the login is interrupted between successful
        ...
        static bool SocialAuthenticationPlatformSupportsLogin(AuthenticationPlatform platform)
        {
            if (platform == AuthenticationPlatform.Development)
                return true;
            if (platform == AuthenticationPlatform.Steam)
                return true;
            return false;
        }
        ...
        public static async Task<SocialAuthenticationResult> AuthenticateAccountViaSocialPlatform(IMetaLogger log, IEntityMessagingApi asker, SocialAuthenticationClaimBase claim, EntityId playerIdHint, bool isBot)
        {
            ...
            SocialAuthenticationResult result = await AuthenticateSocialAccountAsync(log, asker, claim);

            if (result.ExistingPlayerId == EntityId.None && claim is SocialAuthenticationClaimSteam)
            {
                EntityId                     playerId        = await DatabaseEntityUtil.CreateNewPlayerAsync(log);

                AuthenticationKey            authKey         = result.AuthKeys.PrimaryAuthenticationKey;
                PersistedAuthenticationEntry authEntry       = new PersistedAuthenticationEntry(authKey, hashedAuthToken: null, playerId);

                await InsertAuthenticationEntryAsync(authEntry);

                result = new SocialAuthenticationResult(result.AuthKeys, playerId);
            }
            ...
            if (result.ExistingPlayerId == EntityId.None)
                throw new NoBoundPlayerAccountForValidSocialAccount(result.AuthKeys, $"No account binding exists for platform {result.AuthKeys.PrimaryAuthenticationKey.Platform} Id {result.AuthKeys.PrimaryAuthenticationKey.Id} (claimed player id {playerIdHint})");

            // Due to account transfer the client may claim a mismatching player id
            if (playerIdHint != EntityId.None && result.ExistingPlayerId != playerIdHint)
                log.Info("Mismatched playerId at login via social platform: claimed={claimedPlayerId}, actual={PlayerEntityId}.", playerIdHint, result.ExistingPlayerId);
