    ...
    public abstract partial class PlayerActorBase<TModel, TPersisted>
    {
        protected abstract class GuildComponentBase : EntityGuildComponent
        {
            ...
            public AssociatedEntityRefBase GetSessionStartAssociatedGuildEntity()
            {
                ...
                MetaDictionary<int, GuildMemberGuildOpLogEntry> committedPendingGuildOps = null;
            ...
            public async Task<LeaveGuildResults> LeaveGuildAsync(bool forceLeave)
            {
                ...
                for (; ; )
                {
                    InternalGuildLeaveResponse leaveResponse = await PlayerBase.EntityAskAsync(GuildState.GuildId, new InternalGuildLeaveRequest(PlayerBase._entityId, GuildState.GuildMemberInstanceId, _persistedPlayerOpEpoch, GuildState.PendingGuildOps, forceLeave));
            ...
            async Task<InternalPlayerJoinGuildResponse> HandleInternalPlayerJoinGuildRequest(InternalPlayerJoinGuildRequest request)
            {
                ...
                for (; ; )
                {
                    using (EntitySynchronize guildSync = await PlayerBase.EntitySynchronizeAsync(request.OriginalRequest.GuildId, new InternalGuildJoinGuildSync.Begin(PlayerBase._entityId, request.OriginalRequest, playerData)))
                    {
                        ...
                        MetaDictionary<int, GuildMemberGuildOpLogEntry> committedPendingGuildOps = TryGetCommittedPendingGuildOps();
                        InternalOwnedGuildAssociationRef guildAssociation = new InternalOwnedGuildAssociationRef(
                            playerId: PlayerBase._entityId,
                            guildId: GuildState.GuildId,
                            memberInstanceId: GuildState.GuildMemberInstanceId,
                ...
                switch (mode)
                {
                    ...
                    case GuildActionExecutionMode.EventuallyConsistent:
                    {
                        ...
                        if (GuildState.PendingGuildOps == null)
                            GuildState.PendingGuildOps = new MetaDictionary<int, GuildMemberGuildOpLogEntry>();
            ...
            MetaDictionary<int, GuildMemberGuildOpLogEntry> TryGetCommittedPendingGuildOps()
            {
                if (GuildState.PendingGuildOps == null)
                    return null;

                MetaDictionary<int, GuildMemberGuildOpLogEntry> committedOps = null;
                foreach ((int epoch, GuildMemberGuildOpLogEntry pendingOp) in GuildState.PendingGuildOps)
                {
                    ...
                    if (committedOps == null)
                        committedOps = new MetaDictionary<int, GuildMemberGuildOpLogEntry>();
            ...
            void HandleSyncPersistedGuildOpsToGuild(SyncPersistedGuildOpsToGuild command)
            {
                ...
                MetaDictionary<int, GuildMemberGuildOpLogEntry> committedPendingOps = TryGetCommittedPendingGuildOps();
            ...
            async Task<CompleteKickedFromGuildResults> TryCompleteKickedFromGuild()
            {
                ...
                for (; ; )
                {
                    InternalGuildPeekKickedStateResponse isKickedResponse = await PlayerBase.EntityAskAsync(guildId, new InternalGuildPeekKickedStateRequest(PlayerBase._entityId, memberInstanceId, _persistedPlayerOpEpoch));
            ...
            GuildPlayerOpResults ExecuteGuildMemberPlayerOps(MetaDictionary<int, GuildMemberPlayerOpLogEntry> ops)
            {
