...
namespace Metaplay.Server
{
    ...
    public class PlayerOptions : RuntimeOptionsBase
    {
        ...
        [MetaDescription("Configuration of client debugging facilities.")]
        public EntityDebugConfig DebugConfig { get; private set; } = new EntityDebugConfig()
        {
            ClientConsistencyChecks = IsLocalEnvironment || IsDevelopmentEnvironment,
            ServerConsistencyChecks = false,
            ChecksumGranularity     = IsLocalEnvironment || IsDevelopmentEnvironment ? ChecksumGranularity.PerOperation : ChecksumGranularity.PerActionSingleTickPerFrame,
        };
        [MetaDescription("Always enable full debugging for bot players, independent of current debug config.")]
        public bool ForceFullDebugConfigForBots { get; private set; } = IsLocalEnvironment;
    }
    ...
    public class PendingSynchronizedServerActionState
    {
        ...
        public long                         DeadlineBeforeTick; // Action must be executed before this tick starts (i.e. tick number equals this value) (this is an approximation of client clock)
        public readonly MetaTime            DeadlineBeforeTime; // or Action must be executed before this point in time. (represents server clock.)

        public PendingSynchronizedServerActionState(PlayerActionBase action, int trackingId, long deadlineBeforeTick, MetaTime deadlineBeforeTime)
        {
            Action = action;
    ...
    [Index(nameof(Priority), nameof(NamePart), nameof(EntityId))]
    ...
    public class PersistedPlayerSearch : IPersistedItem
    {
        ...
        public const int LowestNormalPriorityLevel   = 14;  // Lowest possible priority level that can be given based on name part length
        public const int ReservedLowestPriorityLevel = 64;  // Reserved lowest priority level used for deleted players and bots

        [Required]
        [Column(TypeName = "tinyint")]
        [DefaultValue(ReservedLowestPriorityLevel)]
    ...
    public sealed class ServerPlayerModelJournal : ModelJournal<IPlayerModelBase>.Follower
    {
        ...
        public ChecksumGranularity ClientChecksumGranularity   { get; private set; }

        public ServerPlayerModelJournal(LogChannel log, IPlayerModelBase model, ChecksumGranularity clientChecksumGranularity, bool enableConsistencyChecks)
            : base(log, clientChecksumGranularity != ChecksumGranularity.None)
        {
            ClientChecksumGranularity = clientChecksumGranularity;
            ...
            Setup(model, JournalPosition.AfterTick(model.CurrentTick), enableCheckpointing: clientChecksumGranularity != ChecksumGranularity.None);
        }
    }
    ...
    public abstract class PlayerConfigBase : PersistedEntityConfig
    {
        ...
        public override List<Type>          EntityComponentTypes    => new List<Type>() { typeof(EntityGuildComponent), typeof(EntityLeagueComponent) };
        public override Type                EntityShardType         => typeof(PlayerEntityShard);
    }
        ...
        where TPersisted : PersistedPlayerBase
    {
        ...
        protected static Prometheus.Counter   c_actionsExecutionTime             = Prometheus.Metrics.CreateCounter("game_player_actions_milliseconds_total", "Duration of player actions", "action");
        ...
        MetaTime?                                       _clientBecameDisconnectedOrPutOnBackgroundAt;
        ...
        public PlayerActorBase(EntityId playerId) : base(playerId)
        {
            ...
            _analyticsEventHandler = new AnalyticsEventHandler<IPlayerModelBase, PlayerEventBase>((model, payload) =>
            {
                ...
                EntityEventLogOptionsBase eventLogOptions = GetEventLogConfiguration();

                // Compute context and labels - but only if we need them:
                // - Needed in event log only if so configured in eventLogOptions.
                // - Needed in analytics always.
                bool needContext = (eventSpec.IncludeInEventLog && eventLogOptions.IncludeAnalyticsContext) || eventSpec.SendToAnalytics;
                bool needLabels = (eventSpec.IncludeInEventLog && eventLogOptions.IncludeAnalyticsLabels) || eventSpec.SendToAnalytics;
                PlayerAnalyticsContext context = needContext ? GetAnalyticsContext((TModel)model) : null;
                MetaDictionary<AnalyticsLabel, string> labels = needLabels ? GetAnalyticsLabels((TModel)model) : null;

                // Add to the model's event log (if enabled)
                if (eventSpec.IncludeInEventLog)
                {
                    // \note We make defensive copies of the event payload, the context (if any), and the labels (if any).
                    //       Caller might modify the objects after this call but those changes should not be visible here.

                    PlayerEventBase payloadCopy = MetaSerialization.CloneTagged<PlayerEventBase>(payload, MetaSerializationFlags.EntityEventLog, model.LogicVersion, resolver);

                    // \note Context is only included if so configured in eventLogOptions.
                    PlayerAnalyticsContext contextCopy =
                        eventLogOptions.IncludeAnalyticsContext && context != null
                        ? MetaSerialization.CloneTagged(context, MetaSerializationFlags.EntityEventLog, model.LogicVersion, resolver)
                        : null;

                    // \note Labels are only included if so configured in eventLogOptions.
                    // \note Labels want to be stored as ints (instead of AnalyticsLabel i.e. DynamicEnum) so we do that conversion at the same time as clone.
                    MetaDictionary<int, string> labelsCopy =
                        eventLogOptions.IncludeAnalyticsLabels && labels != null
                        ? labels.ToMetaDictionary(kv => kv.Key?.Id ?? -1, kv => kv.Value)
                        : null;

                    PlayerEventLogUtil.AddEntry(model.EventLog, eventLogOptions, collectedAt, uniqueId,
                        baseParams => new PlayerEventLogEntry(baseParams, modelTime, schemaVersion, payloadCopy, contextCopy, labelsCopy));
        ...
        protected override sealed Task PostLoad(TModel model, DateTime persistedAt, TimeSpan elapsedTime)
        {
            ...
            _activeLocalizationVersionSubscriber.Update();
        ...
        protected void SwitchToNewModelImmediately(TModel model, TimeSpan elapsedAsPersisted)
        {
            ...
            // This journal is not used for client session, so disable checksum computation & maintaining checkpoints.
            _playerJournal = CreateModelJournal(model, ChecksumGranularity.None, enableConsistencyChecks: false);
        ...
        protected override sealed async Task PersistStateImpl(bool isInitial, bool isFinal)
        {
            ...
            _log.Debug("Persisting state (isInitial={IsInitial}, isFinal={IsFinal}, schemaVersion={SchemaVersion}, persistedLogicVersion={ClientLogicVersion})", isInitial, isFinal, CurrentSchemaVersion, ClientLogicVersion);
            ...
            if (isFinal)
            {
                ...
                // Complete the pending actions. Note that actions may add further actions on the list.
                ...
                _pendingSynchronizedServerActions.Clear(); // Not strictly required as we are shutting down, do anyway for consistency.
                Model.PendingSynchronizedServerActions = null;
            }
            else
            {
            ...
            try
            {
                ...
                persistedPayload = SerializeToPersistedPayload(Model, _baselineGameConfigResolver, ClientLogicVersion);
            }
            catch (MetaTypeMissingInLogicVersionSerializationException e)
            {
                int    initialMinimumLogicVersion = ClientLogicVersion;
                byte[] _                          = MetaSerializationUtil.SerializeWithFirstCompatibleLogicVersion(Model, MetaSerializationFlags.IncludeAll, e.MinimumLogicVersion, out int newMinimumLogicVersion);

                if (newMinimumLogicVersion > initialMinimumLogicVersion)
                {
                    _log.Error("Before persisting, the player model was expected to be compatible with logic version {initialLogicVersion} but instead it was {clientLogicVersion}. A server action likely modified the player model in an incompatible way.", initialMinimumLogicVersion, newMinimumLogicVersion);

                    KickPlayerIfConnected(PlayerForceKickOwnerReason.LogicVersionUpdated);

                    UpdateLogicVersionAndSwitchToNewModelImmediately(newMinimumLogicVersion);
                }
            ...
            await MetaDatabase.Get().UpdatePlayerAsync(_entityId, persisted, searchStrings, Model.DeletionStatus);
        ...
        /// <summary>
        /// Verifies that <paramref name="action"/> is compatible with the players logic version, if not, the player is updated to the minimum required logic version.
        /// </summary>
        /// <param name="action"></param>
        /// <returns></returns>
        protected virtual bool CheckAndHandleServerActionLogicVersionConflict(PlayerActionBase action)
        {
            int actionLogicVersion = ClientLogicVersion;
            while (!MetaSerializationUtil.IsInstanceCompatibleWithLogicVersion(action, actionLogicVersion, out int minimumLogicVersion))
                actionLogicVersion = minimumLogicVersion;

            // All good since logic version hasn't changed
            if (actionLogicVersion == ClientLogicVersion)
                return false;

            _log.Warning("{actionType} is not compatible with '{entityId}'s logic version ({logicVersion}), logic version is being updated to {targetLogicVersion}", action.GetType().ToGenericTypeString(), _entityId, ClientLogicVersion, actionLogicVersion);

            KickPlayerIfConnected(PlayerForceKickOwnerReason.LogicVersionUpdated);

            UpdateLogicVersionAndSwitchToNewModelImmediately(actionLogicVersion);

            return true;
        }

        void UpdateLogicVersionAndSwitchToNewModelImmediately(int newLogicVersion)
        {
            ClientLogicVersion = newLogicVersion;
            Model.LogicVersion = newLogicVersion;
        ...
        void UpdatePlayerExperiments(ActiveGameConfig activeConfig, bool isFirstLogin, PlayerExperimentSubject experimentSubject)
        {
            ...
            MetaDictionary<PlayerExperimentId, PlayerExperimentAssignmentPolicy> visibleExperiments = activeConfig.GetVisibleExperimentsFor(experimentSubject);
        ...
        static EntityActiveExperiment[] GetSessionActiveExperiments(ActiveGameConfig activeConfig, EntityId playerId, PlayerExperimentsState playerExperimentsState, PlayerExperimentSubject experimentSubject)
        {
        ...
        static MetaDictionary<PlayerExperimentId, ExperimentMembershipStatus> GetInstanceActiveExperiment(ActiveGameConfig activeConfig, EntityId playerId, PlayerExperimentsState playerExperimentsState)
        {
            ...
            EntityActiveExperiment[] sessionExperiments = GetSessionActiveExperiments(activeConfig, playerId, playerExperimentsState, experimentSubject);
            MetaDictionary<PlayerExperimentId, ExperimentMembershipStatus> result = new MetaDictionary<PlayerExperimentId, ExperimentMembershipStatus>(sessionExperiments.Length);
        ...
        async Task<InternalPlayerSessionSubscribeResponse> OnSessionSubscriber(EntitySubscriber subscriber, InternalPlayerSessionSubscribeRequest request)
        {
            try
            {
                // Remove any stale "associations" that were added before session was formed.
                RemoveEntityAssociationsOnSessionEnd(isSessionStartRefusal: false);

                return await InnerOnSessionSubscriber(subscriber, request);
            }
            catch (EntityAskRefusal)
            {
                // If session was refused, clean entity associations that were already added but are to be removed on session end.
                // Since there is no session, they should be removed.
                RemoveEntityAssociationsOnSessionEnd(isSessionStartRefusal: true);

                throw;
            }
        }

        async Task<InternalPlayerSessionSubscribeResponse> InnerOnSessionSubscriber(EntitySubscriber subscriber, InternalPlayerSessionSubscribeRequest request)
        {
            #if !METAPLAY_DISABLE_GUILDS
            ...
            if (ClientLogicVersion != request.SessionParams.LogicVersion)
            {
                ...
                else
                {
                    ...
                    UpdateLogicVersionAndSwitchToNewModelImmediately(request.SessionParams.LogicVersion);
                }
            }
            else if (!MetaSerializationUtil.IsInstanceCompatibleWithLogicVersion(Model, request.SessionParams.LogicVersion, out _))
            {
                _log.Info("PlayerActor model's logic version {PersistedVersion} is not compatible with ClientConnection version {StartVersion}. Refusing connection.", ClientLogicVersion, request.SessionParams.LogicVersion);
                RequestShutdown();
                throw InternalPlayerSessionSubscribeRefused.CreateLogicVersionDowngradeNotAllowed();
            }
            ...
            // Reject deleted players
            if (Model.DeletionStatus.IsDeleted())
            {
                _log.Debug("Refusing deleted player login.");

                // Remove all authentication entries, including the one currently used (if it didn't exist in the model
                // before, it was just added in `OnClientSessionHandshakeCoreAsync` above.
                await RemoveAllAuthenticationMethodsAsync();

                throw InternalPlayerSessionSubscribeRefused.CreateDeleted();
            }
            ...
            EntityActiveExperiment[]                    activeExperiments;
            ...
            if (guildEntity != null)
                AddEntityAssociation(guildEntity, removeOnSessionEnd: true);
            #endif
            ...
            if (MetaplayCore.Options.FeatureFlags.EnablePlayerLeagues)
                await Leagues.HandleLeaguesOnSessionSubscriber();

            ContentHash patchesVersion = (experimentSubject == PlayerExperimentSubject.Tester)
                                            ? activeGameConfig.SharedGameConfigPatchesForTestersContentHash
                                            : activeGameConfig.SharedGameConfigPatchesForPlayersContentHash;
            ...
            if (resourceCorrection.HasAnyCorrection())
            {
                throw new InternalPlayerSessionSubscribeRefused(
                    ...
                    associatedEntities:             GetAssociatedEntityEntityRef(),
            ...
            if (new Web3EnabledCondition().IsEnabled)
            {
                ...
                List<MetaNft> ownedNfts = (await EntityAskAsync(NftManager.EntityId, new QueryNftsRequest(owner: _entityId)))
                                            .Items
                                            .Where(item => item.IsSuccess)
                                            .Select(item => item.Nft)
                                            .ToList();
                MetaSerialization.ResolveMetaRefs(ownedNfts, Model.GetDataResolver());
                Model.Nft.OwnedNfts = ownedNfts.ToMetaDictionary(NftTypeRegistry.Instance.GetNftKey);
            ...
            // Resolve current debug config, can be overridden by integration.
            EntityDebugConfig debugConfig = ResolveDebugConfigForSession();

            // Bake changes, start a new journal for this session.
            _playerJournal = CreateModelJournal(_playerJournal.StagedModel, clientChecksumGranularity: debugConfig.ChecksumGranularity, enableConsistencyChecks: debugConfig.ServerConsistencyChecks);
            ...
            initialPlayerState = new SessionProtocol.InitialPlayerState(
                playerModel:                serializedModel,
                currentOperation:           _playerJournal.StagedPosition.Operation,
                debugConfig:                debugConfig,
                sharedGameConfigVersion:    activeGameConfig.ClientSharedGameConfigContentHash,
            ...
            // \note: we do Update() to get the latest changes. We later can use Update() to detect changes.
            _activeLocalizationVersionSubscriber.Update();
            // \note: _activeLocalizationVersionSubscriber.Current is null if localizations are not enabled or available
            MetaDictionary<LanguageId, ContentHash> localizationVersions = _activeLocalizationVersionSubscriber.Current?.Versions;
        ...
        protected bool IsBotPlayer()
        {
            if (!RuntimeOptionsRegistry.Instance.GetCurrent<SystemOptions>().AllowBotAccounts)
                return false;
            return _entityId.Value < Authenticator.NumReservedBotIds;
        }

        /// <summary>
        /// Returns the <see cref="EntityDebugConfig"/> for the current session. Called on session start. This method can be used to fine-tune debugging configuration for certain players.
        /// </summary>
        protected virtual EntityDebugConfig ResolveDebugConfigForSession()
        {
            PlayerOptions playerOpts = RuntimeOptionsRegistry.Instance.GetCurrent<PlayerOptions>();

            // Turn on all debugging utilities for bot players.
            if (playerOpts.ForceFullDebugConfigForBots && IsBotPlayer())
                return EntityDebugConfig.EnableAll();

            // Use per-player override if present
            return Model.SessionDebugModeOverride?.DebugConfigForCurrentSession ?? playerOpts.DebugConfig;
        }

        protected override sealed Task OnSubscriberLostAsync(EntitySubscriber subscriber)
        {
        ...
        void HandleOwnerSessionEndedCore(EntitySubscriber subscriber, bool wasKicked)
        {
            ...
            if (Model.IsOnline)
            {
                ...
                {
                    // Assume the session ended when we last lost connection to client or app was put on background.
                    MetaTime sessionEndedAt = _clientBecameDisconnectedOrPutOnBackgroundAt ?? MetaTime.Now;
                    loginEvent.SessionLengthApprox = sessionEndedAt - Model.Stats.LastLoginAt;

                    ScheduleExecuteOnActorContext(
                        TimeSpan.Zero,
                        async () =>
                            await StatisticsEventWriter.WriteEventAsync(
                                ts => new StatisticsEventPlayerSession(
                                    ts,
                                    _entityId,
                                    loginEvent.SessionLengthApprox.Value,
                                    loginEvent.ClientPlatform,
            ...
            // Clean entity associations that are expected to be removed on session end.
            RemoveEntityAssociationsOnSessionEnd(isSessionStartRefusal: false);

            ResetIncidentUploadStateOnSessionEnd();
        }

        #region Session Resources

        SessionProtocol.SessionResourceCorrection GetNewSessionResourceCorrection(InternalPlayerSessionSubscribeRequest request, ActiveGameConfig sessionActiveGameConfig, FullGameConfig sessionSpecializedGameConfig, ContentHash patchesVersion, ref SessionStartResourceSideEffects sideEffects)
        {
            ...
            AddSpecializationPatchesResourceCorrection(ref correction, request, sessionActiveGameConfig, patchesVersion);
        ...
        void AddConfigurationResourceCorrection(ref SessionProtocol.SessionResourceCorrection correction, InternalPlayerSessionSubscribeRequest request, ActiveGameConfig sessionActiveGameConfig)
        {
            ...
            // Ensure the client has the version.
            // Note that we are using the Baseline config's version here since that is the version in the CDN.
            if (request.SessionParams.SessionResourceProposal.ConfigVersions.Contains(baselineSharedConfig))
            {
                // Client has the game config available.
                return;
            }

            // Client has wrong config version, correct it.
            correction.ConfigUpdates.Add(sessionActiveGameConfig.BaselineGameConfigSharedConfigDeliverySources.GetCorrection(request.SessionParams.SupportedArchiveCompressions));
        }

        void AddSpecializationPatchesResourceCorrection(ref SessionProtocol.SessionResourceCorrection correction, InternalPlayerSessionSubscribeRequest request, ActiveGameConfig sessionActiveGameConfig, ContentHash patchesVersion)
        {
            // Ensure the client has the version
            // \note: for patches, current version is Zero if there are no patches.
            if (patchesVersion == ContentHash.None)
            {
                // Nothing to require
            }
            else if (request.SessionParams.SessionResourceProposal.PatchVersions.Contains(patchesVersion))
            {
                // Client has the patch available.
            }
            else
            {
                // Client doesn't have the patches. Correct it
                correction.PatchUpdates.Add(new SessionProtocol.SessionResourceCorrection.ConfigPatchesUpdateInfo(patchesVersion: patchesVersion));
            }
        }
        ...
        void AddLocalizationResourceCorrection(ref SessionProtocol.SessionResourceCorrection correction, InternalPlayerSessionSubscribeRequest request, FullGameConfig sessionSpecializedGameConfig, ref SessionStartResourceSideEffects sideEffects)
        {
            ...
            MetaDictionary<LanguageId, ContentHash> localizationVersions = GlobalStateProxyActor.ActiveLocalizationVersions.Get().Versions;
        ...
        /// </summary>
        void EnsureModelTickInBounds(long tick, string source)
        {
        ...
        /// </summary>
        protected virtual PlayerAnalyticsContext CreateAnalyticsContext(TModel model, int? sessionNumber, MetaDictionary<string, string> experiments)
        {
        ...
        PlayerAnalyticsContext GetAnalyticsContext(TModel model)
        {
            ...
            MetaDictionary<string, string> experiments = new MetaDictionary<string, string>();
        ...
        /// </summary>
        protected virtual MetaDictionary<AnalyticsLabel, string> GetAnalyticsLabels(TModel model)
        {
            return null;
        }
        ...
        void HandleDoInfrequentPeriodicWork(DoInfrequentPeriodicWork _)
        {
            ...
            if (Model.IsOnline)
            {
                ...
                // Kick players if config has been updated and update is required
                CheckForGameConfigUpdate();
            }

            // Refresh LiveOps Events.
            // \todo #liveops-event Do this more smartly and efficiently than just periodic polling.
        ...
        void CheckForGameConfigUpdate()
        {
            ActiveGameConfig activeGameConfig = GlobalStateProxyActor.ActiveGameConfig.Get();
            if (activeGameConfig.ClientForceUpdateTimestamp > _activeGameConfig.ActiveSince)
            {
                KickPlayerIfConnected(PlayerForceKickOwnerReason.GameConfigUpdated);
            }
        }
        ...
        void HandleFlush(PlayerFlushActions flush)
        {
            ...
            // Early input checks.
            if (flush.Checksums == null && _playerJournal.ClientChecksumGranularity != ChecksumGranularity.None)
            {
                _log.Warning("Client did not send checksums in flush, expecting client checksum granularity {ClientChecksumGranularity}", _playerJournal.ClientChecksumGranularity);
            ...
            {
                long numTicksFlushed = operations.Count(op => op.Action == null);
            ...
            try
            {
                ...
                foreach (var op in operations)
                {
                    if (op.Action != null)
                    {
                        ...
                        if (op.Action is PlayerSynchronizedServerActionMarker synchronizedServerActionMarker)
                        {
                            ...
                            if (!synchronizedActionsQueue.TryPeekNext(out PendingSynchronizedServerActionState nextSynchronizedServerAction))
                            {
                                throw new InvalidOperationException(
                                    $"Client is trying to run to run an unknown Synchronized Server Action. Rejected. " +
                                    $"There are no pending actions. Either client is generating corrupt actions or it has submitted this action already. " +
                                    $"(client tracking id: {synchronizedServerActionMarker.TrackingId}). " +
                                    $"This is a client-side failure.");
                            }
                            else if (nextSynchronizedServerAction.TrackingId != synchronizedServerActionMarker.TrackingId)
                            {
                                throw new InvalidOperationException(
                                    $"Client is trying to run to run an unknown Synchronized Server Action. Rejected. " +
                                    $"Server expected client to submit action id {nextSynchronizedServerAction.TrackingId}, but client sent {synchronizedServerActionMarker.TrackingId}. " +
                                    $"Either client is generating corrupt actions, it has submitted this action already, or client failed to run the preceding (the expected) action. " +
                                    $"This is a client-side failure.");
                            }
                            else
                            {
                        ...
                        else if (op.Action is PlayerUnsynchronizedServerActionMarker unsynchronizedServerActionMarker)
                        {
                            ...
                            {
                                throw new InvalidOperationException(
                                    $"Client is trying to run to run an unknown Unsynchronized Server Action. Rejected. " +
                                    $"There are no pending actions. Either client is generating corrupt actions or it has submitted this action already. " +
                                    $"(client tracking id: {unsynchronizedServerActionMarker.TrackingId}). " +
                                    $"This is a client-side failure.");
                            }
                            else if (nextUnsynchronizedServerAction.TrackingId != unsynchronizedServerActionMarker.TrackingId)
                            {
                                throw new InvalidOperationException(
                                    $"Client is trying to run to run an unknown Unsynchronized Server Action. Rejected. " +
                                    $"Server expected client to submit action id {nextUnsynchronizedServerAction.TrackingId}, but client sent {unsynchronizedServerActionMarker.TrackingId}. " +
                                    $"Either client is generating corrupt actions, it has submitted this action already, or client failed to run the preceding (the expected) action. " +
                                    $"This is a client-side failure.");
                            }
                            else
                ...
                if (checksums != null && checksums.Length != totalNumSteps)
                {
            ...
            foreach (PlayerFlushActions.Operation op in operations)
            {
                if (op.Action != null)
                {
                    ...
                    _playerJournal.StageAction(action, checksums == null ? ArraySegment<uint>.Empty : new ArraySegment<uint>(checksums, checksumOffset, op.NumSteps));
                    double milliseconds = startNew.Elapsed.TotalMilliseconds;

                    string actionName = action.GetType().Name;
                    c_actionsExecutionTime.WithLabels(actionName).Inc(milliseconds);
                    c_actionsExecuted.WithLabels(actionName).Inc();
                }
                else
                {
                    _playerJournal.StageTick(checksums == null ? ArraySegment<uint>.Empty : new ArraySegment<uint>(checksums, checksumOffset, op.NumSteps));
                }
        ...
        {
            bool wasClientConnectedAndForeground = Model.IsClientConnected && Model.ClientAppPauseStatus != ClientAppPauseStatus.Paused;

            Model.IsClientConnected = notify.IsClientConnected;
            Model.ClientAppPauseStatus = notify.PauseStatus;
            OnClientConnectivityStatusChanged();

            bool isClientConnectedAndForeground = Model.IsClientConnected && Model.ClientAppPauseStatus != ClientAppPauseStatus.Paused;
            if (isClientConnectedAndForeground)
            {
                // Client is foreground.
                _clientBecameDisconnectedOrPutOnBackgroundAt = null;
            }
            else if (wasClientConnectedAndForeground)
            {
                // Client was put on background (or we lost connection)
                _clientBecameDisconnectedOrPutOnBackgroundAt = MetaTime.Now;
            }
        }
        ...
        bool TryReceiveBroadcast(BroadcastMessage broadcast)
        {
            ...
            if (Model.PassesFilter(broadcast.Params.PlayerFilter, out bool segmentEvalError))
            {
                ...
                if (!MetaSerializationUtil.IsInstanceCompatibleWithLogicVersion(action, ClientLogicVersion, out int minimumLogicVersion))
                {
                    _log.Info("Mail of {mailType} is not compatible with this player's logic version ({logicVersion}), minimum required version is {minimumLogicVersion}, player will not receive this mail.", mail.GetType().ToGenericTypeString(), ClientLogicVersion, minimumLogicVersion);
                    return false;
                }
        ...
        {
            _activeBroadcastStateSubscriber.Update();

            // Re-evaluate active broadcasts.
            // This needs to be done when _activeBroadcastStateSubscriber changes, or when segment membership changes. As the segments
            // membership can change at any moment, we periodically re-evaluate all broadcasts.
            ...
            IReadOnlyList<int> activeBroadcastIds = _activeBroadcastStateSubscriber.Current.BroadcastIds;
            Model.ReceivedBroadcastIds.RemoveWhere(
                static (broadcastId, state) =>
                {
                    IReadOnlyList<int> broadcastIds = (IReadOnlyList<int>)state;
                    return !broadcastIds.Contains(broadcastId);
                },
                state: activeBroadcastIds);
        }
                ...
                foreach ((string originalTransactionId, SubscriptionInstanceModel subscriptionInstance) in subscriptionModel.SubscriptionInstances)
                {
                     ...
                     && subscriptionInstance.StateQueriedAt < subscriptionState.ExpirationTime)
                    {
                        ...
                        else
                        {
                            ...
                            Task<InAppPurchaseSubscriptionStateResponse> queryTask = EntityAskAsync(GetAssociatedServiceEntityId(EntityKindCloudCore.InAppValidator), new InAppPurchaseSubscriptionStateRequest(subscriptionInfo));
        ...
        void InternalDoFlushActionBatch()
        {
            ...
            using (var commitResult = _playerJournal.Commit(_actionFlushBatchUpperBound))
            {
                ...
                // in not reliable, so don't even report the mismatch.
                ...
                long conflictTick = commitResult.ConflictAfterPosition.Tick;
        ...
        async Task HandleCheckpointDriftDetectedCommand(CheckpointDriftDetectedCommand _)
        {
            ...
            throw new InvalidOperationException(
                "Rerunning actions and ticks on a checkpoint did not result in the same result as with the first evaluation. " +
                "Check logs for details. Crashing actor for safety. " +
                "This can be caused by non-determinism in actions or ticks, or by modifying the model directly in a way that changes its checksum.");
        }
        ...
        void HandlePlayerChecksumMismatchDetails(PlayerChecksumMismatchDetails details)
        {
            ...
            // \note: spaces after newline to make log parses in cloud understand it's not a new log entry.
        ...
        async Task HandleValidateInAppPurchaseResponse(ValidateInAppPurchaseResponse validate)
        {
            ...
            else if (validate.Result == InAppPurchaseValidationResult.Valid)
            {
                ...
                await StatisticsEventWriter.WriteEventAsync((ts) =>
                    new StatisticsEventPlayerPurchase(ts, _entityId, txnInfo.TransactionId, pendingEv.ReferencePrice.Float, GetDaysSinceRegistered()));
        ...
        async Task HandlePlayerScheduleDeletionRequest(PlayerScheduleDeletionRequest _)
        {
            ...
            await ChangeDeletionStatusAsync(deletionTime, PlayerDeletionSource.User);
        }
        ...
        async Task HandlePlayerCancelScheduledDeletionRequest(PlayerCancelScheduledDeletionRequest _)
        {
            ...
            await ChangeDeletionStatusAsync(null, PlayerDeletionSource.User);
        }
        ...
        async Task<InternalPlayerScheduleDeletionResponse> HandleInternalPlayerScheduleDeletionRequest(InternalPlayerScheduleDeletionRequest request)
        {
            ...
            await ChangeDeletionStatusAsync(request.ScheduledAt, request.Source, request.SourceInfo);
            return InternalPlayerScheduleDeletionResponse.Instance;
        }
        ...
        /// </summary>
        async Task ChangeDeletionStatusAsync(MetaTime? deletionTime, PlayerDeletionSource source, string sourceInfo = null)
        {
            // Update state
            if (deletionTime != null)
                ExecuteServerActionImmediately(new PlayerSetIsScheduledForDeletionAt(deletionTime.Value, source));
            else
            ...
            if (deletionTime != null)
            {
                ...
                PersistedPlayerDeletionRecord record = new PersistedPlayerDeletionRecord(_entityId, deletionTime.Value, source, sourceInfo);
                if (recordExists)
        ...
        async Task<PlayerCompleteScheduledDeletionResponse> HandlePlayerCompleteScheduledDeletionRequest(PlayerCompleteScheduledDeletionRequest _)
        {
            ...
            else
            {
                ...
                newPlayerModel.DeletionStatus = Model.DeletionStatus.Source().ToDeletedStatus();
                ...
                // Clear developer player status
                if (GlobalStateProxyActor.ActiveDevelopers.Get().IsPlayerDeveloper(_entityId))
                    await EntityAskAsync(GlobalStateManager.EntityId, new SetDeveloperPlayerRequest(_entityId, false));

                _log.Info("Player deleted successfully");
        ...
        async Task CleanupInternalAndExternalStateOnAdminIssuedStateResetAndSwitchTo(TModel playerModel)
        {
            ...
            if (MetaplayCore.Options.FeatureFlags.EnablePlayerLeagues)
            {
                try
                {
                    foreach (ILeagueIntegrationHandler leagueHandler in Leagues.All)
                    {
                        // \todo [nomi] Check if is player league.
                        if (leagueHandler.DivisionClientState.CurrentDivision != EntityId.None)
                            await EntityAskAsync(leagueHandler.LeagueId, new InternalLeagueLeaveRequest(_entityId, false));
                    }
                }
            ...
            // Initialize Leagues for new model
            Leagues?.PostLoad();
        ...
        InternalEntityStateResponse HandlePlayerStateRequest(InternalEntityStateRequest _)
        {
            ...
            byte[] serialized = MetaSerializationUtil.SerializeWithFirstCompatibleLogicVersion<IModel>(Model, MetaSerializationFlags.IncludeAll, ClientLogicVersion, out int newLogicVersion);

            return new InternalEntityStateResponse(
                model:                  new MetaSerialized<IModel>(serialized, MetaSerializationFlags.IncludeAll),
                ...
                logicVersionMismatched: newLogicVersion > ClientLogicVersion);
        }
        ...
        async Task<PlayerImportModelDataResponse> HandlePlayerImportModelDataRequest(PlayerImportModelDataRequest request)
        {
            ...
            try
            {
                ...
                // Remove Leagues State from imported model
                if (MetaplayCore.Options.FeatureFlags.EnablePlayerLeagues)
                {
                    foreach (ILeagueIntegrationHandler leagueHandler in Leagues.All)
                        importModel.PlayerSubClientStates.Remove(leagueHandler.ClientSlot);

                    #pragma warning disable CS0618 // Type or member is obsolete
                    if (importModel.PlayerSubClientStates.ContainsKey(ClientSlotCore.PlayerDivisionLegacy))
                        importModel.PlayerSubClientStates.Remove(ClientSlotCore.PlayerDivisionLegacy);
        ...
        {
            MetaDictionary<PlayerExperimentId, PlayerExperimentDetails> details = new MetaDictionary<PlayerExperimentId, PlayerExperimentDetails>();
        ...
        [EntityAskHandler]
        EntityAskOk HandlePlayerToggleSessionDebugMode(InternalPlayerToggleSessionDebugMode request)
        {
            Model.SessionDebugModeOverride = request.Enabled ? new PlayerSessionDebugModeCounter(request.Parameters, request.ForNextNumSessions) : null;
            return EntityAskOk.Instance;
        }

        [EntityAskHandler]
        PlayerUpdateLogicVersionResponse HandlePlayerUpdateLogicVersion(PlayerUpdateLogicVersionRequest request)
        {
            if (request.NewLogicVersion < Model.LogicVersion)
                throw new InvalidEntityAsk($"Downgrading player's logic version to {request.NewLogicVersion} is not supported.");

            if (Model.LogicVersion == request.NewLogicVersion)
                throw new InvalidEntityAsk($"Player's logic version is already at requested value ({request.NewLogicVersion}).");

            if (request.NewLogicVersion > GlobalStateProxyActor.ActiveClientCompatibilitySettings.Get().ClientCompatibilitySettings.ActiveLogicVersionRange.MaxVersion)
                throw new InvalidEntityAsk($"Given logic version ({request.NewLogicVersion}) is out of the currently active range and therefore not supported.");

            int oldLogicVersion = Model.LogicVersion;

            KickPlayerIfConnected(PlayerForceKickOwnerReason.LogicVersionUpdated);
            UpdateLogicVersionAndSwitchToNewModelImmediately(request.NewLogicVersion);
            return new PlayerUpdateLogicVersionResponse(oldLogicVersion, request.NewLogicVersion);
        }
        ...
        protected void ExecuteServerActionAfterPendingActions(PlayerActionBase action)
        {
            ...
            {
                _log.Error(
                    "PlayerActor.ExecuteServerActionImmediately called with Action {ActionType} which does not have FollowerUnsynchronized ExecuteFlags. ExecuteFlags={Flags}. Tolerating. "
                    + "Hint: The action should inherit from PlayerUnsynchronizedServerActionCore to be issuable by server.",
        ...
        protected MetaActionResult ExecuteServerActionImmediately(PlayerActionBase action)
        {
            ...
            CheckAndHandleServerActionLogicVersionConflict(action);
            ...
            if (!actionSpec.ExecuteFlags.HasFlag(ModelActionExecuteFlags.FollowerSynchronized))
            {
                if (actionSpec.ExecuteFlags.HasFlag(ModelActionExecuteFlags.FollowerUnsynchronized))
                {
                    ...
                    _log.Warning(
                        ...
                        + "Hint: EnqueueServerAction should be used for actions of type PlayerSynchronizedServerActionCore, and ExecuteServerAction*() for PlayerUnsynchronizedServerActionCore.",
                        action.GetType().ToGenericTypeString(), actionSpec.ExecuteFlags);
                }
                else
                {
                    ...
                    _log.Debug(
                        "PlayerActor.EnqueueServerAction called with Action {ActionType} which does not have FollowerSynchronized ExecuteFlags. ExecuteFlags={Flags}. "
                        + "Hint: If you want this action to be issuable by server (and server only), the action should inherit from PlayerSynchronizedServerActionCore.",
                        action.GetType().ToGenericTypeString(), actionSpec.ExecuteFlags);
                }
            }
        ...
        {
            int trackingId = _serverActionRunningTrackingId++;
            long deadlineTick = Model.CurrentTick + (long)Model.TicksPerSecond * deadline.NumRelaxedSeconds;
        ...
        {
            // Note that actions may add further actions on the list.
            for (int ndx = 0; ndx < _pendingSynchronizedServerActions.Count; ++ndx)
            {
                PendingSynchronizedServerActionState pendingServerAction = _pendingSynchronizedServerActions[ndx];
        ...
        [EntityAskHandler]
        async Task HandleRefreshOwnedNftsRequestAsync(EntityAsk<RefreshOwnedNftsResponse> ask, RefreshOwnedNftsRequest _)
        {
            await StartRefreshOwnedNftsAsync(
        ...
        async Task StartRefreshOwnedNftsAsync(Action<RefreshOwnedNftsResponse> onCompleted = null, Action<string> onError = null)
        {
            ...
            ContinueTaskOnActorContext(
                EntityAskAsync(NftManager.EntityId, new RefreshNftsOwnedByEntityRequest(owner: _entityId, ownedAddresses: ownedAddresses)),
        ...
        void HandleNftOwnershipGained(NftOwnershipGained ownershipGained)
        {
            ...
            MetaSerialization.ResolveMetaRefs(nft, Model.GetDataResolver()); // \todo #nft Use baseline resolver instead? But be consistent with client!
        ...
        async Task HandlePlayerNftTransactionRequestAsync(PlayerNftTransactionRequest transactionRequest)
        {
            ...
            MetaSerialization.ResolveMetaRefs(transaction, Model.GetDataResolver()); // \todo #nft Use baseline resolver instead? But be consistent with client!
            ...
            MetaDictionary<NftKey, MetaNft> nfts = transaction.TargetNfts.ToMetaDictionary(key => key, key => Model.Nft.OwnedNfts[key]);
            ...
            if (Model.Nft.TransactionState.Nfts.Count > 0)
            {
                try
                {
                    _ = await EntityAskAsync(NftManager.EntityId, new OwnerUpdateNftStatesRequest(assertedOwner: _entityId, Model.Nft.TransactionState.Nfts.Values.ToList()));
        ...
        {
            // \note CanFlush was already checked in TryEnqueueTriggerEventLogFlushing, but we check it again here,
            //       because state might've changed in the meantime (in particular TryEnqueueTriggerEventLogFlushing
            //       might've been called multiple times before any of the TriggerEventLogFlushing triggers got handled).
            //       This way we avoid unnecessary database accesses done by TryRemoveOldSegmentsAsync (called via TryFlushAndCullSegmentsAsync).
            EntityEventLogOptionsBase eventLogConfig = GetEventLogConfiguration();
            if (PlayerEventLogUtil.CanFlush(Model.EventLog, eventLogConfig))
                await PlayerEventLogUtil.TryFlushAndCullSegmentsAsync(Model.EventLog, eventLogConfig, _entityId, ServerLogicVersion, _log, PersistStateIntermediate);
        }
        ...
        void IncreaseActivableStatistics(MetaActivableKey activableKey, string eventNameForLog, Func<MetaActivableState, MetaActivableStatistics> getStatisticsDelta)
        {
            ...
            RunAfterNextPersist(() => StatsCollectorProxy.IncreaseActivableStatistics(activableKey, statisticsDelta: statisticsDelta));
        }
        ...
        {
            // \note model.IsDeveloper should always reflect the ActiveDevelopers global state, so here we set it,
            //       no matter where `model` came from. In particular, when our model is overwritten with an exported model,
            //       here we set IsDeveloper based on our PlayerId, no matter what's in `model` originally.
        ...
        {
            return MetaGuid.TryParse(deviceGuid, out MetaGuid _, out string _/*error*/);
        }
        ...
        async Task OnSessionStartCoreAsync(PlayerSessionParamsBase start, bool isFirstLogin)
        {
            ...
            // Resolve if this is the first login during this UTC day (to avoid sending unnecessary statistics events for logins).
            // \note 2nd login on the registration day counts as a first login for that day for tracking D0 retention.
            int currentDate = (int)(now.MillisecondsSinceEpoch / MetaDuration.Day.Milliseconds);
            bool isFirstDailyLogin = true;
            if (Model.LoginHistory.Count > 0)
            {
                PlayerLoginEvent prevLoginEvent = Model.LoginHistory[^1];
                int prevLoginDate = (int)(prevLoginEvent.Timestamp.MillisecondsSinceEpoch / MetaDuration.Day.Milliseconds);
                isFirstDailyLogin = currentDate != prevLoginDate || Model.LoginHistory.Count == 1;
            }

            // If this is the first known login event for today, emit the login event. The events are still de-duplicated in the database
            // in case this doesn't detect the duplicate -- which can happen if the player actor crashes before persisting the login history.
            if (isFirstLogin)
                await StatisticsEventWriter.WriteEventAsync(ts => new StatisticsEventPlayerCreated(ts, _entityId));
            else if (isFirstDailyLogin)
            {
                int accountCreatedDate = (int)(Model.Stats.CreatedAt.MillisecondsSinceEpoch / MetaDuration.Day.Milliseconds);
                int daysSinceRegistered = currentDate - accountCreatedDate;
                await StatisticsEventWriter.WriteEventAsync(ts => new StatisticsEventPlayerLogin(ts, _entityId, daysSinceRegistered));
            }
            ...
            // If per-player override exists, update it for this session.
            Model.SessionDebugModeOverride = Model.SessionDebugModeOverride?.UpdateOnSessionStart();
            ...
            #pragma warning restore CS0618 // Type or member is obsolete
            await OnSessionStartAsync(start, isFirstLogin);
        }
        ...
        protected virtual void ValidateClientOriginatingAction(PlayerActionBase action)
        {
            ...
            {
                if (action is PlayerSynchronizedServerActionBase)
                {
                    throw new InvalidOperationException(
                        $"Client tried to run Action {action.GetType().ToGenericTypeString()} which is an PlayerSynchronizedServerAction " +
                        $"which means it's only usable on server via PlayerActor.EnqueueServerAction(). Client may not issue this Action.");
                }
                else if (action is PlayerUnsynchronizedServerActionBase)
                {
                    throw new InvalidOperationException(
                        $"Client tried to run Action {action.GetType().ToGenericTypeString()} which is an PlayerUnsynchronizedServerAction " +
                        $"which means it's only usable on server via PlayerActor.ExecuteServerActionImmediately() or PlayerActor.ExecuteServerActionAfterPendingActions(). " +
                        $"Client may not issue this Action.");
                }

                throw new InvalidOperationException(
                    $"Client tried to run Action {action.GetType().ToGenericTypeString()} which does not have LeaderSynchronized ExecuteFlags. ExecuteFlags=${actionSpec.ExecuteFlags}. "
                    + $"Hint: On PlayerModel, the client is the Leader. Hence an action missing LeaderSyncronized means the client may not issue the action.");
            }
        ...
        ServerPlayerModelJournal CreateModelJournal(IPlayerModelBase model, ChecksumGranularity clientChecksumGranularity, bool enableConsistencyChecks)
        {
            return new ServerPlayerModelJournal(
                _modelLogChannel,
                model,
                clientChecksumGranularity,
                enableConsistencyChecks: enableConsistencyChecks);
        }
        ...
        protected override sealed EntityComponent CreateComponent(Type componentType)
        {
            ...
            if (componentType.IsSubclassOf(typeof(LeagueComponentBase)))
            {
                Leagues = CreateLeagueComponent();
                return Leagues;
            }
        ...
        protected int GetDaysSinceRegistered()
        {

            int currentDate         = (int)(MetaTime.Now.MillisecondsSinceEpoch / MetaDuration.Day.Milliseconds);
            int accountCreatedDate  = (int)(Model.Stats.CreatedAt.MillisecondsSinceEpoch / MetaDuration.Day.Milliseconds);
            return currentDate - accountCreatedDate;
        }
