...
namespace Metaplay.Server.AdminApi
{
    public class PermissionDefinition
    {
        ...
        /// <summary>
        /// Is this permission enabled, i.e. not disabled via a feature condition?
        /// </summary>
        public readonly bool     IsEnabled;

        public PermissionDefinition(string name, string description, bool isDashboardOnly, string[] defaultRoles, bool isEnabled)
        {
            Name            = name;
    ...
    public class ResolvedPermissionConfig
    {
        ...
        public readonly MetaDictionary<string, string[]> RolesForPermission;     // List of roles for each permission (eg, 'api.general.view' => [ 'game-admin', 'game-viewer' ])
        public readonly Dictionary<string, List<string>>    RolePermissions;        // Resolved permissions for each role (eg, 'game-admin' => [ 'api.general.view', .. ])

        readonly Dictionary<string, int>                _roleStringOffset = new Dictionary<string, int>();
        readonly int                                    _roleStringLength = 0;

        public ResolvedPermissionConfig(PermissionGroupDefinition[] allPermissionGroups, string[] activeRoles, Dictionary<string, List<string>> rolesForPermission)
        {
            ...
            {
                // \note Ignore special permissions (name starts with '<')
                IEnumerable<PermissionDefinition> allPermissionDefinitions = allPermissionGroups.SelectMany(group => group.Permissions).Where(permission => !permission.Name.StartsWith("<", StringComparison.Ordinal));
                string[]                          allPermissionNames       = allPermissionDefinitions.Select(permission => permission.Name).ToArray();
                string[]                          enabledPermissionNames   = allPermissionDefinitions.Where(permission => permission.IsEnabled).Select(permission => permission.Name).ToArray();
                ...
                if (unknownPermissions.Length > 0)
                    throw new InvalidOperationException($"Unknown permissions found in Options.*.yaml, but these do not exist in code: {string.Join(", ", unknownPermissions)}");

                // Check that Permissions contains definitions for all known active permissions
                string[] missingPermissions = enabledPermissionNames.Except(definedPermissions).ToArray();
                if (missingPermissions.Length > 0)
                {
                    throw new InvalidOperationException(
                        $"Permissions are manually overridden in the Options.*.yaml however permissions are missing definitions for the following: {string.Join(", ", missingPermissions)}.\n" +
                        $"For default permissions, you can add the following to the AdminApi.Permission section: \n" +
                        string.Join(
                            "\n",
                            missingPermissions.Select(
                                x =>
                                    $"\t\t{x}: [{string.Join(", ", allPermissionDefinitions.First(y => y.Name == x).DefaultRoles.Where(x => ActiveRoles.Contains(x)))}]")));
                }
            ...
            RolesForPermission = new MetaDictionary<string, string[]>();
            foreach (PermissionGroupDefinition permGroup in allPermissionGroups.OrderBy(group => group.Title, StringComparer.Ordinal))
            {
                foreach (PermissionDefinition perm in permGroup.Permissions.Where(permission => permission.IsEnabled))
                {
            ...
            foreach ((string permission, string[] roles) in RolesForPermission)
            {
                foreach (string role in roles)
                {
                    if (RolePermissions.ContainsKey(role))
                        ...
                        RolePermissions[role] = new List<string>() { permission };
                }
            }
    ...
    public class AuthenticationTypeJwtConfiguration
    {
        ...
        public string UserInfoUri       { get; private set; }       // The URI for querying standard OAuth2 userinfo for the current user.
    ...
    public class AdminApiOptions : RuntimeOptionsBase
    {
        ...
        public int                              ListenPort                  { get; private set; } = IsLocalEnvironment ? 5550 : 8080;
        ...
        public override Task OnLoadedAsync()
        {
            ...
            ResolvedPermissions = new ResolvedPermissionConfig(GetAllPermissions(), Roles, Permissions);

            // Validate Issuer and RolePrefix for this configuration
            string issuer = GetAdminApiDomain();
            string rolePrefix = GetRolePrefix();
            if (Type == AuthenticationType.None)
            {
                MetaDebug.Assert(issuer == null, "Must have Issuer==null when AdminApi AuthenticationType==None");
                MetaDebug.Assert(rolePrefix == "", "Must have RolePrefix==\"\" when AdminApi AuthenticationType==None");
            }
            else
            {
                MetaDebug.Assert(!string.IsNullOrEmpty(issuer), "Must have a valid Issuer when AdminApi authentication is enabled");
                MetaDebug.Assert(!string.IsNullOrEmpty(rolePrefix), "Must have valid RolePrefix when AdminApi authentication is enabled");
            }
        ...
        /// Note: The special permission <c>&lt;anyone&gt;</c> is included in the return value.
        /// </summary>
        /// <returns></returns>
            ...
            foreach (Type groupType in TypeScanner.GetClassesWithAttribute<AdminApiPermissionGroupAttribute>())
            {
                ...
                foreach (FieldInfo fi in groupType.GetFields(BindingFlags.Static | BindingFlags.Public))
                {
                    if (fi.GetCustomAttribute<PermissionAttribute>() is PermissionAttribute attrib)
                    {
                        ...
                        permissions.Add(new PermissionDefinition(name, descriptionAttrib.Description, attrib.IsDashboardOnly, attrib.DefaultRoles, isEnabled: fi.IsMetaFeatureEnabled()));
                    }
                }
