...
namespace Metaplay.Server.AdminApi
{
    ...
    public static class AdminApiJsonSerialization
    {
        ...
        /// <summary>
        /// Json Serialization Settings for AdminAPI without read-only properties, includes read-only meta members.
        /// </summary>
        public static readonly JsonSerializerSettings ExcludeReadOnlyPropertiesSerializationSettings = CreateExcludeReadOnlySerializationSettings();

        /// <summary>
        /// Json Serialization Settings for AdminApi serialization of entity event log data.
        /// This is for excluding members with <see cref="ExcludeFromEventLogAttribute"/>.
        /// </summary>
        public static readonly JsonSerializerSettings EventLogSerializationSettings = CreateEventLogSerializationSettings();
        ...
        static void PopulateAdminApiJsonSettings(JsonSerializerSettings settings, JsonSerialization.Options options)
        {
            ...
            settings.ContractResolver = new MetaplayJsonContractResolver(options);
        ...
        static JsonSerializerSettings CreateExcludeReadOnlySerializationSettings()
        {
            JsonSerializerSettings settings = new JsonSerializerSettings();
            JsonSerialization.Options options = new JsonSerialization.Options(JsonSerialization.Options.DefaultOptions)
            {
                ...
                IncludeReadOnlyMetaMembers = true
            };
            PopulateAdminApiJsonSettings(settings, options);
            return settings;
        }

        static JsonSerializerSettings CreateEventLogSerializationSettings()
        {
            JsonSerializerSettings settings = new JsonSerializerSettings();
            ApplySettingsWithOverrides(settings, (ref JsonSerialization.Options options) =>
            {
                options.ExcludeEventLogExcludedMembers = true;
            });
            return settings;
        }
    }
    ...
    class AdminApiJsonBinder : ISerializationBinder
    {
        ...
        public void BindToName(Type serializedType, out string assemblyName, out string typeName)
        {
            ...
            if (serializedType.HasCustomAttribute<CompilerGeneratedAttribute>())
            {
                assemblyName = null;
