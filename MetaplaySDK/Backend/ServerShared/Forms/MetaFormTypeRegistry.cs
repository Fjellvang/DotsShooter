...
namespace Metaplay.Server.Forms
{
    ...
    public class MetaFormContentType
    {
        public class MetaFormContentMember
        {
            ...
            public bool                    IsValueCollection    => TaggedWireSerializer.GetWireType(Type, MetaSerializerTypeRegistry.Instance) == WireDataType.ValueCollection;
            public bool                    IsKeyValueCollection => TaggedWireSerializer.GetWireType(Type, MetaSerializerTypeRegistry.Instance) == WireDataType.KeyValueCollection;
            ...
            public int? AddedInVersion { get; private set; }
            public int? RemovedInVersion { get; private set; }
    ...
    public class MetaFormClassContentType : MetaFormContentType
    {
        ...
        public System.Type[]                              TypeParams   { get; private set; }
        public bool                                       UseAsContext { get; private set; }

        public MetaFormClassContentType(MetaFormContentTypeKind typeKind, MetaSerializableType serializableType, IReadOnlyCollection<MetaFormContentMember> members, bool useAsContext) : base(typeKind, serializableType)
        {
            Members      = members;
        ...
        public MetaFormClassContentType(MetaFormContentTypeKind typeKind, MetaSerializableType serializableType, IReadOnlyCollection<MetaFormContentMember> members, System.Type[] typeParams,
            bool useAsContext) : base(typeKind, serializableType)
        {
            Members      = members;
            IsLocalized  = false;
            IsGeneric    = true;
    ...
    public class MetaformNullableContentType : MetaFormContentType
    {
        public Type valueType { get; private set; }

        public MetaformNullableContentType(MetaFormContentTypeKind typeKind, MetaSerializableType serializableType, Type valueType) : base(typeKind, serializableType)
        {
    ...
    public class MetaFormTypeRegistry
    {
        ...
        public static MetaFormTypeRegistry Instance => MetaplayServices.Get<MetaFormTypeRegistry>();
        ...
        readonly Dictionary<string, Type>                     _nullablePrimitiveByName = new Dictionary<string, Type>();
        ...
        public MetaFormTypeRegistry(MetaSerializerTypeRegistry typeRegistry)
        {
            IEnumerable<MetaFormContentType> types = SerializableTypesToFormTypes(typeRegistry.AllTypes);
            ...
            foreach (Type primitiveType in _primitiveTypes)
            {
                ...
                // Add nullable types of primitive value types
                if (primitiveType.IsValueType)
                {
                    Type nullablePrimitiveType = typeof(Nullable<>).MakeGenericType(primitiveType);

                    _nullablePrimitiveByName.Add(nullablePrimitiveType.ToNamespaceQualifiedTypeString(), nullablePrimitiveType);
                }
            }
        }
        ...
        static IEnumerable<MetaFormContentType> SerializableTypesToFormTypes(IEnumerable<MetaSerializableType> types)
        {
            ...
            List<MetaformNullableContentType> nullableTypes = new List<MetaformNullableContentType>();
            ...
            foreach (MetaSerializableType serializableType in types)
            {
                ...
                if (!serializableType.IsAbstract && !serializableType.IsCollection)
                {
                    ...
                    if (kind == MetaFormContentTypeKind.Class || kind == MetaFormContentTypeKind.Localized)
                    {
                        ...
                        bool useAsContext = customAttrs.OfType<MetaFormUseAsContextAttribute>().Any();
                            ...
                            foreach (MetaSerializableMember serializableMember in serializableType.Members)
                            {
                                ...
                                if (serializableMember.Type.IsGenericType)
                                {
                                    member = new MetaFormContentType.MetaFormContentMember(
                                        ...
                                        captureDefault,
                                        serializableMember.CodeGenInfo.AddedInLogicVersion,
                                        serializableMember.CodeGenInfo.RemovedInLogicVersion);
                                }
                    ...
                    else if (kind == MetaFormContentTypeKind.ConfigLibraryItem)
                    {
                        ...
                        if (configItemType.IsGenericTypeOf(typeof(MetaRef<>)) || configItemType.IsGenericTypeOf(typeof(MetaConfigId<>)))
                    ...
                    {
                        Type valueType = null;
                        if (serializableType.Type.IsGenericTypeOf(typeof(Nullable<>)))
                            valueType = serializableType.Type.GenericTypeArguments[0];

                        MetaformNullableContentType contentType = new MetaformNullableContentType(kind, serializableType, valueType);

                        if (serializableType.IsSystemNullable && GetTypeKindFor(serializableType) == MetaFormContentTypeKind.Nullable)
                            nullableTypes.Add(contentType);
        ...
        static void BuildValidators(MetaFormContentType contentType)
        {
            ...
            if (contentType is MetaFormClassContentType classType)
            {
                ...
                {
                    MetaSerializableMember member = null;
                    foreach (MetaSerializableMember candidateMember in serializableType.Members)
                    {
                        if (candidateMember.Name == contentMember.FieldName)
                        {
                    ...
                    if (member == null)
                        throw new InvalidOperationException($"Cannot find field {contentMember.FieldName} in {serializableType.Name}");
        ...
        public static MetaFormContentTypeKind GetTypeKindFor(System.Type type)
        {
            ...
            if (type.IsGenericTypeOf(typeof(Nullable<>)))
                return MetaFormContentTypeKind.Nullable;
            if (type.IsAbstract)
        ...
        public bool TryGetNullablePrimitiveByName(string name, out System.Type type)
        {
            return _nullablePrimitiveByName.TryGetValue(name, out type);
        }
