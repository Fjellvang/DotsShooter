...
namespace Metaplay.CodeAnalyzers
{
    ...
    {
        public const string StringInterpolationDiagnosticId               = "MP_STR_00";
        public const string StringAdditionDiagnosticId                    = "MP_STR_01";
        public const string ToStringDiagnosticId                          = "MP_STR_02";
        public const string GeneralDefaultformattedInvocationDiagnosticId = "MP_STR_03";
        public const string StringOrderDiagnosticId                       = "MP_STR_04";
        public const string PreferOrdinalStringDiagnosticId               = "MP_STR_05";

        public static readonly DiagnosticDescriptor StringInterpolationRule = new DiagnosticDescriptor(
            id:             StringInterpolationDiagnosticId,
        ...
        private static readonly DiagnosticDescriptor GeneralDefaultFormattedInvocationRule = new DiagnosticDescriptor(
            id:             GeneralDefaultformattedInvocationDiagnosticId,
        ...
        private static readonly DiagnosticDescriptor PreferOrdinalStringComparison = new DiagnosticDescriptor(
            id:             PreferOrdinalStringDiagnosticId,
        ...
        static void AnalyzeInterpolatedStringExpression(SyntaxNodeAnalysisContext context)
        {
            ...
            foreach (InterpolatedStringContentSyntax content in interpolatedStringExpression.Contents)
            {
                ...
                // \note Some expressions may not have Type (e.g. "method group") but we assume those are OK.
                //       For some types we could use ConvertedType instead of Type but this is probably good enough.
        ...
        static void AnalyzeAddExpression(SyntaxNodeAnalysisContext context)
        {
            ...
            // \note #expression-without-type See comment elsewhere.
        ...
        static void AnalyzeInvocationOrObjectCreationExpression(SyntaxNodeAnalysisContext context)
        {
                     ...
                     && method.TypeArguments[1].ToString() == "string")
            {
                AnalyzeStringOrdering(invocationOrCreation, method, context);
                     ...
                     && (methodName == "Order"
                      || methodName == "OrderDescending"
                      || methodName == "Min"
                      || methodName == "Max")
                     && method.TypeArguments.Length > 0 // \note Min and Max have non-generic overloads (for numeric types) so check this for next line's check.
                     && method.TypeArguments[0].ToString() == "string")
            {
                AnalyzeStringOrdering(invocationOrCreation, method, context);
            }
    ...
    {
        private static readonly DiagnosticDescriptor DateTimeNow = new DiagnosticDescriptor(
            id:             "MP_DT_00",
            title:          "Avoid local time zone dependent DateTime.Now",
            messageFormat:  "Prefer DateTime.UtcNow instead of DateTime.Now which depends on the machine's local time zone",
            category:       "MetaplayCustom",
            DiagnosticSeverity.Warning,
            isEnabledByDefault: false);
        ...
        static void AnalyzeSimpleMemberAccessExpression(SyntaxNodeAnalysisContext context)
        {
            ...
            if (memberAccessExpression.Name.Identifier.Text == "Now" &&
                memberAccessExpression.Expression is IdentifierNameSyntax expressionIdentifierName &&
                expressionIdentifierName.Identifier.Text == "DateTime")
            {
                ITypeSymbol type = context.SemanticModel.GetTypeInfo(memberAccessExpression.Expression).Type;
                string typeString = type?.ToString();
                if (typeString == "System.DateTime")
                    context.ReportDiagnostic(Diagnostic.Create(DateTimeNow, memberAccessExpression.GetLocation(), expressionIdentifierName.Identifier.Text));
            }
        }
    }
}
