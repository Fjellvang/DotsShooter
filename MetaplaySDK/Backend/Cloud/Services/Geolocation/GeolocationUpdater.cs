...
namespace Metaplay.Cloud.Services.Geolocation
{
    ...
    /// </summary>
    internal class GeolocationUpdater : IAsyncDisposable
    {
        IMetaLogger                     _logger;
        ...
        async Task MainLoopAsync(CancellationToken ct)
        {
            while (!ct.IsCancellationRequested)
            {
                ...
                {
                    _logger.Debug($"Geolocation updater canceled during {nameof(TickAsync)}");
                }
                catch (Exception ex)
                {
        ...
        async Task TickAsync(CancellationToken ct)
        {
            ...
            GeolocationDatabaseMetadata? destinationMetadataMaybe = await _destination.TryFetchMetadataAsync(options, ct).ConfigureAwait(false);
            ...
            if (!destinationMetadataMaybe.HasValue)
            {
                ...
                GeolocationDatabase? sourceDatabaseMaybe = await _source.TryFetchDatabaseAsync(options, ct).ConfigureAwait(false);
                if (sourceDatabaseMaybe.HasValue)
                {
                    ...
                    _logger.Information("Installed initial database (id: {DatabaseId}, build date: {BuildDate}, uncompressed size: {NumPayloadBytes} bytes)", sourceDatabase.Metadata.DatabaseId, sourceDatabase.Metadata.BuildDate, sourceDatabase.Payload.Length);

                    _lastUpdateCheckAt = DateTime.UtcNow; // Just got the initial database, so don't do the update check until _updateCheckInterval from now.
                }
            }
            else
            {
                ...
                if (!_lastUpdateCheckAt.HasValue || currentTime >= _lastUpdateCheckAt.Value + _updateCheckInterval)
                {
                    ...
                    GeolocationDatabaseMetadata? sourceMetadataMaybe = await _source.TryFetchMetadataAsync(options, ct).ConfigureAwait(false);
                    ...
                    {
                        GeolocationDatabase sourceDatabase = await _source.TryFetchDatabaseAsync(options, ct).ConfigureAwait(false)
                                                             ?? throw new InvalidOperationException("Geolocation database metadata was found in source, but database itself wasn't");

                        await _destination.StoreDatabaseAsync(options, sourceDatabase).ConfigureAwait(false);
                        _logger.Information("Installed updated database (id: {DatabaseId}, old: {OldBuildDate}, new: {NewBuildDate})", sourceDatabase.Metadata.DatabaseId, destinationMetadata.BuildDate, sourceDatabase.Metadata.BuildDate);
                    }
                }
            }
        }
    ...
    internal interface IGeolocationUpdateSource : IGeolocationDatabaseMetadataProvider
    {
        ...
        public Task<GeolocationDatabase?> TryFetchDatabaseAsync(GeolocationOptions options, CancellationToken ct);
    }
