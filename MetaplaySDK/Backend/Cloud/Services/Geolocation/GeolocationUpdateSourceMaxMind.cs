...
namespace Metaplay.Cloud.Services.Geolocation
{
    ...
    internal class GeolocationUpdateSourceMaxMind : IGeolocationUpdateSource
    {
        ...
        readonly struct OriginResponse
        {
            ...
            /// <summary>
            /// The id of the MaxMind database (as in <see cref="GeolocationOptions.MaxMindDatabaseId"/>).
            /// </summary>
            public readonly GeolocationDatabaseId DatabaseId;

            public OriginResponse(DateTimeOffset? buildDate, byte[] payload, DateTime downloadedAt, GeolocationDatabaseId databaseId)
            {
                BuildDate = buildDate;
                Payload = payload;
        ...
        public async Task<GeolocationDatabaseMetadata?> TryFetchMetadataAsync(GeolocationOptions options, CancellationToken ct)
        {
            ...
            DateTimeOffset? originBuildDateDTO = await GetBuildDateFromOriginAsync(options, ct).ConfigureAwait(false);
            if (!originBuildDateDTO.HasValue)
                return null;

            return new GeolocationDatabaseMetadata(originBuildDateDTO.Value.DateTime, options.MaxMindDatabaseId);
        }
        ...
        public async Task<GeolocationDatabase?> TryFetchDatabaseAsync(GeolocationOptions options, CancellationToken ct)
        {
            ...
            OriginResponse  originResponse  = await GetDatabaseBlobFromOriginAsync(options, ct).ConfigureAwait(false);
            ...
            return new GeolocationDatabase(
                metadata:   new GeolocationDatabaseMetadata(originBuildDate.DateTime, options.MaxMindDatabaseId),
                payload:    GeolocationExtractionUtil.ExtractMaxMindDatabase(originResponse.Payload, options));
        }
        ...
        /// </summary>
        async Task<OriginResponse> GetDatabaseBlobFromOriginAsync(GeolocationOptions options, CancellationToken ct)
        {
            // We keep a cache of the latest successful download.
            // If the latest download was made less than BlobDownloadCacheLifetime ago,
            // and concerns the same database id that is now requested,
            ...
            if (_cachedBlobDownloadResponse.HasValue
                && DateTime.UtcNow < _cachedBlobDownloadResponse.Value.DownloadedAt + BlobDownloadCacheLifetime
                && _cachedBlobDownloadResponse.Value.DatabaseId == options.MaxMindDatabaseId)
            {
            ...
            OriginResponse response = await DownloadFromOriginAsync(OriginDownloadFlavor.Full, options, ct).ConfigureAwait(false);
        ...
        /// </summary>
        async Task<DateTimeOffset?> GetBuildDateFromOriginAsync(GeolocationOptions options, CancellationToken ct)
        {
            OriginResponse response = await DownloadFromOriginAsync(OriginDownloadFlavor.OnlyBuildDate, options, ct).ConfigureAwait(false);
        ...
        /// </summary>
        async Task<OriginResponse> DownloadFromOriginAsync(OriginDownloadFlavor flavor, GeolocationOptions options, CancellationToken ct)
        {
            ...
            string      databaseId      = options.MaxMindDatabaseId.ToMaxMindDatabaseIdString();
            string      uri             = $"https://download.maxmind.com/app/geoip_download?edition_id={databaseId}&license_key={options.MaxMindLicenseKey}&suffix=tar.gz";
            ...
            using (HttpResponseMessage response = await _httpClient.SendAsync(request, ct).ConfigureAwait(false))
            {
                ...
                return new OriginResponse(
                    ...
                    databaseId:     options.MaxMindDatabaseId);
            }
        }
    }
}
