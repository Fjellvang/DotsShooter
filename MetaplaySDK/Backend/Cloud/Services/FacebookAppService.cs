...
namespace Metaplay.Cloud.Services
{
    ...
    public class FacebookAppService
    {
        ...
        public class AppAccessToken
        {
            ...
            public readonly DateTime    AllowRenewAfter;

            public AppAccessToken(string accessToken, DateTime allowRenewAfter)
            {
        ...
        async Task<AppAccessFetchResult> DoGetAppAccessTokenAsync(FacebookOptions options)
        {
            ...
            try
            {
                ...
                using(HttpResponseMessage reply = await _httpClient.GetAsync(uri).ConfigureAwait(false))
                {
                    ...
                    DateTime allowRenewAfter;
                    if (response.expires_in == null || response.expires_in.Value <= 0)
                        allowRenewAfter = DateTime.UtcNow + TimeSpan.FromMinutes(5);
                    else if (response.expires_in.Value <= 60)
                        allowRenewAfter = DateTime.UtcNow + TimeSpan.FromMilliseconds(response.expires_in.Value * 1000 / 2);
                    else
                        allowRenewAfter = DateTime.UtcNow + TimeSpan.FromSeconds(response.expires_in.Value - 60);
