...
namespace Metaplay.Cloud.Services
{
    public class JWKSPublicKeyCache
    {
        ...
        DateTime                        _failureThrottleNextRequestEarliestAt;      // task worker only
        ...
        public JWKSPublicKeyCache(string jwksUrl, string productName)
        {
            ...
            _failureThrottleNextRequestEarliestAt = DateTime.UnixEpoch;
        ...
        async Task<bool> RenewTaskWorker()
        {
            try
            {
                TimeSpan failureThrottle = _failureThrottleNextRequestEarliestAt - DateTime.UtcNow;
                if (failureThrottle > TimeSpan.Zero)
                {
                    if (EnableLogging)
                    ...
                    await Task.Delay(failureThrottle);
                }
            ...
            catch(Exception ex)
            {
                ...
                TimeSpan failureThrottle = TimeSpan.FromSeconds(1 << Util.Clamp<int>(_failureThrottleNumFailedRequestsInARow, 0, 6));
                _failureThrottleNextRequestEarliestAt = DateTime.UtcNow + failureThrottle;
                return false;
            }
            finally
            {
