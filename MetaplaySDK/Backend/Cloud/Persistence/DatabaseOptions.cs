...
namespace Metaplay.Cloud.Persistence
{
    ...
    /// </summary>
    [RuntimeOptions("Database", isStatic: true, "Configuration options for the active database.")]
    public class DatabaseOptions : RuntimeOptionsBase
    {
        ...
        [MetaDescription("Use EFCore EnsureCreated instead of Migrations for initializing the database schema. Only supported for in-memory sqlite database.")]
        public bool                 InitSchemaWithoutMigrations { get; private set; } = false;
        ...
        public override Task OnLoadedAsync()
        {
            ...
            if (InitSchemaWithoutMigrations)
            {
                if (Backend != DatabaseBackendType.Sqlite)
                    throw new InvalidOperationException($"{nameof(InitSchemaWithoutMigrations)} can only be used with DB backend {nameof(DatabaseBackendType.Sqlite)}");
                if (!SqliteInMemory)
                    throw new InvalidOperationException($"{nameof(InitSchemaWithoutMigrations)} can only be used with {nameof(SqliteInMemory)} == true");
            }
