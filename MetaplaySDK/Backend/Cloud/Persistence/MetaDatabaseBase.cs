...
namespace Metaplay.Cloud.Persistence
{
    ...
    /// <summary>
    /// Provides a context for a prioritized access to the underlying sharded database.
    /// <para>
    /// To extend this class with new helpers method, consider using extension methods. Most operations can
    /// be implemented by using the <see cref="Backend"/> and <see cref="Throttle"/>.
    /// </para>
    /// </summary>
    /// <remarks>
    ...
    /// </code>
    /// </remarks>
    public class MetaDatabase : IMetaIntegrationConstructible<MetaDatabase>, IDisposable
    {
        ...
        public DatabaseBackend Backend => _backend;
        public IDatabaseThrottle Throttle => _throttle;

        static Dictionary<QueryPriority, MetaDatabase> _priorityContexts = null;
        ...
        public static void Initialize()
        {
            ...
            _priorityContexts = new Dictionary<QueryPriority, MetaDatabase>();
            foreach (QueryPriority prio in Enum.GetValues(typeof(QueryPriority)))
            {
                MetaDatabase ctx = IntegrationRegistry.Create<MetaDatabase>();
        ...
        public static void Deinitialize()
        {
            if (_priorityContexts == null)
                throw new InvalidOperationException("Cannot deinitialize database as it isn't currently initialized");
            foreach (MetaDatabase db in _priorityContexts.Values)
                db.Dispose();
            _priorityContexts = null;

            DatabaseTypeRegistry.Deinitialize();

            DatabaseBackend.Deinitialize();
        }
        ...
        public static MetaDatabase Get(QueryPriority priority = QueryPriority.Normal)
        {
            if (_priorityContexts == null)
                throw new InvalidOperationException($"{nameof(MetaDatabase)} has not been initialized!");
            return _priorityContexts[priority];
        }

        public static TDatabase Get<TDatabase>(QueryPriority priority) where TDatabase : MetaDatabase
        {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<MetaDictionary<string, int[]>> GetTableItemCountsAsync()
        {
            // Initialize itemCounts with empty
            MetaDictionary<string, int[]> itemCounts = new MetaDictionary<string, int[]>();
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<MetaDictionary<string, int[]>> EstimateTableItemCountsAsync()
        {
            // Initialize itemCounts with empty
            MetaDictionary<string, int[]> itemCounts = new MetaDictionary<string, int[]>();
            foreach (DatabaseItemSpec itemSpec in DatabaseTypeRegistry.ItemSpecs)
        ...
        /// </para>
        /// <para>
        /// WARNING. If the item already exists, it is first deleted and then re-inserted.
        /// This will cause any ON DELETE triggers to be executed.
        /// </para>
        /// <para>
        /// Use <see cref="InsertOrUpdateAsync{T}"/> if this is not desired.
        /// </para>
        /// </summary>
        public Task InsertOrReplaceAsync<T>(T item) where T : IPersistedItem
        {
            DatabaseItemSpec itemSpec = DatabaseTypeRegistry.GetItemSpec<T>();
            if (item.GetType() != itemSpec.ItemType)
                throw new InvalidOperationException($"Trying to write object of type {item.GetType().ToGenericTypeString()} into database table '{itemSpec.TableName}' which has type {itemSpec.ItemType.ToGenericTypeString()}");
            ...
            (string partitionKey, int shardNdx) = itemSpec.GetItemShardNdx(item);
            return _backend.DapperExecuteAsync(
                _throttle,
                DatabaseReplica.ReadWrite,
                itemSpec.TableName,
                shardNdx,
                "Insert",
                async conn => await conn.ExecuteAsync(itemSpec.InsertOrReplaceQuery, item).ConfigureAwait(false));
        }
        ...
        /// <summary>
        /// <para>
        /// Insert the list of items, replacing any existing items.
        /// </para>
        /// <para>
        /// WARNING. If an item already exists, it is first deleted and then re-inserted.
        /// This will cause any ON DELETE triggers to be executed.
        /// </para>
        /// <para>
        /// Use <see cref="MultiInsertOrUpdateAsync{T}"/> if this is not desired.
        /// </para>
        /// </summary>
        public Task MultiInsertOrReplaceAsync<T>(IEnumerable<T> allItems) where T : IPersistedItem
        {
            DatabaseItemSpec itemSpec = DatabaseTypeRegistry.GetItemSpec<T>();
            return MultiInsertQueryImplAsync(allItems, itemSpec.InsertOrReplaceQuery);
        }
        ...
        public async Task<List<T>> QuerySinglePageRangeAsync<T>(string opName, string startKeyExclusive, int pageSize, string rangeFirstKeyInclusive, string rangeLastKeyInclusive) where T : IPersistedItem
        {
            ...
            for (int shard = 0; shard <= NumActiveShards; shard++)
            {
                ...
                items.AddRange(await QueryPagedSingleShardAsync<T>(
                    opName:                     opName,
                    shardNdx:                   shardNdx,
        ...
        async Task<PagedQueryResult<T>> QueryPagedPossiblyRangedAsyncImpl<T>(string opName, PagedIterator iterator, int pageSize, (string firstInclusive, string lastInclusive)? keyRange) where T : IPersistedItem
        {
            ...
            if (iterator.StartKeyExclusive == null)
                throw new ArgumentException("Iterator has null start key", nameof(iterator));
        ...
        Task<IEnumerable<T>> QueryPagedSingleShardAsync<T>(string opName, int shardNdx, string iteratorStartKeyExclusive, int pageSize, (string firstInclusive, string lastInclusive)? keyRange)
        {
            if (keyRange.HasValue)
                return QueryPagedRangeSingleShardAsync<T>(opName, shardNdx, iteratorStartKeyExclusive, pageSize, keyRange.Value.firstInclusive, keyRange.Value.lastInclusive);
            else
                return QueryPagedFullSingleShardAsync<T>(opName, shardNdx, iteratorStartKeyExclusive, pageSize);
        }

        public Task<IEnumerable<T>> QueryPagedFullSingleShardAsync<T>(string opName, int shardNdx, string iteratorStartKeyExclusive, int pageSize)
        {
            if (iteratorStartKeyExclusive == null)
                throw new ArgumentNullException(nameof(iteratorStartKeyExclusive));
        ...
        public Task<IEnumerable<T>> QueryPagedRangeSingleShardAsync<T>(string opName, int shardNdx, string iteratorStartKeyExclusive, int pageSize, string rangeFirstKeyInclusive, string rangeLastKeyInclusive)
        {
            if (iteratorStartKeyExclusive == null)
                throw new ArgumentNullException(nameof(iteratorStartKeyExclusive));
