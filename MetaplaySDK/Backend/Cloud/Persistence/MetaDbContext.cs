        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            foreach (IMutableEntityType entity in modelBuilder.Model.GetEntityTypes())
            {
                foreach (IMutableProperty prop in entity.GetProperties())
                {
                    ...
                    // If there is a [DefaultValue(value)] attribute, set default value for the prop
                    DefaultValueAttribute defaultValueAttribute = prop.PropertyInfo?.GetCustomAttribute<DefaultValueAttribute>();
                    if (defaultValueAttribute != null)
                    {
                        prop.SetDefaultValue(defaultValueAttribute.Value);
                    }
                }
            }
