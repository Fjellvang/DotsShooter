...
namespace Metaplay.Cloud.Persistence
{
    ...
    public class DatabaseItemSpec
    {
        ...
        public readonly string  InsertOrReplaceQuery;           // INSERT OR REPLACE INTO <tableName> (Member1, ..) VALUES (@Member1, ..)
        ...
        {
            bool        hasPrimaryKey       = !itemType.HasCustomAttribute<NoPrimaryKeyAttribute>();
            bool        isPartitioned       = !itemType.HasCustomAttribute<NonPartitionedAttribute>();
            ...
            InsertOrReplaceQuery            = $"{db.GetInsertOrReplaceSql()} INTO {tableName} ({MemberNamesStr}) VALUES ({valuesStr})";
        ...
        {
            MemberInfo memberInfo = itemType.GetMembers().SingleOrDefault(member => member.HasCustomAttribute<KeyAttribute>());
            if (memberInfo == null)
        ...
        {
            MemberInfo memberInfo = itemType.GetMembers().SingleOrDefault(member => member.HasCustomAttribute<PartitionKeyAttribute>());
            if (memberInfo == null)
        ...
        string[] GetPersistedItemMemberNames(Type itemType)
        {
            ...
            IEnumerable<string> propNames =
                ...
                .Where(prop => !prop.HasCustomAttribute<CompilerGeneratedAttribute>())
            ...
            IEnumerable<string> fieldNames =
                ...
                .Where(field => !field.HasCustomAttribute<CompilerGeneratedAttribute>())
    ...
    {
        static MetaDictionary<Type, DatabaseItemSpec> _concreteItemSpecs;
        static MetaDictionary<Type, DatabaseItemSpec> _virtualItemSpecs;
        ...
        {
            if (_concreteItemSpecs == null)
                throw new InvalidOperationException($"Cannot deinitialize DatabaseTypeRegistry as it isn't currently initialized");
            _concreteItemSpecs = null;
            _virtualItemSpecs = null;
        }
        ...
        static (MetaDictionary<Type, DatabaseItemSpec>, MetaDictionary<Type, DatabaseItemSpec>) ResolveItemSpecs(DatabaseBackend database, Type concreteContextType)
        {
            MetaDictionary<Type, DatabaseItemSpec> concreteItemSpecs = new MetaDictionary<Type, DatabaseItemSpec>();
            MetaDictionary<Type, DatabaseItemSpec> virtualItemSpecs = new MetaDictionary<Type, DatabaseItemSpec>();
            ...
            foreach ((Type itemType, string tableName) in ResolveEntityToDatabaseTableMap(concreteContextType))
            {
                ...
                {
                    if (type.HasCustomAttribute<MetaPersistedVirtualItemAttribute>(inherit: false))
                    {
