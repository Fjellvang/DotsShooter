...
namespace Metaplay.Cloud.Persistence
{
    ...
    /// </summary>
    public abstract class DatabaseBackend : IDisposable
    {
        // Metrics
        ...
        {
            if (Instance == null)
                throw new InvalidOperationException("Cannot deinitialize database backend as it isn't currently initialized");
            Instance.Dispose();
            Instance = null;
        }
        ...
        /// <summary>Return the SQL fragment for Insert-Or-Replace operation.</summary>
        public abstract string GetInsertOrReplaceSql();
        ...
        /// <summary>
        /// Execute an operation using Dapper.
        /// </summary>
        /// <typeparam name="TResult">Type of the returned value.</typeparam>
        /// <param name="throttle">Throttle object to use.</param>
        /// <param name="replica">Database replica to use (read-write or read-only).</param>
        /// <param name="tableName">Name of the table to operate on.</param>
        /// <param name="shardNdx">Database shard index.</param>
        /// <param name="opLabel">Operation label (for metrics).</param>
        /// <param name="opFunc">Operation to perform with the connection.</param>
        /// <returns>The object return by <paramref name="opFunc"/>.</returns>
        ...
        /// <summary>
        /// Execute an unbuffered (streaming) read from the database using Dapper.
        ///
        /// Important: Keep the stream lifetimes reasonably short, say a few seconds maximum.
        /// Having long-lived operations in MySQL prevents its history buffer from being purged
        /// which can cause the buffers to start growing unboundedly due to MySQL not being able
        /// to purge the history as fast as new history is generated.
        ///
        /// Remember to dispose the returned object when done with the stream.
        /// </summary>
        /// <typeparam name="TResult">Type of the returned value.</typeparam>
        /// <param name="throttle">Throttle object to use.</param>
        /// <param name="replica">Database replica to use (read-write or read-only).</param>
        ...
        /// <param name="opLabel">Operation label (for metrics).</param>
        /// <param name="opFunc">Operation to perform with the connection.</param>
        /// <returns>The asynchronously enumerable stream of objects<paramref name="opFunc"/>.</returns>
        // \todo Mostly copy-paste of the above -- refactor?
        public async Task<ConnectionWrappingAsyncEnumerable<TResult>> DapperQueryUnbufferedAsync<TResult>(IDatabaseThrottle throttle, DatabaseReplica replica, string tableName, int shardNdx, string opLabel, Func<DbConnection, IAsyncEnumerable<TResult>> opFunc)
        {
            using (await throttle.LockAsync(replica, shardNdx).ConfigureAwait(false))
            {
                string[] labels = new string[] { replica.ToString(), shardNdx.ToString(CultureInfo.InvariantCulture), tableName, opLabel };
                ...
                try
                {
                    DbConnection conn = await CreateConnectionAsync(replica, shardNdx).ConfigureAwait(false);
                    return new ConnectionWrappingAsyncEnumerable<TResult>(opFunc(conn), conn);
                }
                catch (Exception)
                {
                    c_operationErrors.WithLabels(labels).Inc();
