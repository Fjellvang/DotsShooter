    ...
    public static class RuntimeOptionsBinder
    {
        public struct BindingResults
        {
            public struct Section
            {
                ...
                public MetaDictionary<string, RuntimeOptionsSourceSet.Source> MappingsSources; // FieldPath -> Source
            }

            public MetaDictionary<Type, Section> Sections;
            public Warning[] Warnings;
            public Exception[] Errors;

            public BindingResults(MetaDictionary<Type, Section> sections, Warning[] warnings, Exception[] errors)
            {
                Sections = sections;
                Warnings = warnings;
                Errors = errors;
        ...
        struct Context
        {
            public MetaDictionary<string, RuntimeOptionsBase> Options;
            public List<Warning> Warnings;
            public List<Exception> Errors;
            public MetaDictionary<Type, string> ContentHashes;
            public MetaDictionary<Type, MetaDictionary<string, RuntimeOptionsSourceSet.Source>> MappingsSources; // Section -> (FieldPath -> Source)

            public RuntimeOptionsBase             CurrentOptions;
            public RuntimeOptionsSourceSet.Source CurrentSource;
            public bool                           IgnoreUnmatchedSections;

            public static Context New(List<RuntimeOptionDefinition> definitions, bool ignoreUnmatchedSections)
            {
                Context ctx = new Context()
                {
                    Options = new MetaDictionary<string, RuntimeOptionsBase>(),
                    Warnings = new List<Warning>(),
                    Errors = new List<Exception>(),
                    ContentHashes = new MetaDictionary<Type, string>(),
                    MappingsSources = new MetaDictionary<Type, MetaDictionary<string, RuntimeOptionsSourceSet.Source>>(),
                };
                ...
                foreach (RuntimeOptionsBase opts in ctx.Options.Values)
                    ctx.MappingsSources[opts.GetType()] = new MetaDictionary<string, RuntimeOptionsSourceSet.Source>();

                ctx.IgnoreUnmatchedSections = ignoreUnmatchedSections;
            ...
            {
                MetaDictionary<Type, BindingResults.Section> sections = new MetaDictionary<Type, BindingResults.Section>();
        ...
        /// <param name="ignoreUnmatchedSections">When 'true', silently ignore sections in the sources that can't be
        /// matched to any definition given in 'definitions'. This is used by <see cref="RuntimeOptionsRegistry.ReadStartupOptions"/>
        /// to read the subset of options required for early configuration.
        /// </param>
        /// </summary>
        public static BindingResults BindToRuntimeOptions(List<RuntimeOptionDefinition> definitions, RuntimeOptionsSourceSet sources, bool ignoreUnmatchedSections = false)
        {
            Context ctx = Context.New(definitions, ignoreUnmatchedSections);
        ...
        {
            Context ctx = Context.New(definitions, false);
        ...
        static RuntimeOptionsBase TryGetOptionsForSection(Context ctx, RuntimeOptionsSourceSet.Source source, IConfigurationSection section)
        {
            ...
            if (ctx.IgnoreUnmatchedSections)
                return null;
