...
namespace Metaplay.Cloud.Utility
{
        ...
        where TValue : class
    {
        ...
        /// <inheritdoc cref="WeakReferencingCache{TKey, TValue}.Count" />
        public int Count => _weakCache.Count;

        /// <inheritdoc cref="WeakReferencingCache{TKey, TValue}.Prune" />
        public WeakReferencingCachePruneResult Prune()
        {
            return _weakCache.Prune();
        }

        /// <inheritdoc cref="WeakReferencingCache{TKey, TValue}.LockedForEach" />
        public void LockedForEach(Action<TKey, TValue> action)
        {
            _weakCache.LockedForEach(action);
        }
    ...
    public class WeakReferencingCache<TKey, TValue>
        ...
        where TValue : class
    {
        MetaDictionary<TKey, WeakReference<TValue>>  _cache  = new MetaDictionary<TKey, WeakReference<TValue>>();
        object                                          _lock   = new object();
        ...
        /// <summary>
        /// Get the number of entries currently in the underlying storage.
        /// Note that this includes also any possible expired weak references
        /// that have not yet been pruned with <see cref="Prune"/>.
        /// </summary>
        public int Count
        {
            get
        ...
        /// <summary>
        /// Prune the underlying storage to remove weak references that
        /// are no longer live.
        /// Returns info about how many were removed and how many remain.
        /// <para>
        /// The cache is locked for the duration of this whole method,
        /// so you should only call on small-enough caches!
        /// </para>
        /// </summary>
        public WeakReferencingCachePruneResult Prune()
        {
            ...
            lock (_lock)
            {
                _cache.RemoveWhere(kv =>
                {
                    bool remove = !kv.Value.TryGetTarget(out _);
            ...
            return new WeakReferencingCachePruneResult
            {
                NumPruned = numPruned,
                NewCount = newCount,
            };
        ...
        /// <summary>
        /// Invoke the given action for each live entry in this cache.
        /// <para>
        /// The cache is locked for the duration of this whole method,
        /// so you should only call on small-enough caches
        /// and with a cheap-enough action!
        /// </para>
        /// </summary>
        public void LockedForEach(Action<TKey, TValue> action)
        {
            lock (_lock)
            {
                foreach ((TKey key, WeakReference<TValue> weakRef) in _cache)
                {
    ...
    public readonly struct WeakReferencingCachePruneResult
    {
        /// <summary>
        /// Number of (non-live) entries pruned from the underlying storage.
        /// </summary>
        public int NumPruned { get; init; }
        /// <summary>
        /// Number of (live) entries remaining in the storage after the pruning.
        /// </summary>
