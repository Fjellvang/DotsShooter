...
namespace Metaplay.Cloud.Entity
{
    ...
    /// In implicit mode, the Handler takes the EntityId of the sender entity and the message (derived from <see cref="MetaMessage"/>, possibly <see cref="EntityAskRequest{TResponse}"/>) and return the result for the
    /// EntityAsk. The sender Entity Id is optional. Return type must be <see cref="MetaMessage"/> (or a type derived from it) or a <see cref="Task{TResult}"/> for
    /// <c>TResult : MetaMessage</c> (or a type derived from it). If the message parameter type is derived from <see cref="EntityAskRequest{TResponse}"/>,
    /// the return type must be the corresponding <c>TResponse</c> type.
    /// <br/>
    /// In explicit mode, the Handler must take exactly two parameters: a <c>EntityAsk</c> object and the message (derived from <see cref="MetaMessage"/>).
    /// The ask parameter type must be according to the request type: if the request is derived from <see cref="EntityAskRequest{TResponse}"/>, use <see cref="EntityAsk{TReply}"/> with the corresponding <c>TResponse</c> type.
    /// If the request is a plain <see cref="MetaMessage"/>, you can use plain <see cref="EntityAsk"/>.
    /// It must return void or a <see cref="Task"/> (ie, be an async method). To return an value, Handler must call <see cref="EntityActor.ReplyToAsk(EntityAsk, MetaMessage)"/> or <see cref="EntityActor.ReplyToAsk{TResult}(EntityAsk{TResult}, TResult)"/>
    ...
    /// <item><description><c>void HandleMessage(EntityAsk&lt;ResponseMessageType&gt; ask, RequestMessageType request) { ReplyToAsk(ask, message); }</c> // Explicit, Synchronous, response-typed EntityAsk&lt;&gt;</description></item>
    /// <item><description><c>Task HandleMessage(EntityAsk&lt;ResponseMessageType&gt; ask, RequestMessageType request) { ReplyToAsk(ask, message); }</c> // Explicit, Asynchronous, response-typed EntityAsk&lt;&gt;</description></item>
    /// <item><description><c>void HandleMessage(EntityAsk ask, RequestMessageType request) { ReplyToAsk(ask, message); }</c> // Explicit, Synchronous, untyped EntityAsk</description></item>
    /// <item><description><c>Task HandleMessage(EntityAsk ask, RequestMessageType request) { ReplyToAsk(ask, message); }</c> // Explicit, Asynchronous, untyped EntityAsk</description></item>
    /// </list>
    /// </remarks>
    [AttributeUsage(AttributeTargets.Method, AllowMultiple = false, Inherited = false)]
    public sealed class EntityAskHandlerAttribute : Attribute
    {
    }
    ...
    /// </remarks>
    [AttributeUsage(AttributeTargets.Method, AllowMultiple = false, Inherited = false)]
    public sealed class EntitySynchronizeHandlerAttribute : Attribute
    {
    }
    ...
    /// </remarks>
    [AttributeUsage(AttributeTargets.Method, AllowMultiple = false, Inherited = false)]
    public sealed class PubSubMessageHandlerAttribute : Attribute
    {
    }
    ...
    public static class EntityDispatcherBuilder<TReceiverType> where TReceiverType : IEntityReceiver
    {
        ...
        {
            if (!receiverType.IsSubclassOf(typeof(TReceiverType)))
                throw new InvalidOperationException($"Type {receiverType} must be subclass of {typeof(TReceiverType)} to be used with EntityDispatcher");
        ...
        static void RegisterDispatchMessageFunc(EntityDispatcherBase<TReceiverType> dispatcher, Type receiverType, MethodInfo method)
        {
            ...
            if (args.Length == 2)
            {
                if (args[0].ParameterType != typeof(EntityId))
                    throw new InvalidOperationException($"[MessageHandler] method {receiverType.Name}.{method.Name}() first parameter must be EntityId, got {args[0].ParameterType.Name}");
            }
            else if (args.Length != 1)
                throw new InvalidOperationException($"[MessageHandler] method {receiverType.Name}.{method.Name}({string.Join(',', args.Select(paramInfo => paramInfo.ParameterType.Name))}) must take exactly one or two parameters, got {args.Length}");
            if (!args[msgArgNdx].ParameterType.IsSubclassOf(typeof(MetaMessage)))
                throw new InvalidOperationException($"[MessageHandler] method {receiverType.Name}.{method.Name}() last parameter must be derived from MetaMessage, got {args[msgArgNdx].ParameterType.Name}");
        ...
        static void RegisterDispatchCommandFunc(EntityDispatcherBase<TReceiverType> dispatcher, Type actorType, MethodInfo method)
        {
            ParameterInfo[] args = method.GetParameters();
            if (args.Length != 1)
                throw new InvalidOperationException($"[CommandHandler] method {actorType.Name}.{method.Name}({string.Join(',', args.Select(paramInfo => paramInfo.ParameterType.Name))}) must take exactly one parameter");
        ...
        static void RegisterDispatchAskFunc(EntityDispatcherBase<TReceiverType> dispatcher, Type receiverType, MethodInfo method)
        {
            ...
            if (hasReturnValue)
            {
                if (!returnValueType.IsDerivedFrom<MetaMessage>())
                    throw new InvalidOperationException($"[EntityAskHandler] method {receiverType.Name}.{method.Name}({string.Join(',', args.Select(paramInfo => paramInfo.ParameterType.Name))}) must return a MetaMessage or a descendant.");
            }
            ...
            else if (hasReturnValue && args.Length == 2)
            {
                ...
                if (args[0].ParameterType != typeof(EntityId))
                    throw new InvalidOperationException($"[EntityAskHandler] method {receiverType.Name}.{method.Name}(): first parameter must be EntityId. See EntityAskHandler doc-comment for legal signatures.");
            ...
            else if (!hasReturnValue && args.Length == 2)
            {
                ...
                askArgType = args[0].ParameterType;
                if (askArgType != typeof(EntityAsk) && !askArgType.IsGenericTypeOf(typeof(EntityAsk<>)))
                    throw new InvalidOperationException($"[EntityAskHandler] method {receiverType.Name}.{method.Name}(): first parameter must be EntityAsk or EntityAsk<TResult>. See EntityAskHandler doc-comment for legal signatures.");
                msgType = args[1].ParameterType;
                hasAskArg = true;
                hasSourceEntityIdArg = false;
            }
            ...
            {
                throw new InvalidOperationException($"[EntityAskHandler] method {receiverType.Name}.{method.Name}({string.Join(',', args.Select(paramInfo => paramInfo.ParameterType.Name))}) has unrecognized signature. See EntityAskHandler doc-comment for legal signatures.");
            }

            if (!msgType.IsSubclassOf(typeof(MetaMessage)))
                throw new InvalidOperationException($"[EntityAskHandler] method {receiverType.Name}.{method.Name}(): last parameter must be derived from MetaMessage. See EntityAskHandler doc-comment for legal signatures.");

            Type entityAskRequestTypeMaybe = msgType.TryGetGenericAncestor(typeof(EntityAskRequest<>));
            if (entityAskRequestTypeMaybe != null)
            {
                Type expectedResponseType = entityAskRequestTypeMaybe.GetGenericArguments().Single();

                if (hasReturnValue)
                {
                    if (returnValueType != expectedResponseType)
                    {
                        throw new InvalidOperationException(
                            $"[EntityAskHandler] method {receiverType.Name}.{method.Name}(): method returns type {returnValueType.ToGenericTypeString()}, " +
                            $"but the declared response type for {msgType.ToGenericTypeString()} is {expectedResponseType.ToGenericTypeString()}.");
                    }
                }
                else if (askArgType.IsGenericTypeOf(typeof(EntityAsk<>)))
                {
                    Type responseType = askArgType.GetGenericArguments().Single();

                    if (responseType != expectedResponseType)
                    {
                        throw new InvalidOperationException(
                            $"[EntityAskHandler] method {receiverType.Name}.{method.Name}(): method's reply type is {responseType.ToGenericTypeString()} " +
                            $"according to the EntityAsk<> parameter, " +
                            $"but the declared response type for {msgType.ToGenericTypeString()} is {expectedResponseType.ToGenericTypeString()}.");
                    }
                }
                else if (askArgType == typeof(EntityAsk))
                {
                    throw new InvalidOperationException(
                        $"[EntityAskHandler] method {receiverType.Name}.{method.Name}(): must use the typed EntityAsk<{expectedResponseType.ToGenericTypeString()}> parameter " +
                        $"for type safety, instead of plain EntityAsk, in order to match the declared response type of {msgType.ToGenericTypeString()}.");
                }
                else
                    throw new InvalidOperationException($"[EntityAskHandler] method {receiverType.Name}.{method.Name}(): internal error, unhandled ask argument type {askArgType}");
            }
            ...
            if (hasAskArg)
            {
                ...
                // If the handler method has parameter type EntityAsk<TReply> instead of plain EntityAsk,
                // wrap the plain EntityAsk argument into a EntityAsk<TReply>.
                if (askArgType.IsGenericTypeOf(typeof(EntityAsk<>)))
                {
                    ConstructorInfo constructor = askArgType.GetConstructor(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance, new Type[] { typeof(EntityAsk) });
            ...
            if (hasSourceEntityIdArg)
            {
                ...
                if (sourceIdGetter == null)
                    throw new InvalidOperationException($"EntityAsk must have {nameof(EntityAsk.FromEntityId)} getter");
                il.Emit(OpCodes.Ldarg_1);
            ...
            if (hasReturnValue)
            {
                if (isAsync)
                {
                    ...
                    if (taskContinueWith == null)
                        throw new InvalidOperationException($"Unable to find {retType.ToGenericTypeString()}.ContinueWith(Action<{retType.ToGenericTypeString()}>)"); // Task<TReply>.ContinueWith(Action<Task<TReply>>)
                ...
                else // sync: just call ReplyToAsk() with return value
                {
                    ...
                    MethodInfo replyToAsk = receiverType.GetMethod("ReplyToAsk", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance, new Type[] { typeof(EntityShard.EntityAsk), typeof(MetaMessage) });
                    if (replyToAsk == null)
                        throw new InvalidOperationException($"Unable to find {receiverType.ToGenericTypeString()}.ReplyToAsk()");
        ...
        static void RegisterDispatchSynchronizeFunc(EntityDispatcherBase<TReceiverType> dispatcher, Type receiverType, MethodInfo method)
        {
            ParameterInfo[] args = method.GetParameters();
            if (args.Length != 2)
                throw new InvalidOperationException($"[EntitySynchronizeHandler] method {receiverType.Name}.{method.Name}() must take exactly two parameters");
            if (args[0].ParameterType != typeof(EntitySynchronize))
                throw new InvalidOperationException($"[EntitySynchronizeHandler] method {receiverType.Name}.{method.Name}(): first parameter must be EntitySynchronize");
            if (!args[1].ParameterType.IsSubclassOf(typeof(MetaMessage)))
                throw new InvalidOperationException($"[EntitySynchronizeHandler] method {receiverType.Name}.{method.Name}(): second parameter must be derived from MetaMessage");
    ...
    public class EntityDispatcherRegistry
    {
        Dictionary<Type, EntityDispatcher> _dispatchers = new Dictionary<Type, EntityDispatcher>();

        public EntityDispatcherRegistry(EntityConfigRegistry entities)
        {
            IEnumerable<(EntityConfigBase, Type)> EntityConfigs()
            {
                foreach ((_, EntityConfigBase entityConfig) in entities.TypeToEntityConfig)
                {
                    if (entityConfig.IsPolymorphic)
                    {
                        foreach (Type derived in entityConfig.GetAllConcreteActorTypes())
                            yield return (entityConfig, derived);
                    }
                    else
                        yield return (entityConfig, entityConfig.EntityActorType);
                }
            }

            foreach ((EntityConfigBase entityConfig, Type concreteType) in EntityConfigs())
            {
                EntityDispatcher dispatcher = new EntityDispatcher();

                EntityDispatcherBuilder<EntityActor>.Build(dispatcher, concreteType);

                if (EntityActor.TryGetComponentTypes(entityConfig.EntityKind, out List<Type> components))
                {
                    EntityDispatcherCombiner combiner = new EntityDispatcherCombiner(dispatcher, concreteType);
                ...
                _dispatchers.Add(concreteType, dispatcher);
            }
        }

        public EntityDispatcher Get(Type actorType) => _dispatchers[actorType];
    }
}
