...
namespace Metaplay.Cloud.Analytics
{
    ...
    public class AnalyticsSinkBigQueryOptions : RuntimeOptionsBase
    {
        ...
        [MetaDescription("If set, uses the V2 (Write API) version of the BigQuery sink.")]
        public bool                     UseV2                           { get; private set; }
        [MetaDescription("If set, prints the rows to stdout instead of writing them to the BigQueryTable.")]
        public bool                     DebugPrintToStdoutOnly          { get; private set; }
        [MetaDescription("If set, fields in the event's Context are added into event_params column. Context fields have a \"$c:\" prefix.")]
        public bool                     AddContextFieldsAsExtraEventParams { get; private set; } = true;

        [MetaDescription("The maximum duration that an event is held before flushing it out.")]
        public TimeSpan                 MaxPendingDuration              { get; private set; } = TimeSpan.FromMinutes(1);
        ...
        [MetaDescription("Enables the native BigQuery row de-duplication based on `insertId`. When disabled, the data processor should de-duplicate events based on the `event_id` column. (Deprecated in V2)")]
        ...
        {
            // Check BigQuery settings but only when we are using it.
            if (Enabled && !DebugPrintToStdoutOnly)
            {
