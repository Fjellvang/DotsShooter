...
{
    public class BigQueryFormatterV2
    {
        internal BigQueryFormatterV2(IEnumerable<AnalyticsEventSpec> events, bool addContextFieldsAsExtraEventParams)
        {
        ...
        public BigQueryFormatterV2(AnalyticsEventRegistry events, RuntimeOptionsRegistry options) : this(events.EventSpecs.Values, options.GetCurrent<AnalyticsSinkBigQueryOptions>().AddContextFieldsAsExtraEventParams) { }
        ...
        class EventFormatter
        {
            ...
            public List<Dictionary<string, object>> Format(object eventBase)
            {
        ...
        /// This should be inserted into ProtoMessage's EventParams.
        /// </summary>
        List<Event.Types.EventParam> FormatEventArgs(EventFormatter formatter, object eventBase)
        {
            List<Event.Types.EventParam> eventParams = new List<Event.Types.EventParam>();
            foreach (Dictionary<string, object> rows in formatter.Format(eventBase))
            {
                Event.Types.EventParam param = new Event.Types.EventParam();
                param.Key = (string)rows["key"];
        ...
        List<Event.Types.Label> FormatEventLabels(MetaDictionary<AnalyticsLabel, string> labels)
        {
            if (labels == null || labels.Count == 0)
                return new List<Event.Types.Label>();

            List<Event.Types.Label> rows = new List<Event.Types.Label>(capacity: labels.Count);
            foreach ((AnalyticsLabel label, string value) in labels)
            {
                Event.Types.Label protoLabel = new Event.Types.Label();
        ...
        /// <summary>
        /// Formats and writer the analytics event into the write buffer.
        /// </summary>
        public void WriteEvent(IByteStringBatchWriter.BatchBuffer buffer, AnalyticsEventEnvelope eventEnvelope, bool enableBigQueryEventDeduplication)
        {
            ...
            // Instantiate your ProtoBuf message class
            Event protoMessage = new Event();

            // Populating ProtoBuf message from fields obtained above
            protoMessage.SourceId           = eventEnvelope.Source.ToString();
            protoMessage.EventId            = id;
            protoMessage.EventTimestamp     = FormatTimestampString(eventEnvelope.ModelTime);
            protoMessage.EventName          = eventFormatter.EventName;
            protoMessage.EventSchemaVersion = eventEnvelope.SchemaVersion;

            // Format event_params rows and map to ProtoBuf message structure
            List<Event.Types.EventParam> eventParams = FormatEventArgs(eventFormatter, eventBase);

            // Context Rows
            switch (eventEnvelope.Context)
            {
                case PlayerAnalyticsContext playerContext:
                    Event.Types.Player player = new Event.Types.Player();
                    foreach ((string experimentId, string variantId) in playerContext.Experiments)
                    {
                        var experiment = new Event.Types.Experiment();
                    ...
                    break;
                case GuildAnalyticsContext guildContext:
                    // Guild context has not yet any fields and empty records are not supported.
                    break;
                case ServerAnalyticsContext serverContext:
                    // Server context has not yet any fields and empty records are not supported.
                    break;
            }

            // Extra field context
            EventFormatter extraFieldContextFormatter = eventEnvelope.Context != null ? _extraFieldContextFormatters.GetValueOrDefault(eventEnvelope.Context.GetType()) : null;
            if (extraFieldContextFormatter != null)
            {
                List<Event.Types.EventParam> extraEventParams = FormatEventArgs(extraFieldContextFormatter, eventEnvelope.Context);
                if (extraEventParams.Count > 0)
                    eventParams.AddRange(extraEventParams);
            }

            if (eventParams.Count > 0)
                protoMessage.EventParams.AddRange(eventParams);

            // Label rows
            List<Event.Types.Label> labels = FormatEventLabels(eventEnvelope.Labels);
            if (labels.Count > 0)
                protoMessage.Labels.Add(labels);
