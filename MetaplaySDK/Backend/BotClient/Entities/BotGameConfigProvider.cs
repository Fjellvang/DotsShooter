...
namespace Metaplay.BotClient
{
    public class BotGameConfigProvider
    {
        ...
        public async ValueTask<ISharedGameConfig> GetSpecializedGameConfigAsync(ContentHash baselineConfigVersion, ContentHash patchsetVersion, MetaDictionary<PlayerExperimentId, ExperimentVariantId> experimentAssignment)
        {
            if (patchsetVersion == ContentHash.None)
            {
        ...
        {
            MetaDictionary<ExperimentVariantPair, GameConfigPatchEnvelope> patches = new MetaDictionary<ExperimentVariantPair, GameConfigPatchEnvelope>();
            if (specializationPatchesMaybe != null)
            {
                foreach ((PlayerExperimentId experimentId, MetaDictionary<ExperimentVariantId, byte[]> variants) in specializationPatchesMaybe.Patches)
                {
