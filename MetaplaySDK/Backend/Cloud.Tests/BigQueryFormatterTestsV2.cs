...
using Metaplay.Cloud.Analytics.BigQuery;
using Metaplay.Core;
...
{
    public class BigQueryFormatterTestsV2
    {
        ...
        [AnalyticsEvent(10_101, "TestEvent")]
        ...
        [AnalyticsEvent(10_102, "TestNamedEvent")]
        [AnalyticsEventCategory("Test")]
        ...
        [AnalyticsEvent(10_103, "TestIllegalRecursiveEvent")]
        [AnalyticsEventCategory("Test")]
        ...
        [AnalyticsEvent(10_104, "TestIllegalCoRecursiveEvent")]
        [AnalyticsEventCategory("Test")]
        ...
        [AnalyticsEvent(10_105, "TestIllegalTypeRecursiveEvent")]
        [AnalyticsEventCategory("Test")]
        ...
        [AnalyticsEvent(10_106, "TestSingleDirectRecursiveEvent")]
        [AnalyticsEventCategory("Test")]
        ...
        [AnalyticsEvent(10_107, "TestDoubleDirectRecursiveEvent")]
        [AnalyticsEventCategory("Test")]
        ...
        [AnalyticsEvent(10_108, "RecursiveEvent1")]
        [AnalyticsEventCategory("Test")]
        ...
        // Regeression test for a known bug
        [AnalyticsEvent(10_109, "RecursiveEvent2")]
        [AnalyticsEventCategory("Test")]
        ...
        [AnalyticsEvent(10_110, "RecursiveRewardEvent")]
        [AnalyticsEventCategory("Test")]
        ...
        [AnalyticsEvent(10_111, "TestTooRecursiveEvent")]
        [AnalyticsEventCategory("Test")]
        ...
        [AnalyticsEvent(10_112, "TestExtractedFieldsAnalyticsEvent")]
        [AnalyticsEventCategory("Test")]
        ...
        class TestBufferSink : IByteStringBatchWriter.BatchBuffer
        {
        }

        [MetaSerializableDerived(12100)]
        public class TestPlayerAnalyticsContext : PlayerAnalyticsContext
        {
            ...
            public TestPlayerAnalyticsContext(int sessionNumber, MetaDictionary<string, string> experiments) : base(sessionNumber, experiments)
            {
            }
        }

        [MetaSerializable]
        public class TestAnalyticsLabel : AnalyticsLabel
        {
            protected TestAnalyticsLabel(int value, string name) : base(value, name) { }

            public static readonly TestAnalyticsLabel Language = new TestAnalyticsLabel(1, nameof(Language));
            public static readonly TestAnalyticsLabel Country  = new TestAnalyticsLabel(2, nameof(Country));
        }

        [Test]
        public void TestBasicTypes()
        {
            var    testEvent = new TestBasicAnalyticsEvent();
            string result    = FormatEvent(testEvent);
            string expected = """
                source_id: None
                ...
                key: TypedSpecific:$t
                string_value: BigQueryFormatterTestsV2.TestBasicAnalyticsEvent.ConcreteType
        ...
        {
            {
                ...
                experiments.Add("exp1", "variant1");
                experiments.Add("exp2", "variant2");
                TestPlayerAnalyticsContext context = new TestPlayerAnalyticsContext(12, experiments);
                string                     result  = FormatEvent(new TestNamedAnalyticsEvent(), context: context, addContextFieldsAsExtraEventParams: false);
                string expected = """
                      source_id: None
                      event_id: 00000000000000000000000000000001
        ...
        {
            MetaDictionary<AnalyticsLabel, string> labels = new MetaDictionary<AnalyticsLabel, string>();
            labels.Add(TestAnalyticsLabel.Country, "test");
            labels.Add(TestAnalyticsLabel.Language, "te");
            string result = FormatEventWithLabels(new TestNamedAnalyticsEvent(), labels);
            string expected = """
                      source_id: None
                      event_id: 00000000000000000000000000000001
                      event_timestamp: 1970-01-01 00:00:00.000000
        ...
        public void TestRecursiveRewardEvent()
        {
            ...
            string expected = """
                ...
                event_params: [
                key: Reward:$t
                string_value: BigQueryFormatterTestsV2.RecursiveRewardEvent.CompoundReward

                key: Reward:Rewards:0:$t
                string_value: BigQueryFormatterTestsV2.RecursiveRewardEvent.GoldReward
        ...
        string FormatEvent<T>(T eventObject, AnalyticsContextBase context = null, bool addContextFieldsAsExtraEventParams = false) where T : AnalyticsEventBase
        {
            ...
            BigQueryFormatterV2 formatterV2 = new BigQueryFormatterV2(new []{ spec }, addContextFieldsAsExtraEventParams);

            TestBufferSink sink = new TestBufferSink();
            formatterV2.WriteEvent(sink, new AnalyticsEventEnvelope(EntityId.None, MetaTime.Epoch, MetaTime.Epoch, MetaUInt128.One, "TestEvent", 1, eventObject, context, new MetaDictionary<AnalyticsLabel, string>()), enableBigQueryEventDeduplication: false);

            Assert.AreEqual(1, sink.Rows.Count);
            return RowToString(sink.Rows[0]);
        }

        string FormatEventWithLabels<T>(T eventObject, MetaDictionary<AnalyticsLabel, string> labels, AnalyticsContextBase context = null, bool addContextFieldsAsExtraEventParams = false) where T : AnalyticsEventBase
        {
            AnalyticsEventSpec  spec        = new AnalyticsEventSpec(typeof(T), typeof(T).GetCustomAttribute<AnalyticsEventAttribute>(), typeof(T).GetCustomAttribute<AnalyticsEventCategoryAttribute>());
            BigQueryFormatterV2 formatterV2 = new BigQueryFormatterV2(new []{ spec }, addContextFieldsAsExtraEventParams);

            TestBufferSink sink = new TestBufferSink();
            formatterV2.WriteEvent(sink, new AnalyticsEventEnvelope(EntityId.None, MetaTime.Epoch, MetaTime.Epoch, MetaUInt128.One, "TestEvent", 1, eventObject, context, labels), enableBigQueryEventDeduplication: false);

            Assert.AreEqual(1, sink.Rows.Count);
            return RowToString(sink.Rows[0]);
        }

        static string RowToString(ByteString byteString)
        {
            // Deserialize the ByteString into the Protobuf Event message
            Event protoEvent = Event.Parser.ParseFrom(byteString);

            // Convert the Protobuf Event message to a string for comparison
            string result = "";
            result += $"source_id: {protoEvent.SourceId}\n";
            result += $"event_id: {protoEvent.EventId}\n";
            result += $"event_timestamp: {protoEvent.EventTimestamp}\n";
            result += $"event_name: {protoEvent.EventName}\n";
            result += Invariant($"event_schema_version: {protoEvent.EventSchemaVersion}\n");

            if (protoEvent.Player != null)
            {
                result += "player:\n";
                result += Invariant($"session_number: {protoEvent.Player.SessionNumber}\n");
                result += "experiments: [";
                foreach (var experiment in protoEvent.Player.Experiments)
                {
                    result += Invariant($"\nexperiment: {experiment.Experiment_}\n");
                    result += Invariant($"variant: {experiment.Variant}\n");
                }
                result += "]\n";
            }

            result += "event_params: [\n";
            for (int index = 0; index < protoEvent.EventParams.Count; index++)
            {
                Event.Types.EventParam param = protoEvent.EventParams[index];
                result += $"key: {param.Key}\n";
                switch (param.ValueCase)
                {
                    case Event.Types.EventParam.ValueOneofCase.StringValue:
                        result += $"string_value: {param.StringValue}\n";
                        break;
                    case Event.Types.EventParam.ValueOneofCase.IntValue:
                        result += Invariant($"int_value: {param.IntValue}\n");
                        break;
                    case Event.Types.EventParam.ValueOneofCase.DoubleValue:
                        result += Invariant($"double_value: {param.DoubleValue}\n");
                        break;
                }

                if (index < protoEvent.EventParams.Count - 1)
                    result += "\n";
            }

            result += "]\n";
            result += "labels: [";
            foreach (var label in protoEvent.Labels)
            {
                result += $"\nname: {label.Name}\n";
                result += $"string_value: {label.StringValue}\n";
            }
            result += "]\n";
