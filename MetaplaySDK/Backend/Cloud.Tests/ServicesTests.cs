    ...
    class ServicesTests
    {
        ...
        public async Task TestGoogleOAuth2PublicKeyCacheExistingKey()
        {
            using (HttpClient client = HttpUtil.CreateJsonHttpClient())
            {
                ...
                try
                {
                    keyset = await HttpUtil.RequestJsonGetAsync<TestJWKSet>(client, "https://www.googleapis.com/oauth2/v3/certs").ConfigureAwait(false);
                }
                catch (HttpRequestException ex) when (ex.StatusCode == HttpStatusCode.Forbidden)
                {
                    // Google sometimes rejects queries with 403. When and why this happens is not clear,
                    // but it _feels_ more like some kind of anti-abuse system than rate-limiting. Hence,
                    // retrying does not appear to help.
                    //
                    // So far, the behavior has only been seen on third party CI systems, so it's possible
                    // the IP spaces used by those providers are on Santa^H^H^H^H^H Google's naughty list.
                    // Ignore these for now.
                    Assert.Warn("Googleapis.com returned 403 - Forbidden. This machine's IP address may be blacklisted. Skipping the test.");
                    return;
                }
                ...
                catch (GoogleOAuth2PublicKeyCache.KeyCacheTemporarilyUnavailable)
                {
                    // GoogleOAuth2PublicKeyCache's internal fetch can fail
                    Assert.Warn("Googleapis.com is temporarily unavailable. This machine's IP address may be blacklisted. Skipping the test.");
                }
            }
        }

        [TestCase]
        public async Task TestGoogleOAuth2PublicKeyCacheNoSuchKey()
        {
            try
            {
                await GoogleOAuth2PublicKeyCache.Instance.GetPublicKeyAsync("bad-id");
                Assert.Fail("Expect GetPublicKeyAsync to throw NoSuchKeyException");
            }
            catch (GoogleOAuth2PublicKeyCache.NoSuchKeyException)
            {
                // Expected
            }
            catch (GoogleOAuth2PublicKeyCache.KeyCacheTemporarilyUnavailable)
            {
                // See TestGoogleOAuth2PublicKeyCacheExistingKey
                Assert.Warn("Googleapis.com is temporarily unavailable. This machine's IP address may be blacklisted. Skipping the test.");
            }
        }
        ...
        public async Task TestAppleGameCenterPublicKeyCacheFutureProofness()
        {
            ...
            // safe bet.
            string futureKeyUrl = "https://static.gc.apple.com/public-key/gc-prod-11.cer";
            DateTime timestamp = new DateTime(2025, 05, 29, 0, 0, 0, DateTimeKind.Utc);
